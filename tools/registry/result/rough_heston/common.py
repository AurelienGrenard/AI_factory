"""Shared native Rough Heston result execution helpers."""
...
def _rough_heston_row(
    ...
    if strike is None:
        raise ValueError("Rough Heston product row requires a strike.")
    return row_type(
        float(model["spot"]),
...
def cpp_outputs_for_time_grid(
    ...
    *,
    ...
    symbol = (
        f"ai_factory_price_rough_heston_{product_symbol}"
    ...
    for num_steps, indexed_rows in groups.items():
        ...
        row_array = row_array_type(
            *[
                _rough_heston_row(row_type, row, model_by_id, product_by_id)

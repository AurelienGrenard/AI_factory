"""Rough Heston multifactor autocall production and validation pipeline."""
...
def source_files_for_engine(engine: str) -> list[str]:
    common = ["tools/registry/result/rough_heston/autocalls.py"]
    if engine.startswith("python_"):
        return [
            ...
            "src_python/ai_factory/pytorch/rough_heston/autocalls.py", *common,
        ]
    if engine.startswith("cpp_cpu_"):
        return [
            ...
            "src_cpp/ai_factory/cpu/rough_heston/autocalls.cpp",
    ...
    return [
        ...
        "src_cpp/ai_factory/cuda/rough_heston/autocalls.cu",
        "src_cpp/ai_factory/c_api/c_api.cpp", *common,
    ]
...
_PIPELINE = ProductionPipeline(ProductionPipelineConfig(
    ...
    model_name="Rough Heston", payoff_name="Memory Autocall",
    first_seed=DEFAULT_FIRST_SEED, default_num_paths=DEFAULT_NUM_PATHS,
    default_target_dt=DEFAULT_TARGET_DT,
    cpp_price=partial(cpp_outputs, "rough_heston"),
    python_price=partial(python_outputs, "rough_heston"),
...
economic_diagnostics = partial(_economic_diagnostics, "rough_heston")

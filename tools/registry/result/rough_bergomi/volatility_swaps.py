"""Shared Rough Bergomi volatility swap priced result recipes."""
...
from ai_factory.pytorch.rough_bergomi.volatility_swaps import (
...
from tools.registry.result.rough_bergomi.common import (
...
def source_files_for_engine(engine: str) -> list[str]:
    common = ["tools/registry/result/rough_bergomi/volatility_swaps.py"]
    if engine.startswith("python_"):
        return [
            ...
            "src_python/ai_factory/pytorch/rough_bergomi/volatility_swaps.py",
            *common,
        ]
    if engine in {"cpp_cpu_philox", "cpp_cpu_philox_delta_crn"}:
        return [
            ...
            "src_cpp/ai_factory/cpu/rough_bergomi/volatility_swaps.cpp",
    ...
    if engine in {"cpp_gpu_philox", "cpp_gpu_philox_delta_crn"}:
        return [
            ...
            "src_cpp/ai_factory/cuda/rough_bergomi/api.cuh",
            "src_cpp/ai_factory/cuda/rough_bergomi/dynamics.cuh",
            "src_cpp/ai_factory/cuda/rough_bergomi/volatility_swaps.cu",
            "src_cpp/ai_factory/cuda/rough_bergomi/paths.cu",
            "src_cpp/ai_factory/c_api/c_api.cpp",
            *common,
        ]
...
def python_outputs(
    ...
    *,
    ...
    for num_steps, indexed_rows in _rows_grouped_by_step_count(rows, product_by_id, target_dt):
        ...
        group_outputs, group_timing = pytorch_volatility_swap_price_batch(
            group_rows,
            model_by_id,
            product_by_id,
            num_paths=num_paths,
...
def python_delta_crn_outputs(
    ...
    *,
    ...
    for num_steps, indexed_rows in _rows_grouped_by_step_count(rows, product_by_id, target_dt):
        group_rows = [row for _, row in indexed_rows]
        group_outputs, group_timing = pytorch_volatility_swap_delta_crn_batch(
            group_rows,
            model_by_id,
            product_by_id,
            num_paths=num_paths,
...
def cpp_outputs_for_time_grid(
    ...
    **_: Any,
    ...
    return _cpp_native_outputs(
        ...
        model_by_id,
        product_by_id,
        row_type=CRoughBergomiRow,
        output_type=CMonteCarloOutput,
        product_symbol="volatility_swap",
        num_paths=num_paths,
        target_dt=target_dt,
        use_gpu=use_gpu,
    )
...
def cpp_delta_crn_outputs_for_time_grid(
    ...
    **_: Any,
    ...
    raise ValueError("A volatility swap has no spot delta CRN result.")
...
PRODUCTION_MODEL_ID = "rough_bergomi_01"

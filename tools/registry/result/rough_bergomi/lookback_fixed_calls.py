"""Shared Rough Bergomi fixed-lookback priced result recipes."""
...
from ai_factory.pytorch.rough_bergomi.lookback_fixed_calls import (
...
def source_files_for_engine(engine: str) -> list[str]:
    common = ["tools/registry/result/rough_bergomi/lookback_fixed_calls.py"]
    if engine.startswith("python_"):
        return [
            ...
            "src_python/ai_factory/pytorch/rough_bergomi/lookback_fixed_calls.py",
            *common,
        ]
    if engine in {"cpp_cpu_philox", "cpp_cpu_philox_delta_crn"}:
        return [
            ...
            "src_cpp/ai_factory/cpu/rough_bergomi/lookback_fixed_calls.cpp",
    ...
    if engine in {"cpp_gpu_philox", "cpp_gpu_philox_delta_crn"}:
        return [
            ...
            "src_cpp/ai_factory/cuda/rough_bergomi/lookback_fixed_calls.cu",
...
def python_outputs(
    ...
    *,
    ...
    for num_steps, indexed_rows in _rows_grouped_by_step_count(rows, product_by_id, target_dt):
        ...
        group_outputs, group_timing = pytorch_lookback_price_batch(
            group_rows,
            model_by_id,
            product_by_id,
            num_paths=num_paths,
...
def python_delta_crn_outputs(
    ...
    *,
    ...
    for num_steps, indexed_rows in _rows_grouped_by_step_count(rows, product_by_id, target_dt):
        group_rows = [row for _, row in indexed_rows]
        group_outputs, group_timing = pytorch_lookback_delta_crn_batch(
            group_rows,
            model_by_id,
            product_by_id,
            num_paths=num_paths,

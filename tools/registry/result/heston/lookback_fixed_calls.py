"""Shared implementation for Heston fixed-lookback priced result recipes."""
...
from ai_factory.pytorch.heston.lookback_fixed_calls import (
    price_batch as pytorch_lookback_price_batch,
    price_delta_crn_batch as pytorch_lookback_delta_crn_batch,
)
...
def source_files_for_engine(engine: str) -> list[str]:
    common = ["tools/registry/result/heston/lookback_fixed_calls.py"]
    if engine.startswith("python_"):
        return [
            ...
            "src_python/ai_factory/pytorch/heston/lookback_fixed_calls.py",
            *common,
        ]
    if engine in {"cpp_cpu_philox", "cpp_cpu_philox_delta_crn"}:
        return [
            "src_cpp/ai_factory/cpu/common/philox.cpp",
            "src_cpp/ai_factory/cpu/common/payoffs/lookback.hpp",
            "src_cpp/ai_factory/cpu/heston/common.cpp",
            "src_cpp/ai_factory/cpu/heston/lookback_fixed_calls.cpp",
    ...
    if engine in {"cpp_gpu_philox", "cpp_gpu_philox_delta_crn"}:
        return [
            ...
            "src_cpp/ai_factory/cuda/heston/lookback_fixed_calls.cu",
...
def python_outputs(
    ...
    *,
    ...
    if heston_scheme != HESTON_SCHEME:
        raise ValueError(
            f"PyTorch Heston lookback supports {HESTON_SCHEME!r}; got {heston_scheme!r}."
        )
    return pytorch_lookback_price_batch(
        rows,
        model_by_id,
        product_by_id,
        num_paths=num_paths,
...
def python_delta_crn_outputs(
    ...
    *,
    ...
    return pytorch_lookback_delta_crn_batch(
        rows,
        model_by_id,
        product_by_id,
        num_paths=num_paths,
...
def _configure_cpp_library(library: ctypes.CDLL) -> None:
    ...
    library.ai_factory_price_heston_lookback_fixed_cpu.argtypes = [
        row_ptr,
        ctypes.c_size_t,
        ctypes.c_size_t,
        mc_ptr,
    ]
    library.ai_factory_price_heston_lookback_fixed_cpu.restype = ctypes.c_int
    if hasattr(library, "ai_factory_price_heston_lookback_fixed_cpu_batch"):
        library.ai_factory_price_heston_lookback_fixed_cpu_batch.argtypes = [
        ...
        library.ai_factory_price_heston_lookback_fixed_cpu_batch.restype = ctypes.c_int
    library.ai_factory_price_heston_lookback_fixed_gpu.argtypes = [
        row_ptr,
        ctypes.c_size_t,
        ctypes.c_size_t,
        mc_ptr,
    ...
    library.ai_factory_price_heston_lookback_fixed_gpu.restype = ctypes.c_int
    if hasattr(library, "ai_factory_price_heston_lookback_fixed_gpu_batch"):
        library.ai_factory_price_heston_lookback_fixed_gpu_batch.argtypes = [
            row_ptr,
            ctypes.c_size_t,
            ctypes.c_size_t,
            ctypes.c_size_t,
        ...
        library.ai_factory_price_heston_lookback_fixed_gpu_batch.restype = ctypes.c_int
    if hasattr(library, "ai_factory_price_heston_lookback_fixed_delta_crn_cpu"):
        library.ai_factory_price_heston_lookback_fixed_delta_crn_cpu.argtypes = [
            row_ptr,
            ctypes.c_size_t,
            ctypes.c_size_t,
            ctypes.c_double,
        ...
        library.ai_factory_price_heston_lookback_fixed_delta_crn_cpu.restype = ctypes.c_int
    if hasattr(library, "ai_factory_price_heston_lookback_fixed_delta_crn_cpu_batch"):
        library.ai_factory_price_heston_lookback_fixed_delta_crn_cpu_batch.argtypes = [
            ctypes.POINTER(CHestonRow),
            ctypes.c_size_t,
            ctypes.c_size_t,
            ctypes.c_size_t,
        ...
        library.ai_factory_price_heston_lookback_fixed_delta_crn_cpu_batch.restype = ctypes.c_int
    if hasattr(library, "ai_factory_price_heston_lookback_fixed_delta_crn_gpu_batch"):
        library.ai_factory_price_heston_lookback_fixed_delta_crn_gpu_batch.argtypes = [
            row_ptr,
            ctypes.c_size_t,
            ctypes.c_size_t,
            ctypes.c_size_t,
        ...
        library.ai_factory_price_heston_lookback_fixed_delta_crn_gpu_batch.restype = ctypes.c_int
...
def cpp_outputs(
    ...
    *,
    ...
    started = perf_counter()
    kernel_seconds = 0.0
    if not use_gpu and hasattr(library, "ai_factory_price_heston_lookback_fixed_cpu_batch"):
        ...
        status = library.ai_factory_price_heston_lookback_fixed_cpu_batch(
            row_array,
            len(rows),
            num_paths,
            num_steps,
    ...
    if use_gpu and hasattr(library, "ai_factory_price_heston_lookback_fixed_gpu_batch"):
        row_array_type = CHestonRow * len(rows)
        output_array_type = CMonteCarloOutput * len(rows)
        row_array = row_array_type(
            *[
        ...
        status = library.ai_factory_price_heston_lookback_fixed_gpu_batch(
            row_array,
            len(rows),
            num_paths,
            num_steps,
    ...
    for row in rows:
        ...
        if use_gpu:
            elapsed = ctypes.c_double(0.0)
            status = library.ai_factory_price_heston_lookback_fixed_gpu(
            ...
            status = library.ai_factory_price_heston_lookback_fixed_cpu(
                ctypes.byref(cpp_row),
                num_paths,
                num_steps,
                ctypes.byref(output),
...
def cpp_delta_crn_outputs(
    ...
    *,
    ...
    kernel_seconds = 0.0
    if use_gpu:
        if not hasattr(library, "ai_factory_price_heston_lookback_fixed_delta_crn_gpu_batch"):
        ...
        output_array = output_array_type()
        elapsed = ctypes.c_double(0.0)
        status = library.ai_factory_price_heston_lookback_fixed_delta_crn_gpu_batch(
            row_array,
            len(rows),
            num_paths,
            num_steps,
    ...
    if hasattr(library, "ai_factory_price_heston_lookback_fixed_delta_crn_cpu_batch"):
        ...
        output_array = output_array_type()
        status = library.ai_factory_price_heston_lookback_fixed_delta_crn_cpu_batch(
            row_array,
            len(rows),
            num_paths,
            num_steps,
    ...
    if not hasattr(library, "ai_factory_price_heston_lookback_fixed_delta_crn_cpu"):
    ...
    for row in rows:
        ...
        status = library.ai_factory_price_heston_lookback_fixed_delta_crn_cpu(
            ctypes.byref(cpp_row),
            num_paths,
            num_steps,
            relative_bump,
...
PRODUCTION_PRODUCT_ID = "lookback_fixed_calls_01"
...
_PRODUCTION_PIPELINE = ProductionPipeline(
    ProductionPipelineConfig(
        ...
        model_name="Heston",
        payoff_name="Lookback Fixed Call",

"""Shared implementation for Heston volatility swap priced result recipes."""
...
from ai_factory.pytorch.heston.volatility_swaps import (
    price_batch as pytorch_volatility_swap_price_batch,
    price_delta_crn_batch as pytorch_volatility_swap_delta_crn_batch,
)
...
def source_files_for_engine(engine: str) -> list[str]:
    common = ["tools/registry/result/heston/volatility_swaps.py"]
    if engine.startswith("python_"):
        return [
            ...
            "src_python/ai_factory/pytorch/heston/volatility_swaps.py",
            *common,
        ]
    if engine in {"cpp_cpu_philox", "cpp_cpu_philox_delta_crn"}:
        return [
            "src_cpp/ai_factory/cpu/common/philox.cpp",
            "src_cpp/ai_factory/cpu/common/payoffs/volatility_swap.hpp",
            "src_cpp/ai_factory/cpu/heston/common.cpp",
            "src_cpp/ai_factory/cpu/heston/volatility_swaps.cpp",
    ...
    if engine in {"cpp_gpu_philox", "cpp_gpu_philox_delta_crn"}:
        return [
            ...
            "src_cpp/ai_factory/cuda/heston/volatility_swaps.cu",
...
def python_outputs(
    ...
    *,
    ...
    if heston_scheme != HESTON_SCHEME:
        raise ValueError(
            f"PyTorch Heston volatility swap supports {HESTON_SCHEME!r}; got {heston_scheme!r}."
        )
    return pytorch_volatility_swap_price_batch(
        rows,
        model_by_id,
        product_by_id,
        num_paths=num_paths,
...
def python_delta_crn_outputs(
    ...
    *,
    ...
    return pytorch_volatility_swap_delta_crn_batch(
        rows,
        model_by_id,
        product_by_id,
        num_paths=num_paths,
...
def _configure_cpp_library(library: ctypes.CDLL) -> None:
    ...
    if hasattr(library, "ai_factory_price_heston_volatility_swap_cpu_batch"):
        library.ai_factory_price_heston_volatility_swap_cpu_batch.argtypes = [
        ...
        library.ai_factory_price_heston_volatility_swap_cpu_batch.restype = ctypes.c_int
    if hasattr(library, "ai_factory_price_heston_volatility_swap_gpu_batch"):
        library.ai_factory_price_heston_volatility_swap_gpu_batch.argtypes = [
            row_ptr,
            ctypes.c_size_t,
            ctypes.c_size_t,
            ctypes.c_size_t,
        ...
        library.ai_factory_price_heston_volatility_swap_gpu_batch.restype = ctypes.c_int
    if hasattr(library, "ai_factory_price_heston_volatility_swap_delta_crn_cpu"):
        library.ai_factory_price_heston_volatility_swap_delta_crn_cpu.argtypes = [
            row_ptr,
            ctypes.c_size_t,
            ctypes.c_size_t,
            ctypes.c_double,
        ...
        library.ai_factory_price_heston_volatility_swap_delta_crn_cpu.restype = ctypes.c_int
    if hasattr(library, "ai_factory_price_heston_volatility_swap_delta_crn_cpu_batch"):
        library.ai_factory_price_heston_volatility_swap_delta_crn_cpu_batch.argtypes = [
            ctypes.POINTER(CHestonRow),
            ctypes.c_size_t,
            ctypes.c_size_t,
            ctypes.c_size_t,
        ...
        library.ai_factory_price_heston_volatility_swap_delta_crn_cpu_batch.restype = ctypes.c_int
    if hasattr(library, "ai_factory_price_heston_volatility_swap_delta_crn_gpu_batch"):
        library.ai_factory_price_heston_volatility_swap_delta_crn_gpu_batch.argtypes = [
            row_ptr,
            ctypes.c_size_t,
            ctypes.c_size_t,
            ctypes.c_size_t,
        ...
        library.ai_factory_price_heston_volatility_swap_delta_crn_gpu_batch.restype = ctypes.c_int
...
def cpp_outputs(
    ...
    *,
    ...
    started = perf_counter()
    kernel_seconds = 0.0
    if not use_gpu and hasattr(library, "ai_factory_price_heston_volatility_swap_cpu_batch"):
        ...
        status = library.ai_factory_price_heston_volatility_swap_cpu_batch(
            row_array,
            len(rows),
            num_paths,
            num_steps,
    ...
    if use_gpu and hasattr(library, "ai_factory_price_heston_volatility_swap_gpu_batch"):
        row_array_type = CHestonRow * len(rows)
        output_array_type = CMonteCarloOutput * len(rows)
        row_array = row_array_type(
            *[
        ...
        status = library.ai_factory_price_heston_volatility_swap_gpu_batch(
            row_array,
            len(rows),
            num_paths,
            num_steps,
    ...
    raise RuntimeError("C++ library does not expose Volatility Swap batch pricing.")
...
def cpp_delta_crn_outputs(
    ...
    *,
    ...
    kernel_seconds = 0.0
    if use_gpu:
        if not hasattr(library, "ai_factory_price_heston_volatility_swap_delta_crn_gpu_batch"):
        ...
        output_array = output_array_type()
        elapsed = ctypes.c_double(0.0)
        status = library.ai_factory_price_heston_volatility_swap_delta_crn_gpu_batch(
            row_array,
            len(rows),
            num_paths,
            num_steps,
    ...
    if hasattr(library, "ai_factory_price_heston_volatility_swap_delta_crn_cpu_batch"):
        ...
        output_array = output_array_type()
        status = library.ai_factory_price_heston_volatility_swap_delta_crn_cpu_batch(
            row_array,
            len(rows),
            num_paths,
            num_steps,
    ...
    if not hasattr(library, "ai_factory_price_heston_volatility_swap_delta_crn_cpu"):
    ...
    for row in rows:
        ...
        status = library.ai_factory_price_heston_volatility_swap_delta_crn_cpu(
...
PRODUCTION_PRODUCT_ID = "volatility_swaps_01"

"""Shared implementation for Heston American put production-audit recipes."""
...
from ai_factory.pytorch.heston.american_puts import price_batch as python_price_batch
...
def source_files_for_engine(engine: str) -> list[str]:
    common = ["tools/registry/result/heston/american_puts.py"]
    if engine.startswith("python_"):
        return [
            ...
            "src_python/ai_factory/pytorch/heston/american_puts.py",
            *common,
        ]
    if engine == "cpp_cpu_philox":
        return [
            ...
            "src_cpp/ai_factory/cpu/heston/american_puts.hpp",
            "src_cpp/ai_factory/cpu/heston/american_puts.cpp",
    ...
    if engine == "cpp_gpu_philox":
        return [
            ...
            "src_cpp/ai_factory/cuda/heston/american_puts.cuh",
            "src_cpp/ai_factory/cuda/heston/american_puts.cu",
            "src_cpp/ai_factory/c_api/c_api.cpp",
            *common,
        ]
...
def _configure_cpp_library(library: ctypes.CDLL) -> None:
    ...
    library.ai_factory_price_heston_american_put_cpu_batch.argtypes = [
        row_ptr,
        ctypes.c_size_t,
        ctypes.c_size_t,
        ctypes.c_size_t,
    ...
    library.ai_factory_price_heston_american_put_cpu_batch.restype = ctypes.c_int
    library.ai_factory_price_heston_american_put_gpu_batch.argtypes = [
        row_ptr,
        ctypes.c_size_t,
        ctypes.c_size_t,
        ctypes.c_size_t,
    ...
    library.ai_factory_price_heston_american_put_gpu_batch.restype = ctypes.c_int
...
def cpp_outputs_for_time_grid(
    ...
    *,
    ...
    for num_steps, indexed_rows in _rows_grouped_by_step_count(rows, product_by_id, target_dt):
        ...
        row_array_type = CHestonRow * len(group_rows)
        ...
        if use_gpu:
            ...
            status = library.ai_factory_price_heston_american_put_gpu_batch(
                row_array,
                len(group_rows),
                num_paths,
                num_steps,
            ...
            status = library.ai_factory_price_heston_american_put_cpu_batch(
                row_array,
                len(group_rows),
                num_paths,
                num_steps,

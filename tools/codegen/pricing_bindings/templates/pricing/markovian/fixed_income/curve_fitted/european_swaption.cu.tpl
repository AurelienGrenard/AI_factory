...
using Schedule = simulation::ExactTransitionTerminalSchedule<
    ::ai_factory::workbench::model::fixed_income::${model}::joint::DynamicsPolicy>;
...
void launch_${model}_${curve}_european_swaption_cuda(
    ...
    monte_carlo::launch_monte_carlo_cuda<Pricing>(
        ...
        "${model}.${curve}.european_swaption", swaption_side_name(Side),
        "${model_display} European swaption Monte Carlo kernel"
    );
}

template void launch_${model}_${curve}_european_swaption_cuda<SwaptionSide::payer>(
...
template void launch_${model}_${curve}_european_swaption_cuda<SwaptionSide::receiver>(
    const ModelParameters*, std::size_t,
    const curve::${curve}::${curve_type}Parameters*, std::size_t,
    const product::RegularEuropeanSwaptionParameters*,
    const product::RegularEuropeanSwaptionParameters*,

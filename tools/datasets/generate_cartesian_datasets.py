"""Build, inspect, or run Cartesian price and price-delta catalogue recipes."""
...
def cartesian_targets(
    kinds: set[str], models: set[str], requested: set[str]
) -> list[str]:
    available = {
        spec.cmake_target: spec
        for spec in AVAILABLE_DATASET_SPECS
        if spec.dataset_kind in kinds and spec.construction == "cartesian"
    }
    unknown = requested - available.keys()
    if unknown:
        raise ValueError(
            "Unknown Cartesian targets: " + ", ".join(sorted(unknown))
        )
    selected = [
        target for target, spec in available.items()
        if (not models or spec.model in models)
        and (not requested or target in requested)
    ]
    if not selected:
        raise ValueError("No Cartesian generator matches the selection")
...
def main() -> int:
    ...
    parser.add_argument("--kind", choices=("prices", "price_delta", "all"), default="all")
    ...
    kinds = {"prices", "price_delta"} if arguments.kind == "all" else {arguments.kind}
    targets = cartesian_targets(kinds, set(arguments.model), set(arguments.target))

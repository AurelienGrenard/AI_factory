// Generated Rough-Bergomi model-sample recipe composition.
#pragma once
...
namespace ai_factory::workbench::offline::sampling::rough_bergomi {

namespace model_binding = ai_factory::workbench::model::equity::rough_bergomi;
...
inline std::vector<ModelParameters> generate_core_parameters(
    ...
    while (parameters.size() < parameter_count) {
        ...
        if (!(true)) continue;
        parameters.push_back({spot, risk_free_rate, dividend_yield, xi_0, eta, hurst_exponent, rho});
    }
    return parameters;
}
...
inline datasets::ModelSampleRecipe recipe(
    ...
    return {
        id,
        "Rough-Bergomi",
        "datasets/model/equity/rough_bergomi/samples/" + id + ".json",
        "catalog/model/equity/rough_bergomi/samples/" + id + "/dataset.yaml",
        "https://datasets.ai-factory.example/v1/model/equity/rough_bergomi/samples/" + id + ".json",
        parameter_count,
        paths_per_parameter,
        63U,
        504U,
...
inline int generate(int argc, char** argv, datasets::ModelSampleRecipe value) {
    return generate_model_sample_dataset<ModelParameters>(
        ...
        if (outputs.size() != 1U) {
            throw std::invalid_argument("Rough-Bergomi sample output arity mismatch.");
        }
        model_binding::launch_rough_bergomi_random_terminal_samples_cuda(

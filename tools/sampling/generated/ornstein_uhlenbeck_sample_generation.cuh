// Generated Ornstein-Uhlenbeck model-sample recipe composition.
#pragma once
...
namespace ai_factory::workbench::offline::sampling::ornstein_uhlenbeck {

namespace model_binding = ai_factory::workbench::model::fixed_income::ornstein_uhlenbeck;
...
inline std::vector<ModelParameters> generate_core_parameters(
    ...
    while (parameters.size() < parameter_count) {
        ...
        if (!(true)) continue;
        parameters.push_back({{mean_reversion, volatility}, initial_state});
    }
    return parameters;
}
...
inline datasets::ModelSampleRecipe recipe(
    ...
    return {
        ...
        "exact finite-horizon transition",
        {
            ...
            {"proposal_draw_order", {"mean_reversion", "stationary_volatility", "initial_state"}},
            ...
            {"acceptance", "true"},
            {"derived_parameters", {{"volatility", "volatility = stationary_volatility * sqrt(2 * mean_reversion); deterministic instantaneous diffusion scale."}}}
        },
        {
...
inline int generate(int argc, char** argv, datasets::ModelSampleRecipe value) {
    ...
    return generate_model_sample_dataset<ModelParameters>(
        ...
        if (outputs.size() != 1U) {
            throw std::invalid_argument("Ornstein-Uhlenbeck sample output arity mismatch.");
        }
        model_binding::launch_ornstein_uhlenbeck_random_terminal_samples_cuda(

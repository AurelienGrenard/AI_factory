// Generated Merton model-sample recipe composition.
#pragma once
...
namespace ai_factory::workbench::offline::sampling::merton {

namespace model_binding = ai_factory::workbench::model::equity::merton;
...
inline std::vector<ModelParameters> generate_core_parameters(
    ...
    while (parameters.size() < parameter_count) {
        ...
        const float jump_log_mean = uniform({-0.2f, 0.08f}, uniforms);
        const float jump_log_volatility = uniform({0.03f, 0.3f}, uniforms);
        if (!(true)) continue;
        parameters.push_back({spot, risk_free_rate, dividend_yield, volatility, jump_intensity, jump_log_mean, jump_log_volatility});
    }
    return parameters;
}
...
inline datasets::ModelSampleRecipe recipe(
    ...
    return {
        ...
        "catalog/model/equity/markovian/merton/samples/" + id + "/generation.yaml",
...
inline int generate(int argc, char** argv, datasets::ModelSampleRecipe value) {
    ...
    return generate_model_sample_dataset<ModelParameters>(
        ...
        if (outputs.size() != 1U) {
            throw std::invalid_argument("Merton sample output arity mismatch.");
        }
        model_binding::launch_merton_random_terminal_samples_cuda(

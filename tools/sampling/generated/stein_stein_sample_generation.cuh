// Generated Stein-Stein model-sample recipe composition.
#pragma once
...
namespace ai_factory::workbench::offline::sampling::stein_stein {

namespace model_binding = ai_factory::workbench::model::equity::stein_stein;
...
inline std::vector<ModelParameters> generate_core_parameters(
    ...
    while (parameters.size() < parameter_count) {
        ...
        const float initial_volatility = uniform({0.1f, 0.35f}, uniforms);
        const float mean_reversion = uniform({0.5f, 8.0f}, uniforms);
        const float volatility_of_volatility = uniform({0.05f, 0.5f}, uniforms);
        if (!(true)) continue;
        parameters.push_back({spot, risk_free_rate, dividend_yield, initial_volatility, mean_reversion, volatility_of_volatility, 0.0f});
    }
    return parameters;
}
...
inline int generate(int argc, char** argv, datasets::ModelSampleRecipe value) {
    return generate_model_sample_dataset<ModelParameters>(
        ...
        if (outputs.size() != 2U) {
            throw std::invalid_argument("Stein-Stein sample output arity mismatch.");
        }
        model_binding::launch_stein_stein_random_terminal_samples_cuda(

// Generated Rough-SABR model-sample recipe composition.
#pragma once
...
namespace ai_factory::workbench::offline::sampling::rough_sabr {

namespace model_binding = ai_factory::workbench::model::equity::rough_sabr;
...
inline std::vector<ModelParameters> generate_core_parameters(
    ...
    while (parameters.size() < parameter_count) {
        ...
        const float xi_0 = uniform({0.04f, 0.04f}, uniforms);
        const float eta = uniform({0.5f, 3.0f}, uniforms);
        const float hurst_exponent = uniform({0.03f, 0.25f}, uniforms);
        const float rho = uniform({-0.95f, -0.3f}, uniforms);
        const float beta = uniform({0.7f, 1.0f}, uniforms);
        if (!(true)) continue;
        parameters.push_back({spot, risk_free_rate, dividend_yield, xi_0, eta, hurst_exponent, rho, beta});
    }
    return parameters;
}
...
inline datasets::ModelSampleRecipe recipe(
    ...
    return {
        ...
        "catalog/model/equity/rough/rough_sabr/samples/" + id + "/generation.yaml",
        ...
        "block-cooperative hybrid FFT at dt=1/504",
        {
            ...
            {"proposal_draw_order", {"spot", "risk_free_rate", "dividend_yield", "xi_0", "eta", "hurst_exponent", "rho", "beta"}},
...
inline int generate(int argc, char** argv, datasets::ModelSampleRecipe value) {
    const auto native_block = model_binding::rough_sabr_sample_block_dimensions(value.maximum_maturity_days);
    return generate_model_sample_dataset<ModelParameters>(
        ...
        if (outputs.size() != 1U) {
            throw std::invalid_argument("Rough-SABR sample output arity mismatch.");
        }
        model_binding::launch_rough_sabr_random_terminal_samples_cuda(

// Generated Variance-Gamma model-sample recipe composition.
#pragma once
...
namespace ai_factory::workbench::offline::sampling::variance_gamma {

namespace model_binding = ai_factory::workbench::model::equity::variance_gamma;
...
inline std::vector<ModelParameters> generate_core_parameters(
    ...
    while (parameters.size() < parameter_count) {
        ...
        const float sigma = uniform({0.08f, 0.45f}, uniforms);
        const float nu = uniform({0.03f, 0.5f}, uniforms);
        const float theta = uniform({-0.35f, 0.15f}, uniforms);
        ...
        if (!(first_moment > 0.05f && second_moment > 0.05f)) continue;
        parameters.push_back({spot, risk_free_rate, dividend_yield, sigma, nu, theta});
    }
    return parameters;
}
...
inline int generate(int argc, char** argv, datasets::ModelSampleRecipe value) {
    return generate_model_sample_dataset<ModelParameters>(
        ...
        if (outputs.size() != 1U) {
            throw std::invalid_argument("Variance-Gamma sample output arity mismatch.");
        }
        model_binding::launch_variance_gamma_random_terminal_samples_cuda(

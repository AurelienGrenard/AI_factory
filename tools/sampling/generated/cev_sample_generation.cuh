// Generated CEV model-sample recipe composition.
#pragma once
...
namespace ai_factory::workbench::offline::sampling::cev {

namespace model_binding = ai_factory::workbench::model::equity::cev;
...
inline std::vector<ModelParameters> generate_core_parameters(
    ...
    while (parameters.size() < parameter_count) {
        ...
        const float beta = uniform({0.55f, 0.95f}, uniforms);
        if (!(true)) continue;
        parameters.push_back({spot, risk_free_rate, dividend_yield, sigma, beta});
    }
    return parameters;
}
...
inline datasets::ModelSampleRecipe recipe(
    ...
    return {
        ...
        "fixed-step transition at dt=1/504",
        {
            ...
            {"proposal_draw_order", {"spot", "risk_free_rate", "dividend_yield", "sigma", "beta"}},
...
inline int generate(int argc, char** argv, datasets::ModelSampleRecipe value) {
    ...
    return generate_model_sample_dataset<ModelParameters>(
        ...
        if (outputs.size() != 1U) {
            throw std::invalid_argument("CEV sample output arity mismatch.");
        }
        model_binding::launch_cev_random_terminal_samples_cuda(

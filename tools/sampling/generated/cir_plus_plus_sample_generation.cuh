// Generated CIR++ model-sample recipe composition.
#pragma once
...
namespace ai_factory::workbench::offline::sampling::cir_plus_plus {

namespace model_binding = ai_factory::workbench::model::fixed_income::cir_plus_plus;
...
inline datasets::ModelSampleRecipe recipe(
    ...
    return {
        ...
        "catalog/model/fixed_income/cir_plus_plus/samples/" + id + "/dataset.yaml",
        ...
        {
            {"state", {{"description", "Terminal nonnegative CIR factor y; not the curve-shifted short rate."}, {"layout", "sample-major"}}}
        },
...
inline int generate(int argc, char** argv, datasets::ModelSampleRecipe value) {
    ...
    return generate_model_sample_dataset<ModelParameters>(
        ...
        if (outputs.size() != 1U) {
            throw std::invalid_argument("CIR++ sample output arity mismatch.");
        }
        model_binding::launch_cir_plus_plus_random_terminal_samples_cuda(

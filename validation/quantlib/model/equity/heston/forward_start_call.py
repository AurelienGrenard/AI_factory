"""Validate Heston forward-start calls against independent QuantLib MC."""
...
def validation_from_quantlib(path, tolerances=_TOLERANCES):
    """Compare every forward-start-call price with independent QuantLib MC."""

    return validation_from_quantlib_heston_option(
        path, "forward_start_call", tolerances
    )

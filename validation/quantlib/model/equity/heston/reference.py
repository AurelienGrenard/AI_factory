"""Shared QuantLib construction for every Heston product validator."""
...
class HestonReference:
    """QuantLib process and model built from one validated Heston row."""
...
def quantlib_reference(parameters: Mapping[str, Any]) -> HestonReference:
    """Map one workbench Heston row to QuantLib's risk-neutral model."""

    context = "Heston model"
    ...
    return HestonReference(process, ql.HestonModel(process), spot)

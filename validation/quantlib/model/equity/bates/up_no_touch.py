"""Compare discrete up no-touches with QuantLib's independent discrete Bates paths."""
...
_TOLERANCES = ValidationTolerances(
    absolute=3.0e-3, relative=1.5e-1,
    standard_error_multiplier=5.0, bias_standard_errors=5.0,
)


def validation_from_quantlib(path, tolerances=_TOLERANCES):
    """Compare every maturity-paid up no-touch with QuantLib."""

    return validation_from_quantlib_bates_option(path, "up_no_touch", tolerances)

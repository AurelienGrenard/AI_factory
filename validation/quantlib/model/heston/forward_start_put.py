"""Validate Heston forward-start puts against independent QuantLib MC."""
...
def validation_from_quantlib(path, tolerances=_TOLERANCES):
    """Compare every forward-start-put price with independent QuantLib MC."""

    return validation_from_quantlib_heston_option(
        path, "forward_start_put", tolerances
    )

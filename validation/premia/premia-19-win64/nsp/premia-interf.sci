...
function [ts_model,options,family_index]= create_premia_option_tree(pmodel,asset_name)
  ...
  family_index=Top;
  ts_model = gtk_tree_store_new(Top_names); 
...
//
// From this list, a new gtk table is built to reflect the list of setable parameters
//
//  - labels is a list of labels for the different parameters. Each parameter is associated to a
//    gtk widget
//  - entries is a list of gtk widgets. Each widget is associated to the corresponding value of
//    labels
//
function [labels, entries] = recursive_table(win,M,L,typeobj)
  ...
  for it = L
    select it(1) 
    case "entry" then
      labels_i = gtk_label_new(str=it(2));
      labels_i.set_line_wrap[%t];
      labels_i.set_text[catenate(wrap(it(2)))];
      entries_i = gtk_entry_new ();
      entries_i.set_text[it(3)];
      entries_i.connect["activate", activate_entry, list(win,M,typeobj,labels_i)]
      labels.add_last[labels_i];
      entries.add_last[entries_i];
    case "combo" then
      cell_renderer = gtk_cell_renderer_text_new ();
      labels_i = gtk_label_new(str=it(2));
      entries_i = gtk_combo_box_new();
      labels_i.set_text[catenate(wrap(it(2)))];
      ts_gener = gtk_tree_store_new(it(4));
    ...
    case "selectfile" then
      labels_i = gtk_label_new(str=it(2));
      entries_i = gtk_file_chooser_button_new('Select File', GTK.FILE_CHOOSER_ACTION_OPEN)

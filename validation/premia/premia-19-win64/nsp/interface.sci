...
function [M] = premia()
  ...
  win = gtk_window_new();
  win.set_title[title];
  vbox_main = gtk_box_new('vertical',spacing=0);
  win.add[vbox_main];
  ...
  // logo
  stock = gtk_image_new_from_file("premia.gif");
  //help menu
  menubar = gtk_menu_bar_new();
  menu_file = gtk_menu_item_new("File");
  menu = gtk_menu_item_new("Help");
  menu_tools = gtk_menu_item_new("Tools");
  about = gtk_menu_item_new("About");
  // sub menu
  
  sub_menu_file = gtk_menu_new ();
  load_file = gtk_menu_item_new('Load');
  save_file = gtk_menu_item_new('Save');
  quit_file = gtk_menu_item_new('Quit');
  ...
  model_help = gtk_menu_item_new('Model Help');
  option_help = gtk_menu_item_new('Option Help');
  method_help = gtk_menu_item_new('Method Help');
  ...
  draw_graph = gtk_menu_item_new('Draw');
  redraw_graph = gtk_menu_item_new('Redraw');
  legend_graph = gtk_menu_item_new('Legend');
  clear_graph = gtk_menu_item_new('Clear Graph');
  clear_data = gtk_menu_item_new('Clear Data');
  ...
  sub_menu_about = gtk_menu_new();
  premia_about = gtk_radio_menu_item_new(label='Premia');
  ...
  notebook.append_page[scrolled_window, gtk_label_new(str="_Interface")]; 
  hb_inter = gtk_box_new('vertical');
  viewport = gtk_viewport_new()
  viewport.add[hb_inter]
  scrolled_window.add[viewport];
  

  vbox_main.pack_start[ notebook,expand=%t,fill=%t,padding=0]
  hb_graph = gtk_box_new('horizontal');
  notebook.append_page[hb_graph, gtk_label_new(str="_Graphics")]; 
  ...
  hbox1 = gtk_box_new('horizontal',spacing=8);
  hbox1.set_border_width[8]
  // hbox for the textview
  hbox_textview = gtk_box_new('horizontal',spacing=8);
  hbox_textview.set_border_width[8]
  // hbox for the buttons "compute" and "close"
  hbox_but = gtk_box_new('horizontal',spacing=8);
  hbox_but.set_border_width[8]
  
  hb_inter.pack_start[hbox1,expand=%f,fill=%f,padding=0];
  hb_inter.pack_start[hbox_but,expand=%f,fill=%f,padding=0];
  hb_inter.pack_start[gtk_separator_new("horizontal"),expand=%f,fill=%f,padding=0];
  hb_inter.pack_start[hbox_textview,expand=%t,fill=%t,padding=0];

  // asset type
  ts_assets = gtk_tree_store_new(assets);
  [vbox_asset,asset_combo,asset_table] = create_vbox ('Asset type', ts_assets);
  hbox1.pack_start[vbox_asset, expand=%f, fill=%t, padding=0];
  
  // model choice
  ts_models = gtk_tree_store_new(models); 
  ...
  ts_meth=gtk_tree_store_new(methods); 
  ...
  hbox_textview.set_border_width[5]
  ...
  // buttons
  compute_but=gtk_button_new[label='Compute'];
  close_but=gtk_button_new[label='Close'];
  hbox_but.pack_start[compute_but, expand=%f, fill=%f, padding=8];
  hbox_but.pack_start[close_but, expand=%f, fill=%f, padding=8];
...
function throw_message(t,mess)
  dialog = gtk_message_dialog_new (flags= GTK.DIALOG_MODAL, type= t, ...
...
function raise_logo(menuitem, data)
  ...
  stock = gtk_image_new_from_file("premia.gif");
  dialog.get_content_area[].pack_start[stock,expand=%f,fill=%f,padding=0];
  dialog.show_all[];
  reponse=dialog.run[];

...
//
// Go through the list data to search for the key label and
// set the corresponding value to val.
//
// If label corresponds to an entry, val holds the text read in the entry. If label
// corresponds to a combo, val is the index (starting at 0!) of the choice
...
function update_gui_after_signal (win, M, typeobj, label, value)
  ...
  L = tweak_object_values (data, label.get_text[], value);
  set_object_values(M,typeobj,L);
  premia_get_values(win,M,typeobj);
  win.show_all[];
endfunction

function activate_entry (entry, data)
  win = data(1);
  M = data(2);
  typeobj = data(3);
  label = data(4);
  value = evstr(entry.get_text[]);
  update_gui_after_signal (win, M, typeobj, label, value)
endfunction
...
function enum_combo_changed (cb, data)
  ...
  update_gui_after_signal (win, M, typeobj, label, choice)
endfunction

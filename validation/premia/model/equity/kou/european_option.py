"""Shared Premia validation for Kou European calls and puts."""

from pathlib import Path

from validation.premia.bridge import (
    KouEuropeanOptionInput,
    price_kou_european_options,
)
...
def validation_from_premia_kou_option(
    ...
    premia_inputs: list[KouEuropeanOptionInput] = []
    for row in validation_input.rows:
        ...
        model_context = f"Kou model row '{row.model_id}'"
        ...
        premia_inputs.append(
            KouEuropeanOptionInput(
                ...
                up_probability=parameter_number(
                    model,
                    "up_probability",
                    model_context,
                ),
                positive_jump_rate=parameter_number(
                    model,
                    "positive_jump_rate",
                    model_context,
                    positive=True,
                ),
                negative_jump_rate=parameter_number(
                    model,
                    "negative_jump_rate",
                    model_context,
                    positive=True,
    ...
    premia_prices = price_kou_european_options(premia_inputs, option_type)
    comparisons = tuple(
        PremiaPriceComparison(
            row_id=row.row_id,

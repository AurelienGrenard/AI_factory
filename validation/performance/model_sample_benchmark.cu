...
constexpr std::size_t kExactLaunchesPerMeasurement = 256U;
...
void benchmark_black_scholes(const Layout& layout) {
    ...
    DeviceBuffer device_parameters(parameters.size() * sizeof(parameters[0]));
    DeviceBuffer output(sample_count(layout) * sizeof(float));
...
void benchmark_heston(const Layout& layout) {
    ...
    DeviceBuffer device_parameters(parameters.size() * sizeof(parameters[0]));
    DeviceBuffer spots(sample_count(layout) * sizeof(float));
    DeviceBuffer variances(sample_count(layout) * sizeof(float));
    copy_to_device(device_parameters, parameters);
    const unsigned int threads = offline::cuda_tuning::kSampleThreadsPerBlock;
    const std::size_t blocks = block_count(layout, threads);
    const auto launch = [&] {
...
void benchmark_rough_heston(const Layout& layout) {
    ...
    DeviceBuffer device_prepared(prepared.size() * sizeof(prepared[0]));
    DeviceBuffer spots(sample_count(layout) * sizeof(float));

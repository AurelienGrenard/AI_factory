...
#pragma omp parallel for schedule(static)
    ...
    for (std::ptrdiff_t index = 0; index < static_cast<std::ptrdiff_t>(row_count); ++index) {
        ...
        const auto averages = simulation::generate_black_scholes_arithmetic_average_spots(
        ...
        for (double average : averages) {
            const double payoff = discount * payoffs::asian_arithmetic_call(average, row.strike);
            sum += payoff;
            sumsq += payoff * payoff;
        }
...
#pragma omp parallel for schedule(static)
    ...
    for (std::ptrdiff_t index = 0; index < static_cast<std::ptrdiff_t>(row_count); ++index) {
        ...
        for (double average : averages) {
            const double price = discount * payoffs::asian_arithmetic_call(average, row.strike);
            const double up = discount
                              * payoffs::asian_arithmetic_call(average * (1.0 + relative_bump), row.strike);

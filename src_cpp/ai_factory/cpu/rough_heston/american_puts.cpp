...
constexpr std::size_t kBasisSize = 3U + simulation::kRoughHestonFactorCount;
...
std::array<double, kBasisSize> state_basis(
    ...
    std::array<double, kBasisSize> basis{};
    basis[0] = 1.0;
    basis[1] = 1.0 - scaled_spot;
    basis[2] =
        1.0 - 2.0 * scaled_spot + 0.5 * scaled_spot * scaled_spot;
    for (std::size_t factor = 0;
         factor < simulation::kRoughHestonFactorCount;
         ++factor) {
...
void price_american_put_from_paths(
    ...
    const auto state_count = num_paths * (num_steps + 1U);
    if (paths.spots.size() != state_count
        || paths.factors.size()
               != state_count * simulation::kRoughHestonFactorCount) {
        throw std::invalid_argument("Unexpected rough Heston path buffer size.");
    }
    ...
    for (std::size_t reverse_step = num_steps - 1U; reverse_step > 0U; --reverse_step) {
        ...
        for (std::size_t path = 0; path < num_paths; ++path) {
            ...
            const auto factor_offset =
                state_index * simulation::kRoughHestonFactorCount;
            const auto basis = state_basis(
                spot / strike, paths.factors.data() + factor_offset, theta
            );
...
void price_american_put(
    ...
    const auto paths = simulation::generate_rough_heston_state_paths(
        to_model(row),

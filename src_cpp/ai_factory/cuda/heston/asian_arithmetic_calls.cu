...
using heston_detail::simulate_heston_qe_average_spot;

struct HestonAsianWorkspaceTag {};

__device__ __forceinline__ double simulate_heston_euler_average_spot(
    const HestonRow& row,
...
__device__ __forceinline__ double simulate_heston_average_spot(
    const HestonRow& row,
    std::size_t num_steps,
    std::size_t path,
    double dt,
    ...
    if (row.scheme == kHestonEulerFullTruncation) {
        return simulate_heston_euler_average_spot(
            row,
            num_steps,
            path,
            dt,
    ...
    return simulate_heston_qe_average_spot(row, num_steps, path, dt);
}

__global__ void heston_asian_arithmetic_partial_kernel(
    ...
    if (path < num_paths) {
        const double average_spot = simulate_heston_average_spot(
            row,
            num_steps,
            path,
            dt,
        ...
        const double payoff = discount * fmax(average_spot - row.strike, 0.0);
        local_sum = payoff;
...
__global__ void heston_asian_arithmetic_finalize_kernel(
...
__global__ void heston_asian_arithmetic_delta_crn_partial_kernel(
    const HestonRow* rows,
    std::size_t row_count,
    std::size_t num_paths,
    std::size_t num_steps,
    ...
    if (path < num_paths) {
        ...
        const double payoff =
            discount * fmax(average_spot - row.strike, 0.0);
        const double up_payoff =
            discount * fmax(average_spot * (1.0 + relative_bump) - row.strike, 0.0);
...
__global__ void heston_asian_arithmetic_delta_crn_finalize_kernel(
...
void price_heston_asian_arithmetic_cuda(
    ...
    auto& workspace = reusable_cuda_workspace<HestonAsianWorkspaceTag, 3U>();
    ...
    try {
        ...
        heston_asian_arithmetic_partial_kernel<<<
            partial_block_count,
            kThreadsPerBlock,
            shared_bytes
        >>>(
        ...
        check_cuda(cudaGetLastError(), "heston asian partial kernel launch");

        heston_asian_arithmetic_finalize_kernel<<<
        ...
        check_cuda(cudaGetLastError(), "heston asian finalize kernel launch");
...
void price_heston_asian_arithmetic_delta_crn_cuda(
    ...
    try {
        ...
        const auto partial_block_count =
            static_cast<unsigned int>(row_count * path_blocks_per_row);
        heston_asian_arithmetic_delta_crn_partial_kernel<<<
            partial_block_count,
            kThreadsPerBlock,
            shared_bytes
        >>>(
        ...
        check_cuda(cudaGetLastError(), "heston asian delta partial kernel launch");

        heston_asian_arithmetic_delta_crn_finalize_kernel<<<
            static_cast<unsigned int>(row_count),
            kThreadsPerBlock,
            shared_bytes
        >>>(
        ...
        check_cuda(cudaGetLastError(), "heston asian delta finalize kernel launch");
        check_cuda(cudaEventRecord(stop), "cudaEventRecord stop");
        check_cuda(cudaEventSynchronize(stop), "cudaEventSynchronize stop");

...
using black_scholes_detail::simulate_average_spot;

struct BlackScholesAsianWorkspaceTag {};

__global__ void asian_kernel(
    ...
    if (path < num_paths) {
        const double average = simulate_average_spot(row, path, num_steps);
        const double payoff = discount * fmax(average - row.strike, 0.0);
        local_sum += payoff;
...
__global__ void asian_delta_kernel(
    const BlackScholesRow* rows,
    std::size_t row_count,
    std::size_t num_paths,
    std::size_t num_steps,
    ...
    if (path < num_paths) {
        const double average = simulate_average_spot(row, path, num_steps);
        const double price = discount * fmax(average - row.strike, 0.0);
        const double up = discount * fmax(average * (1.0 + relative_bump) - row.strike, 0.0);
        const double down = discount * fmax(average * (1.0 - relative_bump) - row.strike, 0.0);
...
__global__ void asian_finalize_kernel(
...
__global__ void asian_delta_finalize_kernel(
...
void run_price_kernel(
    ...
    auto& workspace = reusable_cuda_workspace<BlackScholesAsianWorkspaceTag, 3U>();
    ...
    check_cuda(cudaGetLastError(), "black_scholes asian partial kernel");
    asian_finalize_kernel<<<
    ...
    check_cuda(cudaGetLastError(), "black_scholes asian finalize kernel");
...
void run_delta_kernel(
    ...
    check_cuda(cudaGetLastError(), "black_scholes asian delta partial kernel");
    asian_delta_finalize_kernel<<<
        static_cast<unsigned int>(row_count),
        kThreadsPerBlock,
        4U * kThreadsPerBlock * sizeof(double)
    >>>(
    ...
    check_cuda(cudaGetLastError(), "black_scholes asian delta finalize kernel");
    check_cuda(cudaEventRecord(stop), "cudaEventRecord stop");
    check_cuda(cudaEventSynchronize(stop), "cudaEventSynchronize stop");
    float elapsed_ms = 0.0F;
    check_cuda(cudaEventElapsedTime(&elapsed_ms, start, stop), "kernel timing");

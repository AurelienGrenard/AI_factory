#include "ai_factory/cuda/cir_plus_plus/swaptions.cuh"
...
struct CirPlusPlusSwaptionWorkspaceTag {};
...
__global__ void swaption_kernel(
    const CirPlusPlusSwaptionRow* rows,
    ...
    if (threadIdx.x < product.payment_count) {
        const double maturity = product.expiry
            + (static_cast<int>(threadIdx.x) + 1) * product.accrual_period;
        bond_a[threadIdx.x] = fixed_income::cir_plus_plus_bond_a(
            product.expiry, maturity, model.initial_factor,
        ...
        bond_b[threadIdx.x] = fixed_income::cir_bond_b(
            model.kappa, model.volatility, maturity - product.expiry
        );
    }
    if (threadIdx.x == 0) {
    ...
    for (std::size_t path = static_cast<std::size_t>(threadIdx.x);
        ...
        const double discount = fixed_income::cir_plus_plus_path_discount(
            integral, product.expiry, model.initial_factor,
            model.kappa, model.theta, model.volatility,
            row.curve.beta0, row.curve.beta1, row.curve.beta2, row.curve.tau
        );
        const double payoff = discount * product.notional
                              * fmax(swap, 0.0);
        local_sum += payoff;
...
void price_cir_plus_plus_swaption_cuda(
    const CirPlusPlusSwaptionRow* host_rows,
    std::size_t row_count,
    std::size_t num_paths,
    double target_dt,
    ...
    auto& workspace = detail::reusable_cuda_workspace<
        CirPlusPlusSwaptionWorkspaceTag, 2U
    >();
    auto* device_rows = workspace.buffer<CirPlusPlusSwaptionRow>(
    ...
    detail::check_cuda(
        cudaMemcpy(
            device_rows,
            host_rows,
            row_count * sizeof(CirPlusPlusSwaptionRow),

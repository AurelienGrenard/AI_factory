#include "ai_factory/cuda/cir/bermudan_swaptions.cuh"
...
struct CirBermudanWorkspaceTag {};
...
__global__ void precompute_bonds_kernel(
    const CirBermudanSwaptionRow* rows, std::size_t row_count,
    ...
    for (std::size_t flat = threadIdx.x; flat < kMaxExercises * kMaxPayments;
        ...
        if (exercise < static_cast<std::size_t>(row.product.exercise_count)
            ...
            const double horizon = (payment + 1U - exercise) * row.product.accrual_period;
            bond_a[output] = fixed_income::cir_bond_a(
                row.model.kappa, row.model.theta, row.model.volatility, horizon
            );
            bond_b[output] = fixed_income::cir_bond_b(
                row.model.kappa, row.model.volatility, horizon
            );
        } else {
            bond_a[output] = 0.0;
            bond_b[output] = 0.0;
...
__global__ void precompute_steps_kernel(
    const CirBermudanSwaptionRow* rows,
...
void price_cir_bermudan_swaption_cuda(
    const CirBermudanSwaptionRow* host_rows, std::size_t row_count,
    std::size_t num_paths, double target_dt,
    MonteCarloOutput* host_outputs, CudaTiming* timing
) {
    if (row_count == 0U) return;
    auto& workspace = detail::reusable_cuda_workspace<CirBermudanWorkspaceTag, 12U>();
    const std::size_t capacity = row_count < kRowsPerBatch ? row_count : kRowsPerBatch;
    auto* rows = workspace.buffer<CirBermudanSwaptionRow>(0, capacity, "CIR Bermudan rows");

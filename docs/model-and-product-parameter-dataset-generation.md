...
Version 1 allocates one `2^32`-key domain per canonical recipe path and one
...
reserve `dynamics`. Analytical recipes reserve no Philox domain.
...
`RNG_COMMON_RANDOM_NUMBER_ALLOWLIST`; the version-1 allowlist is empty. Adding,
reordering, or resizing a domain changes the published random construction and

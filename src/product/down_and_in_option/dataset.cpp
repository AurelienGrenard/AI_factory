...
std::vector<DownAndInOptionParameters> load_down_and_in_options(
    ...
    if (!stream) {
        throw std::runtime_error(
            "Could not open Down-and-in option JSON: " + dataset_path.string()
        );
    }
    ...
    try {
        ...
        throw std::runtime_error(
            "Invalid Down-and-in option JSON '" + dataset_path.string()
            + "': " + error.what()
        );
    }
    ...
    std::vector<DownAndInOptionParameters> products;
    ...
    for (const auto& row : rows) {
        ...
        const std::string prefix =
            "Down-and-in option row id '" + row_id + "': ";

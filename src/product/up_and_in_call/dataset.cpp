// Convert Up-and-in-call JSON rows into compact CUDA parameters.
...
std::vector<UpAndInCallParameters> load_up_and_in_calls(
    ...
    if (!stream) {
        throw std::runtime_error(
            "Could not open Up-and-in call JSON: " + dataset_path.string()
        );
    }
    ...
    try {
        ...
        throw std::runtime_error(
            "Invalid Up-and-in call JSON '" + dataset_path.string()
            + "': " + error.what()
        );
    }
    ...
    std::vector<UpAndInCallParameters> products;

...
std::vector<DoubleKnockOutOptionParameters> load_double_knock_out_options(
    ...
    if (!stream) {
        throw std::runtime_error(
            "Could not open Double-knock-out option JSON: " + dataset_path.string()
        );
    }
    ...
    try {
        ...
        throw std::runtime_error(
            "Invalid Double-knock-out option JSON '" + dataset_path.string()
            + "': " + error.what()
        );
    }
    ...
    std::vector<DoubleKnockOutOptionParameters> products;

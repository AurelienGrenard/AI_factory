// Convert digital-put JSON rows into compact CUDA parameters.
...
std::vector<DigitalPutParameters> load_digital_puts(
    ...
    if (!stream) {
        throw std::runtime_error(
            "Could not open digital put JSON: " + dataset_path.string()
        );
    }
    ...
    try {
        ...
        throw std::runtime_error(
            "Invalid digital put JSON '" + dataset_path.string()
            + "': " + error.what()
        );
    }
    ...
    std::vector<DigitalPutParameters> products;
    ...
    for (const auto& row : rows) {
        ...
        const std::string prefix =
            "Digital put row id '" + row_id + "': ";
        const DigitalPutParameters product = {

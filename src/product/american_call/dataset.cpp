// Convert American-call JSON rows into compact CUDA parameters.
...
std::vector<AmericanCallParameters> load_american_calls(
    ...
    if (!stream) {
        throw std::runtime_error(
            "Could not open American call JSON: " + dataset_path.string()
        );
    }
    ...
    try {
        ...
        throw std::runtime_error(
            "Invalid American call JSON '" + dataset_path.string()
            + "': " + error.what()
        );
    }
    ...
    std::vector<AmericanCallParameters> products;

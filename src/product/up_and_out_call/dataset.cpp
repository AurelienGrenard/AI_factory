// Convert Up-and-out-call JSON rows into compact CUDA parameters.
...
std::vector<UpAndOutCallParameters> load_up_and_out_calls(
    ...
    if (!stream) {
        throw std::runtime_error(
            "Could not open Up-and-out call JSON: " + dataset_path.string()
        );
    }
    ...
    try {
        ...
        throw std::runtime_error(
            "Invalid Up-and-out call JSON '" + dataset_path.string()
            + "': " + error.what()
        );
    }
    ...
    std::vector<UpAndOutCallParameters> products;

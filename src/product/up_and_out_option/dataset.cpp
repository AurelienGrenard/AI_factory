...
std::vector<UpAndOutOptionParameters> load_up_and_out_options(
    ...
    if (!stream) {
        throw std::runtime_error(
            "Could not open Up-and-out option JSON: " + dataset_path.string()
        );
    }
    ...
    try {
        ...
        throw std::runtime_error(
            "Invalid Up-and-out option JSON '" + dataset_path.string()
            + "': " + error.what()
        );
    }
    ...
    std::vector<UpAndOutOptionParameters> products;
    ...
    for (const auto& row : rows) {
        ...
        const std::string prefix =
            "Up-and-out option row id '" + row_id + "': ";

...
std::vector<AsianOptionParameters> load_asian_options(
    ...
    if (!stream) {
        throw std::runtime_error(
            "Could not open Asian option JSON: " + dataset_path.string()
        );
    }
    ...
    try {
        ...
        throw std::runtime_error(
            "Invalid Asian option JSON '" + dataset_path.string()
            + "': " + error.what()
        );
    }
    ...
    std::vector<AsianOptionParameters> products;
    ...
    for (const auto& row : rows) {
        ...
        const std::string prefix = "Asian option row id '" + row_id + "': ";

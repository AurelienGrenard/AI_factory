// Convert Asian-put JSON rows into compact CUDA parameters.
...
std::vector<AsianPutParameters> load_asian_puts(
    ...
    if (!stream) {
        throw std::runtime_error(
            "Could not open Asian put JSON: " + dataset_path.string()
        );
    }
    ...
    try {
        ...
        throw std::runtime_error(
            "Invalid Asian put JSON '" + dataset_path.string()
            + "': " + error.what()
        );
    }
    ...
    std::vector<AsianPutParameters> products;
    ...
    for (const auto& row : rows) {
        ...
        const AsianPutParameters product = {
            parameters.at("strike").get<float>(),
            parameters.at("maturity").get<float>(),
        };
        const std::string prefix = "Asian put row id '" + row_id + "': ";

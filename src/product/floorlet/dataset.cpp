// Convert floorlet JSON rows into compact CUDA parameters.
...
std::vector<FloorletParameters> load_floorlets(
    ...
    if (!stream) {
        throw std::runtime_error(
            "Could not open floorlet JSON: " + dataset_path.string()
        );
    }
    ...
    try {
        ...
        throw std::runtime_error(
            "Invalid floorlet JSON '" + dataset_path.string()
            + "': " + error.what()
        );
    }
    ...
    for (const auto& row : rows) {
        ...
        const std::string prefix = "Floorlet row id '" + row_id + "': ";

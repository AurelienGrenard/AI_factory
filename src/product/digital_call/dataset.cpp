// Convert digital-call JSON rows into compact CUDA parameters.
...
std::vector<DigitalCallParameters> load_digital_calls(
    ...
    if (!stream) {
        throw std::runtime_error(
            "Could not open digital call JSON: " + dataset_path.string()
        );
    }
    ...
    try {
        ...
        throw std::runtime_error(
            "Invalid digital call JSON '" + dataset_path.string()
            + "': " + error.what()
        );
    }
    ...
    std::vector<DigitalCallParameters> products;
    ...
    for (const auto& row : rows) {
        ...
        const std::string prefix =
            "Digital call row id '" + row_id + "': ";
        const DigitalCallParameters product = {

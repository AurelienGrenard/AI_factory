// Convert zero-coupon bond put JSON rows into compact CUDA parameters.
...
std::vector<ZeroCouponBondPutParameters> load_zero_coupon_bond_puts(
    ...
    if (!stream) {
        throw std::runtime_error(
            "Could not open zero-coupon bond put JSON: "
            + dataset_path.string()
        );
    }
    ...
    try {
        ...
        throw std::runtime_error(
            "Invalid zero-coupon bond put JSON '" + dataset_path.string()
            + "': " + error.what()
        );
    }
    ...
    for (const auto& row : rows) {
        ...
        const std::string prefix =
            "Zero-coupon bond put row id '" + row_id + "': ";

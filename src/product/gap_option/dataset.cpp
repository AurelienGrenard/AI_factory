...
std::vector<GapOptionParameters> load_gap_options(
    ...
    if (!stream) {
        throw std::runtime_error(
            "Could not open gap option JSON: " + dataset_path.string()
        );
    }
    ...
    try {
        ...
        throw std::runtime_error(
            "Invalid gap option JSON '" + dataset_path.string()
            + "': " + error.what()
        );
    }

// Convert Down-and-out-put JSON rows into compact CUDA parameters.
...
std::vector<DownAndOutPutParameters> load_down_and_out_puts(
    ...
    if (!stream) {
        throw std::runtime_error(
            "Could not open Down-and-out put JSON: " + dataset_path.string()
        );
    }
    ...
    try {
        ...
        throw std::runtime_error(
            "Invalid Down-and-out put JSON '" + dataset_path.string()
            + "': " + error.what()
        );
    }
    ...
    std::vector<DownAndOutPutParameters> products;

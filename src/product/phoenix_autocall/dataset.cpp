...
std::vector<PhoenixAutocallParameters> load_phoenix_autocalls(
    ...
    if (!stream) {
        throw std::runtime_error(
            "Could not open Phoenix autocall JSON: " + dataset_path.string()
        );
    }
    ...
    try {
        ...
        throw std::runtime_error(
            "Invalid Phoenix autocall JSON '" + dataset_path.string()
            + "': " + error.what()
        );
    }
    ...
    std::vector<PhoenixAutocallParameters> products;

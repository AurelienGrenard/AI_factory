// Convert European-put JSON rows into compact CUDA parameters.
...
std::vector<EuropeanPutParameters> load_european_puts(
    ...
    if (!stream) {
        throw std::runtime_error(
            "Could not open European put JSON: " + dataset_path.string()
        );
    }
    ...
    try {
        ...
        throw std::runtime_error(
            "Invalid European put JSON '" + dataset_path.string()
            + "': " + error.what()
        );
    }
    ...
    std::vector<EuropeanPutParameters> products;

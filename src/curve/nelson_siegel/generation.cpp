...
void validate_bounds(
    ...
    const auto valid_range = [](const NelsonSiegelSamplingRange& range) {
...
bool has_admissible_forwards(
    ...
    float beta1,
    float beta2,
    const NelsonSiegelSamplingRange& accepted_forward
) {
...
nlohmann::ordered_json range_metadata(
    const NelsonSiegelSamplingRange& range
) {
...
datasets::GeneratedRows generate_nelson_siegel_rows(
    ...
    std::vector<datasets::ParameterRow> rows;

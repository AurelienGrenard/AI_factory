...
void validate_heston_european_call_launch(
    ...
    if (result_offset >= result_count
        ...
        throw std::invalid_argument(
            "The Heston call launch batch exceeds the result array."
        );
    }
...
void launch_heston_european_call_cuda(
    ...
    check_cuda(
        ...
        "Heston European call occupancy check"
    );
    if (active_blocks_per_multiprocessor == 0) {
        throw std::invalid_argument(
            "Heston European call kernel cannot launch one block per SM."
        );
    }

    // Launch the Heston European-call kernel.
    heston_european_call_kernel<<<

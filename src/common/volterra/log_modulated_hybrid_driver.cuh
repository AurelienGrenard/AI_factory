...
struct LogModulatedHybridDriverPolicy {
    struct Parameters {
    ...
    __host__ __device__ static float far_cell_weight(
        ...
        float sum = unnormalized_kernel(driver.parameters, lower)
            + unnormalized_kernel(driver.parameters, upper);
        ...
        return driver.normalization * sum * h
            / (3.0f * driver.time_step);
    }
    ...
    __host__ __device__ static float variance(
        ...
        return driver.normalization * driver.normalization
            * unnormalized_power_integral(driver.parameters, time, 2.0f);
    }
...
static_assert(std::is_trivially_copyable_v<
    LogModulatedHybridDriverPolicy::PreparedDriver
>);

// Closed-form zero-coupon bond calls under the Ornstein-Uhlenbeck short rate.
...
__global__ void ornstein_uhlenbeck_zero_coupon_bond_call_kernel(
...
void validate_ornstein_uhlenbeck_zero_coupon_bond_call_launch(
    ...
    if (result_offset >= result_count
        ...
        throw std::invalid_argument(
            "The OU zero-coupon bond call launch batch exceeds the result array."
        );
    }
    ...
    const std::size_t thread_count = checked_workspace_product(
        ...
        "The OU zero-coupon bond call thread count exceeds size_t."
    );
    if (thread_count < launch_result_count) {
        throw std::invalid_argument(
            "The OU zero-coupon bond call launch requires one thread per price."
        );
    }
}
...
void launch_ornstein_uhlenbeck_zero_coupon_bond_call_cuda(
    const OrnsteinUhlenbeckModelParameters* device_models,
    std::size_t model_count,
    const product::ZeroCouponBondCallParameters* device_products,
    std::size_t product_count,
    ...
    validate_ornstein_uhlenbeck_zero_coupon_bond_call_launch(
        device_models,
        model_count,
        device_products,
        product_count,
    ...
    ornstein_uhlenbeck_zero_coupon_bond_call_kernel<<<
    ...
    check_cuda(cudaGetLastError(), "OU zero-coupon bond call kernel");
}

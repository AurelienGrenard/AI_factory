// Closed-form floorlet pricing under the Ornstein-Uhlenbeck short rate.
...
__global__ void ornstein_uhlenbeck_floorlet_kernel(
...
void validate_ornstein_uhlenbeck_floorlet_launch(
    ...
    if (result_offset >= result_count
        ...
        throw std::invalid_argument(
            "The Ornstein-Uhlenbeck floorlet launch batch exceeds the result array."
        );
    }
    ...
    const std::size_t thread_count = checked_workspace_product(
        ...
        "The Ornstein-Uhlenbeck floorlet thread count exceeds size_t."
    );
    if (thread_count < launch_result_count) {
        throw std::invalid_argument(
            "The Ornstein-Uhlenbeck floorlet launch requires one thread per price."
        );
    }
}
...
void launch_ornstein_uhlenbeck_floorlet_cuda(
    ...
    validate_ornstein_uhlenbeck_floorlet_launch(
        device_models,
        model_count,
        device_products,
        product_count,
    ...
    ornstein_uhlenbeck_floorlet_kernel<<<
    ...
    check_cuda(cudaGetLastError(), "Ornstein-Uhlenbeck floorlet kernel");
}

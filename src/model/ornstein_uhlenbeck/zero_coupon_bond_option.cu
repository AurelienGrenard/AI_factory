// Closed-form zero-coupon bond options under the Ornstein-Uhlenbeck short rate.
...
template<OptionSide Side>
__global__ void ornstein_uhlenbeck_zero_coupon_bond_option_kernel(
...
void validate_ornstein_uhlenbeck_zero_coupon_bond_option_launch(
    ...
    if (result_offset >= result_count
        ...
        throw std::invalid_argument(
            "The OU zero-coupon bond option launch batch exceeds the result array."
        );
    }
    ...
    const std::size_t thread_count = checked_workspace_product(
        ...
        "The OU zero-coupon bond option thread count exceeds size_t."
    );
    if (thread_count < launch_result_count) {
        throw std::invalid_argument(
            "The OU zero-coupon bond option launch requires one thread per price."
        );
    }
}
...
void launch_ornstein_uhlenbeck_zero_coupon_bond_option_cuda(
    ...
    validate_ornstein_uhlenbeck_zero_coupon_bond_option_launch(
        device_models,
        model_count,
        device_products,
        product_count,
    ...
    report_cuda_kernel_launch_if_enabled(
        ...
        ornstein_uhlenbeck_zero_coupon_bond_option_kernel<Side>,
    ...
    check_cuda(cudaGetLastError(), "OU zero-coupon bond option kernel");
}

// Build both public payoff specializations in this CUDA translation unit.
using LaunchSignature = decltype(launch_ornstein_uhlenbeck_zero_coupon_bond_option_cuda<OptionSide::call>);
template LaunchSignature launch_ornstein_uhlenbeck_zero_coupon_bond_option_cuda<OptionSide::call>;
template LaunchSignature launch_ornstein_uhlenbeck_zero_coupon_bond_option_cuda<OptionSide::put>;

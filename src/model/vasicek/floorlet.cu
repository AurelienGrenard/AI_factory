// Closed-form floorlet pricing under the Vasicek short rate.
...
__global__ void vasicek_floorlet_kernel(
...
void validate_vasicek_floorlet_launch(
    ...
    if (result_offset >= result_count
        ...
        throw std::invalid_argument(
            "The Vasicek floorlet launch batch exceeds the result array."
        );
    }
    ...
    const std::size_t thread_count = checked_workspace_product(
        ...
        "The Vasicek floorlet thread count exceeds size_t."
    );
    if (thread_count < launch_result_count) {
        throw std::invalid_argument(
            "The Vasicek floorlet launch requires one thread per price."
        );
    }
}
...
void launch_vasicek_floorlet_cuda(
    ...
    validate_vasicek_floorlet_launch(
        device_models,
        model_count,
        device_products,
        product_count,
    ...
    check_cuda(cudaGetLastError(), "Vasicek floorlet kernel");
}

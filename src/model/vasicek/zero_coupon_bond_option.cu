// Closed-form zero-coupon bond options under the Vasicek short rate.
...
template<OptionSide Side>
__global__ void vasicek_zero_coupon_bond_option_kernel(
...
void validate_vasicek_zero_coupon_bond_option_launch(
    ...
    if (result_offset >= result_count
        ...
        throw std::invalid_argument(
            "The Vasicek zero-coupon bond option launch batch exceeds the result array."
        );
    }
    ...
    const std::size_t thread_count = checked_workspace_product(
        ...
        "The Vasicek zero-coupon bond option thread count exceeds size_t."
    );
    if (thread_count < launch_result_count) {
        throw std::invalid_argument(
            "The Vasicek zero-coupon bond option launch requires one thread per price."
        );
    }
}
...
void launch_vasicek_zero_coupon_bond_option_cuda(
    ...
    validate_vasicek_zero_coupon_bond_option_launch(
        device_models,
        model_count,
        device_products,
        product_count,
    ...
    report_cuda_kernel_launch_if_enabled(
        ...
        vasicek_zero_coupon_bond_option_kernel<Side>,
    ...
    check_cuda(cudaGetLastError(), "Vasicek zero-coupon bond option kernel");
}

// Closed-form caplet pricing under the Vasicek short-rate model.
...
__global__ void vasicek_caplet_kernel(
...
void validate_vasicek_caplet_launch(
    ...
    if (result_offset >= result_count
        ...
        throw std::invalid_argument(
            "The Vasicek caplet launch batch exceeds the result array."
        );
    }
    ...
    const std::size_t thread_count = checked_workspace_product(
        ...
        "The Vasicek caplet thread count exceeds size_t."
    );
    if (thread_count < launch_result_count) {
        throw std::invalid_argument(
            "The Vasicek caplet launch requires one thread per price."
        );
    }
}
...
void launch_vasicek_caplet_cuda(
    ...
    validate_vasicek_caplet_launch(
        device_models,
        model_count,
        device_products,
        product_count,
    ...
    check_cuda(cudaGetLastError(), "Vasicek caplet kernel");
}

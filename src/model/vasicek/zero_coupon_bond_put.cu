// Closed-form zero-coupon bond puts under the Vasicek short rate.
...
__global__ void vasicek_zero_coupon_bond_put_kernel(
...
void validate_vasicek_zero_coupon_bond_put_launch(
    ...
    if (result_offset >= result_count
        ...
        throw std::invalid_argument(
            "The Vasicek zero-coupon bond put launch batch exceeds the result array."
        );
    }
    ...
    const std::size_t thread_count = checked_workspace_product(
        ...
        "The Vasicek zero-coupon bond put thread count exceeds size_t."
    );
    if (thread_count < launch_result_count) {
        throw std::invalid_argument(
            "The Vasicek zero-coupon bond put launch requires one thread per price."
        );
    }
}
...
void launch_vasicek_zero_coupon_bond_put_cuda(
    const VasicekModelParameters* device_models,
    std::size_t model_count,
    const product::ZeroCouponBondPutParameters* device_products,
    std::size_t product_count,
    ...
    validate_vasicek_zero_coupon_bond_put_launch(
        device_models,
        model_count,
        device_products,
        product_count,
    ...
    vasicek_zero_coupon_bond_put_kernel<<<
    ...
    check_cuda(cudaGetLastError(), "Vasicek zero-coupon bond put kernel");
}

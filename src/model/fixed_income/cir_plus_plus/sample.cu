// Generated CIR++ composition over the common sample engine.
#include "model/fixed_income/cir_plus_plus/sample.cuh"

#include "common/sample.cuh"
#include "common/simulation/schedule.cuh"
#include "model/fixed_income/cir_plus_plus/dynamics.cuh"
...
using Observation = sample::StateSampleObservation<DynamicsPolicy>;
...
void launch_cir_plus_plus_terminal_samples_cuda(
    ...
    sample::launch_device_terminal_samples_cuda<TerminalPolicy>(
        ...
        dynamics_seed, {device_states}, "cir_plus_plus.samples",
        "CIR++ terminal sample kernel");
}

void launch_cir_plus_plus_random_terminal_samples_cuda(
    const ModelParameters* device_parameters, std::size_t parameter_count,
    ...
    sample::launch_device_random_terminal_samples_cuda<TerminalPolicy>(
        ...
        dynamics_seed, device_maturity_days, {device_states},
        "cir_plus_plus.samples", "CIR++ random terminal sample kernel");
}

void launch_cir_plus_plus_calendar_samples_cuda(
    const ModelParameters* device_parameters, std::size_t parameter_count,

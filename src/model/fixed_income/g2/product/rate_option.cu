// Generated Closed-form rate-option composition under the G2 short-rate model.
...
void launch_g2_rate_option_cuda(
    ...
    closed_form::launch_closed_form_cuda<PricingPolicy<Side>>(
        ...
        "G2 rate_option kernel"
    );
}

template void launch_g2_rate_option_cuda<OptionSide::call>(
...
template void launch_g2_rate_option_cuda<OptionSide::put>(
    const ModelParameters*, std::size_t,
    const product::RateOptionParameters*, std::size_t,
    PriceConstruction, std::size_t, std::size_t, std::size_t, float,
    unsigned int, std::size_t, float*

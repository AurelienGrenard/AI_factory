...
using PricingPolicy = product::StandaloneEuropeanSwaptionMonteCarloPricingPolicy<
    Schedule, Product, Source, Side>;
}  // namespace

template<SwaptionSide Side>
void launch_g2_european_swaption_cuda(
    ...
    monte_carlo::launch_monte_carlo_cuda<Pricing>(
        ...
        "g2.european_swaption", swaption_side_name(Side),
        "G2 European swaption Monte Carlo kernel"
    );
}

template void launch_g2_european_swaption_cuda<SwaptionSide::payer>(
...
template void launch_g2_european_swaption_cuda<SwaptionSide::receiver>(
    const ModelParameters*, std::size_t,
    const product::RegularEuropeanSwaptionParameters*,
    const product::RegularEuropeanSwaptionParameters*,
    std::size_t, PriceConstruction, std::size_t, std::size_t, std::size_t,

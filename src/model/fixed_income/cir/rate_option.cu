// Closed-form rate_option pricing under the CIR short-rate model.
...
template<OptionSide Side>
__global__ void cir_rate_option_kernel(
    const CirModelParameters* __restrict__ models,
...
void validate_cir_rate_option_launch(
    ...
    if (result_offset >= result_count
        ...
        throw std::invalid_argument(
            "The CIR rate_option launch batch exceeds the result array."
        );
    }
    ...
    const std::size_t thread_count = checked_workspace_product(
        ...
        "The CIR rate_option thread count exceeds size_t."
    );
    if (thread_count < launch_result_count) {
        throw std::invalid_argument(
            "The CIR rate_option launch requires one thread per price."
        );
    }
}
...
void launch_cir_rate_option_cuda(
    ...
    validate_cir_rate_option_launch(
        device_models,
        model_count,
        device_products,
        product_count,
    ...
    check_cuda(cudaGetLastError(), "CIR rate_option kernel");
}

// Build both public payoff specializations in this CUDA translation unit.
using LaunchSignature = decltype(launch_cir_rate_option_cuda<OptionSide::call>);
template LaunchSignature launch_cir_rate_option_cuda<OptionSide::call>;
template LaunchSignature launch_cir_rate_option_cuda<OptionSide::put>;

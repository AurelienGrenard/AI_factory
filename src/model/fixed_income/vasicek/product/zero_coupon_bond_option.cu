// Generated Closed-form zero-coupon-bond-option composition under Vasicek.
...
void launch_vasicek_zero_coupon_bond_option_cuda(
    ...
    closed_form::launch_closed_form_cuda<PricingPolicy<Side>>(
        ...
        "Vasicek zero-coupon bond option kernel"
    );
}

template void launch_vasicek_zero_coupon_bond_option_cuda<OptionSide::call>(
...
template void launch_vasicek_zero_coupon_bond_option_cuda<OptionSide::put>(
    const ModelParameters*, std::size_t,
    const product::ZeroCouponBondOptionParameters*, std::size_t,
    PriceConstruction, std::size_t, std::size_t, std::size_t, float,
    unsigned int, std::size_t, float*

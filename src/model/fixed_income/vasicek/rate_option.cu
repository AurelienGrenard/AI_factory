// Closed-form rate_option pricing under the Vasicek short-rate model.
...
template<OptionSide Side>
__global__ void vasicek_rate_option_kernel(
...
void validate_vasicek_rate_option_launch(
    ...
    if (result_offset >= result_count
        ...
        throw std::invalid_argument(
            "The Vasicek rate_option launch batch exceeds the result array."
        );
    }
    ...
    const std::size_t thread_count = checked_workspace_product(
        ...
        "The Vasicek rate_option thread count exceeds size_t."
    );
    if (thread_count < launch_result_count) {
        throw std::invalid_argument(
            "The Vasicek rate_option launch requires one thread per price."
        );
    }
}
...
void launch_vasicek_rate_option_cuda(
    ...
    validate_vasicek_rate_option_launch(
        device_models,
        model_count,
        device_products,
        product_count,
    ...
    report_cuda_kernel_launch_if_enabled(
        ...
        vasicek_rate_option_kernel<Side>,
    ...
    check_cuda(cudaGetLastError(), "Vasicek rate_option kernel");
}

// Build both public payoff specializations in this CUDA translation unit.
using LaunchSignature = decltype(launch_vasicek_rate_option_cuda<OptionSide::call>);
template LaunchSignature launch_vasicek_rate_option_cuda<OptionSide::call>;
template LaunchSignature launch_vasicek_rate_option_cuda<OptionSide::put>;

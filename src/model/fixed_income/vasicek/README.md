...
**Implementation:**
`launch_vasicek_european_swaption_cuda<SwaptionSide::payer>` and the
...
For $`I_t=\int_0^t r_u\,\mathrm du`$, $`D(s,t)=e^{-(I_t-I_s)}`$. A
degree-three Hermite regression of the standardized factor estimates
...
$`\mathbb E[D(0,E_0)V_0]`$. The joint Vasicek endpoint and rate integral are
simulated exactly between exercise dates.

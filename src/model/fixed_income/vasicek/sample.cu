// Persistent vasicek terminal and regular-calendar sample kernels.
...
__global__ void vasicek_calendar_samples_kernel(
    const ModelParameters* __restrict__ models,
    std::size_t paths_per_model,
...
std::size_t validate_vasicek_sample_launch(
...
void launch_vasicek_terminal_samples_cuda(
    ...
    validate_vasicek_sample_launch(
        device_models, model_count, paths_per_model, sample_offset,
        launch_sample_count, threads_per_block, block_count, base_seed,
        device_states
    );
    ...
    report_cuda_kernel_launch_if_enabled(
        "vasicek.samples", "terminal",
        vasicek_terminal_samples_kernel,
    ...
    check_cuda(cudaGetLastError(), "vasicek terminal sample kernel");
}
...
void launch_vasicek_calendar_samples_cuda(
    ...
    report_cuda_kernel_launch_if_enabled(
        "vasicek.samples", "calendar",
        vasicek_calendar_samples_kernel,
        dim3(static_cast<unsigned int>(block_count)),
        dim3(threads_per_block)
    );
    vasicek_calendar_samples_kernel<<<

...
DynamicsPolicy::prepare_model(const Parameters& parameters) {
    PreparedModel model = ornstein_uhlenbeck::prepare_model(
        parameters.process
    );
...
DynamicsPolicy::prepare_transition(
    ...
    return ornstein_uhlenbeck::prepare_transition(model, delta_t);
}
...
DynamicsPolicy::initial_state(const PreparedModel& model) {
    return ornstein_uhlenbeck::initial_state(model);
}
...
__device__ __forceinline__ DynamicsPolicy::State
DynamicsPolicy::initial_state(const PreparedModel& model) {
    return {ornstein_uhlenbeck::initial_state(model), 0.0f};
}

// Persistent ornstein uhlenbeck terminal and regular-calendar sample kernels.
...
__global__ void ornstein_uhlenbeck_terminal_samples_kernel(
...
__global__ void ornstein_uhlenbeck_calendar_samples_kernel(
    const ModelParameters* __restrict__ models,
    std::size_t paths_per_model,
...
std::size_t validate_ornstein_uhlenbeck_sample_launch(
...
void launch_ornstein_uhlenbeck_terminal_samples_cuda(
    ...
    validate_ornstein_uhlenbeck_sample_launch(
        device_models, model_count, paths_per_model, sample_offset,
        launch_sample_count, threads_per_block, block_count, base_seed,
        device_states
    );
    ...
    report_cuda_kernel_launch_if_enabled(
        "ornstein_uhlenbeck.samples", "terminal",
        ornstein_uhlenbeck_terminal_samples_kernel,
    ...
    check_cuda(cudaGetLastError(), "ornstein uhlenbeck terminal sample kernel");
}

void launch_ornstein_uhlenbeck_calendar_samples_cuda(
    const ModelParameters* device_models,
    std::size_t model_count,
    std::size_t paths_per_model,
    float first_observation_time,
    ...
    report_cuda_kernel_launch_if_enabled(
        "ornstein_uhlenbeck.samples", "calendar",
        ornstein_uhlenbeck_calendar_samples_kernel,
        dim3(static_cast<unsigned int>(block_count)),
        dim3(threads_per_block)
    );
    ornstein_uhlenbeck_calendar_samples_kernel<<<
    ...
    check_cuda(cudaGetLastError(), "ornstein uhlenbeck calendar sample kernel");
}

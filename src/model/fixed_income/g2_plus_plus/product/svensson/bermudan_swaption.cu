// G2++/Svensson Bermudans over the common Longstaff-Schwartz engine.
...
using ContinuationState =
    ::ai_factory::workbench::fixed_income::TwoFactorRateContinuationState<
        Dynamics
    >;
...
launch_g2_plus_plus_svensson_bermudan_swaption_cuda(
    ...
    >(
        ...
        "G2++ Svensson Bermudan swaption"
    );
}

template longstaff_schwartz::LaunchResult
launch_g2_plus_plus_svensson_bermudan_swaption_cuda<SwaptionSide::payer>(
...
template longstaff_schwartz::LaunchResult
launch_g2_plus_plus_svensson_bermudan_swaption_cuda<SwaptionSide::receiver>(
    const ModelParameters*, std::size_t,
    const curve::svensson::SvenssonParameters*, std::size_t,
    const product::BermudanSwaptionParameters*,
    const product::BermudanSwaptionParameters*, std::size_t,

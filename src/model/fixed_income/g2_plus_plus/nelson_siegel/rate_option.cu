// Closed-form G2++ rate_option pricing fitted to Nelson-Siegel curves.
...
template<OptionSide Side>
__global__ void g2_plus_plus_nelson_siegel_rate_option_kernel(
...
void validate_g2_plus_plus_nelson_siegel_rate_option_launch(
    ...
    if (result_offset >= result_count
        ...
        throw std::invalid_argument(
            "The G2++ rate_option launch batch exceeds the result array."
        );
    }
    ...
    const std::size_t thread_count = checked_workspace_product(
        ...
        "The G2++ rate_option thread count exceeds size_t."
    );
    if (thread_count < launch_result_count) {
        throw std::invalid_argument(
            "The G2++ rate_option launch requires one thread per price."
        );
    }
}
...
template<OptionSide Side>
void launch_g2_plus_plus_nelson_siegel_rate_option_cuda(
    const ModelParameters* device_models,
    std::size_t model_count,
    const curve::nelson_siegel::NelsonSiegelParameters* device_curves,
    std::size_t curve_count,
    ...
    validate_g2_plus_plus_nelson_siegel_rate_option_launch(
        device_models,
        model_count,
        device_curves,
        curve_count,
    ...
    report_cuda_kernel_launch_if_enabled(
        ...
        g2_plus_plus_nelson_siegel_rate_option_kernel<Side>,
    ...
    check_cuda(cudaGetLastError(), "G2++ rate_option kernel");
}

// Build both public payoff specializations in this CUDA translation unit.
using LaunchSignature = decltype(launch_g2_plus_plus_nelson_siegel_rate_option_cuda<OptionSide::call>);
template LaunchSignature launch_g2_plus_plus_nelson_siegel_rate_option_cuda<OptionSide::call>;
template LaunchSignature launch_g2_plus_plus_nelson_siegel_rate_option_cuda<OptionSide::put>;

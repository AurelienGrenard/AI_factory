// Closed-form G2++ bond options fitted to Nelson-Siegel curves.
...
template<OptionSide Side>
__global__ void g2_plus_plus_nelson_siegel_zero_coupon_bond_option_kernel(
...
void validate_g2_plus_plus_nelson_siegel_zero_coupon_bond_option_launch(
    ...
    if (result_offset >= result_count
        ...
        throw std::invalid_argument(
            "The G2++ bond option launch batch exceeds the result array."
        );
    }
    ...
    const std::size_t thread_count = checked_workspace_product(
        ...
        "The G2++ bond option thread count exceeds size_t."
    );
    if (thread_count < launch_result_count) {
        throw std::invalid_argument(
            "The G2++ bond option launch requires one thread per price."
        );
    }
}
...
void launch_g2_plus_plus_nelson_siegel_zero_coupon_bond_option_cuda(
    ...
    validate_g2_plus_plus_nelson_siegel_zero_coupon_bond_option_launch(
        device_models,
        model_count,
        device_curves,
        curve_count,
    ...
    report_cuda_kernel_launch_if_enabled(
        ...
        g2_plus_plus_nelson_siegel_zero_coupon_bond_option_kernel<Side>,
...
using LaunchSignature = decltype(launch_g2_plus_plus_nelson_siegel_zero_coupon_bond_option_cuda<OptionSide::call>);
template LaunchSignature launch_g2_plus_plus_nelson_siegel_zero_coupon_bond_option_cuda<OptionSide::call>;
template LaunchSignature launch_g2_plus_plus_nelson_siegel_zero_coupon_bond_option_cuda<OptionSide::put>;

...
__device__ __forceinline__ G2State simulate_on_regular_grid(
    ...
    const float first_x_normal = normals.next();
    const float first_y_normal = normals.next();
    one_step_transition(
    ...
    for (std::uint32_t observation = 1U;
         ...
         ++observation) {
        const float state_x_normal = normals.next();
        const float state_y_normal = normals.next();
        one_step_transition(
    ...
    const float terminal_x_normal = normals.next();
    const float terminal_y_normal = normals.next();
    one_step_transition(
...
__device__ __forceinline__ G2JointState simulate_on_regular_grid(
    ...
    const std::uint64_t groups_per_path =
        (3ULL * static_cast<std::uint64_t>(observation_count) + 3ULL) >> 2U;
    philox::NormalSequence normals(
        key, static_cast<std::uint64_t>(path) * groups_per_path
    );

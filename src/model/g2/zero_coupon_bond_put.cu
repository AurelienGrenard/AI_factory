// Closed-form zero-coupon bond puts under the G2 short rate.
...
__global__ void g2_zero_coupon_bond_put_kernel(
...
void validate_g2_zero_coupon_bond_put_launch(
    ...
    if (result_offset >= result_count
        ...
        throw std::invalid_argument(
            "The G2 zero-coupon bond put launch batch exceeds the result array."
        );
    }
    ...
    const std::size_t thread_count = checked_workspace_product(
        ...
        "The G2 zero-coupon bond put thread count exceeds size_t."
    );
    if (thread_count < launch_result_count) {
        throw std::invalid_argument(
            "The G2 zero-coupon bond put launch requires one thread per price."
        );
    }
}
...
void launch_g2_zero_coupon_bond_put_cuda(
    const G2ModelParameters* device_models,
    std::size_t model_count,
    const product::ZeroCouponBondPutParameters* device_products,
    std::size_t product_count,
    ...
    validate_g2_zero_coupon_bond_put_launch(
        device_models,
        model_count,
        device_products,
        product_count,
    ...
    g2_zero_coupon_bond_put_kernel<<<

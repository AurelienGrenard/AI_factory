// Heston Up-no-touch kernel with fused Philox simulation and payoff reduction.
...
__global__ void heston_up_no_touch_kernel(
...
void validate_heston_up_no_touch_launch(
    ...
    if (result_offset >= result_count
        ...
        throw std::invalid_argument(
            "The Heston up-no-touch batch exceeds the result array."
        );
    }
...
void launch_heston_up_no_touch_cuda(
    ...
    validate_heston_up_no_touch_launch(
        device_models,
        model_count,
        device_products,
        product_count,
    ...
    check_cuda(
        cudaOccupancyMaxActiveBlocksPerMultiprocessor(
            ...
            heston_up_no_touch_kernel,
            static_cast<int>(threads_per_block),
            shared_bytes
        ),
        "Heston Up no-touch occupancy check"
    );
    if (active_blocks_per_multiprocessor == 0) {
        throw std::invalid_argument(
            "Heston Up no-touch kernel cannot launch one block per SM."
        );
    }

    // Launch the Heston Up-no-touch kernel.
    heston_up_no_touch_kernel<<<
    ...
    check_cuda(cudaGetLastError(), "Heston Up no-touch kernel");
}

// Heston Forward-start-call kernel with fused Philox simulation and payoff reduction.
...
__device__ __forceinline__ PreparedRow prepare_row(
    ...
    const product::ForwardStartCallParameters& product,
    std::size_t reset_steps,
    std::size_t remaining_steps,
    std::uint64_t seed
) {
...
__global__ void heston_forward_start_call_kernel(
    const HestonModelParameters* __restrict__ models,
    const product::ForwardStartCallParameters* __restrict__ products,
...
void validate_heston_forward_start_call_launch(
    const HestonModelParameters* device_models,
    std::size_t model_count,
    const product::ForwardStartCallParameters* device_products,
    std::size_t product_count,
    bool cartesian_product,
    ...
    if (result_offset >= result_count
        ...
        throw std::invalid_argument(
            "The Heston forward-start-call batch exceeds the result array."
        );
    }
...
void launch_heston_forward_start_call_cuda(
    const HestonModelParameters* device_models,
    std::size_t model_count,
    const product::ForwardStartCallParameters* device_products,
    std::size_t product_count,
    ...
    validate_heston_forward_start_call_launch(
        device_models,
        model_count,
        device_products,
        product_count,
    ...
    check_cuda(
        cudaOccupancyMaxActiveBlocksPerMultiprocessor(
            ...
            heston_forward_start_call_kernel,
            static_cast<int>(threads_per_block),
            shared_bytes
        ),
        "Heston Forward-start call occupancy check"
    );
    if (active_blocks_per_multiprocessor == 0) {
        throw std::invalid_argument(
            "Heston Forward-start call kernel cannot launch one block per SM."
        );
    }

    // Launch the Heston Forward-start-call kernel.
    heston_forward_start_call_kernel<<<
    ...
    check_cuda(cudaGetLastError(), "Heston Forward-start call kernel");
}

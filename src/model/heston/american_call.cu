// Multi-block Heston American-call pricing with GPU Longstaff-Schwartz.
...
std::size_t aligned_offset(std::size_t offset, std::size_t alignment) {
    ...
    if (offset > std::numeric_limits<std::size_t>::max() - padding) {
        throw std::overflow_error("American-call workspace offset exceeds size_t.");
    }
...
std::uint32_t exercise_count(const product::AmericanCallParameters& product) {
    ...
    if (!(count >= 1.0)
        ...
        throw std::overflow_error("American-call exercise count exceeds uint32_t.");
    }
...
std::vector<BatchPlan> plan_batches(
    const product::AmericanCallParameters* host_products,
    ...
    while (result_offset < result_count) {
        ...
        while (result_offset + batch_size < result_count) {
            ...
            const std::size_t row_state_values = checked_workspace_product(
                ...
                "American-call row state count exceeds size_t."
            );
            if (state_value_count
                > std::numeric_limits<std::size_t>::max() - row_state_values) {
                throw std::overflow_error(
                    "American-call batch state count exceeds size_t."
                );
            }
        ...
        if (batch_size == 0U) {
            ...
            throw std::runtime_error(
                "American-call result row " + std::to_string(result_index)
...
__global__ void prepare_rows_kernel(
    ...
    const product::AmericanCallParameters* __restrict__ products,
    std::size_t product_count,
    bool cartesian_product,
    std::size_t batch_size,
    float target_dt,
    ...
    const product::AmericanCallParameters product = products[product_index];
    const std::uint32_t row_exercise_count =
...
__global__ void simulate_paths_kernel(
    ...
    for (std::size_t path = first_path;
        ...
        row_cashflows[path] = fmaxf(terminal_spot - row.strike, 0.0f);
    }
}
...
__global__ void regression_partials_kernel(
    ...
    for (std::size_t path = first_path;
        ...
        const float immediate = fmaxf(spot - row.strike, 0.0f);
...
__global__ void finalize_prices_kernel(
    ...
    const double immediate = static_cast<double>(
        fmaxf(row->initial_spot - row->strike, 0.0f)
    );
    if (immediate > continuation) {
...
void validate_heston_american_call_launch(
    const HestonModelParameters* device_models,
    std::size_t model_count,
    const product::AmericanCallParameters* host_products,
    const product::AmericanCallParameters* device_products,
    std::size_t product_count,
    bool cartesian_product,
    std::size_t result_count,
    std::size_t paths_per_price,
...
AmericanCallLaunchResult launch_heston_american_call_cuda(
    const HestonModelParameters* device_models,
    std::size_t model_count,
    const product::AmericanCallParameters* host_products,
    const product::AmericanCallParameters* device_products,
    ...
    validate_heston_american_call_launch(
        device_models,
        model_count,
        host_products,
        device_products,
    ...
    check_cuda(
        ...
        "cudaMemGetInfo American-call workspace"
    );
    ...
    if (maximum_prices_per_batch
        ...
        throw std::overflow_error(
            "American-call batch exceeds the current gridDim.y limit."
        );
    }
    ...
    try {
        check_cuda(
            ...
            "cudaMalloc American-call workspace"
        );
        ...
        for (const BatchPlan& batch : batches) {
            ...
            if (state_value_cursor != batch.state_value_count) {
                throw std::logic_error(
                    "American-call batch state count changed after planning."
                );
            }
            ...
            check_cuda(
                ...
                "cudaMemcpy American-call exercise counts"
            );
            check_cuda(
                cudaMemcpy(
                ...
                "cudaMemcpy American-call state offsets"
            );
            ...
            check_cuda(cudaGetLastError(), "prepare American-call rows");
            ++kernel_launch_count;
            ...
            check_cuda(cudaGetLastError(), "simulate American-call paths");
            ++kernel_launch_count;
            ...
            for (std::uint32_t backward_level = 1U;
                ...
                check_cuda(
                    cudaGetLastError(), "American-call regression partials"
                );
                ...
                check_cuda(
                    cudaGetLastError(), "solve American-call regressions"
                );
                ...
                check_cuda(
                    cudaGetLastError(), "update American-call cashflows"
                );
            ...
            check_cuda(cudaGetLastError(), "American-call moment partials");
            ++kernel_launch_count;
            ...
            check_cuda(cudaGetLastError(), "finalize American-call prices");
            ++kernel_launch_count;
            ...
            check_cuda(
                ...
                "cudaEventElapsedTime American-call batch"
            );
            kernel_seconds +=
        ...
        check_cuda(
            cudaFree(device_workspace), "cudaFree American-call workspace"
        );
    } catch (...) {

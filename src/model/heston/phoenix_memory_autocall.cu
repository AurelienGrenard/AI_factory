// Heston Phoenix-Memory-autocall kernel with fused Philox simulation and payoff reduction.
...
// ---------------------- Phoenix-Memory-autocall payoff -----------------------
...
__device__ __forceinline__ float evaluate_path(
    ...
    const std::uint64_t total_step_count =
        static_cast<std::uint64_t>(row.observation_count)
        * row.steps_per_observation;
    const std::uint64_t groups_per_path =
        (kUniformsPerStep * total_step_count + 3ULL) >> 2U;
...
__global__ void heston_phoenix_memory_autocall_kernel(
...
void validate_heston_phoenix_memory_autocall_launch(
    ...
    if (result_offset >= result_count
        ...
        throw std::invalid_argument(
            "The Heston Phoenix-Memory-autocall launch batch exceeds the result array."
        );
    }
...
void launch_heston_phoenix_memory_autocall_cuda(
    const HestonModelParameters* device_models,
    std::size_t model_count,
    const product::PhoenixMemoryAutocallParameters* device_products,
    std::size_t product_count,
    ...
    validate_heston_phoenix_memory_autocall_launch(
        device_models,
        model_count,
        device_products,
        product_count,
    ...
    check_cuda(
        cudaOccupancyMaxActiveBlocksPerMultiprocessor(
            ...
            heston_phoenix_memory_autocall_kernel,
            static_cast<int>(threads_per_block),
            shared_bytes
        ),
        "Heston Phoenix Memory autocall occupancy check"
    );
    if (active_blocks_per_multiprocessor == 0) {
        throw std::invalid_argument(
            "Heston Phoenix Memory autocall kernel cannot launch one block per SM."
        );
    }

    // Launch the Heston Phoenix-Memory-autocall kernel.
    ...
    check_cuda(cudaGetLastError(), "Heston Phoenix Memory autocall kernel");
}

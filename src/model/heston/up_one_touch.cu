// Heston Up-one-touch kernel with fused Philox simulation and payoff reduction.
...
__global__ void heston_up_one_touch_kernel(
...
void validate_heston_up_one_touch_launch(
    ...
    if (result_offset >= result_count
        ...
        throw std::invalid_argument(
            "The Heston up-one-touch batch exceeds the result array."
        );
    }
...
void launch_heston_up_one_touch_cuda(
    const HestonModelParameters* device_models,
    std::size_t model_count,
    const product::UpOneTouchParameters* device_products,
    std::size_t product_count,
    ...
    validate_heston_up_one_touch_launch(
        device_models,
        model_count,
        device_products,
        product_count,
    ...
    check_cuda(
        ...
        "Heston Up one-touch occupancy check"
    );
    if (active_blocks_per_multiprocessor == 0) {
        throw std::invalid_argument(
            "Heston Up one-touch kernel cannot launch one block per SM."
        );
    }
    ...
    check_cuda(cudaGetLastError(), "Heston Up one-touch kernel");
}

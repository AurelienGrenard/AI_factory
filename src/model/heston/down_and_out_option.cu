// Heston Down-and-out option kernel with fused Philox simulation and payoff reduction.
...
__device__ __forceinline__ float evaluate_path(
    ...
    const std::uint64_t uniform_count =
        kUniformsPerStep * static_cast<std::uint64_t>(row.num_steps);
    const std::uint64_t groups_per_path = (uniform_count + 3ULL) >> 2U;
    philox::UniformSequence uniforms(
        row.key, static_cast<std::uint64_t>(path) * groups_per_path
    );
    for (std::size_t step_index = 0U;
         step_index < row.num_steps;
        ++step_index) {
        simulate_one_step(row.model, uniforms, state);
...
template<OptionSide Side>
__global__ void heston_down_and_out_option_kernel(
...
void validate_heston_down_and_out_option_launch(
    ...
    if (result_offset >= result_count
        ...
        throw std::invalid_argument(
            "The Heston Down-and-out option launch batch exceeds the result array."
        );
    }
...
template<OptionSide Side>
void launch_heston_down_and_out_option_cuda(
    const HestonModelParameters* device_models,
    std::size_t model_count,
    const product::DownAndOutOptionParameters* device_products,
    std::size_t product_count,
    ...
    validate_heston_down_and_out_option_launch(
        device_models,
        model_count,
        device_products,
        product_count,
    ...
    check_cuda(
        cudaOccupancyMaxActiveBlocksPerMultiprocessor(
            ...
            heston_down_and_out_option_kernel<Side>,
            static_cast<int>(threads_per_block),
            shared_bytes
        ),
        "Heston Down-and-out option occupancy check"
    );
    if (active_blocks_per_multiprocessor == 0) {
        throw std::invalid_argument(
            "Heston Down-and-out option kernel cannot launch one block per SM."
        );
    }

    // Launch the Heston Down-and-out option kernel.
    ...
    check_cuda(cudaGetLastError(), "Heston Down-and-out option kernel");
}

// Build both public payoff specializations in this CUDA translation unit.
using LaunchSignature = decltype(launch_heston_down_and_out_option_cuda<OptionSide::call>);
template LaunchSignature launch_heston_down_and_out_option_cuda<OptionSide::call>;
template LaunchSignature launch_heston_down_and_out_option_cuda<OptionSide::put>;

// Heston straddle kernel with fused Philox simulation and payoff reduction.
...
__global__ void heston_straddle_kernel(
...
void validate_heston_straddle_launch(
    ...
    if (result_offset >= result_count
        ...
        throw std::invalid_argument(
            "The Heston straddle launch batch exceeds the result array."
        );
    }
...
void launch_heston_straddle_cuda(
    ...
    validate_heston_straddle_launch(
        device_models,
        model_count,
        device_products,
        product_count,
    ...
    check_cuda(
        cudaOccupancyMaxActiveBlocksPerMultiprocessor(
            ...
            heston_straddle_kernel,
            static_cast<int>(threads_per_block),
            shared_bytes
        ),
        "Heston straddle occupancy check"
    );
    if (active_blocks_per_multiprocessor == 0) {
        throw std::invalid_argument(
            "Heston straddle kernel cannot launch one block per SM."
        );
    }

    // Launch the Heston Straddle kernel.
    heston_straddle_kernel<<<
    ...
    check_cuda(cudaGetLastError(), "Heston straddle kernel");
}

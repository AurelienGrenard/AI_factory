// Heston digital-call kernel with fused Philox simulation and payoff reduction.
...
__device__ __forceinline__ PreparedRow prepare_row(
    ...
    const product::DigitalCallParameters& product,
    std::size_t num_steps,
    std::uint64_t seed
) {
...
__global__ void heston_digital_call_kernel(
    const HestonModelParameters* __restrict__ models,
    const product::DigitalCallParameters* __restrict__ products,
...
void validate_heston_digital_call_launch(
    const HestonModelParameters* device_models,
    std::size_t model_count,
    const product::DigitalCallParameters* device_products,
    std::size_t product_count,
    bool cartesian_product,
    ...
    if (result_offset >= result_count
        ...
        throw std::invalid_argument(
            "The Heston digital-call launch batch exceeds the result array."
        );
    }
...
void launch_heston_digital_call_cuda(
    const HestonModelParameters* device_models,
    std::size_t model_count,
    const product::DigitalCallParameters* device_products,
    std::size_t product_count,
    ...
    validate_heston_digital_call_launch(
        device_models,
        model_count,
        device_products,
        product_count,
    ...
    check_cuda(
        cudaOccupancyMaxActiveBlocksPerMultiprocessor(
            ...
            heston_digital_call_kernel,
            static_cast<int>(threads_per_block),
            shared_bytes
        ),
        "Heston Digital call occupancy check"
    );
    if (active_blocks_per_multiprocessor == 0) {
        throw std::invalid_argument(
            "Heston Digital call kernel cannot launch one block per SM."
        );
    }

    // Launch the Heston Digital-call kernel.
    heston_digital_call_kernel<<<
    ...
    check_cuda(cudaGetLastError(), "Heston Digital call kernel");
}

// Heston Geometric-Asian-put kernel with fused Philox simulation and payoff reduction.
...
__device__ __forceinline__ PreparedRow prepare_row(
    ...
    const product::GeometricAsianPutParameters& product,
    std::size_t num_steps,
    std::uint64_t seed
) {
...
__global__ void heston_geometric_asian_put_kernel(
    const HestonModelParameters* __restrict__ models,
    const product::GeometricAsianPutParameters* __restrict__ products,
...
void validate_heston_geometric_asian_put_launch(
    const HestonModelParameters* device_models,
    std::size_t model_count,
    const product::GeometricAsianPutParameters* device_products,
    std::size_t product_count,
    bool cartesian_product,
    ...
    if (result_offset >= result_count
        ...
        throw std::invalid_argument(
            "The Heston Geometric-Asian-put launch batch exceeds the result array."
        );
    }
...
void launch_heston_geometric_asian_put_cuda(
    const HestonModelParameters* device_models,
    std::size_t model_count,
    const product::GeometricAsianPutParameters* device_products,
    std::size_t product_count,
    ...
    validate_heston_geometric_asian_put_launch(
        device_models,
        model_count,
        device_products,
        product_count,
    ...
    check_cuda(
        cudaOccupancyMaxActiveBlocksPerMultiprocessor(
            ...
            heston_geometric_asian_put_kernel,
            static_cast<int>(threads_per_block),
            shared_bytes
        ),
        "Heston Geometric Asian put occupancy check"
    );
    if (active_blocks_per_multiprocessor == 0) {
        throw std::invalid_argument(
            "Heston Geometric Asian put kernel cannot launch one block per SM."
        );
    }

    // Launch the Heston Geometric-Asian-put kernel.
    heston_geometric_asian_put_kernel<<<
    ...
    check_cuda(cudaGetLastError(), "Heston Geometric Asian put kernel");
}

// Heston Double-knock-out-put kernel with fused Philox simulation and payoff reduction.
...
__device__ __forceinline__ PreparedRow prepare_row(
    ...
    const product::DoubleKnockOutPutParameters& product,
    std::size_t num_steps,
    std::uint64_t seed
) {
...
__global__ void heston_double_knock_out_put_kernel(
    const HestonModelParameters* __restrict__ models,
    const product::DoubleKnockOutPutParameters* __restrict__ products,
...
void validate_heston_double_knock_out_put_launch(
    const HestonModelParameters* device_models,
    std::size_t model_count,
    const product::DoubleKnockOutPutParameters* device_products,
    std::size_t product_count,
    bool cartesian_product,
    ...
    if (result_offset >= result_count
        ...
        throw std::invalid_argument(
            "The Heston Double-knock-out-put launch batch exceeds the result array."
        );
    }
...
void launch_heston_double_knock_out_put_cuda(
    const HestonModelParameters* device_models,
    std::size_t model_count,
    const product::DoubleKnockOutPutParameters* device_products,
    std::size_t product_count,
    ...
    validate_heston_double_knock_out_put_launch(
        device_models,
        model_count,
        device_products,
        product_count,
    ...
    check_cuda(
        cudaOccupancyMaxActiveBlocksPerMultiprocessor(
            ...
            heston_double_knock_out_put_kernel,
            static_cast<int>(threads_per_block),
            shared_bytes
        ),
        "Heston Double-knock-out put occupancy check"
    );
    if (active_blocks_per_multiprocessor == 0) {
        throw std::invalid_argument(
            "Heston Double-knock-out put kernel cannot launch one block per SM."
        );
    }

    // Launch the Heston Double-knock-out-put kernel.
    heston_double_knock_out_put_kernel<<<
    ...
    check_cuda(cudaGetLastError(), "Heston Double-knock-out put kernel");
}

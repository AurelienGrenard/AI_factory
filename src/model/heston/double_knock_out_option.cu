// Heston Double-knock-out option kernel with fused Philox simulation and payoff reduction.
...
template<OptionSide Side>
__global__ void heston_double_knock_out_option_kernel(
...
void validate_heston_double_knock_out_option_launch(
    ...
    if (result_offset >= result_count
        ...
        throw std::invalid_argument(
            "The Heston double-knock-out option batch exceeds the result array."
        );
    }
...
template<OptionSide Side>
void launch_heston_double_knock_out_option_cuda(
    const HestonModelParameters* device_models,
    std::size_t model_count,
    const product::DoubleKnockOutOptionParameters* device_products,
    std::size_t product_count,
    ...
    validate_heston_double_knock_out_option_launch(
        device_models,
        model_count,
        device_products,
        product_count,
    ...
    check_cuda(
        cudaOccupancyMaxActiveBlocksPerMultiprocessor(
            ...
            heston_double_knock_out_option_kernel<Side>,
            static_cast<int>(threads_per_block),
            shared_bytes
        ),
        "Heston Double-knock-out option occupancy check"
    );
    if (active_blocks_per_multiprocessor == 0) {
        throw std::invalid_argument(
            "Heston Double-knock-out option kernel cannot launch one block per SM."
        );
    }

    // Launch the Heston Double-knock-out option kernel.
    ...
    check_cuda(cudaGetLastError(), "Heston Double-knock-out option kernel");
}

// Build both public payoff specializations in this CUDA translation unit.
using LaunchSignature = decltype(launch_heston_double_knock_out_option_cuda<OptionSide::call>);
template LaunchSignature launch_heston_double_knock_out_option_cuda<OptionSide::call>;
template LaunchSignature launch_heston_double_knock_out_option_cuda<OptionSide::put>;

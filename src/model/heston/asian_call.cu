// Heston Asian-call kernel with fused Philox simulation and payoff reduction.
...
__device__ __forceinline__ PreparedRow prepare_row(
    ...
    const product::AsianCallParameters& product,
    std::size_t num_steps,
    std::uint64_t seed
) {
...
__global__ void heston_asian_call_kernel(
    const HestonModelParameters* __restrict__ models,
    const product::AsianCallParameters* __restrict__ products,
...
void validate_heston_asian_call_launch(
    ...
    if (result_offset >= result_count
        ...
        throw std::invalid_argument(
            "The Heston Asian-call launch batch exceeds the result array."
        );
    }
...
void launch_heston_asian_call_cuda(
    ...
    validate_heston_asian_call_launch(
        device_models,
        model_count,
        device_products,
        product_count,
    ...
    check_cuda(
        cudaOccupancyMaxActiveBlocksPerMultiprocessor(
            ...
            heston_asian_call_kernel,
            static_cast<int>(threads_per_block),
            shared_bytes
        ),
        "Heston Asian call occupancy check"
    );
    if (active_blocks_per_multiprocessor == 0) {
        throw std::invalid_argument(
            "Heston Asian call kernel cannot launch one block per SM."
        );
    }

    // Launch the Heston Asian-call kernel.
    heston_asian_call_kernel<<<
    ...
    check_cuda(cudaGetLastError(), "Heston Asian call kernel");
}

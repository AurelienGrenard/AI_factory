// Heston Cliquet kernel with fused Philox simulation and payoff reduction.
...
__global__ void heston_cliquet_kernel(
...
void validate_heston_cliquet_launch(
    ...
    if (result_offset >= result_count
        ...
        throw std::invalid_argument(
            "The Heston Cliquet launch batch exceeds the result array."
        );
    }
...
void launch_heston_cliquet_cuda(
    const HestonModelParameters* device_models,
    std::size_t model_count,
    const product::CliquetParameters* device_products,
    std::size_t product_count,
    ...
    validate_heston_cliquet_launch(
        device_models,
        model_count,
        device_products,
        product_count,
    ...
    check_cuda(
        ...
        "Heston Cliquet occupancy check"
    );
    if (active_blocks_per_multiprocessor == 0) {
        throw std::invalid_argument(
            "Heston Cliquet kernel cannot launch one block per SM."
        );
    }
    ...
    check_cuda(cudaGetLastError(), "Heston Cliquet kernel");
}

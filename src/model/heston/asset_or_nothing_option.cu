// Heston asset-or-nothing kernel with fused simulation and payoff reduction.
...
template<OptionSide Side>
__global__ void heston_asset_or_nothing_option_kernel(
...
void validate_heston_asset_or_nothing_option_launch(
    ...
    if (result_offset >= result_count
        ...
        throw std::invalid_argument(
            "The Heston asset-or-nothing launch batch exceeds the result array."
        );
    }
...
void launch_heston_asset_or_nothing_option_cuda(
    ...
    validate_heston_asset_or_nothing_option_launch(
        device_models,
        model_count,
        device_products,
        product_count,
    ...
    check_cuda(
        cudaOccupancyMaxActiveBlocksPerMultiprocessor(
            ...
            heston_asset_or_nothing_option_kernel<Side>,
            static_cast<int>(threads_per_block),
            shared_bytes
        ),
        "Heston asset-or-nothing option occupancy check"
    );
    if (active_blocks_per_multiprocessor == 0) {
        throw std::invalid_argument(
            "Heston asset-or-nothing option kernel cannot launch one block per SM."
        );
    }

    // Launch the Heston Asset-or-nothing kernel.
    ...
    check_cuda(cudaGetLastError(), "Heston asset-or-nothing option kernel");
}

// Build both public payoff specializations in this CUDA translation unit.
using LaunchSignature = decltype(launch_heston_asset_or_nothing_option_cuda<OptionSide::call>);
template LaunchSignature launch_heston_asset_or_nothing_option_cuda<OptionSide::call>;
template LaunchSignature launch_heston_asset_or_nothing_option_cuda<OptionSide::put>;

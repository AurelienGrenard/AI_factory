...
__device__ __forceinline__ PreparedRow prepare_row(
    ...
    const product::AssetOrNothingPutParameters& product,
    std::size_t num_steps,
    std::uint64_t seed
) {
...
__global__ void heston_asset_or_nothing_put_kernel(
    const HestonModelParameters* __restrict__ models,
    const product::AssetOrNothingPutParameters* __restrict__ products,
...
void validate_heston_asset_or_nothing_put_launch(
    const HestonModelParameters* device_models,
    std::size_t model_count,
    const product::AssetOrNothingPutParameters* device_products,
    std::size_t product_count,
    bool cartesian_product,
...
void launch_heston_asset_or_nothing_put_cuda(
    const HestonModelParameters* device_models,
    std::size_t model_count,
    const product::AssetOrNothingPutParameters* device_products,
    std::size_t product_count,
    ...
    validate_heston_asset_or_nothing_put_launch(
        device_models,
        model_count,
        device_products,
        product_count,
    ...
    check_cuda(
        cudaOccupancyMaxActiveBlocksPerMultiprocessor(
            ...
            heston_asset_or_nothing_put_kernel,
            static_cast<int>(threads_per_block),
            shared_bytes
        ),
        "Heston asset-or-nothing put occupancy check"
    );
    if (active_blocks_per_multiprocessor == 0) {
        throw std::invalid_argument(
            "Heston asset-or-nothing put kernel cannot launch one block per SM."
        );
    }
    ...
    check_cuda(cudaGetLastError(), "Heston asset-or-nothing put kernel");
}

// Closed-form G2++ bond puts fitted to Svensson curves.
#include "model/g2_plus_plus/svensson/zero_coupon_bond_put.cuh"
...
__global__ void g2_plus_plus_svensson_zero_coupon_bond_put_kernel(
...
void validate_g2_plus_plus_svensson_zero_coupon_bond_put_launch(
    ...
    if (result_offset >= result_count
        ...
        throw std::invalid_argument(
            "The G2++ bond put launch batch exceeds the result array."
        );
    }
    ...
    const std::size_t thread_count = checked_workspace_product(
        ...
        "The G2++ bond put thread count exceeds size_t."
    );
    if (thread_count < launch_result_count) {
        throw std::invalid_argument(
            "The G2++ bond put launch requires one thread per price."
        );
    }
}
...
void launch_g2_plus_plus_svensson_zero_coupon_bond_put_cuda(
    const G2PlusPlusModelParameters* device_models,
    std::size_t model_count,
    const curve::svensson::SvenssonParameters* device_curves,
    std::size_t curve_count,
    ...
    validate_g2_plus_plus_svensson_zero_coupon_bond_put_launch(
        device_models,
        model_count,
        device_curves,
        curve_count,
    ...
    g2_plus_plus_svensson_zero_coupon_bond_put_kernel<<<
    ...
    check_cuda(cudaGetLastError(), "G2++ zero-coupon bond put kernel");
}

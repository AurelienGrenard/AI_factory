// Closed-form G2++ caplet pricing fitted to Nelson-Siegel curves.
...
__global__ void g2_plus_plus_nelson_siegel_caplet_kernel(
...
void validate_g2_plus_plus_nelson_siegel_caplet_launch(
    ...
    if (result_offset >= result_count
        ...
        throw std::invalid_argument(
            "The G2++ caplet launch batch exceeds the result array."
        );
    }
    ...
    const std::size_t thread_count = checked_workspace_product(
        ...
        "The G2++ caplet thread count exceeds size_t."
    );
    if (thread_count < launch_result_count) {
        throw std::invalid_argument(
            "The G2++ caplet launch requires one thread per price."
        );
    }
}
...
void launch_g2_plus_plus_nelson_siegel_caplet_cuda(
    const G2PlusPlusModelParameters* device_models,
    std::size_t model_count,
    const curve::nelson_siegel::NelsonSiegelParameters* device_curves,
    std::size_t curve_count,
    ...
    validate_g2_plus_plus_nelson_siegel_caplet_launch(
        device_models,
        model_count,
        device_curves,
        curve_count,
    ...
    g2_plus_plus_nelson_siegel_caplet_kernel<<<
    ...
    check_cuda(cudaGetLastError(), "G2++ caplet kernel");
}

// Closed-form Hull-White bond options fitted to Nelson-Siegel curves.
#include "model/hull_white/nelson_siegel/zero_coupon_bond_option.cuh"
...
template<OptionSide Side>
__global__ void hull_white_nelson_siegel_zero_coupon_bond_option_kernel(
...
void validate_hull_white_nelson_siegel_zero_coupon_bond_option_launch(
    ...
    if (result_offset >= result_count
        ...
        throw std::invalid_argument(
            "The Hull-White bond option launch batch exceeds the result array."
        );
    }
    ...
    const std::size_t thread_count = checked_workspace_product(
        ...
        "The Hull-White bond option thread count exceeds size_t."
    );
    if (thread_count < launch_result_count) {
        throw std::invalid_argument(
            "The Hull-White bond option launch requires one thread per price."
        );
    }
}
...
void launch_hull_white_nelson_siegel_zero_coupon_bond_option_cuda(
    ...
    validate_hull_white_nelson_siegel_zero_coupon_bond_option_launch(
        device_models,
        model_count,
        device_curves,
        curve_count,
    ...
    // Launch one analytical zero-coupon bond option per CUDA thread.
    report_cuda_kernel_launch_if_enabled(
        "hull_white.nelson_siegel.zero_coupon_bond_option",
        option_side_name(Side),
        hull_white_nelson_siegel_zero_coupon_bond_option_kernel<Side>,
    ...
    check_cuda(cudaGetLastError(), "Hull-White zero-coupon bond option kernel");
}

// Build both public payoff specializations in this CUDA translation unit.
using LaunchSignature = decltype(launch_hull_white_nelson_siegel_zero_coupon_bond_option_cuda<OptionSide::call>);
template LaunchSignature launch_hull_white_nelson_siegel_zero_coupon_bond_option_cuda<OptionSide::call>;
template LaunchSignature launch_hull_white_nelson_siegel_zero_coupon_bond_option_cuda<OptionSide::put>;

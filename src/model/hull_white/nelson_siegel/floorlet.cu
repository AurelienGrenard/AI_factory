// Closed-form Hull-White floorlet pricing fitted to Nelson-Siegel curves.
#include "model/hull_white/nelson_siegel/floorlet.cuh"
...
__global__ void hull_white_nelson_siegel_floorlet_kernel(
...
void validate_hull_white_nelson_siegel_floorlet_launch(
    ...
    if (result_offset >= result_count
        ...
        throw std::invalid_argument(
            "The Hull-White floorlet launch batch exceeds the result array."
        );
    }
    ...
    const std::size_t thread_count = checked_workspace_product(
        ...
        "The Hull-White floorlet thread count exceeds size_t."
    );
    if (thread_count < launch_result_count) {
        throw std::invalid_argument(
            "The Hull-White floorlet launch requires one thread per price."
        );
    }
}
...
void launch_hull_white_nelson_siegel_floorlet_cuda(
    const HullWhiteModelParameters* device_models,
    std::size_t model_count,
    const curve::nelson_siegel::NelsonSiegelParameters* device_curves,
    std::size_t curve_count,
    ...
    validate_hull_white_nelson_siegel_floorlet_launch(
        device_models,
        model_count,
        device_curves,
        curve_count,
    ...
    hull_white_nelson_siegel_floorlet_kernel<<<
    ...
    check_cuda(cudaGetLastError(), "Hull-White floorlet kernel");
}

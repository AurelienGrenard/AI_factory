// Closed-form Hull-White bond puts fitted to Svensson curves.
#include "model/hull_white/svensson/zero_coupon_bond_put.cuh"
...
__global__ void hull_white_svensson_zero_coupon_bond_put_kernel(
...
void validate_hull_white_svensson_zero_coupon_bond_put_launch(
    ...
    if (result_offset >= result_count
        ...
        throw std::invalid_argument(
            "The Hull-White bond put launch batch exceeds the result array."
        );
    }
    ...
    const std::size_t thread_count = checked_workspace_product(
        ...
        "The Hull-White bond put thread count exceeds size_t."
    );
    if (thread_count < launch_result_count) {
        throw std::invalid_argument(
            "The Hull-White bond put launch requires one thread per price."
        );
    }
}
...
void launch_hull_white_svensson_zero_coupon_bond_put_cuda(
    const HullWhiteModelParameters* device_models,
    std::size_t model_count,
    const curve::svensson::SvenssonParameters* device_curves,
    std::size_t curve_count,
    ...
    validate_hull_white_svensson_zero_coupon_bond_put_launch(
        device_models,
        model_count,
        device_curves,
        curve_count,
    ...
    // Launch one analytical zero-coupon bond put per CUDA thread.
    hull_white_svensson_zero_coupon_bond_put_kernel<<<
    ...
    check_cuda(cudaGetLastError(), "Hull-White zero-coupon bond put kernel");
}

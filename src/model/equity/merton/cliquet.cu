// Merton Cliquet kernel with fused Philox simulation and payoff reduction.
...
void validate_merton_cliquet_launch(
    ...
    if (result_offset >= result_count
        ...
        throw std::invalid_argument(
            "The Merton Cliquet launch batch exceeds the result array."
        );
    }
...
void launch_merton_cliquet_cuda(
    ...
    validate_merton_cliquet_launch(
        device_models,
        model_count,
        device_products,
        product_count,
    ...
    check_cuda(
        cudaOccupancyMaxActiveBlocksPerMultiprocessor(
            ...
            merton_cliquet_kernel,
            static_cast<int>(threads_per_block),
            shared_bytes
        ),
        "Merton Cliquet occupancy check"
    );
    if (active_blocks_per_multiprocessor == 0) {
        throw std::invalid_argument(
            "Merton Cliquet kernel cannot launch one block per SM."
        );
    }

    // Launch the Merton Cliquet kernel.
    ...
    check_cuda(cudaGetLastError(), "Merton Cliquet kernel");
}

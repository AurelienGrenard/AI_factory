// Merton straddle kernel with fused Philox simulation and payoff reduction.
...
__global__ void merton_straddle_kernel(
...
void validate_merton_straddle_launch(
    ...
    if (result_offset >= result_count
        ...
        throw std::invalid_argument(
            "The Merton straddle launch batch exceeds the result array."
        );
    }
...
void launch_merton_straddle_cuda(
    ...
    validate_merton_straddle_launch(
        device_models,
        model_count,
        device_products,
        product_count,
    ...
    check_cuda(
        cudaOccupancyMaxActiveBlocksPerMultiprocessor(
            ...
            merton_straddle_kernel,
            static_cast<int>(threads_per_block),
            shared_bytes
        ),
        "Merton straddle occupancy check"
    );
    if (active_blocks_per_multiprocessor == 0) {
        throw std::invalid_argument(
            "Merton straddle kernel cannot launch one block per SM."
        );
    }

    // Launch the Merton Straddle kernel.
    ...
    check_cuda(cudaGetLastError(), "Merton straddle kernel");
}

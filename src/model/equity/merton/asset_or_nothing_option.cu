// Merton asset-or-nothing kernel with fused simulation and payoff reduction.
...
template<OptionSide Side>
__global__ void merton_asset_or_nothing_option_kernel(
...
void validate_merton_asset_or_nothing_option_launch(
    ...
    if (result_offset >= result_count
        ...
        throw std::invalid_argument(
            "The Merton asset-or-nothing launch batch exceeds the result array."
        );
    }
...
template<OptionSide Side>
void launch_merton_asset_or_nothing_option_cuda(
    const MertonModelParameters* device_models,
    std::size_t model_count,
    const product::AssetOrNothingOptionParameters* device_products,
    std::size_t product_count,
    ...
    validate_merton_asset_or_nothing_option_launch(
        device_models,
        model_count,
        device_products,
        product_count,
    ...
    check_cuda(
        cudaOccupancyMaxActiveBlocksPerMultiprocessor(
            ...
            merton_asset_or_nothing_option_kernel<Side>,
            static_cast<int>(threads_per_block),
            shared_bytes
        ),
        "Merton asset-or-nothing option occupancy check"
    );
    if (active_blocks_per_multiprocessor == 0) {
        throw std::invalid_argument(
            "Merton asset-or-nothing option kernel cannot launch one block per SM."
        );
    }

    // Launch the Merton Asset-or-nothing kernel.
    ...
    check_cuda(cudaGetLastError(), "Merton asset-or-nothing option kernel");
}

// Build both public payoff specializations in this CUDA translation unit.
using LaunchSignature = decltype(launch_merton_asset_or_nothing_option_cuda<OptionSide::call>);
template LaunchSignature launch_merton_asset_or_nothing_option_cuda<OptionSide::call>;
template LaunchSignature launch_merton_asset_or_nothing_option_cuda<OptionSide::put>;

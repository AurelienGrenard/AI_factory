// Merton european-option composition over generic CUDA layers.
...
void launch_merton_european_option_cuda(
    ...
    equity::launch_monte_carlo_cuda<PricingPolicy<Side>>(
        ...
        "Merton european option kernel"
    );
}

template void launch_merton_european_option_cuda<OptionSide::call>(
...
template void launch_merton_european_option_cuda<OptionSide::put>(
    const ModelParameters*, std::size_t,
    const product::EuropeanOptionParameters*, std::size_t,
    bool, std::size_t, std::size_t, std::size_t, std::size_t,
    float, unsigned int, std::size_t, std::uint64_t,

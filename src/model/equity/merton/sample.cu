// Persistent merton terminal and regular-calendar sample kernels.
#include "model/equity/merton/sample.cuh"
...
__global__ void merton_terminal_samples_kernel(
...
__global__ void merton_calendar_samples_kernel(
    const ModelParameters* __restrict__ models,
    std::size_t paths_per_model,
...
std::size_t validate_merton_sample_launch(
...
void launch_merton_terminal_samples_cuda(
    ...
    report_cuda_kernel_launch_if_enabled(
        "merton.samples",
        "terminal",
        merton_terminal_samples_kernel,
    ...
    check_cuda(cudaGetLastError(), "merton terminal sample kernel");
}
...
void launch_merton_calendar_samples_cuda(
    ...
    report_cuda_kernel_launch_if_enabled(
        "merton.samples",
        "calendar",
        merton_calendar_samples_kernel,
        dim3(static_cast<unsigned int>(block_count)),
        dim3(threads_per_block)
    );
    merton_calendar_samples_kernel<<<
    ...
    check_cuda(cudaGetLastError(), "merton calendar sample kernel");
}

// Merton digital-option kernel with fused Philox simulation and payoff reduction.
...
template<OptionSide Side>
__global__ void merton_digital_option_kernel(
...
void validate_merton_digital_option_launch(
    ...
    if (result_offset >= result_count
        ...
        throw std::invalid_argument(
            "The Merton digital-option launch batch exceeds the result array."
        );
    }
...
void launch_merton_digital_option_cuda(
    ...
    validate_merton_digital_option_launch(
        device_models,
        model_count,
        device_products,
        product_count,
    ...
    check_cuda(
        cudaOccupancyMaxActiveBlocksPerMultiprocessor(
            ...
            merton_digital_option_kernel<Side>,
            static_cast<int>(threads_per_block),
            shared_bytes
        ),
        "Merton Digital option occupancy check"
    );
    if (active_blocks_per_multiprocessor == 0) {
        throw std::invalid_argument(
            "Merton Digital option kernel cannot launch one block per SM."
        );
    }

    // Launch the Merton Digital-option kernel.
    ...
    check_cuda(cudaGetLastError(), "Merton Digital option kernel");
}

// Build both public payoff specializations in this CUDA translation unit.
using LaunchSignature = decltype(launch_merton_digital_option_cuda<OptionSide::call>);
template LaunchSignature launch_merton_digital_option_cuda<OptionSide::call>;
template LaunchSignature launch_merton_digital_option_cuda<OptionSide::put>;

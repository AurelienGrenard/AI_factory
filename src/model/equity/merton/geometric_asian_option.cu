// Merton Geometric-Asian option kernel with fused Philox simulation and payoff reduction.
...
__device__ __forceinline__ float evaluate_path(
    ...
    const MertonGeometricMeanPathResult simulated =
...
template<OptionSide Side>
__global__ void merton_geometric_asian_option_kernel(
...
void validate_merton_geometric_asian_option_launch(
    ...
    if (result_offset >= result_count
        ...
        throw std::invalid_argument(
            "The Merton Geometric-Asian option launch batch exceeds the result array."
        );
    }
...
void launch_merton_geometric_asian_option_cuda(
    ...
    validate_merton_geometric_asian_option_launch(
        device_models,
        model_count,
        device_products,
        product_count,
    ...
    check_cuda(
        cudaOccupancyMaxActiveBlocksPerMultiprocessor(
            ...
            merton_geometric_asian_option_kernel<Side>,
            static_cast<int>(threads_per_block),
            shared_bytes
        ),
        "Merton Geometric Asian option occupancy check"
    );
    if (active_blocks_per_multiprocessor == 0) {
        throw std::invalid_argument(
            "Merton Geometric Asian option kernel cannot launch one block per SM."
        );
    }

    // Launch the Merton Geometric-Asian option kernel.
    ...
    check_cuda(cudaGetLastError(), "Merton Geometric Asian option kernel");
}

// Build both public payoff specializations in this CUDA translation unit.
using LaunchSignature = decltype(launch_merton_geometric_asian_option_cuda<OptionSide::call>);
template LaunchSignature launch_merton_geometric_asian_option_cuda<OptionSide::call>;
template LaunchSignature launch_merton_geometric_asian_option_cuda<OptionSide::put>;

// Merton Up-one-touch kernel with fused Philox simulation and payoff reduction.
...
__global__ void merton_up_one_touch_kernel(
...
void validate_merton_up_one_touch_launch(
    ...
    if (result_offset >= result_count
        ...
        throw std::invalid_argument(
            "The Merton up-one-touch batch exceeds the result array."
        );
    }
...
void launch_merton_up_one_touch_cuda(
    ...
    validate_merton_up_one_touch_launch(
        device_models,
        model_count,
        device_products,
        product_count,
    ...
    check_cuda(
        cudaOccupancyMaxActiveBlocksPerMultiprocessor(
            ...
            merton_up_one_touch_kernel,
            static_cast<int>(threads_per_block),
            shared_bytes
        ),
        "Merton Up one-touch occupancy check"
    );
    if (active_blocks_per_multiprocessor == 0) {
        throw std::invalid_argument(
            "Merton Up one-touch kernel cannot launch one block per SM."
        );
    }

    // Launch the Merton Up-one-touch kernel.
    ...
    check_cuda(cudaGetLastError(), "Merton Up one-touch kernel");
}

// Merton Range Accrual kernel with fused Philox simulation and payoff reduction.
...
void validate_merton_range_accrual_launch(
    ...
    if (result_offset >= result_count
        ...
        throw std::invalid_argument(
            "The Merton Range Accrual launch batch exceeds the result array."
        );
    }
...
void launch_merton_range_accrual_cuda(
    ...
    validate_merton_range_accrual_launch(
        device_models,
        model_count,
        device_products,
        product_count,
    ...
    check_cuda(
        cudaOccupancyMaxActiveBlocksPerMultiprocessor(
            ...
            merton_range_accrual_kernel,
            static_cast<int>(threads_per_block),
            shared_bytes
        ),
        "Merton Range Accrual occupancy check"
    );
    if (active_blocks_per_multiprocessor == 0) {
        throw std::invalid_argument(
            "Merton Range Accrual kernel cannot launch one block per SM."
        );
    }

    // Launch the Merton Range Accrual kernel.
    ...
    check_cuda(cudaGetLastError(), "Merton Range Accrual kernel");
}

// Merton Phoenix-Memory-autocall kernel with fused Philox simulation and payoff reduction.
...
__global__ void merton_phoenix_memory_autocall_kernel(
...
void validate_merton_phoenix_memory_autocall_launch(
    ...
    if (result_offset >= result_count
        ...
        throw std::invalid_argument(
            "The Merton Phoenix-Memory-autocall launch batch exceeds the result array."
        );
    }
...
void launch_merton_phoenix_memory_autocall_cuda(
    ...
    validate_merton_phoenix_memory_autocall_launch(
        device_models,
        model_count,
        device_products,
        product_count,
    ...
    check_cuda(
        cudaOccupancyMaxActiveBlocksPerMultiprocessor(
            ...
            merton_phoenix_memory_autocall_kernel,
            static_cast<int>(threads_per_block),
            shared_bytes
        ),
        "Merton Phoenix Memory autocall occupancy check"
    );
    if (active_blocks_per_multiprocessor == 0) {
        throw std::invalid_argument(
            "Merton Phoenix Memory autocall kernel cannot launch one block per SM."
        );
    }

    // Launch the Merton Phoenix-Memory-autocall kernel.
    ...
    check_cuda(cudaGetLastError(), "Merton Phoenix Memory autocall kernel");
}

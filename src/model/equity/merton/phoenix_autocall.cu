// Merton Phoenix-autocall kernel with fused Philox simulation and payoff reduction.
...
__global__ void merton_phoenix_autocall_kernel(
...
void validate_merton_phoenix_autocall_launch(
    ...
    if (result_offset >= result_count
        ...
        throw std::invalid_argument(
            "The Merton Phoenix-autocall launch batch exceeds the result array."
        );
    }
...
void launch_merton_phoenix_autocall_cuda(
    const MertonModelParameters* device_models,
    std::size_t model_count,
    const product::PhoenixAutocallParameters* device_products,
    std::size_t product_count,
    ...
    validate_merton_phoenix_autocall_launch(
        device_models,
        model_count,
        device_products,
        product_count,
    ...
    check_cuda(
        cudaOccupancyMaxActiveBlocksPerMultiprocessor(
            ...
            merton_phoenix_autocall_kernel,
            static_cast<int>(threads_per_block),
            shared_bytes
        ),
        "Merton Phoenix autocall occupancy check"
    );
    if (active_blocks_per_multiprocessor == 0) {
        throw std::invalid_argument(
            "Merton Phoenix autocall kernel cannot launch one block per SM."
        );
    }

    // Launch the Merton Phoenix-autocall kernel.
    ...
    check_cuda(cudaGetLastError(), "Merton Phoenix autocall kernel");
}

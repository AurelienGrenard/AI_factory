// Merton Athena-autocall kernel with fused Philox simulation and payoff reduction.
...
__global__ void merton_athena_autocall_kernel(
...
void validate_merton_athena_autocall_launch(
    ...
    if (result_offset >= result_count
        ...
        throw std::invalid_argument(
            "The Merton Athena-autocall launch batch exceeds the result array."
        );
    }
...
void launch_merton_athena_autocall_cuda(
    ...
    validate_merton_athena_autocall_launch(
        device_models,
        model_count,
        device_products,
        product_count,
    ...
    check_cuda(
        cudaOccupancyMaxActiveBlocksPerMultiprocessor(
            ...
            merton_athena_autocall_kernel,
            static_cast<int>(threads_per_block),
            shared_bytes
        ),
        "Merton Athena autocall occupancy check"
    );
    if (active_blocks_per_multiprocessor == 0) {
        throw std::invalid_argument(
            "Merton Athena autocall kernel cannot launch one block per SM."
        );
    }

    // Launch the Merton Athena-autocall kernel.
    ...
    check_cuda(cudaGetLastError(), "Merton Athena autocall kernel");
}

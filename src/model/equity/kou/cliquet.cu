// Kou Cliquet kernel with fused Philox simulation and payoff reduction.
...
__global__ void kou_cliquet_kernel(
...
void validate_kou_cliquet_launch(
    ...
    if (result_offset >= result_count
        ...
        throw std::invalid_argument(
            "The Kou Cliquet launch batch exceeds the result array."
        );
    }
...
void launch_kou_cliquet_cuda(
    ...
    check_cuda(
        ...
        "Kou Cliquet occupancy check"
    );
    if (active_blocks_per_multiprocessor == 0) {
        throw std::invalid_argument(
            "Kou Cliquet kernel cannot launch one block per SM."
        );
    }

    // Launch the Kou Cliquet kernel.
    ...
    check_cuda(cudaGetLastError(), "Kou Cliquet kernel");
}

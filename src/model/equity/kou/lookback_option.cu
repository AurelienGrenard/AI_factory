// Kou lookback kernel with fused Philox simulation and payoff reduction.
...
void validate_kou_lookback_option_launch(
    ...
    if (result_offset >= result_count
        ...
        throw std::invalid_argument(
            "The Kou lookback launch batch exceeds the result array."
        );
    }
...
void launch_kou_lookback_option_cuda(
    ...
    validate_kou_lookback_option_launch(
        device_models,
        model_count,
        device_products,
        product_count,
    ...
    check_cuda(
        cudaOccupancyMaxActiveBlocksPerMultiprocessor(
            ...
            kou_lookback_option_kernel,
            static_cast<int>(threads_per_block),
            shared_bytes
        ),
        "Kou Lookback option occupancy check"
    );
    if (active_blocks_per_multiprocessor == 0) {
        throw std::invalid_argument(
            "Kou Lookback option kernel cannot launch one block per SM."
        );
    }

    // Launch the Kou lookback kernel.
    ...
    check_cuda(cudaGetLastError(), "Kou Lookback option kernel");
}

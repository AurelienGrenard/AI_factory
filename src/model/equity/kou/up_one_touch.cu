// Kou Up-one-touch kernel with fused Philox simulation and payoff reduction.
...
__global__ void kou_up_one_touch_kernel(
...
void validate_kou_up_one_touch_launch(
    ...
    if (result_offset >= result_count
        ...
        throw std::invalid_argument(
            "The Kou up-one-touch batch exceeds the result array."
        );
    }
...
void launch_kou_up_one_touch_cuda(
    const ModelParameters* device_models,
    std::size_t model_count,
    const product::UpOneTouchParameters* device_products,
    std::size_t product_count,
    ...
    validate_kou_up_one_touch_launch(
        device_models,
        model_count,
        device_products,
        product_count,
    ...
    check_cuda(
        cudaOccupancyMaxActiveBlocksPerMultiprocessor(
            ...
            kou_up_one_touch_kernel,
            static_cast<int>(threads_per_block),
            shared_bytes
        ),
        "Kou Up one-touch occupancy check"
    );
    if (active_blocks_per_multiprocessor == 0) {
        throw std::invalid_argument(
            "Kou Up one-touch kernel cannot launch one block per SM."
        );
    }

    // Launch the Kou Up-one-touch kernel.
    ...
    check_cuda(cudaGetLastError(), "Kou Up one-touch kernel");
}

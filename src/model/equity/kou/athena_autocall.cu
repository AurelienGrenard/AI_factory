// Kou Athena-autocall kernel with fused Philox simulation and payoff reduction.
...
void validate_kou_athena_autocall_launch(
    ...
    if (result_offset >= result_count
        ...
        throw std::invalid_argument(
            "The Kou Athena-autocall launch batch exceeds the result array."
        );
    }
...
void launch_kou_athena_autocall_cuda(
    ...
    validate_kou_athena_autocall_launch(
        device_models,
        model_count,
        device_products,
        product_count,
    ...
    check_cuda(
        cudaOccupancyMaxActiveBlocksPerMultiprocessor(
            ...
            kou_athena_autocall_kernel,
            static_cast<int>(threads_per_block),
            shared_bytes
        ),
        "Kou Athena autocall occupancy check"
    );
    if (active_blocks_per_multiprocessor == 0) {
        throw std::invalid_argument(
            "Kou Athena autocall kernel cannot launch one block per SM."
        );
    }

    // Launch the Kou Athena-autocall kernel.
    ...
    check_cuda(cudaGetLastError(), "Kou Athena autocall kernel");
}

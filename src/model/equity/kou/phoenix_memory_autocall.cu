// Kou Phoenix-Memory-autocall kernel with fused Philox simulation and payoff reduction.
...
__global__ void kou_phoenix_memory_autocall_kernel(
...
void validate_kou_phoenix_memory_autocall_launch(
    ...
    if (result_offset >= result_count
        ...
        throw std::invalid_argument(
            "The Kou Phoenix-Memory-autocall launch batch exceeds the result array."
        );
    }
...
void launch_kou_phoenix_memory_autocall_cuda(
    const ModelParameters* device_models,
    std::size_t model_count,
    const product::PhoenixMemoryAutocallParameters* device_products,
    std::size_t product_count,
    ...
    validate_kou_phoenix_memory_autocall_launch(
        device_models,
        model_count,
        device_products,
        product_count,
    ...
    check_cuda(
        cudaOccupancyMaxActiveBlocksPerMultiprocessor(
            ...
            kou_phoenix_memory_autocall_kernel,
            static_cast<int>(threads_per_block),
            shared_bytes
        ),
        "Kou Phoenix Memory autocall occupancy check"
    );
    if (active_blocks_per_multiprocessor == 0) {
        throw std::invalid_argument(
            "Kou Phoenix Memory autocall kernel cannot launch one block per SM."
        );
    }

    // Launch the Kou Phoenix-Memory-autocall kernel.
    ...
    check_cuda(cudaGetLastError(), "Kou Phoenix Memory autocall kernel");
}

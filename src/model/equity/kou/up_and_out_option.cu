// Kou up-and-out-option composition over generic CUDA layers.
...
using Schedule = equity::FixedStepDenseSchedule<kou::DynamicsPolicy>;
...
void launch_kou_up_and_out_option_cuda(
    ...
    equity::launch_monte_carlo_cuda<PricingPolicy<Side>>(
        ...
        "Kou up and out option kernel"
    );
}

template void launch_kou_up_and_out_option_cuda<OptionSide::call>(
...
template void launch_kou_up_and_out_option_cuda<OptionSide::put>(
    const ModelParameters*, std::size_t,
    const product::UpAndOutOptionParameters*, std::size_t,
    bool, std::size_t, std::size_t, std::size_t, std::size_t,
    float, std::uint32_t, unsigned int, std::size_t, std::uint64_t,

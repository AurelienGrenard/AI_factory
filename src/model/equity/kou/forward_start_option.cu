// Kou Forward-start option kernel with fused Philox simulation and payoff reduction.
...
struct PreparedRow {
    KouPreparedParameters reset_model;
    KouPreparedParameters remaining_model;
...
__device__ __forceinline__ float evaluate_path(
    ...
    const KouTwoTimePathResult simulated = simulate_at_two_times(
        row.reset_model,
        row.remaining_model,
        row.key,
        path
...
template<OptionSide Side>
__global__ void kou_forward_start_option_kernel(
...
void validate_kou_forward_start_option_launch(
    ...
    if (result_offset >= result_count
        ...
        throw std::invalid_argument(
            "The Kou forward-start option batch exceeds the result array."
        );
    }
...
void launch_kou_forward_start_option_cuda(
    ...
    validate_kou_forward_start_option_launch(
        device_models,
        model_count,
        device_products,
        product_count,
    ...
    check_cuda(
        cudaOccupancyMaxActiveBlocksPerMultiprocessor(
            ...
            kou_forward_start_option_kernel<Side>,
            static_cast<int>(threads_per_block),
            shared_bytes
        ),
        "Kou Forward-start option occupancy check"
    );
    if (active_blocks_per_multiprocessor == 0) {
        throw std::invalid_argument(
            "Kou Forward-start option kernel cannot launch one block per SM."
        );
    }

    // Launch the Kou Forward-start option kernel.
    ...
    check_cuda(cudaGetLastError(), "Kou Forward-start option kernel");
}

// Build both public payoff specializations in this CUDA translation unit.
using LaunchSignature = decltype(launch_kou_forward_start_option_cuda<OptionSide::call>);
template LaunchSignature launch_kou_forward_start_option_cuda<OptionSide::call>;
template LaunchSignature launch_kou_forward_start_option_cuda<OptionSide::put>;

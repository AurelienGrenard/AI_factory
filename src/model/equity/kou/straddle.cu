// Kou straddle kernel with fused Philox simulation and payoff reduction.
...
void validate_kou_straddle_launch(
    ...
    if (result_offset >= result_count
        ...
        throw std::invalid_argument(
            "The Kou straddle launch batch exceeds the result array."
        );
    }
...
void launch_kou_straddle_cuda(
    ...
    check_cuda(
        ...
        "Kou straddle occupancy check"
    );
    if (active_blocks_per_multiprocessor == 0) {
        throw std::invalid_argument(
            "Kou straddle kernel cannot launch one block per SM."
        );
    }

    // Launch the Kou Straddle kernel.
    ...
    check_cuda(cudaGetLastError(), "Kou straddle kernel");
}

// Kou Range Accrual kernel with fused Philox simulation and payoff reduction.
...
__device__ __forceinline__ PreparedRow prepare_row(
    ...
    const KouPreparedParameters prepared_model = prepare_model(model, product.observation_interval);
...
__global__ void kou_range_accrual_kernel(
...
void validate_kou_range_accrual_launch(
    ...
    if (result_offset >= result_count
        ...
        throw std::invalid_argument(
            "The Kou Range Accrual launch batch exceeds the result array."
        );
    }
...
void launch_kou_range_accrual_cuda(
    const KouModelParameters* device_models,
    std::size_t model_count,
    const product::RangeAccrualParameters* device_products,
    std::size_t product_count,
    ...
    validate_kou_range_accrual_launch(
        device_models,
        model_count,
        device_products,
        product_count,
    ...
    check_cuda(
        cudaOccupancyMaxActiveBlocksPerMultiprocessor(
            ...
            kou_range_accrual_kernel,
            static_cast<int>(threads_per_block),
            shared_bytes
        ),
        "Kou Range Accrual occupancy check"
    );
    if (active_blocks_per_multiprocessor == 0) {
        throw std::invalid_argument(
            "Kou Range Accrual kernel cannot launch one block per SM."
        );
    }

    // Launch the Kou Range Accrual kernel.
    ...
    check_cuda(cudaGetLastError(), "Kou Range Accrual kernel");
}

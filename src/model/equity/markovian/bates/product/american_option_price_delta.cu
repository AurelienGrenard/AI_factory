// bates composes the common central LSM solver with frozen-exercise delta outputs.
...
namespace {
using Schedule = simulation::FixedStepMaturityAlignedExerciseSchedule<bates::DynamicsPolicy>;
using Continuation = product::SpotAndScaledStateContinuationState<bates::DynamicsPolicy, &bates::State::variance, &bates::ModelParameters::theta>;
...
longstaff_schwartz::LaunchResult launch_bates_american_option_price_delta_cuda(
    ...
    return longstaff_schwartz::launch_longstaff_schwartz_cuda<Policy<Side>, Regressor>(
        ...
        "bates.american_option_price_delta", option_side_name(Side), "American price-delta"
    );
}

template longstaff_schwartz::LaunchResult launch_bates_american_option_price_delta_cuda<OptionSide::call>(
...
template longstaff_schwartz::LaunchResult launch_bates_american_option_price_delta_cuda<OptionSide::put>(
    const ModelParameters*, const ModelParameters*, std::size_t,
    const product::AmericanOptionParameters*, const product::AmericanOptionParameters*, std::size_t,
    PriceConstruction, std::size_t, std::size_t, float, std::uint32_t, unsigned, std::size_t, std::uint64_t,
    ::ai_factory::workbench::equity::price_delta::SpotBumpConfiguration, float*, float*, float*, float*);

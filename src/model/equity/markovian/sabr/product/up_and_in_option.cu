// SABR up-and-in-option composition over generic CUDA layers.
...
template<OptionSide Side>
void launch_sabr_up_and_in_option_cuda(
    ...
    monte_carlo::launch_monte_carlo_cuda<PricingPolicy<Side>>(
        ...
        "SABR up and in option kernel"
    );
}

template void launch_sabr_up_and_in_option_cuda<OptionSide::call>(
...
template void launch_sabr_up_and_in_option_cuda<OptionSide::put>(
    const ModelParameters*, std::size_t,
    const product::UpAndInOptionParameters*,
    const product::UpAndInOptionParameters*, std::size_t,
    PriceConstruction, std::size_t, std::size_t, std::size_t, std::size_t,

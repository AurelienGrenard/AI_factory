// SABR forward-start-option composition over generic CUDA layers.
...
using Schedule = simulation::FixedStepCalendarSchedule<sabr::DynamicsPolicy, 2U>;
...
template<OptionSide Side>
void launch_sabr_forward_start_option_cuda(
    ...
    monte_carlo::launch_monte_carlo_cuda<PricingPolicy<Side>>(
        ...
        "SABR forward start option kernel"
    );
}

template void launch_sabr_forward_start_option_cuda<OptionSide::call>(
...
template void launch_sabr_forward_start_option_cuda<OptionSide::put>(
    const ModelParameters*, std::size_t,
    const product::ForwardStartOptionParameters*,
    const product::ForwardStartOptionParameters*, std::size_t,
    PriceConstruction, std::size_t, std::size_t, std::size_t, std::size_t,

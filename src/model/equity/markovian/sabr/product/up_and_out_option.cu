// SABR up-and-out-option composition over generic CUDA layers.
...
template<OptionSide Side>
void launch_sabr_up_and_out_option_cuda(
    ...
    monte_carlo::launch_monte_carlo_cuda<PricingPolicy<Side>>(
        ...
        "SABR up and out option kernel"
    );
}

template void launch_sabr_up_and_out_option_cuda<OptionSide::call>(
...
template void launch_sabr_up_and_out_option_cuda<OptionSide::put>(
    const ModelParameters*, std::size_t,
    const product::UpAndOutOptionParameters*,
    const product::UpAndOutOptionParameters*, std::size_t,
    PriceConstruction, std::size_t, std::size_t, std::size_t, std::size_t,

// Heston 3/2 gap-option composition over generic CUDA layers.
...
template<OptionSide Side>
void launch_heston_3_2_gap_option_cuda(
    ...
    monte_carlo::launch_monte_carlo_cuda<PricingPolicy<Side>>(
        ...
        "Heston 3/2 gap option kernel"
    );
}

template void launch_heston_3_2_gap_option_cuda<OptionSide::call>(
...
template void launch_heston_3_2_gap_option_cuda<OptionSide::put>(
    const ModelParameters*, std::size_t,
    const product::GapOptionParameters*, std::size_t,
    PriceConstruction, std::size_t, std::size_t, std::size_t, std::size_t,
    float, std::uint32_t, unsigned int, std::size_t, std::uint64_t,

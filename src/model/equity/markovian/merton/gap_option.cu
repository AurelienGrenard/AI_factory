// Merton gap-option composition over generic CUDA layers.
...
void launch_merton_gap_option_cuda(
    ...
    monte_carlo::launch_monte_carlo_cuda<PricingPolicy<Side>>(
        ...
        "Merton gap option kernel"
    );
}

template void launch_merton_gap_option_cuda<OptionSide::call>(
...
template void launch_merton_gap_option_cuda<OptionSide::put>(
    const ModelParameters*, std::size_t,
    const product::GapOptionParameters*, std::size_t,
    PriceConstruction, std::size_t, std::size_t, std::size_t, std::size_t,
    float, unsigned int, std::size_t, std::uint64_t,

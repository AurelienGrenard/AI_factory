// Generated merton forward_start_option composition over the shared price-delta engine.
...
using Path = equity::price_delta::MultiplicativeSpotPath<merton::DynamicsPolicy>;
...
template<OptionSide Side>
void launch_merton_forward_start_option_price_delta_cuda(
    ...
    monte_carlo::launch_monte_carlo_price_delta_cuda<PricingPolicy<Side>>(
        ...
        "merton.forward_start_option.price_delta", option_side_name(Side));
}

// Explicit option-side instantiation for the paired-output launcher.
template void launch_merton_forward_start_option_price_delta_cuda<OptionSide::call>(
...
// Explicit option-side instantiation for the paired-output launcher.
template void launch_merton_forward_start_option_price_delta_cuda<OptionSide::put>(
    const ModelParameters*, const ModelParameters*, std::size_t,
    const product::ForwardStartOptionParameters*, const product::ForwardStartOptionParameters*,
    std::size_t, PriceConstruction, std::size_t, std::size_t, std::size_t, std::size_t,
    float, unsigned int, std::size_t, std::uint64_t,

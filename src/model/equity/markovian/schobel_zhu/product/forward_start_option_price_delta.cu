// Generated schobel_zhu forward_start_option composition over the shared price-delta engine.
...
using Path = equity::price_delta::MultiplicativeSpotPath<schobel_zhu::DynamicsPolicy>;
using Schedule = simulation::FixedStepCalendarSchedule<typename Path::Dynamics, 2U>;
...
template<OptionSide Side>
void launch_schobel_zhu_forward_start_option_price_delta_cuda(
    ...
    monte_carlo::launch_monte_carlo_price_delta_cuda<PricingPolicy<Side>>(
        ...
        "schobel_zhu.forward_start_option.price_delta", option_side_name(Side));
}

// Explicit option-side instantiation for the paired-output launcher.
template void launch_schobel_zhu_forward_start_option_price_delta_cuda<OptionSide::call>(
...
// Explicit option-side instantiation for the paired-output launcher.
template void launch_schobel_zhu_forward_start_option_price_delta_cuda<OptionSide::put>(
    const ModelParameters*, const ModelParameters*, std::size_t,
    const product::ForwardStartOptionParameters*, const product::ForwardStartOptionParameters*,
    std::size_t, PriceConstruction, std::size_t, std::size_t, std::size_t, std::size_t,
    float, std::uint32_t, unsigned int, std::size_t, std::uint64_t,

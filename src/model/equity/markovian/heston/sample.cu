// Heston composition over the common model-sampling engine.
...
using TerminalSamplingPolicy = sample::ModelSamplingPolicy<
    simulation::FixedStepTerminalSchedule<DynamicsPolicy>,
    Observation
>;
using CalendarSamplingPolicy = sample::ModelSamplingPolicy<
    simulation::FixedStepStubbedRegularSchedule<DynamicsPolicy>,
    Observation
>;
...
void launch_heston_terminal_samples_cuda(
    ...
    sample::launch_device_terminal_samples_cuda<TerminalSamplingPolicy>(
        ...
        maturity_days, sample_offset, launch_sample_count,
        threads_per_block, block_count, dynamics_seed,
        {device_spots, device_variances},
        "heston.samples", "Heston terminal sample kernel"
    );
}
...
void launch_heston_calendar_samples_cuda(
    ...
    sample::launch_device_calendar_samples_cuda<CalendarSamplingPolicy>(
        ...
        dynamics_seed, {device_spots, device_variances},
        "heston.samples", "Heston calendar sample kernel"
    );
}

// Stein-Stein asset-or-nothing-option composition over generic CUDA layers.
...
using Schedule = simulation::FixedStepTerminalSchedule<stein_stein::DynamicsPolicy>;
...
template<OptionSide Side>
void launch_stein_stein_asset_or_nothing_option_cuda(
    ...
    monte_carlo::launch_monte_carlo_cuda<PricingPolicy<Side>>(
        ...
        "Stein-Stein asset or nothing option kernel"
    );
}

template void launch_stein_stein_asset_or_nothing_option_cuda<OptionSide::call>(
...
template void launch_stein_stein_asset_or_nothing_option_cuda<OptionSide::put>(
    const ModelParameters*, std::size_t,
    const product::AssetOrNothingOptionParameters*,
    const product::AssetOrNothingOptionParameters*, std::size_t,
    PriceConstruction, std::size_t, std::size_t, std::size_t, std::size_t,

// Stein-Stein geometric-asian-option composition over generic CUDA layers.
...
template<OptionSide Side>
void launch_stein_stein_geometric_asian_option_cuda(
    ...
    monte_carlo::launch_monte_carlo_cuda<PricingPolicy<Side>>(
        ...
        "Stein-Stein geometric asian option kernel"
    );
}

template void launch_stein_stein_geometric_asian_option_cuda<OptionSide::call>(
...
template void launch_stein_stein_geometric_asian_option_cuda<OptionSide::put>(
    const ModelParameters*, std::size_t,
    const product::GeometricAsianOptionParameters*, std::size_t,
    PriceConstruction, std::size_t, std::size_t, std::size_t, std::size_t,
    float, std::uint32_t, unsigned int, std::size_t, std::uint64_t,

// Stein-Stein down-and-in-option composition over generic CUDA layers.
...
template<OptionSide Side>
void launch_stein_stein_down_and_in_option_cuda(
    ...
    monte_carlo::launch_monte_carlo_cuda<PricingPolicy<Side>>(
        ...
        "Stein-Stein down and in option kernel"
    );
}

template void launch_stein_stein_down_and_in_option_cuda<OptionSide::call>(
...
template void launch_stein_stein_down_and_in_option_cuda<OptionSide::put>(
    const ModelParameters*, std::size_t,
    const product::DownAndInOptionParameters*, std::size_t,
    PriceConstruction, std::size_t, std::size_t, std::size_t, std::size_t,
    float, std::uint32_t, unsigned int, std::size_t, std::uint64_t,

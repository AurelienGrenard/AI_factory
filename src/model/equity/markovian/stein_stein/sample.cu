// Generated Stein-Stein composition over the common sample engine.
...
using Observation = sample::SpotAndStateSampleObservation<DynamicsPolicy, &State::volatility>;
...
void launch_stein_stein_terminal_samples_cuda(
    ...
    sample::launch_device_terminal_samples_cuda<TerminalPolicy>(
        ...
        dynamics_seed, {device_spots, device_volatilities}, "stein_stein.samples",
        "Stein-Stein terminal sample kernel");
}

void launch_stein_stein_random_terminal_samples_cuda(
    const ModelParameters* device_parameters, std::size_t parameter_count,
    ...
    sample::launch_device_random_terminal_samples_cuda<TerminalPolicy>(
        ...
        dynamics_seed, device_maturity_days, {device_spots, device_volatilities},
        "stein_stein.samples", "Stein-Stein random terminal sample kernel");
}

void launch_stein_stein_calendar_samples_cuda(
    const ModelParameters* device_parameters, std::size_t parameter_count,
    ...
    sample::launch_device_calendar_samples_cuda<CalendarPolicy>(
        ...
        sample_offset, launch_sample_count, threads_per_block, block_count,
        dynamics_seed, {device_spots, device_volatilities}, "stein_stein.samples",
        "Stein-Stein calendar sample kernel");
}

...
|---|---|---|---|
| European option | Closed form | strike $`K`$, maturity $`T`$, side $`\varepsilon`$ | $`[\varepsilon(S_T-K)]^+`$ |
| Digital option | Closed form | strike $`K`$, cash amount $`Q`$, maturity $`T`$, side $`\varepsilon`$ | $`Q\mathbf 1_{\{\varepsilon(S_T-K)\gt 0\}}`$ |
| Asset-or-nothing option | Closed form | strike $`K`$, maturity $`T`$, side $`\varepsilon`$ | $`S_T\mathbf 1_{\{\varepsilon(S_T-K)\gt 0\}}`$ |
| Straddle | Closed form | strike $`K`$, maturity $`T`$ | $`\lvert S_T-K\rvert`$ |
| Gap option | Closed form | trigger $`K_1`$, payoff strike $`K_2`$, maturity $`T`$, side $`\varepsilon`$ | $`\varepsilon(S_T-K_2)\mathbf 1_{\{\varepsilon(S_T-K_1)\gt 0\}}`$ |
| Forward-start option | Closed form | reset $`T_r`$, moneyness $`m`$, maturity $`T`$, side $`\varepsilon`$ | $`[\varepsilon(S_T-mS_{T_r})]^+`$ |
...
| Geometric Asian option | Closed form | strike $`K`$, maturity $`T`$, side $`\varepsilon`$ | $`[\varepsilon(\overline S_G-K)]^+`$ |
...
=\prod_{n=0}^{N}\mathbf 1_{\{B_L\lt S_{u_n}\lt B_H\}}.
```
...
For the scheduled products below, let $`\Delta_o\gt 0`$ be the contractual
...
|---|---|---|---|
| Athena | MC | $`T,\Delta_o`$, autocall barrier $`B_A`$, protection barrier $`B_P`$, annual coupon $`c`$ | At the first $`t_j\lt T`$ with $`S_{t_j}\geq B_A`$, pay $`1+ct_j`$. At $`T`$, pay $`1+cT`$ if $`S_T\geq B_A`$, $`1`$ if $`B_P\leq S_T\lt B_A`$, and $`S_T`$ otherwise. |

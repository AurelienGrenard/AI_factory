// Generated cev European composition over the common selected-gradient MC engine.
...
void launch_cev_european_option_price_gradients_cuda(
    ...
    if (configuration.method != pg::PricingMethod::monte_carlo)
        throw std::invalid_argument("cev European gradients require Monte Carlo.");
    monte_carlo::price_gradients::launch<price_gradients::CoupledDynamics,
        product::EuropeanOptionGradientPathPolicy<Side>>(device, configuration, host.time, outputs,
            host.sensitivity_count(), "cev.european_option.price_gradients.monte_carlo", option_side_name(Side));
}
template void launch_cev_european_option_price_gradients_cuda<OptionSide::call>(
...
template void launch_cev_european_option_price_gradients_cuda<OptionSide::put>(
    const EuropeanOptionPriceGradientPlan&, pg::DeviceInputs<EuropeanOptionPriceGradientPlan::ScenarioType>,
    const pg::LaunchConfiguration&, pg::Outputs);
}  // namespace ai_factory::workbench::model::equity::cev

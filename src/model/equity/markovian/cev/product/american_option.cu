// CEV American-option composition over shared LSM kernels.
...
template<OptionSide Side>
longstaff_schwartz::LaunchResult launch_cev_american_option_cuda(
...
launch_cev_american_option_cuda<OptionSide::call>(
...
template longstaff_schwartz::LaunchResult
launch_cev_american_option_cuda<OptionSide::put>(
    const ModelParameters*, std::size_t,
    const product::AmericanOptionParameters*,
    const product::AmericanOptionParameters*, std::size_t,
    PriceConstruction, std::size_t, std::size_t, float, std::uint32_t,

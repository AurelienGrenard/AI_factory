// CEV asian-option composition over generic CUDA layers.
...
void launch_cev_asian_option_cuda(
    ...
    monte_carlo::launch_monte_carlo_cuda<PricingPolicy<Side>>(
        ...
        "CEV asian option kernel"
    );
}

template void launch_cev_asian_option_cuda<OptionSide::call>(
...
template void launch_cev_asian_option_cuda<OptionSide::put>(
    const ModelParameters*, std::size_t,
    const product::AsianOptionParameters*,
    const product::AsianOptionParameters*, std::size_t,
    PriceConstruction, std::size_t, std::size_t, std::size_t, std::size_t,

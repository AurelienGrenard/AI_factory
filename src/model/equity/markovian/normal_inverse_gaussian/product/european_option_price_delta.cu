// Generated normal_inverse_gaussian european_option composition over the shared price-delta engine.
...
using PricingPolicy = equity::price_delta::PathProductPriceDeltaPolicy<
    Schedule, product::EuropeanOptionPathPolicy<Side>, Path>;
static_assert(monte_carlo::PriceDeltaMonteCarloPolicy<PricingPolicy<OptionSide::call>>);
}  // namespace

template<OptionSide Side>
void launch_normal_inverse_gaussian_european_option_price_delta_cuda(
    ...
    monte_carlo::launch_monte_carlo_price_delta_cuda<PricingPolicy<Side>>(
        ...
        "normal_inverse_gaussian.european_option.price_delta", option_side_name(Side));
}

// Explicit option-side instantiation for the paired-output launcher.
template void launch_normal_inverse_gaussian_european_option_price_delta_cuda<OptionSide::call>(
    const ModelParameters*, const ModelParameters*, std::size_t,
    const product::EuropeanOptionParameters*, const product::EuropeanOptionParameters*,
...
// Explicit option-side instantiation for the paired-output launcher.
template void launch_normal_inverse_gaussian_european_option_price_delta_cuda<OptionSide::put>(
    const ModelParameters*, const ModelParameters*, std::size_t,
    const product::EuropeanOptionParameters*, const product::EuropeanOptionParameters*,
    std::size_t, PriceConstruction, std::size_t, std::size_t, std::size_t, std::size_t,
    float, unsigned int, std::size_t, std::uint64_t,

// Normal-Inverse-Gaussian up-and-out-option composition over generic CUDA layers.
...
using PricingPolicy =
    product::UpAndOutOptionPricingPolicy<Schedule, Side>;
...
void launch_normal_inverse_gaussian_up_and_out_option_cuda(
    ...
    monte_carlo::launch_monte_carlo_cuda<PricingPolicy<Side>>(
        ...
        "Normal-Inverse-Gaussian up and out option kernel"
    );
}

template void launch_normal_inverse_gaussian_up_and_out_option_cuda<OptionSide::call>(
    const ModelParameters*, std::size_t,
    const product::UpAndOutOptionParameters*,
    const product::UpAndOutOptionParameters*, std::size_t,
    PriceConstruction, std::size_t, std::size_t, std::size_t, std::size_t,
    float, std::uint32_t, unsigned int, std::size_t, std::uint64_t,
    float*, float*
);
template void launch_normal_inverse_gaussian_up_and_out_option_cuda<OptionSide::put>(
    const ModelParameters*, std::size_t,
    const product::UpAndOutOptionParameters*,
    const product::UpAndOutOptionParameters*, std::size_t,
    PriceConstruction, std::size_t, std::size_t, std::size_t, std::size_t,

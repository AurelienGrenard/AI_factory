// Public Quadratic rough-Heston Forward-start-option N-factor price/spot-delta launcher.
...
void launch_quadratic_rough_heston_forward_start_option_price_delta_cuda(
    ...
    using Product = product::ForwardStartOptionPathPolicy<Side>;
    ...
    monte_carlo::launch_monte_carlo_price_delta_cuda<Policy>(
        ...
        "quadratic_rough_heston.forward_start_option_price_delta", option_side_name(Side));
}

template void launch_quadratic_rough_heston_forward_start_option_price_delta_cuda<
    OptionSide::call, 2U
>(
    const ModelParameters*, const ModelParameters*, std::size_t,
    const PreparedDynamics<2U>*, std::size_t,
    const product::ForwardStartOptionParameters*,

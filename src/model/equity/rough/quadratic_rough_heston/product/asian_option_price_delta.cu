// Public Quadratic rough-Heston Asian-option N-factor price/spot-delta launcher.
...
void launch_quadratic_rough_heston_asian_option_price_delta_cuda(
    ...
    using Product = product::AsianOptionPathPolicy<Side>;
    ...
    monte_carlo::launch_monte_carlo_price_delta_cuda<Policy>(
        ...
        "quadratic_rough_heston.asian_option_price_delta", option_side_name(Side));
}

template void launch_quadratic_rough_heston_asian_option_price_delta_cuda<
    OptionSide::call, 2U
>(
    const ModelParameters*, const ModelParameters*, std::size_t,
    const PreparedDynamics<2U>*, std::size_t,
    const product::AsianOptionParameters*,

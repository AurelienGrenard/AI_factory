// Schobel-Zhu Range Accrual kernel with fused Philox simulation and payoff reduction.
...
__device__ __forceinline__ PreparedRow prepare_row(
    ...
    const SchobelZhuPreparedParameters prepared_model = prepare_model(
        model,
...
__global__ void schobel_zhu_range_accrual_kernel(
...
void validate_schobel_zhu_range_accrual_launch(
    ...
    if (result_offset >= result_count
        ...
        throw std::invalid_argument(
            "The Schobel-Zhu Range Accrual launch batch exceeds the result array."
        );
    }
...
void launch_schobel_zhu_range_accrual_cuda(
    ...
    validate_schobel_zhu_range_accrual_launch(
        device_models,
        model_count,
        device_products,
        product_count,
    ...
    check_cuda(
        cudaOccupancyMaxActiveBlocksPerMultiprocessor(
            ...
            schobel_zhu_range_accrual_kernel,
            static_cast<int>(threads_per_block),
            shared_bytes
        ),
        "Schobel-Zhu Range Accrual occupancy check"
    );
    if (active_blocks_per_multiprocessor == 0) {
        throw std::invalid_argument(
            "Schobel-Zhu Range Accrual kernel cannot launch one block per SM."
        );
    }

    // Launch the Schobel-Zhu Range Accrual kernel.
    ...
    check_cuda(cudaGetLastError(), "Schobel-Zhu Range Accrual kernel");
}

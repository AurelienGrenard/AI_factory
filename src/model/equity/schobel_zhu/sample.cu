// Persistent schobel zhu terminal and regular-calendar sample kernels.
#include "model/equity/schobel_zhu/sample.cuh"
...
__global__ void schobel_zhu_terminal_samples_kernel(
    ...
    float* __restrict__ volatilities
) {
    ...
    for (std::size_t launch_index = thread;
        ...
        volatilities[sample_index] = terminal.volatility;
    }
}

__global__ void schobel_zhu_calendar_samples_kernel(
    const ModelParameters* __restrict__ models,
    std::size_t paths_per_model,
    ...
    for (std::size_t launch_index = thread;
        ...
        volatilities[terminal_index] = terminal.volatility;
    }
}

std::size_t validate_schobel_zhu_sample_launch(
    ...
    const float* device_volatilities
) {
    ...
    validate_device_pointer(device_volatilities, "device_volatilities");
...
void launch_schobel_zhu_terminal_samples_cuda(
    ...
    report_cuda_kernel_launch_if_enabled(
        "schobel_zhu.samples",
        "terminal",
        schobel_zhu_terminal_samples_kernel,
    ...
    >>>(
        ...
        launch_sample_count, base_seed, device_spots, device_volatilities
    );
    check_cuda(cudaGetLastError(), "schobel zhu terminal sample kernel");
}
...
void launch_schobel_zhu_calendar_samples_cuda(
    ...
    report_cuda_kernel_launch_if_enabled(
        "schobel_zhu.samples",
        "calendar",
        schobel_zhu_calendar_samples_kernel,
        dim3(static_cast<unsigned int>(block_count)),
        dim3(threads_per_block)
    );
    schobel_zhu_calendar_samples_kernel<<<

// Schobel-Zhu Up-no-touch kernel with fused Philox simulation and payoff reduction.
...
__global__ void schobel_zhu_up_no_touch_kernel(
...
void validate_schobel_zhu_up_no_touch_launch(
    ...
    if (result_offset >= result_count
        ...
        throw std::invalid_argument(
            "The Schobel-Zhu up-no-touch batch exceeds the result array."
        );
    }
...
void launch_schobel_zhu_up_no_touch_cuda(
    const ModelParameters* device_models,
    std::size_t model_count,
    const product::UpNoTouchParameters* device_products,
    std::size_t product_count,
    ...
    validate_schobel_zhu_up_no_touch_launch(
        device_models,
        model_count,
        device_products,
        product_count,
    ...
    check_cuda(
        cudaOccupancyMaxActiveBlocksPerMultiprocessor(
            ...
            schobel_zhu_up_no_touch_kernel,
            static_cast<int>(threads_per_block),
            shared_bytes
        ),
        "Schobel-Zhu Up no-touch occupancy check"
    );
    if (active_blocks_per_multiprocessor == 0) {
        throw std::invalid_argument(
            "Schobel-Zhu Up no-touch kernel cannot launch one block per SM."
        );
    }

    // Launch the Schobel-Zhu Up-no-touch kernel.
    ...
    check_cuda(cudaGetLastError(), "Schobel-Zhu Up no-touch kernel");
}

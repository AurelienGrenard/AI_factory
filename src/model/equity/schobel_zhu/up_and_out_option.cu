// Schobel-Zhu Up-and-out option kernel with fused Philox simulation and payoff reduction.
...
template<OptionSide Side>
__global__ void schobel_zhu_up_and_out_option_kernel(
...
void validate_schobel_zhu_up_and_out_option_launch(
    ...
    if (result_offset >= result_count
        ...
        throw std::invalid_argument(
            "The Schobel-Zhu up-and-out option batch exceeds the result array."
        );
    }
...
void launch_schobel_zhu_up_and_out_option_cuda(
    ...
    validate_schobel_zhu_up_and_out_option_launch(
        device_models,
        model_count,
        device_products,
        product_count,
    ...
    check_cuda(
        cudaOccupancyMaxActiveBlocksPerMultiprocessor(
            ...
            schobel_zhu_up_and_out_option_kernel<Side>,
            static_cast<int>(threads_per_block),
            shared_bytes
        ),
        "Schobel-Zhu Up-and-out option occupancy check"
    );
    if (active_blocks_per_multiprocessor == 0) {
        throw std::invalid_argument(
            "Schobel-Zhu Up-and-out option kernel cannot launch one block per SM."
        );
    }

    // Launch the Schobel-Zhu Up-and-out option kernel.
    ...
    check_cuda(cudaGetLastError(), "Schobel-Zhu Up-and-out option kernel");
}

// Build both public payoff specializations in this CUDA translation unit.
using LaunchSignature = decltype(launch_schobel_zhu_up_and_out_option_cuda<OptionSide::call>);
template LaunchSignature launch_schobel_zhu_up_and_out_option_cuda<OptionSide::call>;
template LaunchSignature launch_schobel_zhu_up_and_out_option_cuda<OptionSide::put>;

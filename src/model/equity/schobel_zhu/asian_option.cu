// Schobel-Zhu asian-option composition over generic CUDA layers.
...
void launch_schobel_zhu_asian_option_cuda(
    ...
    equity::launch_monte_carlo_cuda<PricingPolicy<Side>>(
        ...
        "Schobel-Zhu asian option kernel"
    );
}

template void launch_schobel_zhu_asian_option_cuda<OptionSide::call>(
...
template void launch_schobel_zhu_asian_option_cuda<OptionSide::put>(
    const ModelParameters*, std::size_t,
    const product::AsianOptionParameters*, std::size_t,
    bool, std::size_t, std::size_t, std::size_t, std::size_t,
    float, std::uint32_t, unsigned int, std::size_t, std::uint64_t,

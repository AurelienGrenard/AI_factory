// Schobel-Zhu lookback kernel with fused Philox simulation and payoff reduction.
...
__device__ __forceinline__ float evaluate_path(
    ...
    const SchobelZhuMaximumPathResult simulated =
...
__global__ void schobel_zhu_lookback_option_kernel(
...
void validate_schobel_zhu_lookback_option_launch(
    ...
    if (result_offset >= result_count
        ...
        throw std::invalid_argument(
            "The Schobel-Zhu lookback launch batch exceeds the result array."
        );
    }
...
void launch_schobel_zhu_lookback_option_cuda(
    ...
    validate_schobel_zhu_lookback_option_launch(
        device_models,
        model_count,
        device_products,
        product_count,
    ...
    check_cuda(
        cudaOccupancyMaxActiveBlocksPerMultiprocessor(
            ...
            schobel_zhu_lookback_option_kernel,
            static_cast<int>(threads_per_block),
            shared_bytes
        ),
        "Schobel-Zhu Lookback option occupancy check"
    );
    if (active_blocks_per_multiprocessor == 0) {
        throw std::invalid_argument(
            "Schobel-Zhu Lookback option kernel cannot launch one block per SM."
        );
    }

    // Launch the Schobel-Zhu lookback kernel.
    ...
    check_cuda(cudaGetLastError(), "Schobel-Zhu Lookback option kernel");
}

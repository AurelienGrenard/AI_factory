...
std::vector<SchobelZhuModelParameters> load_models(
    ...
    if (!stream) {
        throw std::runtime_error(
            "Could not open Schobel-Zhu JSON: " + dataset_path.string()
        );
    }
    ...
    try {
        ...
        throw std::runtime_error(
            "Invalid Schobel-Zhu JSON '" + dataset_path.string()
            + "': " + error.what()
        );
    }
    ...
    std::vector<SchobelZhuModelParameters> models;
    ...
    for (const auto& row : rows) {
        ...
        const std::string prefix =
            "Schobel-Zhu model row id '" + row_id + "': ";

// Schobel-Zhu Phoenix-autocall kernel with fused Philox simulation and payoff reduction.
...
__global__ void schobel_zhu_phoenix_autocall_kernel(
...
void validate_schobel_zhu_phoenix_autocall_launch(
    ...
    if (result_offset >= result_count
        ...
        throw std::invalid_argument(
            "The Schobel-Zhu Phoenix-autocall launch batch exceeds the result array."
        );
    }
...
void launch_schobel_zhu_phoenix_autocall_cuda(
    ...
    validate_schobel_zhu_phoenix_autocall_launch(
        device_models,
        model_count,
        device_products,
        product_count,
    ...
    check_cuda(
        cudaOccupancyMaxActiveBlocksPerMultiprocessor(
            ...
            schobel_zhu_phoenix_autocall_kernel,
            static_cast<int>(threads_per_block),
            shared_bytes
        ),
        "Schobel-Zhu Phoenix autocall occupancy check"
    );
    if (active_blocks_per_multiprocessor == 0) {
        throw std::invalid_argument(
            "Schobel-Zhu Phoenix autocall kernel cannot launch one block per SM."
        );
    }

    // Launch the Schobel-Zhu Phoenix-autocall kernel.
    ...
    check_cuda(cudaGetLastError(), "Schobel-Zhu Phoenix autocall kernel");
}

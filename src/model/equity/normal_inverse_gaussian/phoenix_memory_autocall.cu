// NormalInverseGaussian Phoenix-Memory-autocall kernel with fused Philox simulation and payoff reduction.
...
void validate_normal_inverse_gaussian_phoenix_memory_autocall_launch(
    ...
    if (result_offset >= result_count
        ...
        throw std::invalid_argument(
            "The NormalInverseGaussian Phoenix-Memory-autocall launch batch exceeds the result array."
        );
    }
...
void launch_normal_inverse_gaussian_phoenix_memory_autocall_cuda(
    ...
    validate_normal_inverse_gaussian_phoenix_memory_autocall_launch(
        device_models,
        model_count,
        device_products,
        product_count,
    ...
    check_cuda(
        cudaOccupancyMaxActiveBlocksPerMultiprocessor(
            ...
            normal_inverse_gaussian_phoenix_memory_autocall_kernel,
            static_cast<int>(threads_per_block),
            shared_bytes
        ),
        "NormalInverseGaussian Phoenix Memory autocall occupancy check"
    );
    if (active_blocks_per_multiprocessor == 0) {
        throw std::invalid_argument(
            "NormalInverseGaussian Phoenix Memory autocall kernel cannot launch one block per SM."
        );
    }

    // Launch the NormalInverseGaussian Phoenix-Memory-autocall kernel.
    ...
    check_cuda(cudaGetLastError(), "NormalInverseGaussian Phoenix Memory autocall kernel");
}

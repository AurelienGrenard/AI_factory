// Normal-Inverse-Gaussian digital-option composition over generic CUDA layers.
...
void launch_normal_inverse_gaussian_digital_option_cuda(
    ...
    equity::launch_monte_carlo_cuda<PricingPolicy<Side>>(
        ...
        "Normal-Inverse-Gaussian digital option kernel"
    );
}

template void launch_normal_inverse_gaussian_digital_option_cuda<OptionSide::call>(
...
template void launch_normal_inverse_gaussian_digital_option_cuda<OptionSide::put>(
    const ModelParameters*, std::size_t,
    const product::DigitalOptionParameters*, std::size_t,
    bool, std::size_t, std::size_t, std::size_t, std::size_t,
    float, unsigned int, std::size_t, std::uint64_t,

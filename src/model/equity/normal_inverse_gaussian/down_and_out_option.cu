// NormalInverseGaussian Down-and-out option kernel with fused Philox simulation and payoff reduction.
...
__device__ __forceinline__ PreparedRow prepare_row(
    ...
    const product::DownAndOutOptionParameters& product,
    float dt,
...
template<OptionSide Side>
__global__ void normal_inverse_gaussian_down_and_out_option_kernel(
    const ModelParameters* __restrict__ models,
    const product::DownAndOutOptionParameters* __restrict__ products,
...
void validate_normal_inverse_gaussian_down_and_out_option_launch(
    ...
    if (result_offset >= result_count
        ...
        throw std::invalid_argument(
            "The NormalInverseGaussian Down-and-out option launch batch exceeds the result array."
        );
    }
...
template<OptionSide Side>
void launch_normal_inverse_gaussian_down_and_out_option_cuda(
    const ModelParameters* device_models,
    std::size_t model_count,
    const product::DownAndOutOptionParameters* device_products,
    std::size_t product_count,
    ...
    validate_normal_inverse_gaussian_down_and_out_option_launch(
        device_models,
        model_count,
        device_products,
        product_count,
    ...
    check_cuda(
        cudaOccupancyMaxActiveBlocksPerMultiprocessor(
            ...
            normal_inverse_gaussian_down_and_out_option_kernel<Side>,
            static_cast<int>(threads_per_block),
            shared_bytes
        ),
        "NormalInverseGaussian Down-and-out option occupancy check"
    );
    if (active_blocks_per_multiprocessor == 0) {
        throw std::invalid_argument(
            "NormalInverseGaussian Down-and-out option kernel cannot launch one block per SM."
        );
    }

    // Launch the NormalInverseGaussian Down-and-out option kernel.
    ...
    check_cuda(cudaGetLastError(), "NormalInverseGaussian Down-and-out option kernel");
}

// Build both public payoff specializations in this CUDA translation unit.
using LaunchSignature = decltype(launch_normal_inverse_gaussian_down_and_out_option_cuda<OptionSide::call>);
template LaunchSignature launch_normal_inverse_gaussian_down_and_out_option_cuda<OptionSide::call>;
template LaunchSignature launch_normal_inverse_gaussian_down_and_out_option_cuda<OptionSide::put>;

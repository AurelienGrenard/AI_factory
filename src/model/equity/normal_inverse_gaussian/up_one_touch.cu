// NormalInverseGaussian Up-one-touch kernel with fused Philox simulation and payoff reduction.
...
__global__ void normal_inverse_gaussian_up_one_touch_kernel(
...
void validate_normal_inverse_gaussian_up_one_touch_launch(
    ...
    if (result_offset >= result_count
        ...
        throw std::invalid_argument(
            "The NormalInverseGaussian up-one-touch batch exceeds the result array."
        );
    }
...
void launch_normal_inverse_gaussian_up_one_touch_cuda(
    ...
    validate_normal_inverse_gaussian_up_one_touch_launch(
        device_models,
        model_count,
        device_products,
        product_count,
    ...
    check_cuda(
        cudaOccupancyMaxActiveBlocksPerMultiprocessor(
            ...
            normal_inverse_gaussian_up_one_touch_kernel,
            static_cast<int>(threads_per_block),
            shared_bytes
        ),
        "NormalInverseGaussian Up one-touch occupancy check"
    );
    if (active_blocks_per_multiprocessor == 0) {
        throw std::invalid_argument(
            "NormalInverseGaussian Up one-touch kernel cannot launch one block per SM."
        );
    }

    // Launch the NormalInverseGaussian Up-one-touch kernel.
    ...
    check_cuda(cudaGetLastError(), "NormalInverseGaussian Up one-touch kernel");
}

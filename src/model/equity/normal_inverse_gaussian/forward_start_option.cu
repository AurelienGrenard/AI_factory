// NormalInverseGaussian Forward-start option kernel with fused Philox simulation and payoff reduction.
...
struct PreparedRow {
    NormalInverseGaussianPreparedParameters reset_model;
    NormalInverseGaussianPreparedParameters remaining_model;
...
__device__ __forceinline__ float evaluate_path(
    ...
    const NormalInverseGaussianTwoTimePathResult simulated = simulate_at_two_times(
        row.reset_model,
        row.remaining_model,
        row.key,
        path,
...
template<OptionSide Side>
__global__ void normal_inverse_gaussian_forward_start_option_kernel(
...
void validate_normal_inverse_gaussian_forward_start_option_launch(
    ...
    if (result_offset >= result_count
        ...
        throw std::invalid_argument(
            "The NormalInverseGaussian forward-start option batch exceeds the result array."
        );
    }
...
void launch_normal_inverse_gaussian_forward_start_option_cuda(
    ...
    validate_normal_inverse_gaussian_forward_start_option_launch(
        device_models,
        model_count,
        device_products,
        product_count,
    ...
    check_cuda(
        cudaOccupancyMaxActiveBlocksPerMultiprocessor(
            ...
            normal_inverse_gaussian_forward_start_option_kernel<Side>,
            static_cast<int>(threads_per_block),
            shared_bytes
        ),
        "NormalInverseGaussian Forward-start option occupancy check"
    );
    if (active_blocks_per_multiprocessor == 0) {
        throw std::invalid_argument(
            "NormalInverseGaussian Forward-start option kernel cannot launch one block per SM."
        );
    }

    // Launch the NormalInverseGaussian Forward-start option kernel.
    ...
    check_cuda(cudaGetLastError(), "NormalInverseGaussian Forward-start option kernel");
}

// Build both public payoff specializations in this CUDA translation unit.
using LaunchSignature = decltype(launch_normal_inverse_gaussian_forward_start_option_cuda<OptionSide::call>);
template LaunchSignature launch_normal_inverse_gaussian_forward_start_option_cuda<OptionSide::call>;
template LaunchSignature launch_normal_inverse_gaussian_forward_start_option_cuda<OptionSide::put>;

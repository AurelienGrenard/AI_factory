// NormalInverseGaussian Up-no-touch kernel with fused Philox simulation and payoff reduction.
...
__device__ __forceinline__ PreparedRow prepare_row(
    ...
    const product::UpNoTouchParameters& product,
    float dt,
...
__device__ __forceinline__ float evaluate_path(
    ...
    if (expf(state.log_spot) >= row.barrier) return 0.0f;
...
__global__ void normal_inverse_gaussian_up_no_touch_kernel(
    const ModelParameters* __restrict__ models,
    const product::UpNoTouchParameters* __restrict__ products,
...
void validate_normal_inverse_gaussian_up_no_touch_launch(
    ...
    if (result_offset >= result_count
        ...
        throw std::invalid_argument(
            "The NormalInverseGaussian up-no-touch batch exceeds the result array."
        );
    }
...
void launch_normal_inverse_gaussian_up_no_touch_cuda(
    ...
    validate_normal_inverse_gaussian_up_no_touch_launch(
        device_models,
        model_count,
        device_products,
        product_count,
    ...
    check_cuda(
        cudaOccupancyMaxActiveBlocksPerMultiprocessor(
            ...
            normal_inverse_gaussian_up_no_touch_kernel,
            static_cast<int>(threads_per_block),
            shared_bytes
        ),
        "NormalInverseGaussian Up no-touch occupancy check"
    );
    if (active_blocks_per_multiprocessor == 0) {
        throw std::invalid_argument(
            "NormalInverseGaussian Up no-touch kernel cannot launch one block per SM."
        );
    }

    // Launch the NormalInverseGaussian Up-no-touch kernel.
    ...
    check_cuda(cudaGetLastError(), "NormalInverseGaussian Up no-touch kernel");
}

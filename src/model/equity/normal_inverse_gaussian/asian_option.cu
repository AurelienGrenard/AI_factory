// NormalInverseGaussian Asian-option kernel with fused Philox simulation and payoff reduction.
...
__device__ __forceinline__ float evaluate_path(
    ...
    const NormalInverseGaussianMeanPathResult simulated =
        simulate_mean_state(row.model, row.key, path, row.num_steps);
    if constexpr (Side == OptionSide::call)
        return row.discount
            * fmaxf(simulated.arithmetic_mean - row.strike, 0.0f);
    else
        return row.discount
            * fmaxf(row.strike - simulated.arithmetic_mean, 0.0f);
}

// Price rows through a bounded persistent grid and write FP32 result moments.
template<OptionSide Side>
__global__ void normal_inverse_gaussian_asian_option_kernel(
    const NormalInverseGaussianModelParameters* __restrict__ models,
    const product::AsianOptionParameters* __restrict__ products,
...
void validate_normal_inverse_gaussian_asian_option_launch(
    ...
    if (result_offset >= result_count
        ...
        throw std::invalid_argument(
            "The NormalInverseGaussian Asian-option launch batch exceeds the result array."
        );
    }
...
template<OptionSide Side>
void launch_normal_inverse_gaussian_asian_option_cuda(
    const NormalInverseGaussianModelParameters* device_models,
    std::size_t model_count,
    const product::AsianOptionParameters* device_products,
    std::size_t product_count,
    ...
    validate_normal_inverse_gaussian_asian_option_launch(
        device_models,
        model_count,
        device_products,
        product_count,
    ...
    check_cuda(
        cudaOccupancyMaxActiveBlocksPerMultiprocessor(
            ...
            normal_inverse_gaussian_asian_option_kernel<Side>,
            static_cast<int>(threads_per_block),
            shared_bytes
        ),
        "NormalInverseGaussian Asian option occupancy check"
    );
    if (active_blocks_per_multiprocessor == 0) {
        throw std::invalid_argument(
            "NormalInverseGaussian Asian option kernel cannot launch one block per SM."
        );
    }

    // Launch the NormalInverseGaussian Asian-option kernel.
    ...
    check_cuda(cudaGetLastError(), "NormalInverseGaussian Asian option kernel");
}

// Build both public payoff specializations in this CUDA translation unit.
using LaunchSignature = decltype(launch_normal_inverse_gaussian_asian_option_cuda<OptionSide::call>);
template LaunchSignature launch_normal_inverse_gaussian_asian_option_cuda<OptionSide::call>;
template LaunchSignature launch_normal_inverse_gaussian_asian_option_cuda<OptionSide::put>;

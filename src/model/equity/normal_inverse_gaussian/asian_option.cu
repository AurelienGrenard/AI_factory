// Normal-Inverse-Gaussian asian-option composition over generic CUDA layers.
...
using PricingPolicy =
    product::AsianOptionPricingPolicy<Schedule, Side>;
...
void launch_normal_inverse_gaussian_asian_option_cuda(
    ...
    monte_carlo::launch_monte_carlo_cuda<PricingPolicy<Side>>(
        ...
        "Normal-Inverse-Gaussian asian option kernel"
    );
}

template void launch_normal_inverse_gaussian_asian_option_cuda<OptionSide::call>(
...
template void launch_normal_inverse_gaussian_asian_option_cuda<OptionSide::put>(
    const ModelParameters*, std::size_t,
    const product::AsianOptionParameters*, std::size_t,
    bool, std::size_t, std::size_t, std::size_t, std::size_t,
    float, std::uint32_t, unsigned int, std::size_t, std::uint64_t,

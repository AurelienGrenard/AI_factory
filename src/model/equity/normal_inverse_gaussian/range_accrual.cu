// NormalInverseGaussian Range Accrual kernel with fused Philox simulation and payoff reduction.
...
__device__ __forceinline__ PreparedRow prepare_row(
    ...
    const NormalInverseGaussianPreparedParameters prepared_model = prepare_model(model, product.observation_interval);
...
__global__ void normal_inverse_gaussian_range_accrual_kernel(
...
void validate_normal_inverse_gaussian_range_accrual_launch(
    ...
    if (result_offset >= result_count
        ...
        throw std::invalid_argument(
            "The NormalInverseGaussian Range Accrual launch batch exceeds the result array."
        );
    }
...
void launch_normal_inverse_gaussian_range_accrual_cuda(
    ...
    validate_normal_inverse_gaussian_range_accrual_launch(
        device_models,
        model_count,
        device_products,
        product_count,
    ...
    check_cuda(
        cudaOccupancyMaxActiveBlocksPerMultiprocessor(
            ...
            normal_inverse_gaussian_range_accrual_kernel,
            static_cast<int>(threads_per_block),
            shared_bytes
        ),
        "NormalInverseGaussian Range Accrual occupancy check"
    );
    if (active_blocks_per_multiprocessor == 0) {
        throw std::invalid_argument(
            "NormalInverseGaussian Range Accrual kernel cannot launch one block per SM."
        );
    }

    // Launch the NormalInverseGaussian Range Accrual kernel.
    ...
    check_cuda(cudaGetLastError(), "NormalInverseGaussian Range Accrual kernel");
}

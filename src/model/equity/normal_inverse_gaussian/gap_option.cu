// NormalInverseGaussian gap-option kernel with fused Philox simulation and payoff reduction.
...
template<OptionSide Side>
__global__ void normal_inverse_gaussian_gap_option_kernel(
...
void validate_normal_inverse_gaussian_gap_option_launch(
    ...
    if (result_offset >= result_count
        ...
        throw std::invalid_argument(
            "The NormalInverseGaussian gap-option launch batch exceeds the result array."
        );
    }
...
template<OptionSide Side>
void launch_normal_inverse_gaussian_gap_option_cuda(
    const ModelParameters* device_models,
    std::size_t model_count,
    const product::GapOptionParameters* device_products,
    std::size_t product_count,
    ...
    validate_normal_inverse_gaussian_gap_option_launch(
        device_models,
        model_count,
        device_products,
        product_count,
    ...
    check_cuda(
        cudaOccupancyMaxActiveBlocksPerMultiprocessor(
            ...
            normal_inverse_gaussian_gap_option_kernel<Side>,
            static_cast<int>(threads_per_block),
            shared_bytes
        ),
        "NormalInverseGaussian Gap option occupancy check"
    );
    if (active_blocks_per_multiprocessor == 0) {
        throw std::invalid_argument(
            "NormalInverseGaussian Gap option kernel cannot launch one block per SM."
        );
    }

    // Launch the NormalInverseGaussian Gap-option kernel.
    ...
    check_cuda(cudaGetLastError(), "NormalInverseGaussian Gap option kernel");
}

// Build both public payoff specializations in this CUDA translation unit.
using LaunchSignature = decltype(launch_normal_inverse_gaussian_gap_option_cuda<OptionSide::call>);
template LaunchSignature launch_normal_inverse_gaussian_gap_option_cuda<OptionSide::call>;
template LaunchSignature launch_normal_inverse_gaussian_gap_option_cuda<OptionSide::put>;

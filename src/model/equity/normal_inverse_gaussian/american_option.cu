...
StateRegions normal_inverse_gaussian_state_regions(const lsm::WorkspaceLayout& layout) {
...
lsm::LaunchResult launch_normal_inverse_gaussian_american_option_cuda(
    ...
    for (const lsm::BatchPlan& batch : plan.batches) {
        ...
        const StateRegions states = normal_inverse_gaussian_state_regions(layout);

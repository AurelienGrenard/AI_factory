// Shared multi-block NormalInverseGaussian American-option pricing with GPU Longstaff-Schwartz.
...
struct PreparedRow {
    NormalInverseGaussianPreparedParameters initial_stub_model;
    NormalInverseGaussianPreparedParameters regular_model;
...
struct NormalInverseGaussianStateRegions {
...
NormalInverseGaussianStateRegions normal_inverse_gaussian_state_regions(const lsm::WorkspaceLayout& layout) {
    if (layout.state_fields.size() != 1U) {
        throw std::logic_error(
            "Normal-Inverse-Gaussian American options require one state field."
        );
    }
...
__global__ void prepare_rows_kernel(
    ...
    const NormalInverseGaussianModelParameters model = models[model_index];
...
__global__ void simulate_paths_kernel(
    ...
    for (std::size_t path = first_path;
         ...
         path += path_stride) {
        const NormalInverseGaussianState terminal = simulate_on_regular_grid(
...
void validate_normal_inverse_gaussian_american_option_launch(
...
template <OptionSide Side>
lsm::LaunchResult launch_normal_inverse_gaussian_american_option_cuda(
    const NormalInverseGaussianModelParameters* device_models,
    std::size_t model_count,
    const product::AmericanOptionParameters* host_products,
    const product::AmericanOptionParameters* device_products,
    ...
    validate_normal_inverse_gaussian_american_option_launch(
        device_models,
        model_count,
        host_products,
        device_products,
    ...
    for (const lsm::BatchPlan& batch : plan.batches) {
        ...
        const NormalInverseGaussianStateRegions states = normal_inverse_gaussian_state_regions(layout);
        ...
        report_cuda_kernel_launch_if_enabled(
            "normal_inverse_gaussian.american_option.prepare_rows",
            "common",
            prepare_rows_kernel,
            dim3(row_blocks),
        ...
        report_cuda_kernel_launch_if_enabled(
            "normal_inverse_gaussian.american_option.simulate_paths",
        ...
        for (std::uint32_t backward_level = 1U;
             ...
             ++backward_level) {
            report_cuda_kernel_launch_if_enabled(
                "normal_inverse_gaussian.american_option.regression_partials",
                option_side_name(Side),
            ...
            report_cuda_kernel_launch_if_enabled(
                "normal_inverse_gaussian.american_option.solve_regressions",
                "common",
            ...
            report_cuda_kernel_launch_if_enabled(
                "normal_inverse_gaussian.american_option.update_cashflows",
                option_side_name(Side),
        ...
        report_cuda_kernel_launch_if_enabled(
            "normal_inverse_gaussian.american_option.moment_partials",
            "common",
        ...
        report_cuda_kernel_launch_if_enabled(
            "normal_inverse_gaussian.american_option.finalize_prices",
            option_side_name(Side),

// CEV asset-or-nothing kernel with fused simulation and payoff reduction.
...
template<OptionSide Side>
__global__ void cev_asset_or_nothing_option_kernel(
...
void validate_cev_asset_or_nothing_option_launch(
    ...
    if (result_offset >= result_count
        ...
        throw std::invalid_argument(
            "The CEV asset-or-nothing launch batch exceeds the result array."
        );
    }
...
void launch_cev_asset_or_nothing_option_cuda(
    ...
    validate_cev_asset_or_nothing_option_launch(
        device_models,
        model_count,
        device_products,
        product_count,
    ...
    check_cuda(
        cudaOccupancyMaxActiveBlocksPerMultiprocessor(
            ...
            cev_asset_or_nothing_option_kernel<Side>,
            static_cast<int>(threads_per_block),
            shared_bytes
        ),
        "CEV asset-or-nothing option occupancy check"
    );
    if (active_blocks_per_multiprocessor == 0) {
        throw std::invalid_argument(
            "CEV asset-or-nothing option kernel cannot launch one block per SM."
        );
    }

    // Launch the CEV Asset-or-nothing kernel.
    ...
    check_cuda(cudaGetLastError(), "CEV asset-or-nothing option kernel");
}

// Build both public payoff specializations in this CUDA translation unit.
using LaunchSignature = decltype(launch_cev_asset_or_nothing_option_cuda<OptionSide::call>);
template LaunchSignature launch_cev_asset_or_nothing_option_cuda<OptionSide::call>;
template LaunchSignature launch_cev_asset_or_nothing_option_cuda<OptionSide::put>;

// CEV Range Accrual kernel with fused Philox simulation and payoff reduction.
...
__device__ __forceinline__ PreparedRow prepare_row(
    ...
    return {
        ...
        prepared_model.initial_spot * product.lower_barrier,
        prepared_model.initial_spot * product.upper_barrier,
        maturity_discount,
        maturity_discount
            * product.coupon_rate
            * product.observation_interval,
...
__device__ __forceinline__ float evaluate_path(
    ...
    for (std::uint32_t observation = 0U;
        ...
        in_range_count += static_cast<std::uint32_t>(
            state.spot >= row.lower_spot
            && state.spot <= row.upper_spot
        );
    }
    return fmaf(
...
void validate_cev_range_accrual_launch(
    ...
    if (result_offset >= result_count
        ...
        throw std::invalid_argument(
            "The CEV Range Accrual launch batch exceeds the result array."
        );
    }
...
void launch_cev_range_accrual_cuda(
    ...
    validate_cev_range_accrual_launch(
        device_models,
        model_count,
        device_products,
        product_count,
    ...
    check_cuda(
        cudaOccupancyMaxActiveBlocksPerMultiprocessor(
            ...
            cev_range_accrual_kernel,
            static_cast<int>(threads_per_block),
            shared_bytes
        ),
        "CEV Range Accrual occupancy check"
    );
    if (active_blocks_per_multiprocessor == 0) {
        throw std::invalid_argument(
            "CEV Range Accrual kernel cannot launch one block per SM."
        );
    }

    // Launch the CEV Range Accrual kernel.
    ...
    check_cuda(cudaGetLastError(), "CEV Range Accrual kernel");
}

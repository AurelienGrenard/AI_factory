// CEV geometric-asian-option composition over generic CUDA layers.
...
void launch_cev_geometric_asian_option_cuda(
    ...
    equity::launch_monte_carlo_cuda<PricingPolicy<Side>>(
        ...
        "CEV geometric asian option kernel"
    );
}

template void launch_cev_geometric_asian_option_cuda<OptionSide::call>(
...
template void launch_cev_geometric_asian_option_cuda<OptionSide::put>(
    const ModelParameters*, std::size_t,
    const product::GeometricAsianOptionParameters*, std::size_t,
    bool, std::size_t, std::size_t, std::size_t, std::size_t,
    float, std::uint32_t, unsigned int, std::size_t, std::uint64_t,

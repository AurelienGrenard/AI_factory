// CEV Geometric-Asian option kernel with fused Philox simulation and payoff reduction.
...
__device__ __forceinline__ float evaluate_path(
    ...
    const CevGeometricMeanPathResult simulated =
...
template<OptionSide Side>
__global__ void cev_geometric_asian_option_kernel(
...
void validate_cev_geometric_asian_option_launch(
    ...
    if (result_offset >= result_count
        ...
        throw std::invalid_argument(
            "The CEV Geometric-Asian option launch batch exceeds the result array."
        );
    }
...
template<OptionSide Side>
void launch_cev_geometric_asian_option_cuda(
    const CevModelParameters* device_models,
    std::size_t model_count,
    const product::GeometricAsianOptionParameters* device_products,
    std::size_t product_count,
    ...
    validate_cev_geometric_asian_option_launch(
        device_models,
        model_count,
        device_products,
        product_count,
    ...
    check_cuda(
        cudaOccupancyMaxActiveBlocksPerMultiprocessor(
            ...
            cev_geometric_asian_option_kernel<Side>,
            static_cast<int>(threads_per_block),
            shared_bytes
        ),
        "CEV Geometric Asian option occupancy check"
    );
    if (active_blocks_per_multiprocessor == 0) {
        throw std::invalid_argument(
            "CEV Geometric Asian option kernel cannot launch one block per SM."
        );
    }

    // Launch the CEV Geometric-Asian option kernel.
    ...
    check_cuda(cudaGetLastError(), "CEV Geometric Asian option kernel");
}

// Build both public payoff specializations in this CUDA translation unit.
using LaunchSignature = decltype(launch_cev_geometric_asian_option_cuda<OptionSide::call>);
template LaunchSignature launch_cev_geometric_asian_option_cuda<OptionSide::call>;
template LaunchSignature launch_cev_geometric_asian_option_cuda<OptionSide::put>;

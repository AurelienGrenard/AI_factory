// CEV European-option kernel with fused Philox simulation and payoff reduction.
#include "model/equity/cev/european_option.cuh"
...
template<OptionSide Side>
__global__ void cev_european_option_kernel(
...
void validate_cev_european_option_launch(
    ...
    if (result_offset >= result_count
        ...
        throw std::invalid_argument(
            "The CEV European-option launch batch exceeds the result array."
        );
    }
...
template<OptionSide Side>
void launch_cev_european_option_cuda(
    const CevModelParameters* device_models,
    std::size_t model_count,
    const product::EuropeanOptionParameters* device_products,
    std::size_t product_count,
    ...
    validate_cev_european_option_launch(
        device_models,
        model_count,
        device_products,
        product_count,
    ...
    check_cuda(
        cudaOccupancyMaxActiveBlocksPerMultiprocessor(
            ...
            cev_european_option_kernel<Side>,
            static_cast<int>(threads_per_block),
            shared_bytes
        ),
        "CEV European option occupancy check"
    );
    if (active_blocks_per_multiprocessor == 0) {
        throw std::invalid_argument(
            "CEV European option kernel cannot launch one block per SM."
        );
    }

    // Launch the CEV European-option kernel.
    ...
    check_cuda(cudaGetLastError(), "CEV European option kernel");
}

// Build both public payoff specializations in this CUDA translation unit.
using LaunchSignature = decltype(launch_cev_european_option_cuda<OptionSide::call>);
template LaunchSignature launch_cev_european_option_cuda<OptionSide::call>;
template LaunchSignature launch_cev_european_option_cuda<OptionSide::put>;

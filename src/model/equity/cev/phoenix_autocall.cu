// CEV Phoenix-autocall kernel with fused Philox simulation and payoff reduction.
...
__global__ void cev_phoenix_autocall_kernel(
...
void validate_cev_phoenix_autocall_launch(
    ...
    if (result_offset >= result_count
        ...
        throw std::invalid_argument(
            "The CEV Phoenix-autocall launch batch exceeds the result array."
        );
    }
...
void launch_cev_phoenix_autocall_cuda(
    const ModelParameters* device_models,
    std::size_t model_count,
    const product::PhoenixAutocallParameters* device_products,
    std::size_t product_count,
    ...
    validate_cev_phoenix_autocall_launch(
        device_models,
        model_count,
        device_products,
        product_count,
    ...
    check_cuda(
        cudaOccupancyMaxActiveBlocksPerMultiprocessor(
            ...
            cev_phoenix_autocall_kernel,
            static_cast<int>(threads_per_block),
            shared_bytes
        ),
        "CEV Phoenix autocall occupancy check"
    );
    if (active_blocks_per_multiprocessor == 0) {
        throw std::invalid_argument(
            "CEV Phoenix autocall kernel cannot launch one block per SM."
        );
    }

    // Launch the CEV Phoenix-autocall kernel.
    ...
    check_cuda(cudaGetLastError(), "CEV Phoenix autocall kernel");
}

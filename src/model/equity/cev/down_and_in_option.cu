// CEV Down-and-in option kernel with fused Philox simulation and payoff reduction.
...
__device__ __forceinline__ float evaluate_path(
    ...
    bool activated = state.spot <= row.barrier;
    ...
    for (std::size_t step_index = 0U;
        ...
        if (!activated) activated = state.spot <= row.barrier;
    }
    if (!activated) return 0.0f;
    const float terminal_spot = state.spot;
...
template<OptionSide Side>
__global__ void cev_down_and_in_option_kernel(
...
void validate_cev_down_and_in_option_launch(
    ...
    if (result_offset >= result_count
        ...
        throw std::invalid_argument(
            "The CEV Down-and-in option launch batch exceeds the result array."
        );
    }
...
void launch_cev_down_and_in_option_cuda(
    ...
    validate_cev_down_and_in_option_launch(
        device_models,
        model_count,
        device_products,
        product_count,
    ...
    check_cuda(
        cudaOccupancyMaxActiveBlocksPerMultiprocessor(
            ...
            cev_down_and_in_option_kernel<Side>,
            static_cast<int>(threads_per_block),
            shared_bytes
        ),
        "CEV Down-and-in option occupancy check"
    );
    if (active_blocks_per_multiprocessor == 0) {
        throw std::invalid_argument(
            "CEV Down-and-in option kernel cannot launch one block per SM."
        );
    }

    // Launch the CEV Down-and-in option kernel.
    ...
    check_cuda(cudaGetLastError(), "CEV Down-and-in option kernel");
}

// Build both public payoff specializations in this CUDA translation unit.
using LaunchSignature = decltype(launch_cev_down_and_in_option_cuda<OptionSide::call>);
template LaunchSignature launch_cev_down_and_in_option_cuda<OptionSide::call>;
template LaunchSignature launch_cev_down_and_in_option_cuda<OptionSide::put>;

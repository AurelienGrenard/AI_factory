// CEV Double-knock-out option kernel with fused Philox simulation and payoff reduction.
#include "model/equity/cev/double_knock_out_option.cuh"
...
template<OptionSide Side>
__global__ void cev_double_knock_out_option_kernel(
...
void validate_cev_double_knock_out_option_launch(
    ...
    if (result_offset >= result_count
        ...
        throw std::invalid_argument(
            "The CEV double-knock-out option batch exceeds the result array."
        );
    }
...
template<OptionSide Side>
void launch_cev_double_knock_out_option_cuda(
    const ModelParameters* device_models,
    std::size_t model_count,
    const product::DoubleKnockOutOptionParameters* device_products,
    std::size_t product_count,
    ...
    validate_cev_double_knock_out_option_launch(
        device_models,
        model_count,
        device_products,
        product_count,
    ...
    check_cuda(
        cudaOccupancyMaxActiveBlocksPerMultiprocessor(
            ...
            cev_double_knock_out_option_kernel<Side>,
            static_cast<int>(threads_per_block),
            shared_bytes
        ),
        "CEV Double-knock-out option occupancy check"
    );
    if (active_blocks_per_multiprocessor == 0) {
        throw std::invalid_argument(
            "CEV Double-knock-out option kernel cannot launch one block per SM."
        );
    }

    // Launch the CEV Double-knock-out option kernel.
    ...
    check_cuda(cudaGetLastError(), "CEV Double-knock-out option kernel");
}

// Build both public payoff specializations in this CUDA translation unit.
using LaunchSignature = decltype(launch_cev_double_knock_out_option_cuda<OptionSide::call>);
template LaunchSignature launch_cev_double_knock_out_option_cuda<OptionSide::call>;
template LaunchSignature launch_cev_double_knock_out_option_cuda<OptionSide::put>;

// CEV Forward-start option kernel with fused Philox simulation and payoff reduction.
...
__device__ __forceinline__ float evaluate_path(
    ...
    const float reset_spot = simulated.first_state.spot;
    const float terminal_spot = simulated.terminal_state.spot;
...
void validate_cev_forward_start_option_launch(
    ...
    if (result_offset >= result_count
        ...
        throw std::invalid_argument(
            "The CEV forward-start option batch exceeds the result array."
        );
    }
...
void launch_cev_forward_start_option_cuda(
    ...
    validate_cev_forward_start_option_launch(
        device_models,
        model_count,
        device_products,
        product_count,
    ...
    check_cuda(
        cudaOccupancyMaxActiveBlocksPerMultiprocessor(
            ...
            cev_forward_start_option_kernel<Side>,
            static_cast<int>(threads_per_block),
            shared_bytes
        ),
        "CEV Forward-start option occupancy check"
    );
    if (active_blocks_per_multiprocessor == 0) {
        throw std::invalid_argument(
            "CEV Forward-start option kernel cannot launch one block per SM."
        );
    }

    // Launch the CEV Forward-start option kernel.
    ...
    check_cuda(cudaGetLastError(), "CEV Forward-start option kernel");
}

// Build both public payoff specializations in this CUDA translation unit.
using LaunchSignature = decltype(launch_cev_forward_start_option_cuda<OptionSide::call>);
template LaunchSignature launch_cev_forward_start_option_cuda<OptionSide::call>;
template LaunchSignature launch_cev_forward_start_option_cuda<OptionSide::put>;

// CEV lookback kernel with fused Philox simulation and payoff reduction.
...
__device__ __forceinline__ float evaluate_path(
    ...
    const CevMaximumPathResult simulated =
...
__global__ void cev_lookback_option_kernel(
...
void validate_cev_lookback_option_launch(
    ...
    if (result_offset >= result_count
        ...
        throw std::invalid_argument(
            "The CEV lookback launch batch exceeds the result array."
        );
    }
...
void launch_cev_lookback_option_cuda(
    ...
    validate_cev_lookback_option_launch(
        device_models,
        model_count,
        device_products,
        product_count,
    ...
    check_cuda(
        cudaOccupancyMaxActiveBlocksPerMultiprocessor(
            ...
            cev_lookback_option_kernel,
            static_cast<int>(threads_per_block),
            shared_bytes
        ),
        "CEV Lookback option occupancy check"
    );
    if (active_blocks_per_multiprocessor == 0) {
        throw std::invalid_argument(
            "CEV Lookback option kernel cannot launch one block per SM."
        );
    }

    // Launch the CEV lookback kernel.
    ...
    check_cuda(cudaGetLastError(), "CEV Lookback option kernel");
}

// CEV straddle kernel with fused Philox simulation and payoff reduction.
...
__global__ void cev_straddle_kernel(
...
void validate_cev_straddle_launch(
    ...
    if (result_offset >= result_count
        ...
        throw std::invalid_argument(
            "The CEV straddle launch batch exceeds the result array."
        );
    }
...
void launch_cev_straddle_cuda(
    ...
    check_cuda(
        ...
        "CEV straddle occupancy check"
    );
    if (active_blocks_per_multiprocessor == 0) {
        throw std::invalid_argument(
            "CEV straddle kernel cannot launch one block per SM."
        );
    }

    // Launch the CEV Straddle kernel.
    ...
    check_cuda(cudaGetLastError(), "CEV straddle kernel");
}

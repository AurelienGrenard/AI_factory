// CEV Up-no-touch kernel with fused Philox simulation and payoff reduction.
...
__device__ __forceinline__ float evaluate_path(
    ...
    if (state.spot >= row.barrier) return 0.0f;
...
__global__ void cev_up_no_touch_kernel(
...
void validate_cev_up_no_touch_launch(
    ...
    if (result_offset >= result_count
        ...
        throw std::invalid_argument(
            "The CEV up-no-touch batch exceeds the result array."
        );
    }
...
void launch_cev_up_no_touch_cuda(
    ...
    validate_cev_up_no_touch_launch(
        device_models,
        model_count,
        device_products,
        product_count,
    ...
    check_cuda(
        cudaOccupancyMaxActiveBlocksPerMultiprocessor(
            ...
            cev_up_no_touch_kernel,
            static_cast<int>(threads_per_block),
            shared_bytes
        ),
        "CEV Up no-touch occupancy check"
    );
    if (active_blocks_per_multiprocessor == 0) {
        throw std::invalid_argument(
            "CEV Up no-touch kernel cannot launch one block per SM."
        );
    }

    // Launch the CEV Up-no-touch kernel.
    ...
    check_cuda(cudaGetLastError(), "CEV Up no-touch kernel");
}

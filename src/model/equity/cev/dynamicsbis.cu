// Experimental autonomous implementation of absorbed Milstein CEV paths.
#include "model/equity/cev/dynamicsbis.cuh"
...
__device__ __forceinline__ void one_step_transition(
    ...
    const float proposal = spot
        ...
        + prepared_model.milstein_scale * spot_milstein_power
            * (normal * normal - 1.0f);
...
__device__ __forceinline__ void DynamicsPolicy::simulate_one_step(
    ...
    const float normal = philox::next_normal(
        random.uniforms,
        random.normals
    );
    cev::one_step_transition(dynamics, normal, state);
}

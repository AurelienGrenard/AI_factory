// CEV Cliquet kernel with fused Philox simulation and payoff reduction.
...
__device__ __forceinline__ float evaluate_path(
    ...
    float previous_spot = state.spot;
...
__global__ void cev_cliquet_kernel(
...
void validate_cev_cliquet_launch(
    ...
    if (result_offset >= result_count
        ...
        throw std::invalid_argument(
            "The CEV Cliquet launch batch exceeds the result array."
        );
    }
...
void launch_cev_cliquet_cuda(
    ...
    check_cuda(
        ...
        "CEV Cliquet occupancy check"
    );
    if (active_blocks_per_multiprocessor == 0) {
        throw std::invalid_argument(
            "CEV Cliquet kernel cannot launch one block per SM."
        );
    }

    // Launch the CEV Cliquet kernel.
    ...
    check_cuda(cudaGetLastError(), "CEV Cliquet kernel");
}

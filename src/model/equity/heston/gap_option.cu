// Heston gap-option kernel with fused Philox simulation and payoff reduction.
...
template<OptionSide Side>
__global__ void heston_gap_option_kernel(
...
void validate_heston_gap_option_launch(
    ...
    if (result_offset >= result_count
        ...
        throw std::invalid_argument(
            "The Heston gap-option launch batch exceeds the result array."
        );
    }
...
void launch_heston_gap_option_cuda(
    ...
    validate_heston_gap_option_launch(
        device_models,
        model_count,
        device_products,
        product_count,
    ...
    check_cuda(
        cudaOccupancyMaxActiveBlocksPerMultiprocessor(
            ...
            heston_gap_option_kernel<Side>,
            static_cast<int>(threads_per_block),
            shared_bytes
        ),
        "Heston Gap option occupancy check"
    );
    if (active_blocks_per_multiprocessor == 0) {
        throw std::invalid_argument(
            "Heston Gap option kernel cannot launch one block per SM."
        );
    }

    // Launch the Heston Gap-option kernel.
    ...
    check_cuda(cudaGetLastError(), "Heston Gap option kernel");
}

// Build both public payoff specializations in this CUDA translation unit.
using LaunchSignature = decltype(launch_heston_gap_option_cuda<OptionSide::call>);
template LaunchSignature launch_heston_gap_option_cuda<OptionSide::call>;
template LaunchSignature launch_heston_gap_option_cuda<OptionSide::put>;

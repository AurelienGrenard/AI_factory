// Heston Asian-option kernel with fused Philox simulation and payoff reduction.
...
__device__ __forceinline__ float evaluate_path(
    ...
    const HestonMeanPathResult simulated =
...
template<OptionSide Side>
__global__ void heston_asian_option_kernel(
...
void validate_heston_asian_option_launch(
    ...
    if (result_offset >= result_count
        ...
        throw std::invalid_argument(
            "The Heston Asian-option launch batch exceeds the result array."
        );
    }
...
void launch_heston_asian_option_cuda(
    ...
    validate_heston_asian_option_launch(
        device_models,
        model_count,
        device_products,
        product_count,
    ...
    check_cuda(
        cudaOccupancyMaxActiveBlocksPerMultiprocessor(
            ...
            heston_asian_option_kernel<Side>,
            static_cast<int>(threads_per_block),
            shared_bytes
        ),
        "Heston Asian option occupancy check"
    );
    if (active_blocks_per_multiprocessor == 0) {
        throw std::invalid_argument(
            "Heston Asian option kernel cannot launch one block per SM."
        );
    }

    // Launch the Heston Asian-option kernel.
    ...
    check_cuda(cudaGetLastError(), "Heston Asian option kernel");
}

// Build both public payoff specializations in this CUDA translation unit.
using LaunchSignature = decltype(launch_heston_asian_option_cuda<OptionSide::call>);
template LaunchSignature launch_heston_asian_option_cuda<OptionSide::call>;
template LaunchSignature launch_heston_asian_option_cuda<OptionSide::put>;

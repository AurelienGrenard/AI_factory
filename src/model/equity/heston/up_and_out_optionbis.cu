// Experimental Heston up-and-out composition over generic CUDA layers.
...
#include "product/up_and_out_option/pricing_policybis.cuh"
...
template<OptionSide Side>
void launch_heston_up_and_out_optionbis_cuda(
    ...
    equity::launch_monte_carlo_cuda<PricingPolicy<Side>>(
        ...
        "heston.up_and_out_optionbis",
        option_side_name(Side),
        "Heston up-and-out option bis kernel"
    );
}

template void launch_heston_up_and_out_optionbis_cuda<OptionSide::call>(
...
template void launch_heston_up_and_out_optionbis_cuda<OptionSide::put>(
    const ModelParameters*, std::size_t,
    const product::UpAndOutOptionParameters*, std::size_t,
    bool, std::size_t, std::size_t, std::size_t, std::size_t,
    float, std::uint32_t, unsigned int, std::size_t, std::uint64_t,

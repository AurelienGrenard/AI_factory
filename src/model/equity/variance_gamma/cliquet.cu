// VarianceGamma Cliquet kernel with fused Philox simulation and payoff reduction.
...
__global__ void variance_gamma_cliquet_kernel(
...
void validate_variance_gamma_cliquet_launch(
    ...
    if (result_offset >= result_count
        ...
        throw std::invalid_argument(
            "The VarianceGamma Cliquet launch batch exceeds the result array."
        );
    }
...
void launch_variance_gamma_cliquet_cuda(
    ...
    validate_variance_gamma_cliquet_launch(
        device_models,
        model_count,
        device_products,
        product_count,
    ...
    check_cuda(
        cudaOccupancyMaxActiveBlocksPerMultiprocessor(
            ...
            variance_gamma_cliquet_kernel,
            static_cast<int>(threads_per_block),
            shared_bytes
        ),
        "VarianceGamma Cliquet occupancy check"
    );
    if (active_blocks_per_multiprocessor == 0) {
        throw std::invalid_argument(
            "VarianceGamma Cliquet kernel cannot launch one block per SM."
        );
    }

    // Launch the VarianceGamma Cliquet kernel.
    ...
    check_cuda(cudaGetLastError(), "VarianceGamma Cliquet kernel");
}

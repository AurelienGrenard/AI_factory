// VarianceGamma Athena-autocall kernel with fused Philox simulation and payoff reduction.
...
__global__ void variance_gamma_athena_autocall_kernel(
...
void validate_variance_gamma_athena_autocall_launch(
    ...
    if (result_offset >= result_count
        ...
        throw std::invalid_argument(
            "The VarianceGamma Athena-autocall launch batch exceeds the result array."
        );
    }
...
void launch_variance_gamma_athena_autocall_cuda(
    ...
    validate_variance_gamma_athena_autocall_launch(
        device_models,
        model_count,
        device_products,
        product_count,
    ...
    check_cuda(
        cudaOccupancyMaxActiveBlocksPerMultiprocessor(
            ...
            variance_gamma_athena_autocall_kernel,
            static_cast<int>(threads_per_block),
            shared_bytes
        ),
        "VarianceGamma Athena autocall occupancy check"
    );
    if (active_blocks_per_multiprocessor == 0) {
        throw std::invalid_argument(
            "VarianceGamma Athena autocall kernel cannot launch one block per SM."
        );
    }

    // Launch the VarianceGamma Athena-autocall kernel.
    ...
    check_cuda(cudaGetLastError(), "VarianceGamma Athena autocall kernel");
}

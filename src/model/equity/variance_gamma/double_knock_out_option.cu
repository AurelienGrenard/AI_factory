// VarianceGamma Double-knock-out option kernel with fused Philox simulation and payoff reduction.
...
template<OptionSide Side>
__global__ void variance_gamma_double_knock_out_option_kernel(
...
void validate_variance_gamma_double_knock_out_option_launch(
    ...
    if (result_offset >= result_count
        ...
        throw std::invalid_argument(
            "The VarianceGamma double-knock-out option batch exceeds the result array."
        );
    }
...
template<OptionSide Side>
void launch_variance_gamma_double_knock_out_option_cuda(
    const ModelParameters* device_models,
    std::size_t model_count,
    const product::DoubleKnockOutOptionParameters* device_products,
    std::size_t product_count,
    ...
    validate_variance_gamma_double_knock_out_option_launch(
        device_models,
        model_count,
        device_products,
        product_count,
    ...
    check_cuda(
        cudaOccupancyMaxActiveBlocksPerMultiprocessor(
            ...
            variance_gamma_double_knock_out_option_kernel<Side>,
            static_cast<int>(threads_per_block),
            shared_bytes
        ),
        "VarianceGamma Double-knock-out option occupancy check"
    );
    if (active_blocks_per_multiprocessor == 0) {
        throw std::invalid_argument(
            "VarianceGamma Double-knock-out option kernel cannot launch one block per SM."
        );
    }

    // Launch the VarianceGamma Double-knock-out option kernel.
    ...
    check_cuda(cudaGetLastError(), "VarianceGamma Double-knock-out option kernel");
}

// Build both public payoff specializations in this CUDA translation unit.
using LaunchSignature = decltype(launch_variance_gamma_double_knock_out_option_cuda<OptionSide::call>);
template LaunchSignature launch_variance_gamma_double_knock_out_option_cuda<OptionSide::call>;
template LaunchSignature launch_variance_gamma_double_knock_out_option_cuda<OptionSide::put>;

// Variance-Gamma forward-start-option composition over generic CUDA layers.
...
using Schedule = simulation::ExactTransitionCalendarSchedule<variance_gamma::DynamicsPolicy, 2U>;
...
void launch_variance_gamma_forward_start_option_cuda(
    ...
    monte_carlo::launch_monte_carlo_cuda<PricingPolicy<Side>>(
        ...
        "Variance-Gamma forward start option kernel"
    );
}

template void launch_variance_gamma_forward_start_option_cuda<OptionSide::call>(
...
template void launch_variance_gamma_forward_start_option_cuda<OptionSide::put>(
    const ModelParameters*, std::size_t,
    const product::ForwardStartOptionParameters*, std::size_t,
    bool, std::size_t, std::size_t, std::size_t, std::size_t,
    float, unsigned int, std::size_t, std::uint64_t,

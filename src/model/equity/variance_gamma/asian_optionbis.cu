// Experimental Variance-Gamma Asian composition over generic CUDA layers.
...
#include "model/equity/variance_gamma/dynamicsbis.cu"
...
using Schedule =
    equity::FixedStepDenseSchedule<variance_gamma::DynamicsPolicy>;
using Discount =
    equity::ConstantRateDiscountPolicy<variance_gamma::DynamicsPolicy>;
...
template<OptionSide Side>
void launch_variance_gamma_asian_optionbis_cuda(
    ...
    equity::launch_monte_carlo_cuda<PricingPolicy<Side>>(
        ...
        "variance_gamma.asian_optionbis",
        option_side_name(Side),
        "Variance-Gamma Asian option bis kernel"
    );
}

template void launch_variance_gamma_asian_optionbis_cuda<OptionSide::call>(
...
template void launch_variance_gamma_asian_optionbis_cuda<OptionSide::put>(
    const ModelParameters*, std::size_t,
    const product::AsianOptionParameters*, std::size_t,
    bool, std::size_t, std::size_t, std::size_t, std::size_t,
    float, std::uint32_t, unsigned int, std::size_t, std::uint64_t,

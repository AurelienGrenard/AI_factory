// Variance-Gamma down-and-out-option composition over generic CUDA layers.
...
using Schedule = equity::FixedStepDenseSchedule<variance_gamma::DynamicsPolicy>;
using Discount = equity::ConstantRateDiscountPolicy<variance_gamma::DynamicsPolicy>;
template<OptionSide Side>
using PricingPolicy =
    product::DownAndOutOptionPricingPolicy<Schedule, Discount, Side>;
...
void launch_variance_gamma_down_and_out_option_cuda(
    ...
    equity::launch_monte_carlo_cuda<PricingPolicy<Side>>(
        ...
        "Variance-Gamma down and out option kernel"
    );
}

template void launch_variance_gamma_down_and_out_option_cuda<OptionSide::call>(
...
template void launch_variance_gamma_down_and_out_option_cuda<OptionSide::put>(
    const ModelParameters*, std::size_t,
    const product::DownAndOutOptionParameters*, std::size_t,
    bool, std::size_t, std::size_t, std::size_t, std::size_t,
    float, std::uint32_t, unsigned int, std::size_t, std::uint64_t,

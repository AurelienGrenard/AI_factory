// VarianceGamma Geometric-Asian option kernel with fused Philox simulation and payoff reduction.
...
__device__ __forceinline__ float evaluate_path(
    ...
    const VarianceGammaGeometricMeanPathResult simulated =
...
template<OptionSide Side>
__global__ void variance_gamma_geometric_asian_option_kernel(
...
void validate_variance_gamma_geometric_asian_option_launch(
    ...
    if (result_offset >= result_count
        ...
        throw std::invalid_argument(
            "The VarianceGamma Geometric-Asian option launch batch exceeds the result array."
        );
    }
...
void launch_variance_gamma_geometric_asian_option_cuda(
    ...
    validate_variance_gamma_geometric_asian_option_launch(
        device_models,
        model_count,
        device_products,
        product_count,
    ...
    check_cuda(
        cudaOccupancyMaxActiveBlocksPerMultiprocessor(
            ...
            variance_gamma_geometric_asian_option_kernel<Side>,
            static_cast<int>(threads_per_block),
            shared_bytes
        ),
        "VarianceGamma Geometric Asian option occupancy check"
    );
    if (active_blocks_per_multiprocessor == 0) {
        throw std::invalid_argument(
            "VarianceGamma Geometric Asian option kernel cannot launch one block per SM."
        );
    }

    // Launch the VarianceGamma Geometric-Asian option kernel.
    ...
    check_cuda(cudaGetLastError(), "VarianceGamma Geometric Asian option kernel");
}

// Build both public payoff specializations in this CUDA translation unit.
using LaunchSignature = decltype(launch_variance_gamma_geometric_asian_option_cuda<OptionSide::call>);
template LaunchSignature launch_variance_gamma_geometric_asian_option_cuda<OptionSide::call>;
template LaunchSignature launch_variance_gamma_geometric_asian_option_cuda<OptionSide::put>;

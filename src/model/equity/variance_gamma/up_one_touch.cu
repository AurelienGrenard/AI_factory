// VarianceGamma Up-one-touch kernel with fused Philox simulation and payoff reduction.
...
void validate_variance_gamma_up_one_touch_launch(
    ...
    if (result_offset >= result_count
        ...
        throw std::invalid_argument(
            "The VarianceGamma up-one-touch batch exceeds the result array."
        );
    }
...
void launch_variance_gamma_up_one_touch_cuda(
    ...
    validate_variance_gamma_up_one_touch_launch(
        device_models,
        model_count,
        device_products,
        product_count,
    ...
    check_cuda(
        cudaOccupancyMaxActiveBlocksPerMultiprocessor(
            ...
            variance_gamma_up_one_touch_kernel,
            static_cast<int>(threads_per_block),
            shared_bytes
        ),
        "VarianceGamma Up one-touch occupancy check"
    );
    if (active_blocks_per_multiprocessor == 0) {
        throw std::invalid_argument(
            "VarianceGamma Up one-touch kernel cannot launch one block per SM."
        );
    }

    // Launch the VarianceGamma Up-one-touch kernel.
    ...
    check_cuda(cudaGetLastError(), "VarianceGamma Up one-touch kernel");
}

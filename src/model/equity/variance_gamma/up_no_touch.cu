// VarianceGamma Up-no-touch kernel with fused Philox simulation and payoff reduction.
...
__global__ void variance_gamma_up_no_touch_kernel(
...
void validate_variance_gamma_up_no_touch_launch(
    ...
    if (result_offset >= result_count
        ...
        throw std::invalid_argument(
            "The VarianceGamma up-no-touch batch exceeds the result array."
        );
    }
...
void launch_variance_gamma_up_no_touch_cuda(
    const VarianceGammaModelParameters* device_models,
    std::size_t model_count,
    const product::UpNoTouchParameters* device_products,
    std::size_t product_count,
    ...
    validate_variance_gamma_up_no_touch_launch(
        device_models,
        model_count,
        device_products,
        product_count,
    ...
    check_cuda(
        cudaOccupancyMaxActiveBlocksPerMultiprocessor(
            ...
            variance_gamma_up_no_touch_kernel,
            static_cast<int>(threads_per_block),
            shared_bytes
        ),
        "VarianceGamma Up no-touch occupancy check"
    );
    if (active_blocks_per_multiprocessor == 0) {
        throw std::invalid_argument(
            "VarianceGamma Up no-touch kernel cannot launch one block per SM."
        );
    }

    // Launch the VarianceGamma Up-no-touch kernel.
    ...
    check_cuda(cudaGetLastError(), "VarianceGamma Up no-touch kernel");
}

// VarianceGamma lookback kernel with fused Philox simulation and payoff reduction.
...
__device__ __forceinline__ float evaluate_path(
    ...
    const VarianceGammaMaximumPathResult simulated =
...
__global__ void variance_gamma_lookback_option_kernel(
...
void validate_variance_gamma_lookback_option_launch(
    ...
    if (result_offset >= result_count
        ...
        throw std::invalid_argument(
            "The VarianceGamma lookback launch batch exceeds the result array."
        );
    }
...
void launch_variance_gamma_lookback_option_cuda(
    ...
    validate_variance_gamma_lookback_option_launch(
        device_models,
        model_count,
        device_products,
        product_count,
    ...
    check_cuda(
        cudaOccupancyMaxActiveBlocksPerMultiprocessor(
            ...
            variance_gamma_lookback_option_kernel,
            static_cast<int>(threads_per_block),
            shared_bytes
        ),
        "VarianceGamma Lookback option occupancy check"
    );
    if (active_blocks_per_multiprocessor == 0) {
        throw std::invalid_argument(
            "VarianceGamma Lookback option kernel cannot launch one block per SM."
        );
    }

    // Launch the VarianceGamma lookback kernel.
    ...
    check_cuda(cudaGetLastError(), "VarianceGamma Lookback option kernel");
}

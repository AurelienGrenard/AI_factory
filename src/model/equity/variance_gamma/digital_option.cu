// Variance-Gamma digital-option composition over generic CUDA layers.
...
void launch_variance_gamma_digital_option_cuda(
    ...
    monte_carlo::launch_monte_carlo_cuda<PricingPolicy<Side>>(
        ...
        "Variance-Gamma digital option kernel"
    );
}

template void launch_variance_gamma_digital_option_cuda<OptionSide::call>(
...
template void launch_variance_gamma_digital_option_cuda<OptionSide::put>(
    const ModelParameters*, std::size_t,
    const product::DigitalOptionParameters*, std::size_t,
    bool, std::size_t, std::size_t, std::size_t, std::size_t,
    float, unsigned int, std::size_t, std::uint64_t,

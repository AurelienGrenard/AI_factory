// VarianceGamma asset-or-nothing kernel with fused simulation and payoff reduction.
...
template<OptionSide Side>
__global__ void variance_gamma_asset_or_nothing_option_kernel(
...
void validate_variance_gamma_asset_or_nothing_option_launch(
    ...
    if (result_offset >= result_count
        ...
        throw std::invalid_argument(
            "The VarianceGamma asset-or-nothing launch batch exceeds the result array."
        );
    }
...
void launch_variance_gamma_asset_or_nothing_option_cuda(
    ...
    validate_variance_gamma_asset_or_nothing_option_launch(
        device_models,
        model_count,
        device_products,
        product_count,
    ...
    check_cuda(
        cudaOccupancyMaxActiveBlocksPerMultiprocessor(
            ...
            variance_gamma_asset_or_nothing_option_kernel<Side>,
            static_cast<int>(threads_per_block),
            shared_bytes
        ),
        "VarianceGamma asset-or-nothing option occupancy check"
    );
    if (active_blocks_per_multiprocessor == 0) {
        throw std::invalid_argument(
            "VarianceGamma asset-or-nothing option kernel cannot launch one block per SM."
        );
    }

    // Launch the VarianceGamma Asset-or-nothing kernel.
    ...
    check_cuda(cudaGetLastError(), "VarianceGamma asset-or-nothing option kernel");
}

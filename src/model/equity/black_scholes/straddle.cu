// Closed-form Black-Scholes Straddle kernel.
...
void validate_black_scholes_straddle_launch(
    ...
    if (result_offset >= result_count
        ...
        throw std::invalid_argument(
            "The Black-Scholes Straddle launch batch exceeds the result array."
        );
    }
    ...
    const std::size_t thread_count = checked_workspace_product(
        ...
        "The Black-Scholes Straddle thread count exceeds size_t."
    );
    if (thread_count < launch_result_count) {
        throw std::invalid_argument(
            "The Black-Scholes Straddle launch requires one thread per price."
        );
    }
}

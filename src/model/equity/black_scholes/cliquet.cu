// BlackScholes Cliquet kernel with fused Philox simulation and payoff reduction.
...
__global__ void black_scholes_cliquet_kernel(
...
void validate_black_scholes_cliquet_launch(
    ...
    if (result_offset >= result_count
        ...
        throw std::invalid_argument(
            "The BlackScholes Cliquet launch batch exceeds the result array."
        );
    }
...
void launch_black_scholes_cliquet_cuda(
    ...
    validate_black_scholes_cliquet_launch(
        device_models,
        model_count,
        device_products,
        product_count,
    ...
    check_cuda(
        cudaOccupancyMaxActiveBlocksPerMultiprocessor(
            ...
            black_scholes_cliquet_kernel,
            static_cast<int>(threads_per_block),
            shared_bytes
        ),
        "BlackScholes Cliquet occupancy check"
    );
    if (active_blocks_per_multiprocessor == 0) {
        throw std::invalid_argument(
            "BlackScholes Cliquet kernel cannot launch one block per SM."
        );
    }

    // Launch the BlackScholes Cliquet kernel.
    ...
    check_cuda(cudaGetLastError(), "BlackScholes Cliquet kernel");
}

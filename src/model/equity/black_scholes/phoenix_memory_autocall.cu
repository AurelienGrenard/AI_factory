// BlackScholes Phoenix-Memory-autocall kernel with fused Philox simulation and payoff reduction.
...
__global__ void black_scholes_phoenix_memory_autocall_kernel(
...
void validate_black_scholes_phoenix_memory_autocall_launch(
    ...
    if (result_offset >= result_count
        ...
        throw std::invalid_argument(
            "The BlackScholes Phoenix-Memory-autocall launch batch exceeds the result array."
        );
    }
...
void launch_black_scholes_phoenix_memory_autocall_cuda(
    ...
    validate_black_scholes_phoenix_memory_autocall_launch(
        device_models,
        model_count,
        device_products,
        product_count,
    ...
    check_cuda(
        cudaOccupancyMaxActiveBlocksPerMultiprocessor(
            ...
            black_scholes_phoenix_memory_autocall_kernel,
            static_cast<int>(threads_per_block),
            shared_bytes
        ),
        "BlackScholes Phoenix Memory autocall occupancy check"
    );
    if (active_blocks_per_multiprocessor == 0) {
        throw std::invalid_argument(
            "BlackScholes Phoenix Memory autocall kernel cannot launch one block per SM."
        );
    }

    // Launch the BlackScholes Phoenix-Memory-autocall kernel.
    ...
    check_cuda(cudaGetLastError(), "BlackScholes Phoenix Memory autocall kernel");
}

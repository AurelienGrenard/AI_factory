// BlackScholes Up-one-touch kernel with fused Philox simulation and payoff reduction.
...
__global__ void black_scholes_up_one_touch_kernel(
...
void validate_black_scholes_up_one_touch_launch(
    ...
    if (result_offset >= result_count
        ...
        throw std::invalid_argument(
            "The BlackScholes up-one-touch batch exceeds the result array."
        );
    }
...
void launch_black_scholes_up_one_touch_cuda(
    const BlackScholesModelParameters* device_models,
    std::size_t model_count,
    const product::UpOneTouchParameters* device_products,
    std::size_t product_count,
    ...
    validate_black_scholes_up_one_touch_launch(
        device_models,
        model_count,
        device_products,
        product_count,
    ...
    check_cuda(
        cudaOccupancyMaxActiveBlocksPerMultiprocessor(
            ...
            black_scholes_up_one_touch_kernel,
            static_cast<int>(threads_per_block),
            shared_bytes
        ),
        "BlackScholes Up one-touch occupancy check"
    );
    if (active_blocks_per_multiprocessor == 0) {
        throw std::invalid_argument(
            "BlackScholes Up one-touch kernel cannot launch one block per SM."
        );
    }

    // Launch the BlackScholes Up-one-touch kernel.
    ...
    check_cuda(cudaGetLastError(), "BlackScholes Up one-touch kernel");
}

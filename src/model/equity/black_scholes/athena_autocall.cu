// BlackScholes Athena-autocall kernel with fused Philox simulation and payoff reduction.
...
__device__ __forceinline__ float evaluate_path(
    ...
    for (std::uint32_t observation = 0U;
        ...
        if (!maturity && spot >= row.autocall_barrier) {
            return discount * (1.0f + accumulated_gain);
        }
        if (maturity) {
            const float redemption = spot >= row.autocall_barrier
                ? 1.0f + accumulated_gain
                : (spot >= row.protection_barrier ? 1.0f : spot);
...
__global__ void black_scholes_athena_autocall_kernel(
...
void validate_black_scholes_athena_autocall_launch(
    ...
    if (result_offset >= result_count
        ...
        throw std::invalid_argument(
            "The BlackScholes Athena-autocall launch batch exceeds the result array."
        );
    }
...
void launch_black_scholes_athena_autocall_cuda(
    const BlackScholesModelParameters* device_models,
    std::size_t model_count,
    const product::AthenaAutocallParameters* device_products,
    std::size_t product_count,
    ...
    validate_black_scholes_athena_autocall_launch(
        device_models,
        model_count,
        device_products,
        product_count,
    ...
    check_cuda(
        cudaOccupancyMaxActiveBlocksPerMultiprocessor(
            ...
            black_scholes_athena_autocall_kernel,
            static_cast<int>(threads_per_block),
            shared_bytes
        ),
        "BlackScholes Athena autocall occupancy check"
    );
    if (active_blocks_per_multiprocessor == 0) {
        throw std::invalid_argument(
            "BlackScholes Athena autocall kernel cannot launch one block per SM."
        );
    }

    // Launch the BlackScholes Athena-autocall kernel.
    ...
    check_cuda(cudaGetLastError(), "BlackScholes Athena autocall kernel");
}

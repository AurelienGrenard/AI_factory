// BlackScholes lookback kernel with fused Philox simulation and payoff reduction.
...
__device__ __forceinline__ float evaluate_path(
    ...
    const BlackScholesMaximumPathResult simulated =
...
__global__ void black_scholes_lookback_option_kernel(
...
void validate_black_scholes_lookback_option_launch(
    ...
    if (result_offset >= result_count
        ...
        throw std::invalid_argument(
            "The BlackScholes lookback launch batch exceeds the result array."
        );
    }
...
void launch_black_scholes_lookback_option_cuda(
    const BlackScholesModelParameters* device_models,
    std::size_t model_count,
    const product::LookbackOptionParameters* device_products,
    std::size_t product_count,
    ...
    validate_black_scholes_lookback_option_launch(
        device_models,
        model_count,
        device_products,
        product_count,
    ...
    check_cuda(
        cudaOccupancyMaxActiveBlocksPerMultiprocessor(
            ...
            black_scholes_lookback_option_kernel,
            static_cast<int>(threads_per_block),
            shared_bytes
        ),
        "BlackScholes Lookback option occupancy check"
    );
    if (active_blocks_per_multiprocessor == 0) {
        throw std::invalid_argument(
            "BlackScholes Lookback option kernel cannot launch one block per SM."
        );
    }

    // Launch the BlackScholes lookback kernel.
    ...
    check_cuda(cudaGetLastError(), "BlackScholes Lookback option kernel");
}

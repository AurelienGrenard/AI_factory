// Closed-form Black-Scholes AssetOrNothingOption kernel.
...
template<OptionSide Side>
__global__ void black_scholes_asset_or_nothing_option_kernel(
...
void validate_black_scholes_asset_or_nothing_option_launch(
    ...
    if (result_offset >= result_count
        ...
        throw std::invalid_argument(
            "The Black-Scholes AssetOrNothingOption launch batch exceeds the result array."
        );
    }
    ...
    const std::size_t thread_count = checked_workspace_product(
        ...
        "The Black-Scholes AssetOrNothingOption thread count exceeds size_t."
    );
    if (thread_count < launch_result_count) {
        throw std::invalid_argument(
            "The Black-Scholes AssetOrNothingOption launch requires one thread per price."
        );
    }
}
...
void launch_black_scholes_asset_or_nothing_option_cuda(
    ...
    validate_black_scholes_asset_or_nothing_option_launch(
    ...
    report_cuda_kernel_launch_if_enabled(
        ...
        black_scholes_asset_or_nothing_option_kernel<Side>,
    ...
    check_cuda(cudaGetLastError(), "Black-Scholes AssetOrNothingOption kernel");
}

using LaunchSignature =
    decltype(launch_black_scholes_asset_or_nothing_option_cuda<OptionSide::call>);
namespace {
[[maybe_unused]] LaunchSignature* launch_instantiation_0 =
    &launch_black_scholes_asset_or_nothing_option_cuda<OptionSide::call>;
}  // namespace
namespace {
[[maybe_unused]] LaunchSignature* launch_instantiation_1 =
    &launch_black_scholes_asset_or_nothing_option_cuda<OptionSide::put>;
}  // namespace

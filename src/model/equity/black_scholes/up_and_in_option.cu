// BlackScholes Up-and-in option kernel with fused Philox simulation and payoff reduction.
...
template<OptionSide Side>
__global__ void black_scholes_up_and_in_option_kernel(
...
void validate_black_scholes_up_and_in_option_launch(
    ...
    if (result_offset >= result_count
        ...
        throw std::invalid_argument(
            "The BlackScholes up-and-in option batch exceeds the result array."
        );
    }
...
template<OptionSide Side>
void launch_black_scholes_up_and_in_option_cuda(
    const BlackScholesModelParameters* device_models,
    std::size_t model_count,
    const product::UpAndInOptionParameters* device_products,
    std::size_t product_count,
    ...
    validate_black_scholes_up_and_in_option_launch(
        device_models,
        model_count,
        device_products,
        product_count,
    ...
    check_cuda(
        cudaOccupancyMaxActiveBlocksPerMultiprocessor(
            ...
            black_scholes_up_and_in_option_kernel<Side>,
            static_cast<int>(threads_per_block),
            shared_bytes
        ),
        "BlackScholes Up-and-in option occupancy check"
    );
    if (active_blocks_per_multiprocessor == 0) {
        throw std::invalid_argument(
            "BlackScholes Up-and-in option kernel cannot launch one block per SM."
        );
    }

    // Launch the BlackScholes Up-and-in option kernel.
    ...
    check_cuda(cudaGetLastError(), "BlackScholes Up-and-in option kernel");
}

// Build both public payoff specializations in this CUDA translation unit.
using LaunchSignature = decltype(launch_black_scholes_up_and_in_option_cuda<OptionSide::call>);
template LaunchSignature launch_black_scholes_up_and_in_option_cuda<OptionSide::call>;
template LaunchSignature launch_black_scholes_up_and_in_option_cuda<OptionSide::put>;

// BlackScholes Down-and-out option kernel with fused Philox simulation and payoff reduction.
...
struct PreparedRow {
    BlackScholesPreparedParameters model;
...
__device__ __forceinline__ float evaluate_path(
    ...
    BlackScholesState state = initial_state(row.model);
...
template<OptionSide Side>
__global__ void black_scholes_down_and_out_option_kernel(
...
void validate_black_scholes_down_and_out_option_launch(
    ...
    if (result_offset >= result_count
        ...
        throw std::invalid_argument(
            "The BlackScholes Down-and-out option launch batch exceeds the result array."
        );
    }
...
template<OptionSide Side>
void launch_black_scholes_down_and_out_option_cuda(
    const BlackScholesModelParameters* device_models,
    std::size_t model_count,
    const product::DownAndOutOptionParameters* device_products,
    std::size_t product_count,
    ...
    validate_black_scholes_down_and_out_option_launch(
        device_models,
        model_count,
        device_products,
        product_count,
    ...
    check_cuda(
        cudaOccupancyMaxActiveBlocksPerMultiprocessor(
            ...
            black_scholes_down_and_out_option_kernel<Side>,
            static_cast<int>(threads_per_block),
            shared_bytes
        ),
        "BlackScholes Down-and-out option occupancy check"
    );
    if (active_blocks_per_multiprocessor == 0) {
        throw std::invalid_argument(
            "BlackScholes Down-and-out option kernel cannot launch one block per SM."
        );
    }

    // Launch the BlackScholes Down-and-out option kernel.
    ...
    check_cuda(cudaGetLastError(), "BlackScholes Down-and-out option kernel");
}

// Build both public payoff specializations in this CUDA translation unit.
using LaunchSignature = decltype(launch_black_scholes_down_and_out_option_cuda<OptionSide::call>);
template LaunchSignature launch_black_scholes_down_and_out_option_cuda<OptionSide::call>;
template LaunchSignature launch_black_scholes_down_and_out_option_cuda<OptionSide::put>;

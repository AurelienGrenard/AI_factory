// Closed-form Black-Scholes ForwardStartOption kernel.
...
void validate_black_scholes_forward_start_option_launch(
    ...
    if (result_offset >= result_count
        ...
        throw std::invalid_argument(
            "The Black-Scholes ForwardStartOption launch batch exceeds the result array."
        );
    }
    ...
    const std::size_t thread_count = checked_workspace_product(
        ...
        "The Black-Scholes ForwardStartOption thread count exceeds size_t."
    );
    if (thread_count < launch_result_count) {
        throw std::invalid_argument(
            "The Black-Scholes ForwardStartOption launch requires one thread per price."
        );
    }
}
...
void launch_black_scholes_forward_start_option_cuda(
    ...
    report_cuda_kernel_launch_if_enabled(
        ...
        black_scholes_forward_start_option_kernel<Side>,
...
using LaunchSignature =
    decltype(launch_black_scholes_forward_start_option_cuda<OptionSide::call>);
template LaunchSignature
launch_black_scholes_forward_start_option_cuda<OptionSide::call>;
template LaunchSignature
launch_black_scholes_forward_start_option_cuda<OptionSide::put>;

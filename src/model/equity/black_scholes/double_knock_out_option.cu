// BlackScholes Double-knock-out option kernel with fused Philox simulation and payoff reduction.
...
template<OptionSide Side>
__global__ void black_scholes_double_knock_out_option_kernel(
...
void validate_black_scholes_double_knock_out_option_launch(
    ...
    if (result_offset >= result_count
        ...
        throw std::invalid_argument(
            "The BlackScholes double-knock-out option batch exceeds the result array."
        );
    }
...
void launch_black_scholes_double_knock_out_option_cuda(
    ...
    validate_black_scholes_double_knock_out_option_launch(
        device_models,
        model_count,
        device_products,
        product_count,
    ...
    check_cuda(
        cudaOccupancyMaxActiveBlocksPerMultiprocessor(
            ...
            black_scholes_double_knock_out_option_kernel<Side>,
            static_cast<int>(threads_per_block),
            shared_bytes
        ),
        "BlackScholes Double-knock-out option occupancy check"
    );
    if (active_blocks_per_multiprocessor == 0) {
        throw std::invalid_argument(
            "BlackScholes Double-knock-out option kernel cannot launch one block per SM."
        );
    }

    // Launch the BlackScholes Double-knock-out option kernel.
    ...
    check_cuda(cudaGetLastError(), "BlackScholes Double-knock-out option kernel");
}

// Build both public payoff specializations in this CUDA translation unit.
using LaunchSignature = decltype(launch_black_scholes_double_knock_out_option_cuda<OptionSide::call>);
template LaunchSignature launch_black_scholes_double_knock_out_option_cuda<OptionSide::call>;
template LaunchSignature launch_black_scholes_double_knock_out_option_cuda<OptionSide::put>;

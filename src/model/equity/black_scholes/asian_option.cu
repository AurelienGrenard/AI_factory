// BlackScholes Asian-option kernel with fused Philox simulation and payoff reduction.
...
__device__ __forceinline__ float evaluate_path(
    ...
    const BlackScholesMeanPathResult simulated =
...
template<OptionSide Side>
__global__ void black_scholes_asian_option_kernel(
...
void validate_black_scholes_asian_option_launch(
    ...
    if (result_offset >= result_count
        ...
        throw std::invalid_argument(
            "The BlackScholes Asian-option launch batch exceeds the result array."
        );
    }
...
template<OptionSide Side>
void launch_black_scholes_asian_option_cuda(
    const BlackScholesModelParameters* device_models,
    std::size_t model_count,
    const product::AsianOptionParameters* device_products,
    std::size_t product_count,
    ...
    validate_black_scholes_asian_option_launch(
        device_models,
        model_count,
        device_products,
        product_count,
    ...
    check_cuda(
        cudaOccupancyMaxActiveBlocksPerMultiprocessor(
            ...
            black_scholes_asian_option_kernel<Side>,
            static_cast<int>(threads_per_block),
            shared_bytes
        ),
        "BlackScholes Asian option occupancy check"
    );
    if (active_blocks_per_multiprocessor == 0) {
        throw std::invalid_argument(
            "BlackScholes Asian option kernel cannot launch one block per SM."
        );
    }

    // Launch the BlackScholes Asian-option kernel.
    ...
    check_cuda(cudaGetLastError(), "BlackScholes Asian option kernel");
}

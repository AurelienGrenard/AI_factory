// Bates down-and-in-option composition over generic CUDA layers.
...
void launch_bates_down_and_in_option_cuda(
    ...
    equity::launch_monte_carlo_cuda<PricingPolicy<Side>>(
        ...
        "Bates down and in option kernel"
    );
}

template void launch_bates_down_and_in_option_cuda<OptionSide::call>(
...
template void launch_bates_down_and_in_option_cuda<OptionSide::put>(
    const ModelParameters*, std::size_t,
    const product::DownAndInOptionParameters*, std::size_t,
    bool, std::size_t, std::size_t, std::size_t, std::size_t,
    float, std::uint32_t, unsigned int, std::size_t, std::uint64_t,

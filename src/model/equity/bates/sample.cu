// Persistent bates terminal and regular-calendar sample kernels.
#include "model/equity/bates/sample.cuh"
...
__global__ void bates_terminal_samples_kernel(
...
__global__ void bates_calendar_samples_kernel(
    const ModelParameters* __restrict__ models,
    std::size_t paths_per_model,
...
std::size_t validate_bates_sample_launch(
...
void launch_bates_terminal_samples_cuda(
    ...
    check_cuda(cudaGetLastError(), "bates terminal sample kernel");
}
...
void launch_bates_calendar_samples_cuda(
    ...
    check_cuda(cudaGetLastError(), "bates calendar sample kernel");
}

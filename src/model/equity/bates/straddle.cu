// Bates straddle kernel with fused Philox simulation and payoff reduction.
...
__global__ void bates_straddle_kernel(
...
void validate_bates_straddle_launch(
    ...
    if (result_offset >= result_count
        ...
        throw std::invalid_argument(
            "The Bates straddle launch batch exceeds the result array."
        );
    }
...
void launch_bates_straddle_cuda(
    ...
    validate_bates_straddle_launch(
        device_models,
        model_count,
        device_products,
        product_count,
    ...
    check_cuda(
        cudaOccupancyMaxActiveBlocksPerMultiprocessor(
            ...
            bates_straddle_kernel,
            static_cast<int>(threads_per_block),
            shared_bytes
        ),
        "Bates straddle occupancy check"
    );
    if (active_blocks_per_multiprocessor == 0) {
        throw std::invalid_argument(
            "Bates straddle kernel cannot launch one block per SM."
        );
    }

    // Launch the Bates Straddle kernel.
    ...
    check_cuda(cudaGetLastError(), "Bates straddle kernel");
}

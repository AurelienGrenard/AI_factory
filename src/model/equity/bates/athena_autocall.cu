// Bates Athena-autocall kernel with fused Philox simulation and payoff reduction.
...
__global__ void bates_athena_autocall_kernel(
...
void validate_bates_athena_autocall_launch(
    ...
    if (result_offset >= result_count
        ...
        throw std::invalid_argument(
            "The Bates Athena-autocall launch batch exceeds the result array."
        );
    }
...
void launch_bates_athena_autocall_cuda(
    const BatesModelParameters* device_models,
    std::size_t model_count,
    const product::AthenaAutocallParameters* device_products,
    std::size_t product_count,
    ...
    validate_bates_athena_autocall_launch(
        device_models,
        model_count,
        device_products,
        product_count,
    ...
    check_cuda(
        cudaOccupancyMaxActiveBlocksPerMultiprocessor(
            ...
            bates_athena_autocall_kernel,
            static_cast<int>(threads_per_block),
            shared_bytes
        ),
        "Bates Athena autocall occupancy check"
    );
    if (active_blocks_per_multiprocessor == 0) {
        throw std::invalid_argument(
            "Bates Athena autocall kernel cannot launch one block per SM."
        );
    }

    // Launch the Bates Athena-autocall kernel.
    ...
    check_cuda(cudaGetLastError(), "Bates Athena autocall kernel");
}

// Bates lookback kernel with fused Philox simulation and payoff reduction.
...
__device__ __forceinline__ float evaluate_path(
    ...
    const BatesMaximumPathResult simulated =
...
__global__ void bates_lookback_option_kernel(
...
void validate_bates_lookback_option_launch(
    ...
    if (result_offset >= result_count
        ...
        throw std::invalid_argument(
            "The Bates lookback launch batch exceeds the result array."
        );
    }
...
void launch_bates_lookback_option_cuda(
    const BatesModelParameters* device_models,
    std::size_t model_count,
    const product::LookbackOptionParameters* device_products,
    std::size_t product_count,
    ...
    validate_bates_lookback_option_launch(
        device_models,
        model_count,
        device_products,
        product_count,
    ...
    check_cuda(
        cudaOccupancyMaxActiveBlocksPerMultiprocessor(
            ...
            bates_lookback_option_kernel,
            static_cast<int>(threads_per_block),
            shared_bytes
        ),
        "Bates Lookback option occupancy check"
    );
    if (active_blocks_per_multiprocessor == 0) {
        throw std::invalid_argument(
            "Bates Lookback option kernel cannot launch one block per SM."
        );
    }

    // Launch the Bates lookback kernel.
    ...
    check_cuda(cudaGetLastError(), "Bates Lookback option kernel");
}

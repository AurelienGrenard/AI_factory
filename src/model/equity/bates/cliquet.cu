// Bates Cliquet kernel with fused Philox simulation and payoff reduction.
...
__global__ void bates_cliquet_kernel(
...
void validate_bates_cliquet_launch(
    ...
    if (result_offset >= result_count
        ...
        throw std::invalid_argument(
            "The Bates Cliquet launch batch exceeds the result array."
        );
    }
...
void launch_bates_cliquet_cuda(
    ...
    check_cuda(
        cudaOccupancyMaxActiveBlocksPerMultiprocessor(
            ...
            bates_cliquet_kernel,
            static_cast<int>(threads_per_block),
            shared_bytes
        ),
        "Bates Cliquet occupancy check"
    );
    if (active_blocks_per_multiprocessor == 0) {
        throw std::invalid_argument(
            "Bates Cliquet kernel cannot launch one block per SM."
        );
    }

    // Launch the Bates Cliquet kernel.
    ...
    check_cuda(cudaGetLastError(), "Bates Cliquet kernel");
}

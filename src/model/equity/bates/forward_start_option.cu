// Bates forward-start-option composition over generic CUDA layers.
...
using Schedule = equity::FixedStepCalendarSchedule<bates::DynamicsPolicy, 2U>;
...
void launch_bates_forward_start_option_cuda(
    ...
    equity::launch_monte_carlo_cuda<PricingPolicy<Side>>(
        ...
        "Bates forward start option kernel"
    );
}

template void launch_bates_forward_start_option_cuda<OptionSide::call>(
...
template void launch_bates_forward_start_option_cuda<OptionSide::put>(
    const ModelParameters*, std::size_t,
    const product::ForwardStartOptionParameters*, std::size_t,
    bool, std::size_t, std::size_t, std::size_t, std::size_t,
    float, std::uint32_t, unsigned int, std::size_t, std::uint64_t,

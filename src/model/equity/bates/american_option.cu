// Shared multi-block Bates American-option pricing with GPU Longstaff-Schwartz.
...
struct PreparedRow {
    BatesQeParameters initial_stub_model;
    BatesQeParameters regular_model;
...
BatesStateRegions bates_state_regions(const lsm::WorkspaceLayout& layout) {
    if (layout.state_fields.size() != 2U) {
        throw std::logic_error("Bates American options require two state fields.");
    }
    return {layout.state_fields[0], layout.state_fields[1]};
}
...
__global__ void prepare_rows_kernel(
    ...
    const BatesModelParameters model = models[model_index];
...
__global__ void simulate_paths_kernel(
    ...
    for (std::size_t path = first_path;
         ...
         path += path_stride) {
        const BatesState terminal = simulate_on_regular_grid(
...
void validate_bates_american_option_launch(
...
template <OptionSide Side>
lsm::LaunchResult launch_bates_american_option_cuda(
    const BatesModelParameters* device_models,
    std::size_t model_count,
    const product::AmericanOptionParameters* host_products,
    const product::AmericanOptionParameters* device_products,
    ...
    validate_bates_american_option_launch(
        device_models,
        model_count,
        host_products,
        device_products,
    ...
    for (const lsm::BatchPlan& batch : plan.batches) {
        ...
        const BatesStateRegions states = bates_state_regions(layout);
        ...
        float* const observed_variances = lsm::workspace_pointer<float>(
            device_workspace, states.variances
        );
        ...
        report_cuda_kernel_launch_if_enabled(
            "bates.american_option.prepare_rows",
            "common",
            prepare_rows_kernel,
            dim3(row_blocks),
        ...
        report_cuda_kernel_launch_if_enabled(
            "bates.american_option.simulate_paths",
        ...
        for (std::uint32_t backward_level = 1U;
             ...
             ++backward_level) {
            report_cuda_kernel_launch_if_enabled(
                "bates.american_option.regression_partials",
                option_side_name(Side),
            ...
            report_cuda_kernel_launch_if_enabled(
                "bates.american_option.solve_regressions",
                "common",
            ...
            report_cuda_kernel_launch_if_enabled(
                "bates.american_option.update_cashflows",
                option_side_name(Side),
        ...
        report_cuda_kernel_launch_if_enabled(
            "bates.american_option.moment_partials",
            "common",
        ...
        report_cuda_kernel_launch_if_enabled(
            "bates.american_option.finalize_prices",
            option_side_name(Side),
...
using LaunchSignature =
    decltype(launch_bates_american_option_cuda<OptionSide::call>);
template LaunchSignature
launch_bates_american_option_cuda<OptionSide::call>;
template LaunchSignature
launch_bates_american_option_cuda<OptionSide::put>;

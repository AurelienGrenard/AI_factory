// Bates asset-or-nothing kernel with fused simulation and payoff reduction.
...
template<OptionSide Side>
__global__ void bates_asset_or_nothing_option_kernel(
...
void validate_bates_asset_or_nothing_option_launch(
    ...
    if (result_offset >= result_count
        ...
        throw std::invalid_argument(
            "The Bates asset-or-nothing launch batch exceeds the result array."
        );
    }
...
template<OptionSide Side>
void launch_bates_asset_or_nothing_option_cuda(
    const BatesModelParameters* device_models,
    std::size_t model_count,
    const product::AssetOrNothingOptionParameters* device_products,
    std::size_t product_count,
    ...
    validate_bates_asset_or_nothing_option_launch(
        device_models,
        model_count,
        device_products,
        product_count,
    ...
    check_cuda(
        cudaOccupancyMaxActiveBlocksPerMultiprocessor(
            ...
            bates_asset_or_nothing_option_kernel<Side>,
            static_cast<int>(threads_per_block),
            shared_bytes
        ),
        "Bates asset-or-nothing option occupancy check"
    );
    if (active_blocks_per_multiprocessor == 0) {
        throw std::invalid_argument(
            "Bates asset-or-nothing option kernel cannot launch one block per SM."
        );
    }

    // Launch the Bates Asset-or-nothing kernel.
    ...
    check_cuda(cudaGetLastError(), "Bates asset-or-nothing option kernel");
}

// Build both public payoff specializations in this CUDA translation unit.
using LaunchSignature = decltype(launch_bates_asset_or_nothing_option_cuda<OptionSide::call>);
template LaunchSignature launch_bates_asset_or_nothing_option_cuda<OptionSide::call>;
template LaunchSignature launch_bates_asset_or_nothing_option_cuda<OptionSide::put>;

// Bates asset-or-nothing-option composition over generic CUDA layers.
...
void launch_bates_asset_or_nothing_option_cuda(
    ...
    equity::launch_monte_carlo_cuda<PricingPolicy<Side>>(
        ...
        "Bates asset or nothing option kernel"
    );
}

template void launch_bates_asset_or_nothing_option_cuda<OptionSide::call>(
...
template void launch_bates_asset_or_nothing_option_cuda<OptionSide::put>(
    const ModelParameters*, std::size_t,
    const product::AssetOrNothingOptionParameters*, std::size_t,
    bool, std::size_t, std::size_t, std::size_t, std::size_t,
    float, std::uint32_t, unsigned int, std::size_t, std::uint64_t,

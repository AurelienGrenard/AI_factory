// Bates Range Accrual kernel with fused Philox simulation and payoff reduction.
...
__device__ __forceinline__ PreparedRow prepare_row(
    ...
    const BatesQeParameters prepared_model = prepare_model(
        model,
    ...
    return {
        ...
        prepared_model.heston.initial_log_spot + logf(product.lower_barrier),
        prepared_model.heston.initial_log_spot + logf(product.upper_barrier),
        maturity_discount,
        maturity_discount
            * product.coupon_rate
            * product.observation_interval,
...
__global__ void bates_range_accrual_kernel(
...
void validate_bates_range_accrual_launch(
    ...
    if (result_offset >= result_count
        ...
        throw std::invalid_argument(
            "The Bates Range Accrual launch batch exceeds the result array."
        );
    }
...
void launch_bates_range_accrual_cuda(
    ...
    validate_bates_range_accrual_launch(
        device_models,
        model_count,
        device_products,
        product_count,
    ...
    check_cuda(
        cudaOccupancyMaxActiveBlocksPerMultiprocessor(
            ...
            bates_range_accrual_kernel,
            static_cast<int>(threads_per_block),
            shared_bytes
        ),
        "Bates Range Accrual occupancy check"
    );
    if (active_blocks_per_multiprocessor == 0) {
        throw std::invalid_argument(
            "Bates Range Accrual kernel cannot launch one block per SM."
        );
    }

    // Launch the Bates Range Accrual kernel.
    ...
    check_cuda(cudaGetLastError(), "Bates Range Accrual kernel");
}

// Bates Phoenix-Memory-autocall kernel with fused Philox simulation and payoff reduction.
...
__global__ void bates_phoenix_memory_autocall_kernel(
...
void validate_bates_phoenix_memory_autocall_launch(
    ...
    if (result_offset >= result_count
        ...
        throw std::invalid_argument(
            "The Bates Phoenix-Memory-autocall launch batch exceeds the result array."
        );
    }
...
void launch_bates_phoenix_memory_autocall_cuda(
    const ModelParameters* device_models,
    std::size_t model_count,
    const product::PhoenixMemoryAutocallParameters* device_products,
    std::size_t product_count,
    ...
    validate_bates_phoenix_memory_autocall_launch(
        device_models,
        model_count,
        device_products,
        product_count,
    ...
    check_cuda(
        cudaOccupancyMaxActiveBlocksPerMultiprocessor(
            ...
            bates_phoenix_memory_autocall_kernel,
            static_cast<int>(threads_per_block),
            shared_bytes
        ),
        "Bates Phoenix Memory autocall occupancy check"
    );
    if (active_blocks_per_multiprocessor == 0) {
        throw std::invalid_argument(
            "Bates Phoenix Memory autocall kernel cannot launch one block per SM."
        );
    }

    // Launch the Bates Phoenix-Memory-autocall kernel.
    ...
    check_cuda(cudaGetLastError(), "Bates Phoenix Memory autocall kernel");
}

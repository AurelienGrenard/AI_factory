// Bates European-option kernel with fused Philox simulation and payoff reduction.
...
template<OptionSide Side>
__global__ void bates_european_option_kernel(
...
void validate_bates_european_option_launch(
    ...
    if (result_offset >= result_count
        ...
        throw std::invalid_argument(
            "The Bates European-option launch batch exceeds the result array."
        );
    }
...
template<OptionSide Side>
void launch_bates_european_option_cuda(
    const BatesModelParameters* device_models,
    std::size_t model_count,
    const product::EuropeanOptionParameters* device_products,
    std::size_t product_count,
    ...
    validate_bates_european_option_launch(
        device_models,
        model_count,
        device_products,
        product_count,
    ...
    check_cuda(
        cudaOccupancyMaxActiveBlocksPerMultiprocessor(
            ...
            bates_european_option_kernel<Side>,
            static_cast<int>(threads_per_block),
            shared_bytes
        ),
        "Bates European option occupancy check"
    );
    if (active_blocks_per_multiprocessor == 0) {
        throw std::invalid_argument(
            "Bates European option kernel cannot launch one block per SM."
        );
    }

    // Launch the Bates European-option kernel.
    ...
    check_cuda(cudaGetLastError(), "Bates European option kernel");
}

// Build both public payoff specializations in this CUDA translation unit.
using LaunchSignature = decltype(launch_bates_european_option_cuda<OptionSide::call>);
template LaunchSignature launch_bates_european_option_cuda<OptionSide::call>;
template LaunchSignature launch_bates_european_option_cuda<OptionSide::put>;

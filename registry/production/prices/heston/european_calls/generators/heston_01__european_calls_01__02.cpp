// Build one Heston European-call price database from JSON inputs to CUDA outputs.
...
// Input databases and rule used to construct the final price rows.
...
constexpr ai_factory::workbench::registry::ResultConstruction construction =
    ai_factory::workbench::registry::ResultConstruction::CartesianProduct;

// Numerical and CUDA configuration used by the pricing algorithm.
constexpr std::size_t monte_carlo_paths_per_price = 131'072U;
constexpr float target_dt = 1.0f / 252.0f;
constexpr unsigned int threads_per_block = 512U;
constexpr std::size_t results_per_kernel_launch = 512U;
constexpr std::size_t block_count = 512U;
constexpr std::uint64_t seed = 900000001ULL;

// Descriptive metadata used only to write the result JSON and YAML.
const std::string target_dt_description = "1 / 252";
const std::string random_generator = "Philox";
const std::filesystem::path output_root =
    "registry/production/prices/heston/european_calls";
const std::filesystem::path generation_script =
    "registry/production/prices/heston/european_calls/generators/"
    "heston_01__european_calls_01__02.cpp";

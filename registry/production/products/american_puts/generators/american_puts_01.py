"""Generate the american_puts_01 production product database."""
...
EXERCISE_DATES_PER_YEAR = 52


def product_parameters() -> list[dict[str, float | int | str]]:
    ...
    return [
        {
            ...
            "exercise_dates_per_year": EXERCISE_DATES_PER_YEAR,
        }
...
def main() -> None:
    write_product_recipe_database(
        ...
        title="American put production product parameter database",
        payoff_key="american_put",
        expression="sup_tau E[exp(-r tau) max(K - S_tau, 0)]",
        scaling_rule="V(s, K) = s * V(1, K / s)",
        parameters=product_parameters(),
        construction={
            "method": "random sample",
            "rule": (
                "T is uniform on [1/12, 3]. Conditional on T, log(K) is "
                "uniform on [-aT, aT]. Exercise dates are weekly."
            ),
        ...
        parameter_docs={
            ...
            "exercise_frequency": "Exercise-date convention.",
            "exercise_dates_per_year": "Number of exercise dates per year.",
        },

...
#include "model/vasicek/floorlet.cuh"
#include "model/vasicek/zero_coupon_bond_call.cuh"
#include "model/vasicek/zero_coupon_bond_put.cuh"
...
void check_launcher(
    ...
    try {
        ai_factory::workbench::check_cuda(
            ...
            "Vasicek rate-option test cudaMalloc models"
        );
        ai_factory::workbench::check_cuda(
            cudaMalloc(&device_products, row_count * sizeof(Product)),
            "Vasicek rate-option test cudaMalloc products"
        );
        ai_factory::workbench::check_cuda(
            cudaMalloc(&device_prices, cartesian_count * sizeof(float)),
            "Vasicek rate-option test cudaMalloc prices"
        );
        ai_factory::workbench::check_cuda(
            cudaMemcpy(
                device_models,
            ...
            "Vasicek rate-option test cudaMemcpy models"
        );
        ai_factory::workbench::check_cuda(
            cudaMemcpy(
                device_products,
            ...
            "Vasicek rate-option test cudaMemcpy products"
        );
        ...
        ai_factory::workbench::check_cuda(
            ...
            "Vasicek rate-option test cudaMemcpy aligned prices"
        );
        ...
        ai_factory::workbench::check_cuda(
            ...
            "Vasicek rate-option test cudaMemcpy Cartesian prices"
        );
    ...
    ai_factory::workbench::check_cuda(
        cudaFree(device_models), "Vasicek rate-option test cudaFree models"
    );
    ai_factory::workbench::check_cuda(
        cudaFree(device_products), "Vasicek rate-option test cudaFree products"
    );
    ai_factory::workbench::check_cuda(
        cudaFree(device_prices), "Vasicek rate-option test cudaFree prices"
    );
}
...
int main() {
    ...
    check_cuda(availability, "Vasicek rate-option test cudaGetDeviceCount");
    ...
    check_launcher(
        models,
        floorlets,
        vasicek::launch_vasicek_floorlet_cuda,
        ...
        "Vasicek floorlet CUDA price differs from the FP64 formula"
    );
    check_launcher(
        models,
        calls,
        vasicek::launch_vasicek_zero_coupon_bond_call_cuda,
        [](const Model& model,
        ...
        "Vasicek zero-coupon bond call differs from the FP64 formula"
    );
    check_launcher(
        models,
        puts,
        vasicek::launch_vasicek_zero_coupon_bond_put_cuda,
        [](const Model& model,
        ...
        "Vasicek zero-coupon bond put differs from the FP64 formula"
    );
}

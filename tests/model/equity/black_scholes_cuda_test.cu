...
double range_accrual_price(
    ...
    for (std::uint32_t observation = 1U;
        ...
        probability_sum += interval_probability(
            model,
            // Contract bounds are relative to S(0); this oracle takes levels.
            static_cast<double>(model.spot) * product.lower_barrier,
            static_cast<double>(model.spot) * product.upper_barrier,
...
int main() {
    ...
    const ModelParameters nonunit_spot{1.2f, 0.0f, 0.0f, 0.01f};
    const product::RangeAccrualParameters narrow_range{252U, 63U, 0.8f, 1.1f, 0.1f};
    const float full_coupon = price_one(
        nonunit_spot, narrow_range,
        ai_factory::workbench::model::equity::black_scholes::launch_black_scholes_range_accrual_cuda
    );
    require(close(full_coupon, 1.1f, 2.0e-6f),
            "BS relative range lost its coupon for a nonunit initial spot");
}

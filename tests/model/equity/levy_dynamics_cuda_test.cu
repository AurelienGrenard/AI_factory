...
__global__ void vg_small_nu_drift(float nu, float theta, float* output) {
    namespace vg = ai_factory::workbench::model::equity::variance_gamma;
    const auto prepared = vg::prepare_model({1.0f, 0.0f, 0.0f, 0.2f, nu, theta});
    output[0] = prepared.drift_rate;
    output[1] = vg::prepare_transition(prepared, 0.25f).drift;
}

void check_vg_brownian_limit() {
    using ai_factory::workbench::check_cuda;
    float* device = nullptr;
    check_cuda(cudaMalloc(&device, 2U * sizeof(float)), "VG drift allocation");
    for (const float theta : {-0.1f, 0.0f, 0.1f}) {
        for (const float nu : {1.0e-9f, 1.0e-8f, 1.0e-7f, 1.0e-6f,
                               1.0e-4f, 0.01f, 0.25f}) {
            vg_small_nu_drift<<<1, 1>>>(nu, theta, device);
            check_cuda(cudaGetLastError(), "VG drift launch");
            float actual[2];
            check_cuda(cudaMemcpy(actual, device, sizeof(actual), cudaMemcpyDeviceToHost),
                       "VG drift copy");
            // Host long-double oracle; the limiting drift is -(theta+sigma²/2).
            const long double limit = -(static_cast<long double>(theta)
                + 0.5L * static_cast<long double>(0.2f) * 0.2f);
            const long double expected = std::log1p(nu * limit) / nu;
            require(std::isfinite(actual[0]) && std::isfinite(actual[1])
                        && std::abs(actual[0] - expected) < 3.0e-8L
                        && std::abs(actual[1] - 0.25L * expected) < 1.0e-8L,
                    "VG small-nu drift differs from the independent oracle");
            if (nu <= 1.0e-7f) {
                require(std::abs(actual[0] - limit) < 3.0e-8L,
                        "VG drift loses its Brownian limit");
            }
        }
    }
    check_cuda(cudaFree(device), "VG drift release");
}

...
#include "tests/model/fixed_income/fixtures/jamshidian_stress_rows.hpp"
...
// CPU-only independent CIR discount formula. The stress cases below have a
// coupon bond below par even at r=0, so the payer is certainly exercised.
double cir_stress_price(const CirModel& model, const Product& product, bool payer) {
    const auto discount = [&](double state, double maturity) {
        const double a = model.process.mean_reversion;
        const double sigma = model.process.volatility;
        const double h = std::sqrt(a * a + 2.0 * sigma * sigma);
        const double decay = std::exp(-h * maturity);
        const double complement = -std::expm1(-h * maturity);
        const double denominator = 2.0 * h * decay + (a + h) * complement;
        const double log_a = 2.0 * a * model.process.long_term_mean / (sigma * sigma)
            * (std::log(2.0 * h / denominator) + 0.5 * (a - h) * maturity);
        return std::exp(log_a - 2.0 * complement / denominator * state);
    };
    double maximum_exercise_bond = 0.0;
    const double exercise = product.exercise_time_days * kDayFraction;
    double price = discount(model.initial_state, exercise);
    for (unsigned int i = 0U; i < product.payment_count; ++i) {
        const double coefficient = static_cast<double>(product.strike)
            * product.accrual_fraction + (i + 1U == product.payment_count ? 1.0 : 0.0);
        const double tenor = (i + 1U) * product.payment_interval_days * kDayFraction;
        maximum_exercise_bond += coefficient * discount(0.0, tenor);
        price -= coefficient * discount(model.initial_state, exercise + tenor);
    }
    require(maximum_exercise_bond < 1.0,
        "CIR stress reference requires a certainly exercised payer");
    return payer ? product.notional * price : 0.0;
}

// Replay frozen failures and one-ULP volatility neighbours through the public
// scalar/cooperative launchers. All independent FP64 references run on CPU.
template<class Model, std::size_t Count, class Launcher, class Reference>
void check_stress_rows(
    const std::array<jamshidian_stress_fixtures::Row<Model>, Count>& fixtures,
    Launcher launch, Reference reference
) {
    ...
    for (const auto& fixture : fixtures) {
        for (int neighbour = -1; neighbour <= 1; ++neighbour) {
            ...
            if (neighbour != 0) {
                model.process.volatility = std::nextafter(
                    model.process.volatility,
                    neighbour < 0 ? 0.0f : std::numeric_limits<float>::infinity()
                );
            }
            models.push_back(model);
            products.push_back(fixture.product);
        }
    }
    offline::cuda::DeviceBuffer<Model> device_models(models.size());
    offline::cuda::DeviceBuffer<Product> device_products(products.size());
    offline::cuda::DeviceBuffer<float> device_prices(products.size());
    device_models.copy_from(models.data());
    device_products.copy_from(products.data());
    std::vector<float> prices(products.size());
    for (bool payer : {true, false}) {
        std::vector<double> expected;
        for (std::size_t i = 0U; i < models.size(); ++i)
            expected.push_back(reference(models[i], products[i], payer));
        for (auto distribution : {closed_form::WorkDistribution::scalar,
                                  closed_form::WorkDistribution::cooperative}) {
            for (unsigned int threads : {128U, 256U, 512U}) {
                for (unsigned int blocks : {1U, 4U, static_cast<unsigned int>(models.size())}) {
                    launch(payer, device_models.data(), models.size(),
                        device_products.data(), products.size(),
                        PriceConstruction::Aligned, products.size(), 0U,
                        products.size(), static_cast<float>(kDayFraction),
                        threads, blocks, device_prices.data(), 600U, distribution);
                    check_cuda(cudaMemcpy(prices.data(), device_prices.data(),
                        prices.size() * sizeof(float), cudaMemcpyDeviceToHost),
                        "Jamshidian stress replay copy");
                    for (std::size_t i = 0U; i < prices.size(); ++i) {
                        if (!std::isfinite(prices[i]) || prices[i] < 0.0f
                            || std::abs(prices[i] - expected[i])
                                > 2.0e-6 + 2.0e-5 * std::abs(expected[i])) {
                            throw std::runtime_error("Jamshidian stress CPU-reference mismatch: row "
                                + std::to_string(fixtures[i / 3U].source_id));
                        }
                    }
                }
            }
...
void check_catalogue_stress_rows() {
    using namespace ai_factory::workbench;
    check_stress_rows(jamshidian_stress_fixtures::cir_rows,
        [](bool payer, auto... arguments) {
            if (payer) model::fixed_income::cir::launch_cir_european_swaption_cuda<SwaptionSide::payer>(arguments...);
            else model::fixed_income::cir::launch_cir_european_swaption_cuda<SwaptionSide::receiver>(arguments...);
        }, cir_stress_price);
    check_stress_rows(jamshidian_stress_fixtures::vasicek_rows,
        [](bool payer, auto... arguments) {
            if (payer) model::fixed_income::vasicek::launch_vasicek_european_swaption_cuda<SwaptionSide::payer>(arguments...);
            else model::fixed_income::vasicek::launch_vasicek_european_swaption_cuda<SwaptionSide::receiver>(arguments...);
        }, [](const auto& model, const auto& product, bool payer) {
            return jamshidian_price(product, payer,
                [&](double state, double start, double maturity) {
                    return vasicek_zero_coupon(model, state, start, maturity);
                }, [&](double sign, double expiry, double maturity, double strike) {
                    return vasicek_bond_option(model, sign, expiry, maturity, strike);
                });
        });
}
...
int main() {
    ...
    for (const auto distribution : {closed_form::WorkDistribution::scalar,
                                    closed_form::WorkDistribution::cooperative}) {
    ...
    for (const bool payer : {true, false}) {
        ...
        if (payer) {
            check_two_input_launcher(
                vasicek_models,
                fixtures,
                [maximum = maximum_payment_count(fixtures), distribution](auto... arguments) {
            ai_factory::workbench::model::fixed_income::vasicek::launch_vasicek_european_swaption_cuda<SwaptionSide::payer>(arguments..., maximum, distribution);
        },
                expected,
                8.0e-5,
            ...
            check_two_input_launcher(
                ...
                fixtures,
                [maximum = maximum_payment_count(fixtures), distribution](auto... arguments) {
            ai_factory::workbench::model::fixed_income::vasicek::launch_vasicek_european_swaption_cuda<SwaptionSide::receiver>(arguments..., maximum, distribution);
        },
                expected,
                8.0e-5,
    ...
    check_fitted_curve(
        ...
        [maximum = maximum_payment_count(fixtures), distribution](auto... arguments) {
            hw_ns::launch_hull_white_nelson_siegel_european_swaption_cuda<SwaptionSide::payer>(arguments..., maximum, distribution);
        },
        [maximum = maximum_payment_count(fixtures), distribution](auto... arguments) {
            hw_ns::launch_hull_white_nelson_siegel_european_swaption_cuda<SwaptionSide::receiver>(arguments..., maximum, distribution);
        },
    ...
    check_fitted_curve(
        svensson_curves,
        [maximum = maximum_payment_count(fixtures), distribution](auto... arguments) {
            hw_sv::launch_hull_white_svensson_european_swaption_cuda<SwaptionSide::payer>(arguments..., maximum, distribution);
        },
        [maximum = maximum_payment_count(fixtures), distribution](auto... arguments) {
            hw_sv::launch_hull_white_svensson_european_swaption_cuda<SwaptionSide::receiver>(arguments..., maximum, distribution);
        },

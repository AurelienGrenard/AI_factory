...
int main() {
    ...
    // Define padding copied to the host; poison floating fields with NaNs so
    // missing writes do not silently pass assertions expecting a finite zero.
    check_cuda(
        cudaMemset(device_results, 0xff, sizeof(NumericalResults)),
        "initialize numerical results storage"
    );

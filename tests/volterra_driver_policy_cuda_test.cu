// Host/device-compilation and stress checks for reusable Volterra drivers.
...
int main() {
    ...
    const auto fractional = FractionalHybridDriverPolicy::prepare(
        0.10f, 1.0f / 360.0f
    );
    require(
        relative_close(
            FractionalHybridDriverPolicy::variance(fractional, 1.0f),
            1.0,
            2.0e-6
        ),
        "Normalized fractional driver has incorrect one-year variance."
    );

    const auto log_modulated = LogModulatedHybridDriverPolicy::prepare(
        {0.10f, 0.10f, 2.0f},
        1.0f / 360.0f
    );
    require(
        std::isfinite(log_modulated.normalization)
            ...
            && relative_close(
                LogModulatedHybridDriverPolicy::variance(
                    log_modulated, 1.0f
                ),
                1.0,
                3.0e-5
            ),
        "Log-modulated driver normalization is incorrect."
    );
    const auto logarithmic_stress = LogModulatedHybridDriverPolicy::prepare(
        {0.0f, 0.05f, 1.05f},
        1.0f / 252.0f
    );
    require(
    ...
    const auto narrow_logarithmic_tail =
        LogModulatedHybridDriverPolicy::prepare(
    ...
    const double mittag_high = FractionalResolventHybridDriverPolicy::
        mittag_leffler_alpha_alpha(0.95, -10.0);
    const double mittag_low = FractionalResolventHybridDriverPolicy::
    ...
    const auto resolvent = FractionalResolventHybridDriverPolicy::prepare(
        {0.45f, 8.0f},
        1.0f / 252.0f
    );
    for (const float time : {1.0f, 5.0f, 7.0f}) {
        const float value = FractionalResolventHybridDriverPolicy::kernel(
            resolvent, time
        );
        require(

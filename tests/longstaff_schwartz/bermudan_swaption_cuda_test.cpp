...
struct CirAdapter {
    using ModelParameters = ai_factory::workbench::model::fixed_income::cir::ModelParameters;
    template<SwaptionSide Side>
    static lsm::LaunchResult launch(
        const ModelParameters* models,
        const product::BermudanSwaptionParameters* host_products,
        const product::BermudanSwaptionParameters* products,
        float* prices, float* errors
    ) {
        return ai_factory::workbench::model::fixed_income::cir::
            launch_cir_bermudan_swaption_cuda<Side>(
                models, kRowCount, host_products, products, kRowCount,
                ai_factory::workbench::PriceConstruction::Aligned, kRowCount,
                kPathsPerPrice, kDayFraction, 512U, 4U, kSeed, prices, errors
            );
    }
};
...
int main() {
    ...
    const std::array<CirAdapter::ModelParameters, kRowCount> cir_models{{
        {{0.5f, 0.04f, 0.1f}, 0.04f},
        {{0.3f, 0.02f, 0.2f}, 0.01f},
    }};
    validate_model<CirAdapter>(cir_models, products);
    // No fixed 32-exercise array, including daily exercises near the boundary.
    const std::array<product::BermudanSwaptionParameters, kRowCount> long_products{{
        {1.0f, 0.035f, 1.0f / 252.0f, 1U, 1U, 48U, 48U},
        {1.0f, 0.04f, 0.25f, 63U, 63U, 65U, 65U},
    }};
    validate_model<CirAdapter>(cir_models, long_products);
}

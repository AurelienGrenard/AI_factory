// Check small-basis evaluation, FP64 solves, statuses and LSM workspace planning.
...
void validate_schedule_and_workspace_planning() {
    ...
    auto with_observations = descriptor;
    with_observations.observation_fields = {{16U, alignof(float)}};
    const auto observation_layout = lsm::make_workspace_layout(
        with_observations, 2U, 448U, paths_per_price, blocks_per_price, "observation-test"
    );
    require(observation_layout.total_bytes == first_two.total_bytes + 7U * 16U,
        "Prepared observations were multiplied by the path count.");
    require(observation_layout.observation_fields.size() == 1U,
        "Missing prepared observation region.");
    bool invalid_observation_rows = false;
    try {
        lsm::make_workspace_layout(with_observations, 2U, 447U, paths_per_price,
            blocks_per_price, "observation-test");
    } catch (const std::invalid_argument&) { invalid_observation_rows = true; }
    require(invalid_observation_rows, "Partial observation rows were accepted.");
}

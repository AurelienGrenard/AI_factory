...
struct DeviceArrays {
    ai_factory::workbench::bates::BatesModelParameters* model = nullptr;
...
int main() {
    ...
    const bates::BatesModelParameters model = {
    ...
    require(
        ...
        "Bates path-product launcher returned a non-finite price"
    );
    require(
        european_error > 0.0f && asian_error > 0.0f
            && lookback_error > 0.0f,
        "Bates path-product launcher returned an invalid standard error"
    );
    require(
        lookback_price >= european_price && lookback_price >= asian_price,
        "Bates path-product payoffs violate their pathwise ordering"
    );
}

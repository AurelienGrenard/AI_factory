...
const std::vector<std::filesystem::path> catalog_paths = {
    ...
    cir_catalog_path,
    "catalog/model/fixed_income/cir_plus_plus/parameters/cir_plus_plus_01/dataset.yaml",
...
void validate_price_validation_metadata(const std::filesystem::path& path) {
    ...
    if (validation.find("  status: \"pending\"") != std::string::npos
        && validation.find("  dataset:") != std::string::npos) {
        require(
            occurrence_count(validation, "  verified: false") == 1U
            ...
            "pending validation must be compact and explicitly unverified: "
                + path.string()
        );
        return;
    }
    ...
    require(
        ...
        "price YAML validation reference is missing or unsupported: " + path.string()
    );
    require(
        none

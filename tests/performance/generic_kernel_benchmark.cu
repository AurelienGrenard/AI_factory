...
using ai_factory::workbench::report_cuda_kernel_launch_if_enabled;
...
void benchmark_indexing() {
    ...
    DeviceBuffer output(
        result_count * sizeof(std::uint64_t), DeviceMemoryRole::output
    );

    const auto runtime_launch = [&] {
        report_cuda_kernel_launch_if_enabled(
            "runtime_index_kernel",
            "runtime_enum_size_t",
            runtime_index_kernel,
...
Measurement measure_accumulation(
    ...
    return measure_cuda(
        [&] {
            const char* variant = Mode == AccumulationMode::Fp64
                ? "fp64"
                : Mode == AccumulationMode::CompensatedFp32
                    ? "compensated_fp32"
                    : "chunked_fp32_fp64";
            report_cuda_kernel_launch_if_enabled(
                "accumulation_kernel",
                variant,
                accumulation_kernel<Mode>,
                dim3(static_cast<unsigned int>(block_count)),
                dim3(kThreads)
            );
...
void benchmark_accumulation() {
    ...
    constexpr std::uint32_t iterations = 1'024U;
    DeviceBuffer output(
        value_count * sizeof(double), DeviceMemoryRole::output
    );
...
void benchmark_closed_form_overhead(std::size_t result_count) {
    ...
    DeviceBuffer device_prices(
        result_count * sizeof(float), DeviceMemoryRole::output
    );
...
void benchmark_rich_policy(
    ...
    for (const unsigned int threads : {128U, 256U, 512U}) {
        ...
        const Measurement measurement = measure_cuda(
            ...
            kWarmups,
            kRepetitions,
            kRichPolicyOperationsPerSample
        );
        const std::vector<float> prices = copy_from_device<float>(
            device_prices, result_count
        );
...
void benchmark_rich_policies() {
    ...
    benchmark_rich_policy(
        ...
        [&](const auto* models, const auto* host_products,
            const auto* products, unsigned int threads,
            float* prices, float* errors, std::size_t count,
            std::size_t paths) {
            common_launch(
                bates::launch_bates_phoenix_autocall_cuda,
                models, host_products, products, threads,
                prices, errors, count, paths
            );
        }
    );
    benchmark_rich_policy(
...
void benchmark_ragged_schedules(ScheduleVariant variant) {
    ...
    const std::size_t result_count = variant == ScheduleVariant::Regular
        ? 524'288U
        : 65'536U;

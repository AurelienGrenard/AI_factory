...
TimingResult measure(
    ...
    const auto measurement =
        ai_factory::workbench::performance::measure_cuda(
            launch, 5, repetitions, operations_per_sample
        );
...
void benchmark_heston(
    ...
    DeviceBuffer device_models(
        models.size() * sizeof(ModelParameters),
    ...
    DeviceBuffer device_prices(
        price_count * sizeof(float), DeviceMemoryRole::output
    );
...
void benchmark_rough_bergomi(
    ...
    DeviceBuffer device_workspace(
        plan.workspace_bytes, DeviceMemoryRole::caller_workspace
    );
    DeviceBuffer device_prices(
        price_count * sizeof(float), DeviceMemoryRole::output
    );
...
int main(int argument_count, char** arguments) {
    ...
    const std::size_t path_count = argument_count >= 8
        ...
        : 1U << 20U;
    const std::size_t operations_per_sample = argument_count == 9
        ? parse_size(arguments[8], "operations per sample")
        : 1U;
    if (maturity_days == 0U || steps_per_day == 0U || price_count == 0U
        || tuning == 0U || repetitions < 3 || path_count < 2U
        || operations_per_sample == 0U) {

// Benchmark 50 stratified Heston American-put rows without writing a registry.
...
std::vector<Value> stratified_rows(const std::vector<Value>& source) {
    if (source.size() != kSourceRowCount) {
        throw std::runtime_error("benchmark expects exactly 1000 source rows");
    }
...
int main(int argc, char** argv) {
    ...
    const std::vector<heston::HestonModelParameters> models =
        stratified_rows(heston::load_heston(model_json_path));
    const std::vector<products::AmericanPutInput> products =
        stratified_rows(products::load_american_puts(product_json_path));

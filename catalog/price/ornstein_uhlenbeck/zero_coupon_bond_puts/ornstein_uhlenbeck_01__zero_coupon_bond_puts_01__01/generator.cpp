// Build one Ornstein-Uhlenbeck zero-coupon bond put price dataset.
...
const std::filesystem::path product_dataset_path =
    "datasets/product/fixed_income/zero_coupon_bond_puts/"
    "zero_coupon_bond_puts_01.json";
...
const std::filesystem::path dataset_path =
    "datasets/price/ornstein_uhlenbeck/zero_coupon_bond_puts/"
    "ornstein_uhlenbeck_01__zero_coupon_bond_puts_01__01.json";
...
const std::string url =
    ...
    "ornstein_uhlenbeck/zero_coupon_bond_puts/"
    "ornstein_uhlenbeck_01__zero_coupon_bond_puts_01__01.json";
const std::string numerical_method =
    "Ornstein-Uhlenbeck closed-form zero-coupon bond put";
...
int main() {
    ...
    const std::vector<ou::OrnsteinUhlenbeckModelParameters> models =
        ou::load_models(model_dataset_path);
    const std::vector<product::ZeroCouponBondPutParameters> products =
        product::load_zero_coupon_bond_puts(product_dataset_path);
    ...
    product::ZeroCouponBondPutParameters* device_products = nullptr;
    ...
    try {
        ...
        check_cuda(
            ...
            "cudaMalloc OU zero-coupon bond put prices"
        );
        check_cuda(
            cudaMemcpy(
                device_models,
        ...
        check_cuda(cudaDeviceSynchronize(), "OU ZCB put warmup");
        ...
        check_cuda(
            ...
            "cudaMemcpy OU zero-coupon bond put prices"
        );
    } catch (...) {
    ...
    check_cuda(cudaFree(device_models), "cudaFree OU models");
    check_cuda(cudaFree(device_products), "cudaFree zero-coupon bond puts");
    check_cuda(cudaFree(device_prices), "cudaFree OU ZCB put prices");

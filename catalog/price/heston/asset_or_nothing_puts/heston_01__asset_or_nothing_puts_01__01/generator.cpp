...
const std::filesystem::path product_dataset_path =
    "datasets/product/equity/asset_or_nothing_puts/asset_or_nothing_puts_01.json";
...
const std::filesystem::path dataset_path =
    "datasets/price/heston/asset_or_nothing_puts/"
    "heston_01__asset_or_nothing_puts_01__01.json";
...
int main() {
    ...
    const std::vector<product::AssetOrNothingPutParameters> products =
        product::load_asset_or_nothing_puts(product_dataset_path);
    ...
    product::AssetOrNothingPutParameters* device_products = nullptr;
    ...
    try {
        ...
        check_cuda(
            cudaMalloc(
                &device_products,
                products.size() * sizeof(product::AssetOrNothingPutParameters)
            ),
        ...
        check_cuda(
            ...
            "cudaMalloc Heston asset-or-nothing put prices"
        );
        check_cuda(
            cudaMalloc(
            ...
            "cudaMalloc Heston asset-or-nothing put standard errors"
        );
        ...
        check_cuda(cudaDeviceSynchronize(), "Heston asset-or-nothing put warmup");
        ...
        check_cuda(
            ...
            "cudaMemcpy Heston asset-or-nothing put prices"
        );
        check_cuda(
            cudaMemcpy(
            ...
            "cudaMemcpy Heston asset-or-nothing put standard errors"
        );
    } catch (...) {
    ...
    check_cuda(cudaFree(device_prices), "cudaFree Heston asset-or-nothing put prices");
    check_cuda(
        cudaFree(device_standard_errors),
        "cudaFree Heston asset-or-nothing put standard errors"
    );

// Build one Heston Asian-call price dataset from JSON inputs.
...
const std::filesystem::path product_dataset_path =
    "datasets/product/equity/asian_calls/asian_calls_01.json";
...
int main() {
    ...
    const std::vector<product::AsianCallParameters> products =
        product::load_asian_calls(product_dataset_path);
    ...
    product::AsianCallParameters* device_products = nullptr;
    ...
    try {
        ...
        check_cuda(
            cudaMalloc(
                &device_products,
                products.size() * sizeof(product::AsianCallParameters)
            ),
        ...
        check_cuda(
            ...
            "cudaMalloc Heston Asian call prices"
        );
        check_cuda(
            cudaMalloc(
            ...
            "cudaMalloc Heston Asian call standard errors"
        );
        ...
        check_cuda(cudaDeviceSynchronize(), "Heston Asian call warmup");
        ...
        check_cuda(
            ...
            "cudaMemcpy Heston Asian call prices"
        );
        check_cuda(
            cudaMemcpy(
            ...
            "cudaMemcpy Heston Asian call standard errors"
        );
    } catch (...) {
    ...
    check_cuda(cudaFree(device_prices), "cudaFree Heston Asian call prices");
    check_cuda(
        cudaFree(device_standard_errors),
        "cudaFree Heston Asian call standard errors"
    );

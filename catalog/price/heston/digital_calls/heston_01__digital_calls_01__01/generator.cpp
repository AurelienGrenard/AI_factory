...
const std::filesystem::path product_dataset_path =
    "datasets/product/equity/digital_calls/digital_calls_01.json";
...
const std::filesystem::path dataset_path =
    "datasets/price/heston/digital_calls/"
    "heston_01__digital_calls_01__01.json";
const std::filesystem::path catalog_path =
    "catalog/price/heston/digital_calls/"
    "heston_01__digital_calls_01__01/dataset.yaml";
const std::string url =
    "https://datasets.ai-factory.example/v1/price/heston/digital_calls/"
    "heston_01__digital_calls_01__01.json";
...
int main() {
    ...
    const std::vector<product::DigitalCallParameters> products =
        product::load_digital_calls(product_dataset_path);
    ...
    product::DigitalCallParameters* device_products = nullptr;
    ...
    try {
        ...
        check_cuda(
            cudaMalloc(
                &device_products,
                products.size() * sizeof(product::DigitalCallParameters)
            ),
        ...
        check_cuda(
            ...
            "cudaMalloc Heston digital call prices"
        );
        check_cuda(
            cudaMalloc(
            ...
            "cudaMalloc Heston digital call standard errors"
        );
        ...
        check_cuda(cudaDeviceSynchronize(), "Heston digital call warmup");
        ...
        check_cuda(
            ...
            "cudaMemcpy Heston digital call prices"
        );
        check_cuda(
            cudaMemcpy(
            ...
            "cudaMemcpy Heston digital call standard errors"
        );
    } catch (...) {
    ...
    check_cuda(cudaFree(device_prices), "cudaFree Heston digital call prices");
    check_cuda(
        cudaFree(device_standard_errors),
        "cudaFree Heston digital call standard errors"
    );

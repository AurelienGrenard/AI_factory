...
const std::filesystem::path product_dataset_path =
    "datasets/product/equity/gap_puts/gap_puts_01.json";
...
int main() {
    ...
    const std::vector<product::GapPutParameters> products =
        product::load_gap_puts(product_dataset_path);
    ...
    product::GapPutParameters* device_products = nullptr;
    ...
    try {
        ...
        check_cuda(
            cudaMalloc(
                &device_products,
                products.size() * sizeof(product::GapPutParameters)
            ),
            "cudaMalloc Gap puts"
        );
        check_cuda(

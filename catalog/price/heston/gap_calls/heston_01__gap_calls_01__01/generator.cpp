...
const std::filesystem::path dataset_path =
    "datasets/price/heston/gap_calls/"
    "heston_01__gap_calls_01__01.json";
const std::filesystem::path catalog_path =
    "catalog/price/heston/gap_calls/"
    "heston_01__gap_calls_01__01/dataset.yaml";
const std::string url =
    "https://datasets.ai-factory.example/v1/price/heston/gap_calls/"
    "heston_01__gap_calls_01__01.json";
...
int main() {
    ...
    const std::vector<product::GapCallParameters> products =
        product::load_gap_calls(product_dataset_path);
    ...
    product::GapCallParameters* device_products = nullptr;
    ...
    try {
        ...
        check_cuda(
            cudaMalloc(
                &device_products,
                products.size() * sizeof(product::GapCallParameters)
            ),
        ...
        check_cuda(
            ...
            "cudaMalloc Heston gap call prices"
        );
        check_cuda(
            cudaMalloc(
            ...
            "cudaMalloc Heston gap call standard errors"
        );
        ...
        check_cuda(cudaDeviceSynchronize(), "Heston gap call warmup");
        ...
        check_cuda(
            ...
            "cudaMemcpy Heston gap call prices"
        );
        check_cuda(
            cudaMemcpy(
            ...
            "cudaMemcpy Heston gap call standard errors"
        );
    } catch (...) {
    ...
    check_cuda(cudaFree(device_prices), "cudaFree Heston gap call prices");
    check_cuda(
        cudaFree(device_standard_errors),
        "cudaFree Heston gap call standard errors"
    );

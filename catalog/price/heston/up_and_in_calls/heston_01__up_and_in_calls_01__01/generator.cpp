// Build one Heston Up-and-in-call price dataset from JSON inputs.
...
const std::filesystem::path product_dataset_path =
    "datasets/product/equity/up_and_in_calls/up_and_in_calls_01.json";
...
const std::filesystem::path dataset_path =
    "datasets/price/heston/up_and_in_calls/"
    "heston_01__up_and_in_calls_01__01.json";
const std::filesystem::path catalog_path =
    "catalog/price/heston/up_and_in_calls/"
    "heston_01__up_and_in_calls_01__01/dataset.yaml";
const std::string url =
    "https://mlp.lpma.math.upmc.fr/DataCarlo/Assets/Heston/UpAndInCall/"
    "heston_01__up_and_in_calls_01__01.json";
...
int main() {
    ...
    const std::vector<product::UpAndInCallParameters> products =
        product::load_up_and_in_calls(product_dataset_path);
    ...
    product::UpAndInCallParameters* device_products = nullptr;
    ...
    try {
        ...
        check_cuda(
            cudaMalloc(
                &device_products,
                products.size() * sizeof(product::UpAndInCallParameters)
            ),
        ...
        check_cuda(
            ...
            "cudaMalloc Heston up-and-in call prices"
        );
        check_cuda(
            cudaMalloc(
            ...
            "cudaMalloc Heston up-and-in call standard errors"
        );
        ...
        check_cuda(cudaDeviceSynchronize(), "Heston up-and-in call warmup");
        ...
        check_cuda(
            ...
            "cudaMemcpy Heston up-and-in call prices"
        );
        check_cuda(
            cudaMemcpy(
            ...
            "cudaMemcpy Heston up-and-in call standard errors"
        );
    } catch (...) {
    ...
    check_cuda(cudaFree(device_prices), "cudaFree Heston up-and-in call prices");
    check_cuda(
        cudaFree(device_standard_errors),
        "cudaFree Heston up-and-in call standard errors"
    );

// Build one Heston Asian-put price dataset from JSON inputs.
...
const std::filesystem::path product_dataset_path =
    "datasets/product/equity/asian_puts/asian_puts_01.json";
...
int main() {
    ...
    const std::vector<product::AsianPutParameters> products =
        product::load_asian_puts(product_dataset_path);
    ...
    product::AsianPutParameters* device_products = nullptr;
    ...
    try {
        ...
        check_cuda(
            cudaMalloc(
                &device_products,
                products.size() * sizeof(product::AsianPutParameters)
            ),
        ...
        check_cuda(
            ...
            "cudaMalloc Heston Asian put prices"
        );
        check_cuda(
            cudaMalloc(
            ...
            "cudaMalloc Heston Asian put standard errors"
        );
        ...
        check_cuda(cudaDeviceSynchronize(), "Heston Asian put warmup");
        ...
        check_cuda(
            ...
            "cudaMemcpy Heston Asian put prices"
        );
        check_cuda(
            cudaMemcpy(
            ...
            "cudaMemcpy Heston Asian put standard errors"
        );
    } catch (...) {
    ...
    check_cuda(cudaFree(device_prices), "cudaFree Heston Asian put prices");
    check_cuda(
        cudaFree(device_standard_errors),
        "cudaFree Heston Asian put standard errors"
    );

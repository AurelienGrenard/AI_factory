// Build one Heston asset-or-nothing price dataset from JSON inputs.
...
const std::filesystem::path product_dataset_path =
    "datasets/product/equity/asset_or_nothing_calls/asset_or_nothing_calls_01.json";
...
const std::filesystem::path dataset_path =
    "datasets/price/heston/asset_or_nothing_calls/"
    "heston_01__asset_or_nothing_calls_01__01.json";
const std::filesystem::path catalog_path =
    "catalog/price/heston/asset_or_nothing_calls/"
    "heston_01__asset_or_nothing_calls_01__01/dataset.yaml";
const std::string url =
    "https://datasets.ai-factory.example/v1/price/heston/"
...
int main() {
    ...
    const std::vector<product::AssetOrNothingCallParameters> products =
        product::load_asset_or_nothing_calls(product_dataset_path);
    ...
    product::AssetOrNothingCallParameters* device_products = nullptr;
    ...
    try {
        ...
        check_cuda(
            cudaMalloc(
                &device_products,
                products.size() * sizeof(product::AssetOrNothingCallParameters)
            ),
        ...
        check_cuda(
            ...
            "cudaMalloc Heston asset-or-nothing call prices"
        );
        check_cuda(
            cudaMalloc(
            ...
            "cudaMalloc Heston asset-or-nothing call standard errors"
        );
        ...
        check_cuda(cudaDeviceSynchronize(), "Heston asset-or-nothing call warmup");
        ...
        check_cuda(
            ...
            "cudaMemcpy Heston asset-or-nothing call prices"
        );
        check_cuda(
            cudaMemcpy(
            ...
            "cudaMemcpy Heston asset-or-nothing call standard errors"
        );
    } catch (...) {
    ...
    check_cuda(cudaFree(device_prices), "cudaFree Heston asset-or-nothing call prices");
    check_cuda(
        cudaFree(device_standard_errors),
        "cudaFree Heston asset-or-nothing call standard errors"
    );

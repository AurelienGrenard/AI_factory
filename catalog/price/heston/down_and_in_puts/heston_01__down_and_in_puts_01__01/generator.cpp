...
const std::filesystem::path product_dataset_path =
    "datasets/product/equity/down_and_in_puts/down_and_in_puts_01.json";
...
int main() {
    ...
    const std::vector<product::DownAndInPutParameters> products =
        product::load_down_and_in_puts(product_dataset_path);
    ...
    product::DownAndInPutParameters* device_products = nullptr;
    ...
    try {
        ...
        check_cuda(
            cudaMalloc(
                &device_products,
                products.size() * sizeof(product::DownAndInPutParameters)
            ),

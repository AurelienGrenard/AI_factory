// Build one Heston fixed-strike lookback price dataset from JSON inputs.
...
const std::filesystem::path dataset_path =
    "datasets/price/equity/heston/lookback_options/"
    "heston_01__lookback_options_01__01.json";
const std::filesystem::path catalog_path =
    "catalog/price/equity/heston/lookback_options/"
    "heston_01__lookback_options_01__01/dataset.yaml";
const std::string url =
    "https://mlp.lpma.math.upmc.fr/DataCarlo/Assets/Heston/LookbackOption/"
    "heston_01__lookback_options_01__01.json";
...
int main() {
    ...
    try {
        ...
        check_cuda(
            ...
            "cudaMalloc Heston lookback prices"
        );
        check_cuda(
            cudaMalloc(
            ...
            "cudaMalloc Heston lookback standard errors"
        );
        ...
        check_cuda(cudaDeviceSynchronize(), "Heston lookback warmup");
        ...
        check_cuda(
            ...
            "cudaMemcpy Heston lookback prices"
        );
        check_cuda(
            cudaMemcpy(
            ...
            "cudaMemcpy Heston lookback standard errors"
        );
    } catch (...) {
    ...
    check_cuda(cudaFree(device_prices), "cudaFree Heston lookback prices");
    check_cuda(
        cudaFree(device_standard_errors),
        "cudaFree Heston lookback standard errors"
    );

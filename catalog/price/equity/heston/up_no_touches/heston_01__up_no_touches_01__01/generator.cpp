// Build one Heston Up-no-touch price dataset from JSON inputs.
...
const std::filesystem::path dataset_path =
    "datasets/price/equity/heston/up_no_touches/"
    "heston_01__up_no_touches_01__01.json";
const std::filesystem::path catalog_path =
    "catalog/price/equity/heston/up_no_touches/"
    "heston_01__up_no_touches_01__01/dataset.yaml";
const std::string url =
    "https://mlp.lpma.math.upmc.fr/DataCarlo/Assets/Heston/UpNoTouch/"
    "heston_01__up_no_touches_01__01.json";
...
int main() {
    ...
    try {
        ...
        check_cuda(
            ...
            "cudaMalloc Heston up no-touch prices"
        );
        check_cuda(
            cudaMalloc(
            ...
            "cudaMalloc Heston up no-touch standard errors"
        );
        ...
        check_cuda(cudaDeviceSynchronize(), "Heston up no-touch warmup");
        ...
        check_cuda(
            ...
            "cudaMemcpy Heston up no-touch prices"
        );
        check_cuda(
            cudaMemcpy(
            ...
            "cudaMemcpy Heston up no-touch standard errors"
        );
    } catch (...) {
    ...
    check_cuda(cudaFree(device_prices), "cudaFree Heston up no-touch prices");
    check_cuda(
        cudaFree(device_standard_errors),
        "cudaFree Heston up no-touch standard errors"
    );

// Build one Heston Up-one-touch price dataset from JSON inputs.
...
const std::filesystem::path dataset_path =
    "datasets/price/equity/heston/up_one_touches/"
    "heston_01__up_one_touches_01__01.json";
const std::filesystem::path catalog_path =
    "catalog/price/equity/heston/up_one_touches/"
    "heston_01__up_one_touches_01__01/dataset.yaml";
const std::string url =
    "https://mlp.lpma.math.upmc.fr/DataCarlo/Assets/Heston/UpOneTouch/"
    "heston_01__up_one_touches_01__01.json";
...
int main() {
    ...
    try {
        ...
        check_cuda(
            ...
            "cudaMalloc Heston up one-touch prices"
        );
        check_cuda(
            cudaMalloc(
            ...
            "cudaMalloc Heston up one-touch standard errors"
        );
        ...
        check_cuda(cudaDeviceSynchronize(), "Heston up one-touch warmup");
        ...
        check_cuda(
            ...
            "cudaMemcpy Heston up one-touch prices"
        );
        check_cuda(
            cudaMemcpy(
            ...
            "cudaMemcpy Heston up one-touch standard errors"
        );
    } catch (...) {
    ...
    check_cuda(cudaFree(device_prices), "cudaFree Heston up one-touch prices");
    check_cuda(
        cudaFree(device_standard_errors),
        "cudaFree Heston up one-touch standard errors"
    );

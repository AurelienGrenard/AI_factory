// Build one Black-Scholes gap_calls price dataset.
...
const std::filesystem::path dataset_path =
    "datasets/price/equity/black_scholes/gap_calls/"
    "black_scholes_01__gap_calls_01__01.json";
const std::filesystem::path catalog_path =
    "catalog/price/equity/black_scholes/gap_calls/"
    "black_scholes_01__gap_calls_01__01/dataset.yaml";
const std::string url =
    "https://datasets.ai-factory.example/v1/price/"
    "equity/black_scholes/gap_calls/"
    "black_scholes_01__gap_calls_01__01.json";
const std::string numerical_method =
    "Black-Scholes closed-form gap call";
...
int main() {
    ...
    try {
        ...
        check_cuda(
            ...
            "cudaMalloc gap_calls"
        );
        check_cuda(
            cudaMalloc(&device_prices, result_count * sizeof(float)),
            "cudaMalloc Black-Scholes gap_calls prices"
        );
        check_cuda(
            cudaMemcpy(
                device_models,
        ...
        bs::launch_black_scholes_gap_option_cuda<OptionSide::call>(
            device_models,
            warmup_count,
            device_products,
            warmup_count,
        ...
        check_cuda(cudaDeviceSynchronize(), "Black-Scholes gap_calls warmup");
        ...
        check_cuda(
            ...
            "cudaMemcpy Black-Scholes gap_calls prices"
        );
    } catch (...) {
    ...
    check_cuda(cudaFree(device_products), "cudaFree gap_calls");
    check_cuda(cudaFree(device_prices), "cudaFree Black-Scholes gap_calls prices");

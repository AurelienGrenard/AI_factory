// Build one Black-Scholes geometric_asian_calls price dataset.
...
const std::filesystem::path dataset_path =
    "datasets/price/equity/black_scholes/geometric_asian_calls/"
    "black_scholes_01__geometric_asian_calls_01__01.json";
const std::filesystem::path catalog_path =
    "catalog/price/equity/black_scholes/geometric_asian_calls/"
    "black_scholes_01__geometric_asian_calls_01__01/dataset.yaml";
const std::string url =
    "https://datasets.ai-factory.example/v1/price/"
    "equity/black_scholes/geometric_asian_calls/"
    "black_scholes_01__geometric_asian_calls_01__01.json";
const std::string numerical_method =
    "Black-Scholes closed-form geometric Asian call";
...
int main() {
    ...
    const std::vector<bs::BlackScholesModelParameters> models =
    ...
    bs::BlackScholesModelParameters* device_models = nullptr;
    ...
    try {
        ...
        check_cuda(
            ...
            "cudaMalloc geometric_asian_calls"
        );
        check_cuda(
            cudaMalloc(&device_prices, result_count * sizeof(float)),
            "cudaMalloc Black-Scholes geometric_asian_calls prices"
        );
        check_cuda(
            cudaMemcpy(
                device_models,
        ...
        check_cuda(
            ...
            "cudaMemcpy geometric_asian_calls"
        );
        ...
        bs::launch_black_scholes_geometric_asian_option_cuda<OptionSide::call>(
            device_models,
            warmup_count,
            device_products,
            warmup_count,
        ...
        check_cuda(cudaDeviceSynchronize(), "Black-Scholes geometric_asian_calls warmup");
        ...
        check_cuda(
            ...
            "cudaMemcpy Black-Scholes geometric_asian_calls prices"
        );
    } catch (...) {
    ...
    check_cuda(cudaFree(device_products), "cudaFree geometric_asian_calls");
    check_cuda(cudaFree(device_prices), "cudaFree Black-Scholes geometric_asian_calls prices");

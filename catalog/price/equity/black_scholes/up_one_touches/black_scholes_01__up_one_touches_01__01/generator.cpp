// Build one BlackScholes Up-one-touch price dataset from JSON inputs.
#include "common/check_cuda.cuh"
#include "model/equity/black_scholes/up_one_touch.cuh"
...
const std::filesystem::path dataset_path =
    "datasets/price/equity/black_scholes/up_one_touches/"
    "black_scholes_01__up_one_touches_01__01.json";
const std::filesystem::path catalog_path =
    "catalog/price/equity/black_scholes/up_one_touches/"
    "black_scholes_01__up_one_touches_01__01/dataset.yaml";
const std::string url =
    "https://mlp.lpma.math.upmc.fr/DataCarlo/Assets/BlackScholes/UpOneTouch/"
    "black_scholes_01__up_one_touches_01__01.json";
...
int main() {
    ...
    try {
        ...
        check_cuda(
            ...
            "cudaMalloc BlackScholes up one-touch prices"
        );
        check_cuda(
            cudaMalloc(
            ...
            "cudaMalloc BlackScholes up one-touch standard errors"
        );
        ...
        black_scholes::launch_black_scholes_up_one_touch_cuda(
            device_models,
            warmup_row_count,
            device_products,
            warmup_row_count,
        ...
        check_cuda(cudaDeviceSynchronize(), "BlackScholes up one-touch warmup");
        ...
        check_cuda(
            ...
            "cudaMemcpy BlackScholes up one-touch prices"
        );
        check_cuda(
            cudaMemcpy(
            ...
            "cudaMemcpy BlackScholes up one-touch standard errors"
        );
    } catch (...) {
    ...
    check_cuda(cudaFree(device_prices), "cudaFree BlackScholes up one-touch prices");
    check_cuda(
        cudaFree(device_standard_errors),
        "cudaFree BlackScholes up one-touch standard errors"
    );

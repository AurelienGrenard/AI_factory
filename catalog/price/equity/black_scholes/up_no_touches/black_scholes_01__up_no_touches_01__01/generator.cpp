// Build one BlackScholes Up-no-touch price dataset from JSON inputs.
#include "common/check_cuda.cuh"
#include "model/equity/black_scholes/up_no_touch.cuh"
...
const std::filesystem::path dataset_path =
    "datasets/price/equity/black_scholes/up_no_touches/"
    "black_scholes_01__up_no_touches_01__01.json";
const std::filesystem::path catalog_path =
    "catalog/price/equity/black_scholes/up_no_touches/"
    "black_scholes_01__up_no_touches_01__01/dataset.yaml";
const std::string url =
    "https://mlp.lpma.math.upmc.fr/DataCarlo/Assets/BlackScholes/UpNoTouch/"
    "black_scholes_01__up_no_touches_01__01.json";
...
int main() {
    ...
    try {
        ...
        check_cuda(
            ...
            "cudaMalloc BlackScholes up no-touch prices"
        );
        check_cuda(
            cudaMalloc(
            ...
            "cudaMalloc BlackScholes up no-touch standard errors"
        );
        ...
        black_scholes::launch_black_scholes_up_no_touch_cuda(
            device_models,
            warmup_row_count,
            device_products,
            warmup_row_count,
        ...
        check_cuda(cudaDeviceSynchronize(), "BlackScholes up no-touch warmup");
        ...
        check_cuda(
            ...
            "cudaMemcpy BlackScholes up no-touch prices"
        );
        check_cuda(
            cudaMemcpy(
            ...
            "cudaMemcpy BlackScholes up no-touch standard errors"
        );
    } catch (...) {
    ...
    check_cuda(cudaFree(device_prices), "cudaFree BlackScholes up no-touch prices");
    check_cuda(
        cudaFree(device_standard_errors),
        "cudaFree BlackScholes up no-touch standard errors"
    );

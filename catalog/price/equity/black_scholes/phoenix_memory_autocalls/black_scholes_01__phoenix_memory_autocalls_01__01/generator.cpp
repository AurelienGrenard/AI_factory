// Build one BlackScholes Phoenix-Memory-autocall price dataset from JSON inputs.
#include "common/check_cuda.cuh"
#include "model/equity/black_scholes/phoenix_memory_autocall.cuh"
...
const std::filesystem::path model_dataset_path =
    "datasets/model/equity/black_scholes/black_scholes_01.json";
...
constexpr std::size_t monte_carlo_paths_per_price = 65'536U;
...
const std::filesystem::path dataset_path =
    "datasets/price/equity/black_scholes/phoenix_memory_autocalls/"
    "black_scholes_01__phoenix_memory_autocalls_01__01.json";
const std::filesystem::path catalog_path =
    "catalog/price/equity/black_scholes/phoenix_memory_autocalls/"
    "black_scholes_01__phoenix_memory_autocalls_01__01/dataset.yaml";
const std::string url =
    "https://mlp.lpma.math.upmc.fr/DataCarlo/Assets/BlackScholes/PhoenixMemoryAutocall/"
    "black_scholes_01__phoenix_memory_autocalls_01__01.json";
const std::string numerical_method = "Exact Gaussian log-price transitions";
...
int main() {
    ...
    const std::vector<black_scholes::BlackScholesModelParameters> models =
        black_scholes::load_models(model_dataset_path);
    ...
    black_scholes::BlackScholesModelParameters* device_models = nullptr;
    ...
    try {
        ...
        check_cuda(
            cudaMalloc(
                &device_models,
                models.size() * sizeof(black_scholes::BlackScholesModelParameters)
            ),
            "cudaMalloc BlackScholes models"
        );
        check_cuda(
            cudaMalloc(
                &device_products,
        ...
        check_cuda(
            ...
            "cudaMalloc BlackScholes Phoenix Memory autocall prices"
        );
        check_cuda(
            cudaMalloc(
            ...
            "cudaMalloc BlackScholes Phoenix Memory autocall standard errors"
        );
        ...
        check_cuda(
            ...
            "cudaMemcpy BlackScholes models"
        );
        check_cuda(
            cudaMemcpy(
                device_products,
        ...
        black_scholes::launch_black_scholes_phoenix_memory_autocall_cuda(
            device_models,
            warmup_row_count,
            device_products,
            warmup_row_count,
        ...
        check_cuda(cudaDeviceSynchronize(), "BlackScholes Phoenix Memory autocall warmup");
        ...
        check_cuda(
            ...
            "cudaMemcpy BlackScholes Phoenix Memory autocall prices"
        );
        check_cuda(
            cudaMemcpy(
            ...
            "cudaMemcpy BlackScholes Phoenix Memory autocall standard errors"
        );
    } catch (...) {
    ...
    check_cuda(cudaFree(device_models), "cudaFree BlackScholes models");
    check_cuda(cudaFree(device_products), "cudaFree Phoenix Memory autocalls");
    check_cuda(cudaFree(device_prices), "cudaFree BlackScholes Phoenix Memory autocall prices");
    check_cuda(
        cudaFree(device_standard_errors),
        "cudaFree BlackScholes Phoenix Memory autocall standard errors"
    );

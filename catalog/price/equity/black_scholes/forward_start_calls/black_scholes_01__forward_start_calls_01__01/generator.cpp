// Build one Black-Scholes forward_start_calls price dataset.
...
const std::filesystem::path dataset_path =
    "datasets/price/equity/black_scholes/forward_start_calls/"
    "black_scholes_01__forward_start_calls_01__01.json";
const std::filesystem::path catalog_path =
    "catalog/price/equity/black_scholes/forward_start_calls/"
    "black_scholes_01__forward_start_calls_01__01/dataset.yaml";
const std::string url =
    "https://datasets.ai-factory.example/v1/price/"
    "equity/black_scholes/forward_start_calls/"
    "black_scholes_01__forward_start_calls_01__01.json";
const std::string numerical_method =
    "Black-Scholes closed-form forward-start call";
...
int main() {
    ...
    try {
        ...
        check_cuda(
            ...
            "cudaMalloc forward_start_calls"
        );
        check_cuda(
            cudaMalloc(&device_prices, result_count * sizeof(float)),
            "cudaMalloc Black-Scholes forward_start_calls prices"
        );
        check_cuda(
            cudaMemcpy(
                device_models,
        ...
        check_cuda(
            ...
            "cudaMemcpy forward_start_calls"
        );
        ...
        bs::launch_black_scholes_forward_start_option_cuda<OptionSide::call>(
            device_models,
            warmup_count,
            device_products,
            warmup_count,
        ...
        check_cuda(cudaDeviceSynchronize(), "Black-Scholes forward_start_calls warmup");
        ...
        check_cuda(
            ...
            "cudaMemcpy Black-Scholes forward_start_calls prices"
        );
    } catch (...) {
    ...
    check_cuda(cudaFree(device_products), "cudaFree forward_start_calls");
    check_cuda(cudaFree(device_prices), "cudaFree Black-Scholes forward_start_calls prices");

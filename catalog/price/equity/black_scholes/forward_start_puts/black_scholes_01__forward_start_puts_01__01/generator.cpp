// Build one Black-Scholes forward_start_puts price dataset.
...
const std::filesystem::path dataset_path =
    "datasets/price/equity/black_scholes/forward_start_puts/"
    "black_scholes_01__forward_start_puts_01__01.json";
const std::filesystem::path catalog_path =
    "catalog/price/equity/black_scholes/forward_start_puts/"
    "black_scholes_01__forward_start_puts_01__01/dataset.yaml";
const std::string url =
    "https://datasets.ai-factory.example/v1/price/"
    "equity/black_scholes/forward_start_puts/"
    "black_scholes_01__forward_start_puts_01__01.json";
const std::string numerical_method =
    "Black-Scholes closed-form forward-start put";
...
int main() {
    ...
    try {
        ...
        check_cuda(
            ...
            "cudaMalloc forward_start_puts"
        );
        check_cuda(
            cudaMalloc(&device_prices, result_count * sizeof(float)),
            "cudaMalloc Black-Scholes forward_start_puts prices"
        );
        check_cuda(
            cudaMemcpy(
                device_models,
        ...
        bs::launch_black_scholes_forward_start_option_cuda<OptionSide::put>(
            device_models,
            warmup_count,
            device_products,
            warmup_count,
        ...
        check_cuda(cudaDeviceSynchronize(), "Black-Scholes forward_start_puts warmup");
        ...
        check_cuda(
            ...
            "cudaMemcpy Black-Scholes forward_start_puts prices"
        );
    } catch (...) {
    ...
    check_cuda(cudaFree(device_products), "cudaFree forward_start_puts");
    check_cuda(cudaFree(device_prices), "cudaFree Black-Scholes forward_start_puts prices");

// Build one Black-Scholes asset_or_nothing_calls price dataset.
...
const std::filesystem::path dataset_path =
    "datasets/price/equity/black_scholes/asset_or_nothing_calls/"
    "black_scholes_01__asset_or_nothing_calls_01__01.json";
const std::filesystem::path catalog_path =
    "catalog/price/equity/black_scholes/asset_or_nothing_calls/"
    "black_scholes_01__asset_or_nothing_calls_01__01/dataset.yaml";
const std::string url =
    "https://datasets.ai-factory.example/v1/price/"
    "equity/black_scholes/asset_or_nothing_calls/"
    "black_scholes_01__asset_or_nothing_calls_01__01.json";
const std::string numerical_method =
    "Black-Scholes closed-form asset-or-nothing call";
...
int main() {
    ...
    try {
        ...
        check_cuda(
            ...
            "cudaMalloc asset_or_nothing_calls"
        );
        check_cuda(
            cudaMalloc(&device_prices, result_count * sizeof(float)),
            "cudaMalloc Black-Scholes asset_or_nothing_calls prices"
        );
        check_cuda(
            cudaMemcpy(
                device_models,
        ...
        check_cuda(
            ...
            "cudaMemcpy asset_or_nothing_calls"
        );
        ...
        bs::launch_black_scholes_asset_or_nothing_option_cuda<OptionSide::call>(
            device_models,
            warmup_count,
            device_products,
            warmup_count,
        ...
        check_cuda(cudaDeviceSynchronize(), "Black-Scholes asset_or_nothing_calls warmup");
        ...
        check_cuda(
            ...
            "cudaMemcpy Black-Scholes asset_or_nothing_calls prices"
        );
    } catch (...) {
    ...
    check_cuda(cudaFree(device_products), "cudaFree asset_or_nothing_calls");
    check_cuda(cudaFree(device_prices), "cudaFree Black-Scholes asset_or_nothing_calls prices");

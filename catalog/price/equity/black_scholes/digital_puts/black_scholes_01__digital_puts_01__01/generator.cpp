...
int main() {
    ...
    try {
        ...
        check_cuda(
            ...
            "cudaMalloc digital_puts"
        );
        check_cuda(
            cudaMalloc(&device_prices, result_count * sizeof(float)),
            "cudaMalloc Black-Scholes digital_puts prices"
        );
        check_cuda(
            cudaMemcpy(
                device_models,
        ...
        check_cuda(cudaDeviceSynchronize(), "Black-Scholes digital_puts warmup");
        ...
        check_cuda(
            ...
            "cudaMemcpy Black-Scholes digital_puts prices"
        );
    } catch (...) {
    ...
    check_cuda(cudaFree(device_products), "cudaFree digital_puts");
    check_cuda(cudaFree(device_prices), "cudaFree Black-Scholes digital_puts prices");

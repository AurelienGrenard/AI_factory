// Build one Merton fixed-strike lookback price dataset from JSON inputs.
#include "common/check_cuda.cuh"
#include "model/equity/merton/lookback_option.cuh"
...
const std::filesystem::path dataset_path =
    "datasets/price/equity/merton/lookback_options/"
    "merton_01__lookback_options_01__01.json";
const std::filesystem::path catalog_path =
    "catalog/price/equity/merton/lookback_options/"
    "merton_01__lookback_options_01__01/dataset.yaml";
const std::string url =
    "https://mlp.lpma.math.upmc.fr/DataCarlo/Assets/Merton/LookbackOption/"
    "merton_01__lookback_options_01__01.json";
...
int main() {
    ...
    try {
        ...
        check_cuda(
            ...
            "cudaMalloc Merton lookback prices"
        );
        check_cuda(
            cudaMalloc(
            ...
            "cudaMalloc Merton lookback standard errors"
        );
        ...
        merton::launch_merton_lookback_option_cuda(
            device_models,
            warmup_row_count,
            device_products,
            warmup_row_count,
        ...
        check_cuda(cudaDeviceSynchronize(), "Merton lookback warmup");
        ...
        check_cuda(
            ...
            "cudaMemcpy Merton lookback prices"
        );
        check_cuda(
            cudaMemcpy(
            ...
            "cudaMemcpy Merton lookback standard errors"
        );
    } catch (...) {
    ...
    check_cuda(cudaFree(device_prices), "cudaFree Merton lookback prices");
    check_cuda(
        cudaFree(device_standard_errors),
        "cudaFree Merton lookback standard errors"
    );

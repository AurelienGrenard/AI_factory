// Build one Merton Up-one-touch price dataset from JSON inputs.
...
const std::filesystem::path dataset_path =
    "datasets/price/equity/merton/up_one_touches/"
    "merton_01__up_one_touches_01__01.json";
const std::filesystem::path catalog_path =
    "catalog/price/equity/merton/up_one_touches/"
    "merton_01__up_one_touches_01__01/dataset.yaml";
const std::string url =
    "https://mlp.lpma.math.upmc.fr/DataCarlo/Assets/Merton/UpOneTouch/"
    "merton_01__up_one_touches_01__01.json";
...
int main() {
    ...
    try {
        ...
        check_cuda(
            ...
            "cudaMalloc Merton up one-touch prices"
        );
        check_cuda(
            cudaMalloc(
            ...
            "cudaMalloc Merton up one-touch standard errors"
        );
        ...
        merton::launch_merton_up_one_touch_cuda(
            device_models,
            warmup_row_count,
            device_products,
            warmup_row_count,
        ...
        check_cuda(cudaDeviceSynchronize(), "Merton up one-touch warmup");
        ...
        check_cuda(
            ...
            "cudaMemcpy Merton up one-touch prices"
        );
        check_cuda(
            cudaMemcpy(
            ...
            "cudaMemcpy Merton up one-touch standard errors"
        );
    } catch (...) {
    ...
    check_cuda(cudaFree(device_prices), "cudaFree Merton up one-touch prices");
    check_cuda(
        cudaFree(device_standard_errors),
        "cudaFree Merton up one-touch standard errors"
    );

// Build one Merton gap-call price dataset from JSON inputs.
...
const std::filesystem::path dataset_path =
    "datasets/price/equity/merton/gap_calls/"
    "merton_01__gap_calls_01__01.json";
const std::filesystem::path catalog_path =
    "catalog/price/equity/merton/gap_calls/"
    "merton_01__gap_calls_01__01/dataset.yaml";
const std::string url =
    "https://datasets.ai-factory.example/v1/price/equity/merton/gap_calls/"
    "merton_01__gap_calls_01__01.json";
...
int main() {
    ...
    try {
        ...
        check_cuda(
            ...
            "cudaMalloc Merton gap call prices"
        );
        check_cuda(
            cudaMalloc(
            ...
            "cudaMalloc Merton gap call standard errors"
        );
        ...
        merton::launch_merton_gap_option_cuda<OptionSide::call>(
            device_models,
            warmup_row_count,
            device_products,
            warmup_row_count,
        ...
        check_cuda(cudaDeviceSynchronize(), "Merton gap call warmup");
        ...
        check_cuda(
            ...
            "cudaMemcpy Merton gap call prices"
        );
        check_cuda(
            cudaMemcpy(
            ...
            "cudaMemcpy Merton gap call standard errors"
        );
    } catch (...) {
    ...
    check_cuda(cudaFree(device_prices), "cudaFree Merton gap call prices");
    check_cuda(
        cudaFree(device_standard_errors),
        "cudaFree Merton gap call standard errors"
    );

// Build one VarianceGamma Up-no-touch price dataset from JSON inputs.
#include "common/check_cuda.cuh"
#include "model/equity/variance_gamma/up_no_touch.cuh"
...
const std::filesystem::path dataset_path =
    "datasets/price/equity/variance_gamma/up_no_touches/"
    "variance_gamma_01__up_no_touches_01__01.json";
const std::filesystem::path catalog_path =
    "catalog/price/equity/variance_gamma/up_no_touches/"
    "variance_gamma_01__up_no_touches_01__01/dataset.yaml";
const std::string url =
    "https://mlp.lpma.math.upmc.fr/DataCarlo/Assets/VarianceGamma/UpNoTouch/"
    "variance_gamma_01__up_no_touches_01__01.json";
...
int main() {
    ...
    try {
        ...
        check_cuda(
            ...
            "cudaMalloc VarianceGamma up no-touch prices"
        );
        check_cuda(
            cudaMalloc(
            ...
            "cudaMalloc VarianceGamma up no-touch standard errors"
        );
        ...
        variance_gamma::launch_variance_gamma_up_no_touch_cuda(
            device_models,
            warmup_row_count,
            device_products,
            warmup_row_count,
        ...
        check_cuda(cudaDeviceSynchronize(), "VarianceGamma up no-touch warmup");
        ...
        check_cuda(
            ...
            "cudaMemcpy VarianceGamma up no-touch prices"
        );
        check_cuda(
            cudaMemcpy(
            ...
            "cudaMemcpy VarianceGamma up no-touch standard errors"
        );
    } catch (...) {
    ...
    check_cuda(cudaFree(device_prices), "cudaFree VarianceGamma up no-touch prices");
    check_cuda(
        cudaFree(device_standard_errors),
        "cudaFree VarianceGamma up no-touch standard errors"
    );

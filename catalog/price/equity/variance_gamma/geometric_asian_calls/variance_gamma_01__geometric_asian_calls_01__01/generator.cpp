// Build one VarianceGamma Geometric-Asian-call price dataset from JSON inputs.
#include "common/check_cuda.cuh"
#include "model/equity/variance_gamma/geometric_asian_option.cuh"
...
const std::filesystem::path dataset_path =
    "datasets/price/equity/variance_gamma/geometric_asian_calls/"
    "variance_gamma_01__geometric_asian_calls_01__01.json";
const std::filesystem::path catalog_path =
    "catalog/price/equity/variance_gamma/geometric_asian_calls/"
    "variance_gamma_01__geometric_asian_calls_01__01/dataset.yaml";
const std::string url =
    "https://mlp.lpma.math.upmc.fr/DataCarlo/Assets/VarianceGamma/GeometricAsianCall/"
    "variance_gamma_01__geometric_asian_calls_01__01.json";
...
int main() {
    ...
    const std::vector<variance_gamma::VarianceGammaModelParameters> models =
    ...
    variance_gamma::VarianceGammaModelParameters* device_models = nullptr;
    ...
    try {
        ...
        check_cuda(
            cudaMalloc(
                &device_models,
                models.size() * sizeof(variance_gamma::VarianceGammaModelParameters)
            ),
            "cudaMalloc VarianceGamma models"
        );
        check_cuda(
            cudaMalloc(
                &device_products,
        ...
        check_cuda(
            ...
            "cudaMalloc VarianceGamma Geometric Asian call prices"
        );
        check_cuda(
            cudaMalloc(
            ...
            "cudaMalloc VarianceGamma Geometric Asian call standard errors"
        );
        ...
        check_cuda(
            ...
            "cudaMemcpy VarianceGamma models"
        );
        check_cuda(
            cudaMemcpy(
                device_products,
        ...
        variance_gamma::launch_variance_gamma_geometric_asian_option_cuda<OptionSide::call>(
            device_models,
            warmup_row_count,
            device_products,
            warmup_row_count,
        ...
        check_cuda(cudaDeviceSynchronize(), "VarianceGamma Geometric Asian call warmup");
        ...
        check_cuda(
            ...
            "cudaMemcpy VarianceGamma Geometric Asian call prices"
        );
        check_cuda(
            cudaMemcpy(
            ...
            "cudaMemcpy VarianceGamma Geometric Asian call standard errors"
        );
    } catch (...) {
    ...
    check_cuda(cudaFree(device_models), "cudaFree VarianceGamma models");
    check_cuda(cudaFree(device_products), "cudaFree Geometric Asian calls");
    check_cuda(cudaFree(device_prices), "cudaFree VarianceGamma Geometric Asian call prices");
    check_cuda(
        cudaFree(device_standard_errors),
        "cudaFree VarianceGamma Geometric Asian call standard errors"
    );

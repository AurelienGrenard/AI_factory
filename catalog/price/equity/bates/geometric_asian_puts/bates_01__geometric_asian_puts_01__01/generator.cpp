// Build one Bates Geometric-Asian-put price dataset from JSON inputs.
...
const std::filesystem::path model_dataset_path =
    "datasets/model/equity/bates/bates_01.json";
...
const std::filesystem::path dataset_path =
    "datasets/price/equity/bates/geometric_asian_puts/"
    "bates_01__geometric_asian_puts_01__01.json";
const std::filesystem::path catalog_path =
    "catalog/price/equity/bates/geometric_asian_puts/"
    "bates_01__geometric_asian_puts_01__01/dataset.yaml";
const std::string url =
    "https://datasets.ai-factory.example/v1/price/equity/bates/geometric_asian_puts/"
    "bates_01__geometric_asian_puts_01__01.json";
...
int main() {
    ...
    const std::vector<bates::BatesModelParameters> models =
    ...
    bates::BatesModelParameters* device_models = nullptr;
    ...
    try {
        ...
        check_cuda(
            cudaMalloc(
                &device_models,
                models.size() * sizeof(bates::BatesModelParameters)
            ),
            "cudaMalloc Bates models"
        );
        check_cuda(
            cudaMalloc(
                &device_products,
        ...
        check_cuda(
            ...
            "cudaMalloc Bates Geometric Asian put prices"
        );
        check_cuda(
            cudaMalloc(
            ...
            "cudaMalloc Bates Geometric Asian put standard errors"
        );
        ...
        check_cuda(cudaDeviceSynchronize(), "Bates Geometric Asian put warmup");
        ...
        check_cuda(
            ...
            "cudaMemcpy Bates Geometric Asian put prices"
        );
        check_cuda(
            cudaMemcpy(
            ...
            "cudaMemcpy Bates Geometric Asian put standard errors"
        );
    } catch (...) {
    ...
    check_cuda(cudaFree(device_models), "cudaFree Bates models");
    check_cuda(cudaFree(device_products), "cudaFree Geometric Asian puts");
    check_cuda(cudaFree(device_prices), "cudaFree Bates Geometric Asian put prices");
    check_cuda(
        cudaFree(device_standard_errors),
        "cudaFree Bates Geometric Asian put standard errors"
    );

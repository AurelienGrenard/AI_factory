// Build one Bates Athena-autocall price dataset from JSON inputs.
...
const std::filesystem::path dataset_path =
    "datasets/price/equity/bates/athena_autocalls/"
    "bates_01__athena_autocalls_01__01.json";
const std::filesystem::path catalog_path =
    "catalog/price/equity/bates/athena_autocalls/"
    "bates_01__athena_autocalls_01__01/dataset.yaml";
const std::string url =
    "https://mlp.lpma.math.upmc.fr/DataCarlo/Assets/Bates/AthenaAutocall/"
    "bates_01__athena_autocalls_01__01.json";
...
int main() {
    ...
    try {
        ...
        check_cuda(
            ...
            "cudaMalloc Bates Athena autocall prices"
        );
        check_cuda(
            cudaMalloc(
            ...
            "cudaMalloc Bates Athena autocall standard errors"
        );
        ...
        check_cuda(cudaDeviceSynchronize(), "Bates Athena autocall warmup");
        ...
        check_cuda(
            ...
            "cudaMemcpy Bates Athena autocall prices"
        );
        check_cuda(
            cudaMemcpy(
            ...
            "cudaMemcpy Bates Athena autocall standard errors"
        );
    } catch (...) {
    ...
    check_cuda(cudaFree(device_prices), "cudaFree Bates Athena autocall prices");
    check_cuda(
        cudaFree(device_standard_errors),
        "cudaFree Bates Athena autocall standard errors"
    );

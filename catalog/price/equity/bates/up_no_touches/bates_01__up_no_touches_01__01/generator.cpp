...
const std::filesystem::path dataset_path =
    "datasets/price/equity/bates/up_no_touches/"
    "bates_01__up_no_touches_01__01.json";
const std::filesystem::path catalog_path =
    "catalog/price/equity/bates/up_no_touches/"
    "bates_01__up_no_touches_01__01/dataset.yaml";
const std::string url =
    "https://mlp.lpma.math.upmc.fr/DataCarlo/Assets/Bates/UpNoTouch/"
    "bates_01__up_no_touches_01__01.json";
...
int main() {
    ...
    try {
        ...
        check_cuda(
            ...
            "cudaMalloc Bates up no-touch prices"
        );
        check_cuda(
            cudaMalloc(
            ...
            "cudaMalloc Bates up no-touch standard errors"
        );
        ...
        check_cuda(cudaDeviceSynchronize(), "Bates up no-touch warmup");
        ...
        check_cuda(
            ...
            "cudaMemcpy Bates up no-touch prices"
        );
        check_cuda(
            cudaMemcpy(
            ...
            "cudaMemcpy Bates up no-touch standard errors"
        );
    } catch (...) {
    ...
    check_cuda(cudaFree(device_prices), "cudaFree Bates up no-touch prices");
    check_cuda(
        cudaFree(device_standard_errors),
        "cudaFree Bates up no-touch standard errors"
    );

// Build one Bates American-call price dataset from JSON inputs.
...
const std::filesystem::path dataset_path =
    "datasets/price/equity/bates/american_calls/"
    "bates_01__american_calls_01__01.json";
const std::filesystem::path catalog_path =
    "catalog/price/equity/bates/american_calls/"
    "bates_01__american_calls_01__01/dataset.yaml";
const std::string url =
    "https://datasets.ai-factory.example/v1/price/equity/bates/american_calls/"
    "bates_01__american_calls_01__01.json";
const std::string numerical_method = "Andersen QE-M with compound-Poisson lognormal jumps + Longstaff-Schwartz";
...
int main() {
    ...
    try {
        ...
        check_cuda(
            ...
            "cudaMalloc Bates American call prices"
        );
        check_cuda(
            cudaMalloc(
            ...
            "cudaMalloc Bates American call standard errors"
        );
        ...
        bates::launch_bates_american_option_cuda<OptionSide::call>(
            device_models,
            1U,
            products.data(),
            device_products,
        ...
        check_cuda(cudaDeviceSynchronize(), "Bates American-call warmup");

        // Plan the reusable workspace and time every production batch.
        wall_start = std::chrono::steady_clock::now();
        execution = bates::launch_bates_american_option_cuda<OptionSide::call>(
            device_models,
            models.size(),
            products.data(),
            device_products,
        ...
        check_cuda(
            ...
            "cudaMemcpy Bates American call prices"
        );
        check_cuda(
            cudaMemcpy(
            ...
            "cudaMemcpy Bates American call standard errors"
        );
    ...
    check_cuda(cudaFree(device_prices), "cudaFree Bates American call prices");
    check_cuda(
        cudaFree(device_standard_errors),
        "cudaFree Bates American call standard errors"
    );

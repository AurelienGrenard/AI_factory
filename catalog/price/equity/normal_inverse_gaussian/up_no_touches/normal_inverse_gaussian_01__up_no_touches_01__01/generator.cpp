// Build one NormalInverseGaussian Up-no-touch price dataset from JSON inputs.
#include "common/check_cuda.cuh"
#include "model/equity/normal_inverse_gaussian/up_no_touch.cuh"
...
const std::filesystem::path dataset_path =
    "datasets/price/equity/normal_inverse_gaussian/up_no_touches/"
    "normal_inverse_gaussian_01__up_no_touches_01__01.json";
const std::filesystem::path catalog_path =
    "catalog/price/equity/normal_inverse_gaussian/up_no_touches/"
    "normal_inverse_gaussian_01__up_no_touches_01__01/dataset.yaml";
const std::string url =
    "https://mlp.lpma.math.upmc.fr/DataCarlo/Assets/NormalInverseGaussian/UpNoTouch/"
    "normal_inverse_gaussian_01__up_no_touches_01__01.json";
...
int main() {
    ...
    const std::vector<product::UpNoTouchParameters> products =
        product::load_up_no_touches(product_dataset_path);
    ...
    product::UpNoTouchParameters* device_products = nullptr;
    ...
    try {
        ...
        check_cuda(
            cudaMalloc(
                &device_products,
                products.size() * sizeof(product::UpNoTouchParameters)
            ),
            "cudaMalloc Up no-touchs"
        );
        check_cuda(
            cudaMalloc(
                &device_prices,
            ...
            "cudaMalloc NormalInverseGaussian up no-touch prices"
        );
        check_cuda(
            cudaMalloc(
            ...
            "cudaMalloc NormalInverseGaussian up no-touch standard errors"
        );
        ...
        normal_inverse_gaussian::launch_normal_inverse_gaussian_up_no_touch_cuda(
            device_models,
            warmup_row_count,
            device_products,
            warmup_row_count,
        ...
        check_cuda(cudaDeviceSynchronize(), "NormalInverseGaussian up no-touch warmup");
        ...
        check_cuda(
            ...
            "cudaMemcpy NormalInverseGaussian up no-touch prices"
        );
        check_cuda(
            cudaMemcpy(
            ...
            "cudaMemcpy NormalInverseGaussian up no-touch standard errors"
        );
    } catch (...) {
    ...
    check_cuda(cudaFree(device_products), "cudaFree Up no-touchs");
    check_cuda(cudaFree(device_prices), "cudaFree NormalInverseGaussian up no-touch prices");
    check_cuda(
        cudaFree(device_standard_errors),
        "cudaFree NormalInverseGaussian up no-touch standard errors"
    );

// Build one NormalInverseGaussian Athena-autocall price dataset from JSON inputs.
...
const std::filesystem::path dataset_path =
    "datasets/price/equity/normal_inverse_gaussian/athena_autocalls/"
    "normal_inverse_gaussian_01__athena_autocalls_01__01.json";
const std::filesystem::path catalog_path =
    "catalog/price/equity/normal_inverse_gaussian/athena_autocalls/"
    "normal_inverse_gaussian_01__athena_autocalls_01__01/dataset.yaml";
const std::string url =
    "https://mlp.lpma.math.upmc.fr/DataCarlo/Assets/NormalInverseGaussian/AthenaAutocall/"
    "normal_inverse_gaussian_01__athena_autocalls_01__01.json";
...
int main() {
    ...
    const std::vector<product::AthenaAutocallParameters> products =
        product::load_athena_autocalls(product_dataset_path);
    ...
    product::AthenaAutocallParameters* device_products = nullptr;
    ...
    try {
        ...
        check_cuda(
            cudaMalloc(
                &device_products,
                products.size() * sizeof(product::AthenaAutocallParameters)
            ),
            "cudaMalloc Athena autocalls"
        );
        check_cuda(
            cudaMalloc(
                &device_prices,
            ...
            "cudaMalloc NormalInverseGaussian Athena autocall prices"
        );
        check_cuda(
            cudaMalloc(
            ...
            "cudaMalloc NormalInverseGaussian Athena autocall standard errors"
        );
        ...
        normal_inverse_gaussian::launch_normal_inverse_gaussian_athena_autocall_cuda(
            device_models,
            warmup_row_count,
            device_products,
            warmup_row_count,
        ...
        check_cuda(cudaDeviceSynchronize(), "NormalInverseGaussian Athena autocall warmup");
        ...
        check_cuda(
            ...
            "cudaMemcpy NormalInverseGaussian Athena autocall prices"
        );
        check_cuda(
            cudaMemcpy(
            ...
            "cudaMemcpy NormalInverseGaussian Athena autocall standard errors"
        );
    } catch (...) {
    ...
    check_cuda(cudaFree(device_products), "cudaFree Athena autocalls");
    check_cuda(cudaFree(device_prices), "cudaFree NormalInverseGaussian Athena autocall prices");
    check_cuda(
        cudaFree(device_standard_errors),
        "cudaFree NormalInverseGaussian Athena autocall standard errors"
    );

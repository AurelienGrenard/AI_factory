// Build one NormalInverseGaussian American-call price dataset from JSON inputs.
#include "common/check_cuda.cuh"
#include "model/equity/normal_inverse_gaussian/american_option.cuh"
...
const std::filesystem::path dataset_path =
    "datasets/price/equity/normal_inverse_gaussian/american_calls/"
    "normal_inverse_gaussian_01__american_calls_01__01.json";
const std::filesystem::path catalog_path =
    "catalog/price/equity/normal_inverse_gaussian/american_calls/"
    "normal_inverse_gaussian_01__american_calls_01__01/dataset.yaml";
const std::string url =
    "https://datasets.ai-factory.example/v1/price/equity/normal_inverse_gaussian/american_calls/"
    "normal_inverse_gaussian_01__american_calls_01__01.json";
...
int main() {
    ...
    try {
        ...
        check_cuda(
            ...
            "cudaMalloc NormalInverseGaussian American call prices"
        );
        check_cuda(
            cudaMalloc(
            ...
            "cudaMalloc NormalInverseGaussian American call standard errors"
        );
        ...
        normal_inverse_gaussian::launch_normal_inverse_gaussian_american_option_cuda<OptionSide::call>(
            device_models,
            1U,
            products.data(),
            device_products,
        ...
        check_cuda(cudaDeviceSynchronize(), "NormalInverseGaussian American-call warmup");

        // Plan the reusable workspace and time every production batch.
        wall_start = std::chrono::steady_clock::now();
        execution = normal_inverse_gaussian::launch_normal_inverse_gaussian_american_option_cuda<OptionSide::call>(
            device_models,
            models.size(),
            products.data(),
            device_products,
        ...
        check_cuda(
            ...
            "cudaMemcpy NormalInverseGaussian American call prices"
        );
        check_cuda(
            cudaMemcpy(
            ...
            "cudaMemcpy NormalInverseGaussian American call standard errors"
        );
    ...
    check_cuda(cudaFree(device_products), "cudaFree American calls");
    check_cuda(cudaFree(device_prices), "cudaFree NormalInverseGaussian American call prices");
    check_cuda(
        cudaFree(device_standard_errors),
        "cudaFree NormalInverseGaussian American call standard errors"
    );

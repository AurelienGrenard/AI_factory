// Build one NormalInverseGaussian Down-and-in-put price dataset from JSON inputs.
#include "common/check_cuda.cuh"
#include "model/equity/normal_inverse_gaussian/down_and_in_option.cuh"
...
const std::filesystem::path model_dataset_path =
    "datasets/model/equity/normal_inverse_gaussian/normal_inverse_gaussian_01.json";
const std::filesystem::path product_dataset_path =
    "datasets/product/equity/down_and_in_options/down_and_in_options_01.json";
...
const std::filesystem::path dataset_path =
    "datasets/price/equity/normal_inverse_gaussian/down_and_in_puts/"
    "normal_inverse_gaussian_01__down_and_in_puts_01__01.json";
const std::filesystem::path catalog_path =
    "catalog/price/equity/normal_inverse_gaussian/down_and_in_puts/"
    "normal_inverse_gaussian_01__down_and_in_puts_01__01/dataset.yaml";
const std::string url =
    "https://datasets.ai-factory.example/v1/price/equity/normal_inverse_gaussian/down_and_in_puts/"
    "normal_inverse_gaussian_01__down_and_in_puts_01__01.json";
const std::string numerical_method = "Exact inverse-Gaussian subordination";
...
int main() {
    ...
    const std::vector<normal_inverse_gaussian::NormalInverseGaussianModelParameters> models =
        normal_inverse_gaussian::load_models(model_dataset_path);
    const std::vector<product::DownAndInOptionParameters> products =
        product::load_down_and_in_options(product_dataset_path);
    ...
    normal_inverse_gaussian::NormalInverseGaussianModelParameters* device_models = nullptr;
    product::DownAndInOptionParameters* device_products = nullptr;
    ...
    try {
        ...
        check_cuda(
            cudaMalloc(
                &device_models,
                models.size() * sizeof(normal_inverse_gaussian::NormalInverseGaussianModelParameters)
            ),
            "cudaMalloc NormalInverseGaussian models"
        );
        check_cuda(
            cudaMalloc(
                &device_products,
                products.size() * sizeof(product::DownAndInOptionParameters)
            ),
            "cudaMalloc Down-and-in puts"
        );
        check_cuda(
            cudaMalloc(
                &device_prices,
            ...
            "cudaMalloc NormalInverseGaussian down-and-in put prices"
        );
        check_cuda(
            cudaMalloc(
            ...
            "cudaMalloc NormalInverseGaussian down-and-in put standard errors"
        );
        ...
        check_cuda(
            ...
            "cudaMemcpy NormalInverseGaussian models"
        );
        check_cuda(
            cudaMemcpy(
                device_products,
        ...
        normal_inverse_gaussian::launch_normal_inverse_gaussian_down_and_in_option_cuda<OptionSide::put>(
            device_models,
            warmup_row_count,
            device_products,
            warmup_row_count,
        ...
        check_cuda(cudaDeviceSynchronize(), "NormalInverseGaussian down-and-in put warmup");
        ...
        check_cuda(
            ...
            "cudaMemcpy NormalInverseGaussian down-and-in put prices"
        );
        check_cuda(
            cudaMemcpy(
            ...
            "cudaMemcpy NormalInverseGaussian down-and-in put standard errors"
        );
    } catch (...) {
    ...
    check_cuda(cudaFree(device_models), "cudaFree NormalInverseGaussian models");
    check_cuda(cudaFree(device_products), "cudaFree Down-and-in puts");
    check_cuda(cudaFree(device_prices), "cudaFree NormalInverseGaussian down-and-in put prices");
    check_cuda(
        cudaFree(device_standard_errors),
        "cudaFree NormalInverseGaussian down-and-in put standard errors"
    );

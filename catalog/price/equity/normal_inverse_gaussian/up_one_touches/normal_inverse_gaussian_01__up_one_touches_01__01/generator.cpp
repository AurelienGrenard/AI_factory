// Build one NormalInverseGaussian Up-one-touch price dataset from JSON inputs.
#include "common/check_cuda.cuh"
#include "model/equity/normal_inverse_gaussian/up_one_touch.cuh"
...
const std::filesystem::path product_dataset_path =
    "datasets/product/equity/up_one_touches/up_one_touches_01.json";
...
const std::filesystem::path dataset_path =
    "datasets/price/equity/normal_inverse_gaussian/up_one_touches/"
    "normal_inverse_gaussian_01__up_one_touches_01__01.json";
const std::filesystem::path catalog_path =
    "catalog/price/equity/normal_inverse_gaussian/up_one_touches/"
    "normal_inverse_gaussian_01__up_one_touches_01__01/dataset.yaml";
const std::string url =
    "https://mlp.lpma.math.upmc.fr/DataCarlo/Assets/NormalInverseGaussian/UpOneTouch/"
    "normal_inverse_gaussian_01__up_one_touches_01__01.json";
...
int main() {
    ...
    const std::vector<product::UpOneTouchParameters> products =
        product::load_up_one_touches(product_dataset_path);
    ...
    product::UpOneTouchParameters* device_products = nullptr;
    ...
    try {
        ...
        check_cuda(
            cudaMalloc(
                &device_products,
                products.size() * sizeof(product::UpOneTouchParameters)
            ),
            "cudaMalloc Up one-touchs"
        );
        check_cuda(
            cudaMalloc(
                &device_prices,
            ...
            "cudaMalloc NormalInverseGaussian up one-touch prices"
        );
        check_cuda(
            cudaMalloc(
            ...
            "cudaMalloc NormalInverseGaussian up one-touch standard errors"
        );
        ...
        normal_inverse_gaussian::launch_normal_inverse_gaussian_up_one_touch_cuda(
            device_models,
            warmup_row_count,
            device_products,
            warmup_row_count,
        ...
        check_cuda(cudaDeviceSynchronize(), "NormalInverseGaussian up one-touch warmup");
        ...
        check_cuda(
            ...
            "cudaMemcpy NormalInverseGaussian up one-touch prices"
        );
        check_cuda(
            cudaMemcpy(
            ...
            "cudaMemcpy NormalInverseGaussian up one-touch standard errors"
        );
    } catch (...) {
    ...
    check_cuda(cudaFree(device_products), "cudaFree Up one-touchs");
    check_cuda(cudaFree(device_prices), "cudaFree NormalInverseGaussian up one-touch prices");
    check_cuda(
        cudaFree(device_standard_errors),
        "cudaFree NormalInverseGaussian up one-touch standard errors"
    );

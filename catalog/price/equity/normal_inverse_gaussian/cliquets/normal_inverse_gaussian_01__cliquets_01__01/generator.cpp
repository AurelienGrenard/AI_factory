// Build one NormalInverseGaussian Cliquet price dataset from JSON inputs.
#include "common/check_cuda.cuh"
#include "model/equity/normal_inverse_gaussian/cliquet.cuh"
...
const std::filesystem::path dataset_path =
    "datasets/price/equity/normal_inverse_gaussian/cliquets/"
    "normal_inverse_gaussian_01__cliquets_01__01.json";
const std::filesystem::path catalog_path =
    "catalog/price/equity/normal_inverse_gaussian/cliquets/"
    "normal_inverse_gaussian_01__cliquets_01__01/dataset.yaml";
const std::string url =
    "https://mlp.lpma.math.upmc.fr/DataCarlo/Assets/NormalInverseGaussian/Cliquet/"
    "normal_inverse_gaussian_01__cliquets_01__01.json";
...
int main() {
    ...
    const std::vector<product::CliquetParameters> products =
        product::load_cliquets(product_dataset_path);
    ...
    product::CliquetParameters* device_products = nullptr;
    ...
    try {
        ...
        check_cuda(
            cudaMalloc(
                &device_products,
                products.size() * sizeof(product::CliquetParameters)
            ),
            "cudaMalloc Cliquets"
        );
        check_cuda(
            ...
            "cudaMalloc NormalInverseGaussian Cliquet prices"
        );
        check_cuda(
            cudaMalloc(
            ...
            "cudaMalloc NormalInverseGaussian Cliquet standard errors"
        );
        ...
        normal_inverse_gaussian::launch_normal_inverse_gaussian_cliquet_cuda(
            device_models,
            warmup_row_count,
            device_products,
            warmup_row_count,
        ...
        check_cuda(cudaDeviceSynchronize(), "NormalInverseGaussian Cliquet warmup");
        ...
        check_cuda(
            ...
            "cudaMemcpy NormalInverseGaussian Cliquet prices"
        );
        check_cuda(
            cudaMemcpy(
            ...
            "cudaMemcpy NormalInverseGaussian Cliquet standard errors"
        );
    } catch (...) {
    ...
    check_cuda(cudaFree(device_products), "cudaFree Cliquets");
    check_cuda(cudaFree(device_prices), "cudaFree NormalInverseGaussian Cliquet prices");
    check_cuda(
        cudaFree(device_standard_errors),
        "cudaFree NormalInverseGaussian Cliquet standard errors"
    );

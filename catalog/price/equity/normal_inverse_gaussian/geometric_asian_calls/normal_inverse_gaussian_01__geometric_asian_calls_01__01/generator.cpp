// Build one NormalInverseGaussian Geometric-Asian-call price dataset from JSON inputs.
...
const std::filesystem::path dataset_path =
    "datasets/price/equity/normal_inverse_gaussian/geometric_asian_calls/"
    "normal_inverse_gaussian_01__geometric_asian_calls_01__01.json";
const std::filesystem::path catalog_path =
    "catalog/price/equity/normal_inverse_gaussian/geometric_asian_calls/"
    "normal_inverse_gaussian_01__geometric_asian_calls_01__01/dataset.yaml";
const std::string url =
    "https://mlp.lpma.math.upmc.fr/DataCarlo/Assets/NormalInverseGaussian/GeometricAsianCall/"
    "normal_inverse_gaussian_01__geometric_asian_calls_01__01.json";
...
int main() {
    ...
    const std::vector<product::GeometricAsianOptionParameters> products =
        product::load_geometric_asian_options(product_dataset_path);
    ...
    product::GeometricAsianOptionParameters* device_products = nullptr;
    ...
    try {
        ...
        check_cuda(
            cudaMalloc(
                &device_products,
                products.size() * sizeof(product::GeometricAsianOptionParameters)
            ),
            "cudaMalloc Geometric Asian calls"
        );
        check_cuda(
            cudaMalloc(
                &device_prices,
            ...
            "cudaMalloc NormalInverseGaussian Geometric Asian call prices"
        );
        check_cuda(
            cudaMalloc(
            ...
            "cudaMalloc NormalInverseGaussian Geometric Asian call standard errors"
        );
        ...
        check_cuda(
            ...
            "cudaMemcpy Geometric Asian calls"
        );
        ...
        normal_inverse_gaussian::launch_normal_inverse_gaussian_geometric_asian_option_cuda<OptionSide::call>(
            device_models,
            warmup_row_count,
            device_products,
            warmup_row_count,
        ...
        check_cuda(cudaDeviceSynchronize(), "NormalInverseGaussian Geometric Asian call warmup");
        ...
        check_cuda(
            ...
            "cudaMemcpy NormalInverseGaussian Geometric Asian call prices"
        );
        check_cuda(
            cudaMemcpy(
            ...
            "cudaMemcpy NormalInverseGaussian Geometric Asian call standard errors"
        );
    } catch (...) {
    ...
    check_cuda(cudaFree(device_products), "cudaFree Geometric Asian calls");
    check_cuda(cudaFree(device_prices), "cudaFree NormalInverseGaussian Geometric Asian call prices");
    check_cuda(
        cudaFree(device_standard_errors),
        "cudaFree NormalInverseGaussian Geometric Asian call standard errors"
    );

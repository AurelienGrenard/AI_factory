// Build one NormalInverseGaussian Asian-call price dataset from JSON inputs.
#include "common/check_cuda.cuh"
#include "model/equity/normal_inverse_gaussian/asian_option.cuh"
...
const std::filesystem::path dataset_path =
    "datasets/price/equity/normal_inverse_gaussian/asian_calls/"
    "normal_inverse_gaussian_01__asian_calls_01__01.json";
const std::filesystem::path catalog_path =
    "catalog/price/equity/normal_inverse_gaussian/asian_calls/"
    "normal_inverse_gaussian_01__asian_calls_01__01/dataset.yaml";
const std::string url =
    "https://mlp.lpma.math.upmc.fr/DataCarlo/Assets/NormalInverseGaussian/AsianCall/"
    "normal_inverse_gaussian_01__asian_calls_01__01.json";
...
int main() {
    ...
    const std::vector<product::AsianOptionParameters> products =
        product::load_asian_options(product_dataset_path);
    ...
    product::AsianOptionParameters* device_products = nullptr;
    ...
    try {
        ...
        check_cuda(
            cudaMalloc(
                &device_products,
                products.size() * sizeof(product::AsianOptionParameters)
            ),
            "cudaMalloc Asian calls"
        );
        check_cuda(
            cudaMalloc(
                &device_prices,
            ...
            "cudaMalloc NormalInverseGaussian Asian call prices"
        );
        check_cuda(
            cudaMalloc(
            ...
            "cudaMalloc NormalInverseGaussian Asian call standard errors"
        );
        ...
        normal_inverse_gaussian::launch_normal_inverse_gaussian_asian_option_cuda<OptionSide::call>(
            device_models,
            warmup_row_count,
            device_products,
            warmup_row_count,
        ...
        check_cuda(cudaDeviceSynchronize(), "NormalInverseGaussian Asian call warmup");
        ...
        check_cuda(
            ...
            "cudaMemcpy NormalInverseGaussian Asian call prices"
        );
        check_cuda(
            cudaMemcpy(
            ...
            "cudaMemcpy NormalInverseGaussian Asian call standard errors"
        );
    } catch (...) {
    ...
    check_cuda(cudaFree(device_products), "cudaFree Asian calls");
    check_cuda(cudaFree(device_prices), "cudaFree NormalInverseGaussian Asian call prices");
    check_cuda(
        cudaFree(device_standard_errors),
        "cudaFree NormalInverseGaussian Asian call standard errors"
    );

// Build one NormalInverseGaussian straddle price dataset from JSON inputs.
#include "common/check_cuda.cuh"
#include "model/equity/normal_inverse_gaussian/straddle.cuh"
...
const std::filesystem::path dataset_path =
    "datasets/price/equity/normal_inverse_gaussian/straddles/"
    "normal_inverse_gaussian_01__straddles_01__01.json";
const std::filesystem::path catalog_path =
    "catalog/price/equity/normal_inverse_gaussian/straddles/"
    "normal_inverse_gaussian_01__straddles_01__01/dataset.yaml";
const std::string url =
    "https://datasets.ai-factory.example/v1/price/equity/normal_inverse_gaussian/straddles/"
    "normal_inverse_gaussian_01__straddles_01__01.json";
...
int main() {
    ...
    try {
        ...
        check_cuda(
            cudaMalloc(
                &device_products,
                products.size() * sizeof(product::StraddleParameters)
            ),
            "cudaMalloc Straddles"
        );
        check_cuda(
            cudaMalloc(
                &device_prices,
            ...
            "cudaMalloc NormalInverseGaussian straddle prices"
        );
        check_cuda(
            cudaMalloc(
            ...
            "cudaMalloc NormalInverseGaussian straddle standard errors"
        );
        ...
        normal_inverse_gaussian::launch_normal_inverse_gaussian_straddle_cuda(
            device_models,
            warmup_row_count,
            device_products,
            warmup_row_count,
        ...
        check_cuda(cudaDeviceSynchronize(), "NormalInverseGaussian straddle warmup");
        ...
        check_cuda(
            ...
            "cudaMemcpy NormalInverseGaussian straddle prices"
        );
        check_cuda(
            cudaMemcpy(
            ...
            "cudaMemcpy NormalInverseGaussian straddle standard errors"
        );
    } catch (...) {
    ...
    check_cuda(cudaFree(device_products), "cudaFree Straddles");
    check_cuda(cudaFree(device_prices), "cudaFree NormalInverseGaussian straddle prices");
    check_cuda(
        cudaFree(device_standard_errors),
        "cudaFree NormalInverseGaussian straddle standard errors"
    );

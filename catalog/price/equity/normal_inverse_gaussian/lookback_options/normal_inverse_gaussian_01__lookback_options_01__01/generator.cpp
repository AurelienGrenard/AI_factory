// Build one NormalInverseGaussian fixed-strike lookback price dataset from JSON inputs.
#include "common/check_cuda.cuh"
#include "model/equity/normal_inverse_gaussian/lookback_option.cuh"
...
const std::filesystem::path dataset_path =
    "datasets/price/equity/normal_inverse_gaussian/lookback_options/"
    "normal_inverse_gaussian_01__lookback_options_01__01.json";
const std::filesystem::path catalog_path =
    "catalog/price/equity/normal_inverse_gaussian/lookback_options/"
    "normal_inverse_gaussian_01__lookback_options_01__01/dataset.yaml";
const std::string url =
    "https://mlp.lpma.math.upmc.fr/DataCarlo/Assets/NormalInverseGaussian/LookbackOption/"
    "normal_inverse_gaussian_01__lookback_options_01__01.json";
...
int main() {
    ...
    const std::vector<product::LookbackOptionParameters> products =
        product::load_lookback_options(product_dataset_path);
    ...
    product::LookbackOptionParameters* device_products = nullptr;
    ...
    try {
        ...
        check_cuda(
            cudaMalloc(
                &device_products,
                products.size() * sizeof(product::LookbackOptionParameters)
            ),
            "cudaMalloc lookback options"
        );
        check_cuda(
            cudaMalloc(
                &device_prices,
            ...
            "cudaMalloc NormalInverseGaussian lookback prices"
        );
        check_cuda(
            cudaMalloc(
            ...
            "cudaMalloc NormalInverseGaussian lookback standard errors"
        );
        ...
        normal_inverse_gaussian::launch_normal_inverse_gaussian_lookback_option_cuda(
            device_models,
            warmup_row_count,
            device_products,
            warmup_row_count,
        ...
        check_cuda(cudaDeviceSynchronize(), "NormalInverseGaussian lookback warmup");
        ...
        check_cuda(
            ...
            "cudaMemcpy NormalInverseGaussian lookback prices"
        );
        check_cuda(
            cudaMemcpy(
            ...
            "cudaMemcpy NormalInverseGaussian lookback standard errors"
        );
    } catch (...) {
    ...
    check_cuda(cudaFree(device_products), "cudaFree lookback options");
    check_cuda(cudaFree(device_prices), "cudaFree NormalInverseGaussian lookback prices");
    check_cuda(
        cudaFree(device_standard_errors),
        "cudaFree NormalInverseGaussian lookback standard errors"
    );

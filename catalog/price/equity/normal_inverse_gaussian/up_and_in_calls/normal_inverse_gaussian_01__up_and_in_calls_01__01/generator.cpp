// Build one NormalInverseGaussian Up-and-in-call price dataset from JSON inputs.
#include "common/check_cuda.cuh"
#include "model/equity/normal_inverse_gaussian/up_and_in_option.cuh"
...
const std::filesystem::path dataset_path =
    "datasets/price/equity/normal_inverse_gaussian/up_and_in_calls/"
    "normal_inverse_gaussian_01__up_and_in_calls_01__01.json";
const std::filesystem::path catalog_path =
    "catalog/price/equity/normal_inverse_gaussian/up_and_in_calls/"
    "normal_inverse_gaussian_01__up_and_in_calls_01__01/dataset.yaml";
const std::string url =
    "https://mlp.lpma.math.upmc.fr/DataCarlo/Assets/NormalInverseGaussian/UpAndInCall/"
    "normal_inverse_gaussian_01__up_and_in_calls_01__01.json";
...
int main() {
    ...
    const std::vector<product::UpAndInOptionParameters> products =
        product::load_up_and_in_options(product_dataset_path);
    ...
    product::UpAndInOptionParameters* device_products = nullptr;
    ...
    try {
        ...
        check_cuda(
            cudaMalloc(
                &device_products,
                products.size() * sizeof(product::UpAndInOptionParameters)
            ),
            "cudaMalloc Up-and-in calls"
        );
        check_cuda(
            cudaMalloc(
                &device_prices,
            ...
            "cudaMalloc NormalInverseGaussian up-and-in call prices"
        );
        check_cuda(
            cudaMalloc(
            ...
            "cudaMalloc NormalInverseGaussian up-and-in call standard errors"
        );
        ...
        normal_inverse_gaussian::launch_normal_inverse_gaussian_up_and_in_option_cuda<OptionSide::call>(
            device_models,
            warmup_row_count,
            device_products,
            warmup_row_count,
        ...
        check_cuda(cudaDeviceSynchronize(), "NormalInverseGaussian up-and-in call warmup");
        ...
        check_cuda(
            ...
            "cudaMemcpy NormalInverseGaussian up-and-in call prices"
        );
        check_cuda(
            cudaMemcpy(
            ...
            "cudaMemcpy NormalInverseGaussian up-and-in call standard errors"
        );
    } catch (...) {
    ...
    check_cuda(cudaFree(device_products), "cudaFree Up-and-in calls");
    check_cuda(cudaFree(device_prices), "cudaFree NormalInverseGaussian up-and-in call prices");
    check_cuda(
        cudaFree(device_standard_errors),
        "cudaFree NormalInverseGaussian up-and-in call standard errors"
    );

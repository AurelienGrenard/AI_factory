// Build one NormalInverseGaussian Phoenix-autocall price dataset from JSON inputs.
...
const std::filesystem::path dataset_path =
    "datasets/price/equity/normal_inverse_gaussian/phoenix_autocalls/"
    "normal_inverse_gaussian_01__phoenix_autocalls_01__01.json";
const std::filesystem::path catalog_path =
    "catalog/price/equity/normal_inverse_gaussian/phoenix_autocalls/"
    "normal_inverse_gaussian_01__phoenix_autocalls_01__01/dataset.yaml";
const std::string url =
    "https://mlp.lpma.math.upmc.fr/DataCarlo/Assets/NormalInverseGaussian/PhoenixAutocall/"
    "normal_inverse_gaussian_01__phoenix_autocalls_01__01.json";
...
int main() {
    ...
    const std::vector<product::PhoenixAutocallParameters> products =
        product::load_phoenix_autocalls(product_dataset_path);
    ...
    product::PhoenixAutocallParameters* device_products = nullptr;
    ...
    try {
        ...
        check_cuda(
            cudaMalloc(
                &device_products,
                products.size() * sizeof(product::PhoenixAutocallParameters)
            ),
            "cudaMalloc Phoenix autocalls"
        );
        check_cuda(
            cudaMalloc(
                &device_prices,
            ...
            "cudaMalloc NormalInverseGaussian Phoenix autocall prices"
        );
        check_cuda(
            cudaMalloc(
            ...
            "cudaMalloc NormalInverseGaussian Phoenix autocall standard errors"
        );
        ...
        normal_inverse_gaussian::launch_normal_inverse_gaussian_phoenix_autocall_cuda(
            device_models,
            warmup_row_count,
            device_products,
            warmup_row_count,
        ...
        check_cuda(cudaDeviceSynchronize(), "NormalInverseGaussian Phoenix autocall warmup");
        ...
        check_cuda(
            ...
            "cudaMemcpy NormalInverseGaussian Phoenix autocall prices"
        );
        check_cuda(
            cudaMemcpy(
            ...
            "cudaMemcpy NormalInverseGaussian Phoenix autocall standard errors"
        );
    } catch (...) {
    ...
    check_cuda(cudaFree(device_products), "cudaFree Phoenix autocalls");
    check_cuda(cudaFree(device_prices), "cudaFree NormalInverseGaussian Phoenix autocall prices");
    check_cuda(
        cudaFree(device_standard_errors),
        "cudaFree NormalInverseGaussian Phoenix autocall standard errors"
    );

// Build one NormalInverseGaussian Forward-start-call price dataset from JSON inputs.
...
const std::filesystem::path dataset_path =
    "datasets/price/equity/normal_inverse_gaussian/forward_start_calls/"
    "normal_inverse_gaussian_01__forward_start_calls_01__01.json";
const std::filesystem::path catalog_path =
    "catalog/price/equity/normal_inverse_gaussian/forward_start_calls/"
    "normal_inverse_gaussian_01__forward_start_calls_01__01/dataset.yaml";
const std::string url =
    "https://mlp.lpma.math.upmc.fr/DataCarlo/Assets/NormalInverseGaussian/ForwardStartCall/"
    "normal_inverse_gaussian_01__forward_start_calls_01__01.json";
...
int main() {
    ...
    const std::vector<product::ForwardStartOptionParameters> products =
        product::load_forward_start_options(product_dataset_path);
    ...
    product::ForwardStartOptionParameters* device_products = nullptr;
    ...
    try {
        ...
        check_cuda(
            cudaMalloc(
                &device_products,
                products.size() * sizeof(product::ForwardStartOptionParameters)
            ),
            "cudaMalloc Forward-start calls"
        );
        check_cuda(
            cudaMalloc(
                &device_prices,
            ...
            "cudaMalloc NormalInverseGaussian call prices"
        );
        check_cuda(
            cudaMalloc(
            ...
            "cudaMalloc NormalInverseGaussian call standard errors"
        );
        ...
        check_cuda(
            ...
            "cudaMemcpy Forward-start calls"
        );
        ...
        normal_inverse_gaussian::launch_normal_inverse_gaussian_forward_start_option_cuda<OptionSide::call>(
            device_models,
            warmup_row_count,
            device_products,
            warmup_row_count,
        ...
        check_cuda(cudaDeviceSynchronize(), "NormalInverseGaussian call warmup");
        ...
        check_cuda(
            ...
            "cudaMemcpy NormalInverseGaussian call prices"
        );
        check_cuda(
            cudaMemcpy(
            ...
            "cudaMemcpy NormalInverseGaussian call standard errors"
        );
    } catch (...) {
    ...
    check_cuda(cudaFree(device_products), "cudaFree Forward-start calls");
    check_cuda(cudaFree(device_prices), "cudaFree NormalInverseGaussian call prices");
    check_cuda(
        cudaFree(device_standard_errors),
        "cudaFree NormalInverseGaussian call standard errors"
    );

// Build one NormalInverseGaussian Range Accrual price dataset from JSON inputs.
#include "common/check_cuda.cuh"
#include "model/equity/normal_inverse_gaussian/range_accrual.cuh"
...
const std::filesystem::path dataset_path =
    "datasets/price/equity/normal_inverse_gaussian/range_accruals/"
    "normal_inverse_gaussian_01__range_accruals_01__01.json";
const std::filesystem::path catalog_path =
    "catalog/price/equity/normal_inverse_gaussian/range_accruals/"
    "normal_inverse_gaussian_01__range_accruals_01__01/dataset.yaml";
const std::string url =
    "https://mlp.lpma.math.upmc.fr/DataCarlo/Assets/NormalInverseGaussian/RangeAccrual/"
    "normal_inverse_gaussian_01__range_accruals_01__01.json";
...
int main() {
    ...
    try {
        ...
        check_cuda(
            cudaMalloc(
                &device_products,
                products.size() * sizeof(product::RangeAccrualParameters)
            ),
            "cudaMalloc Range Accruals"
        );
        check_cuda(
            cudaMalloc(
                &device_prices,
            ...
            "cudaMalloc NormalInverseGaussian Range Accrual prices"
        );
        check_cuda(
            cudaMalloc(
            ...
            "cudaMalloc NormalInverseGaussian Range Accrual standard errors"
        );
        ...
        normal_inverse_gaussian::launch_normal_inverse_gaussian_range_accrual_cuda(
            device_models,
            warmup_row_count,
            device_products,
            warmup_row_count,
        ...
        check_cuda(cudaDeviceSynchronize(), "NormalInverseGaussian Range Accrual warmup");
        ...
        check_cuda(
            ...
            "cudaMemcpy NormalInverseGaussian Range Accrual prices"
        );
        check_cuda(
            cudaMemcpy(
            ...
            "cudaMemcpy NormalInverseGaussian Range Accrual standard errors"
        );
    } catch (...) {
    ...
    check_cuda(cudaFree(device_products), "cudaFree Range Accruals");
    check_cuda(cudaFree(device_prices), "cudaFree NormalInverseGaussian Range Accrual prices");
    check_cuda(
        cudaFree(device_standard_errors),
        "cudaFree NormalInverseGaussian Range Accrual standard errors"
    );

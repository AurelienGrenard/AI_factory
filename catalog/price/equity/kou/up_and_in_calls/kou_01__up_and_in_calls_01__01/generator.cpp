// Build one Kou Up-and-in-call price dataset from JSON inputs.
...
const std::filesystem::path dataset_path =
    "datasets/price/equity/kou/up_and_in_calls/"
    "kou_01__up_and_in_calls_01__01.json";
const std::filesystem::path catalog_path =
    "catalog/price/equity/kou/up_and_in_calls/"
    "kou_01__up_and_in_calls_01__01/dataset.yaml";
const std::string url =
    "https://mlp.lpma.math.upmc.fr/DataCarlo/Assets/Kou/UpAndInCall/"
    "kou_01__up_and_in_calls_01__01.json";
...
int main() {
    ...
    try {
        ...
        check_cuda(
            ...
            "cudaMalloc Kou up-and-in call prices"
        );
        check_cuda(
            cudaMalloc(
            ...
            "cudaMalloc Kou up-and-in call standard errors"
        );
        ...
        kou::launch_kou_up_and_in_option_cuda<OptionSide::call>(
            device_models,
            warmup_row_count,
            device_products,
            warmup_row_count,
        ...
        check_cuda(cudaDeviceSynchronize(), "Kou up-and-in call warmup");
        ...
        check_cuda(
            ...
            "cudaMemcpy Kou up-and-in call prices"
        );
        check_cuda(
            cudaMemcpy(
            ...
            "cudaMemcpy Kou up-and-in call standard errors"
        );
    } catch (...) {
    ...
    check_cuda(cudaFree(device_prices), "cudaFree Kou up-and-in call prices");
    check_cuda(
        cudaFree(device_standard_errors),
        "cudaFree Kou up-and-in call standard errors"
    );

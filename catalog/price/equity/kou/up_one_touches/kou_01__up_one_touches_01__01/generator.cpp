// Build one Kou Up-one-touch price dataset from JSON inputs.
#include "common/check_cuda.cuh"
#include "model/equity/kou/up_one_touch.cuh"
...
const std::filesystem::path dataset_path =
    "datasets/price/equity/kou/up_one_touches/"
    "kou_01__up_one_touches_01__01.json";
const std::filesystem::path catalog_path =
    "catalog/price/equity/kou/up_one_touches/"
    "kou_01__up_one_touches_01__01/dataset.yaml";
const std::string url =
    "https://mlp.lpma.math.upmc.fr/DataCarlo/Assets/Kou/UpOneTouch/"
    "kou_01__up_one_touches_01__01.json";
...
int main() {
    ...
    try {
        ...
        check_cuda(
            ...
            "cudaMalloc Kou up one-touch prices"
        );
        check_cuda(
            cudaMalloc(
            ...
            "cudaMalloc Kou up one-touch standard errors"
        );
        ...
        kou::launch_kou_up_one_touch_cuda(
            device_models,
            warmup_row_count,
            device_products,
            warmup_row_count,
        ...
        check_cuda(cudaDeviceSynchronize(), "Kou up one-touch warmup");
        ...
        check_cuda(
            ...
            "cudaMemcpy Kou up one-touch prices"
        );
        check_cuda(
            cudaMemcpy(
            ...
            "cudaMemcpy Kou up one-touch standard errors"
        );
    } catch (...) {
    ...
    check_cuda(cudaFree(device_prices), "cudaFree Kou up one-touch prices");
    check_cuda(
        cudaFree(device_standard_errors),
        "cudaFree Kou up one-touch standard errors"
    );

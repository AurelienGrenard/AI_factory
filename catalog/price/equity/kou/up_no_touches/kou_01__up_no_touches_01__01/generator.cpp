// Build one Kou Up-no-touch price dataset from JSON inputs.
#include "common/check_cuda.cuh"
#include "model/equity/kou/up_no_touch.cuh"
...
const std::filesystem::path dataset_path =
    "datasets/price/equity/kou/up_no_touches/"
    "kou_01__up_no_touches_01__01.json";
const std::filesystem::path catalog_path =
    "catalog/price/equity/kou/up_no_touches/"
    "kou_01__up_no_touches_01__01/dataset.yaml";
const std::string url =
    "https://mlp.lpma.math.upmc.fr/DataCarlo/Assets/Kou/UpNoTouch/"
    "kou_01__up_no_touches_01__01.json";
...
int main() {
    ...
    try {
        ...
        check_cuda(
            ...
            "cudaMalloc Kou up no-touch prices"
        );
        check_cuda(
            cudaMalloc(
            ...
            "cudaMalloc Kou up no-touch standard errors"
        );
        ...
        kou::launch_kou_up_no_touch_cuda(
            device_models,
            warmup_row_count,
            device_products,
            warmup_row_count,
        ...
        check_cuda(cudaDeviceSynchronize(), "Kou up no-touch warmup");
        ...
        check_cuda(
            ...
            "cudaMemcpy Kou up no-touch prices"
        );
        check_cuda(
            cudaMemcpy(
            ...
            "cudaMemcpy Kou up no-touch standard errors"
        );
    } catch (...) {
    ...
    check_cuda(cudaFree(device_prices), "cudaFree Kou up no-touch prices");
    check_cuda(
        cudaFree(device_standard_errors),
        "cudaFree Kou up no-touch standard errors"
    );

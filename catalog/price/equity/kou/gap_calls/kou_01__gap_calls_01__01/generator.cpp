...
const std::filesystem::path dataset_path =
    "datasets/price/equity/kou/gap_calls/"
    "kou_01__gap_calls_01__01.json";
const std::filesystem::path catalog_path =
    "catalog/price/equity/kou/gap_calls/"
    "kou_01__gap_calls_01__01/dataset.yaml";
const std::string url =
    "https://datasets.ai-factory.example/v1/price/equity/kou/gap_calls/"
    "kou_01__gap_calls_01__01.json";
...
int main() {
    ...
    try {
        ...
        check_cuda(
            ...
            "cudaMalloc Kou gap call prices"
        );
        check_cuda(
            cudaMalloc(
            ...
            "cudaMalloc Kou gap call standard errors"
        );
        ...
        kou::launch_kou_gap_option_cuda<OptionSide::call>(
            device_models,
            warmup_row_count,
            device_products,
            warmup_row_count,
        ...
        check_cuda(cudaDeviceSynchronize(), "Kou gap call warmup");
        ...
        check_cuda(
            ...
            "cudaMemcpy Kou gap call prices"
        );
        check_cuda(
            cudaMemcpy(
            ...
            "cudaMemcpy Kou gap call standard errors"
        );
    } catch (...) {
    ...
    check_cuda(cudaFree(device_prices), "cudaFree Kou gap call prices");
    check_cuda(
        cudaFree(device_standard_errors),
        "cudaFree Kou gap call standard errors"
    );

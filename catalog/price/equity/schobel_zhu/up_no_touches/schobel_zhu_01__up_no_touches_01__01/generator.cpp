// Build one SchobelZhu Up-no-touch price dataset from JSON inputs.
#include "common/check_cuda.cuh"
#include "model/equity/schobel_zhu/up_no_touch.cuh"
...
const std::filesystem::path dataset_path =
    "datasets/price/equity/schobel_zhu/up_no_touches/"
    "schobel_zhu_01__up_no_touches_01__01.json";
const std::filesystem::path catalog_path =
    "catalog/price/equity/schobel_zhu/up_no_touches/"
    "schobel_zhu_01__up_no_touches_01__01/dataset.yaml";
const std::string url =
    "https://mlp.lpma.math.upmc.fr/DataCarlo/Assets/SchobelZhu/UpNoTouch/"
    "schobel_zhu_01__up_no_touches_01__01.json";
...
int main() {
    ...
    try {
        ...
        check_cuda(
            ...
            "cudaMalloc SchobelZhu up no-touch prices"
        );
        check_cuda(
            cudaMalloc(
            ...
            "cudaMalloc SchobelZhu up no-touch standard errors"
        );
        ...
        schobel_zhu::launch_schobel_zhu_up_no_touch_cuda(
            device_models,
            warmup_row_count,
            device_products,
            warmup_row_count,
        ...
        check_cuda(cudaDeviceSynchronize(), "SchobelZhu up no-touch warmup");
        ...
        check_cuda(
            ...
            "cudaMemcpy SchobelZhu up no-touch prices"
        );
        check_cuda(
            cudaMemcpy(
            ...
            "cudaMemcpy SchobelZhu up no-touch standard errors"
        );
    } catch (...) {
    ...
    check_cuda(cudaFree(device_prices), "cudaFree SchobelZhu up no-touch prices");
    check_cuda(
        cudaFree(device_standard_errors),
        "cudaFree SchobelZhu up no-touch standard errors"
    );

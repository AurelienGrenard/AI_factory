// Build one SchobelZhu Up-one-touch price dataset from JSON inputs.
#include "common/check_cuda.cuh"
#include "model/equity/schobel_zhu/up_one_touch.cuh"
...
const std::filesystem::path dataset_path =
    "datasets/price/equity/schobel_zhu/up_one_touches/"
    "schobel_zhu_01__up_one_touches_01__01.json";
const std::filesystem::path catalog_path =
    "catalog/price/equity/schobel_zhu/up_one_touches/"
    "schobel_zhu_01__up_one_touches_01__01/dataset.yaml";
const std::string url =
    "https://mlp.lpma.math.upmc.fr/DataCarlo/Assets/SchobelZhu/UpOneTouch/"
    "schobel_zhu_01__up_one_touches_01__01.json";
...
int main() {
    ...
    try {
        ...
        check_cuda(
            ...
            "cudaMalloc SchobelZhu up one-touch prices"
        );
        check_cuda(
            cudaMalloc(
            ...
            "cudaMalloc SchobelZhu up one-touch standard errors"
        );
        ...
        schobel_zhu::launch_schobel_zhu_up_one_touch_cuda(
            device_models,
            warmup_row_count,
            device_products,
            warmup_row_count,
        ...
        check_cuda(cudaDeviceSynchronize(), "SchobelZhu up one-touch warmup");
        ...
        check_cuda(
            ...
            "cudaMemcpy SchobelZhu up one-touch prices"
        );
        check_cuda(
            cudaMemcpy(
            ...
            "cudaMemcpy SchobelZhu up one-touch standard errors"
        );
    } catch (...) {
    ...
    check_cuda(cudaFree(device_prices), "cudaFree SchobelZhu up one-touch prices");
    check_cuda(
        cudaFree(device_standard_errors),
        "cudaFree SchobelZhu up one-touch standard errors"
    );

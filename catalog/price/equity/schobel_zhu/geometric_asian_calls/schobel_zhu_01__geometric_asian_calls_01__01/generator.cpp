// Build one SchobelZhu Geometric-Asian-call price dataset from JSON inputs.
#include "common/check_cuda.cuh"
#include "model/equity/schobel_zhu/geometric_asian_option.cuh"
...
const std::filesystem::path model_dataset_path =
    "datasets/model/equity/schobel_zhu/schobel_zhu_01.json";
...
const std::filesystem::path dataset_path =
    "datasets/price/equity/schobel_zhu/geometric_asian_calls/"
    "schobel_zhu_01__geometric_asian_calls_01__01.json";
const std::filesystem::path catalog_path =
    "catalog/price/equity/schobel_zhu/geometric_asian_calls/"
    "schobel_zhu_01__geometric_asian_calls_01__01/dataset.yaml";
const std::string url =
    "https://mlp.lpma.math.upmc.fr/DataCarlo/Assets/SchobelZhu/GeometricAsianCall/"
    "schobel_zhu_01__geometric_asian_calls_01__01.json";
const std::string numerical_method = "exact OU factor with log-spot Euler";
...
int main() {
    ...
    const std::vector<schobel_zhu::SchobelZhuModelParameters> models =
        schobel_zhu::load_models(model_dataset_path);
    ...
    schobel_zhu::SchobelZhuModelParameters* device_models = nullptr;
    ...
    try {
        ...
        check_cuda(
            cudaMalloc(
                &device_models,
                models.size() * sizeof(schobel_zhu::SchobelZhuModelParameters)
            ),
            "cudaMalloc SchobelZhu models"
        );
        check_cuda(
            cudaMalloc(
                &device_products,
        ...
        check_cuda(
            ...
            "cudaMalloc SchobelZhu Geometric Asian call prices"
        );
        check_cuda(
            cudaMalloc(
            ...
            "cudaMalloc SchobelZhu Geometric Asian call standard errors"
        );
        ...
        schobel_zhu::launch_schobel_zhu_geometric_asian_option_cuda<OptionSide::call>(
            device_models,
            warmup_row_count,
            device_products,
            warmup_row_count,
        ...
        check_cuda(cudaDeviceSynchronize(), "SchobelZhu Geometric Asian call warmup");
        ...
        check_cuda(
            ...
            "cudaMemcpy SchobelZhu Geometric Asian call prices"
        );
        check_cuda(
            cudaMemcpy(
            ...
            "cudaMemcpy SchobelZhu Geometric Asian call standard errors"
        );
    } catch (...) {
    ...
    check_cuda(cudaFree(device_models), "cudaFree SchobelZhu models");
    check_cuda(cudaFree(device_products), "cudaFree Geometric Asian calls");
    check_cuda(cudaFree(device_prices), "cudaFree SchobelZhu Geometric Asian call prices");
    check_cuda(
        cudaFree(device_standard_errors),
        "cudaFree SchobelZhu Geometric Asian call standard errors"
    );

// Build one CIR zero-coupon bond put price dataset.
...
const std::filesystem::path dataset_path =
    "datasets/model/fixed_income/cir/prices/zero_coupon_bond_puts/"
    "cir_01__zero_coupon_bond_puts_01__01.json";
...
const std::string url =
    ...
    "fixed_income/cir/prices/zero_coupon_bond_puts/"
    "cir_01__zero_coupon_bond_puts_01__01.json";
...
int main() {
    ...
    const std::vector<cir::ModelParameters> models =
        cir::load_models(model_dataset_path);
    ...
    try {
        ...
        check_cuda(
            ...
            "cudaMalloc CIR zero-coupon bond put prices"
        );
        check_cuda(
            cudaMemcpy(
                device_models,
        ...
        check_cuda(cudaDeviceSynchronize(), "CIR ZCB put warmup");
        ...
        check_cuda(
            ...
            "cudaMemcpy CIR zero-coupon bond put prices"
        );
    } catch (...) {
    ...
    check_cuda(cudaFree(device_models), "cudaFree CIR models");
    check_cuda(cudaFree(device_products), "cudaFree zero-coupon bond puts");
    check_cuda(cudaFree(device_prices), "cudaFree CIR ZCB put prices");

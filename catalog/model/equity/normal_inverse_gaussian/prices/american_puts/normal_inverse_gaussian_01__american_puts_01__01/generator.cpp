// Build one NormalInverseGaussian American-put price dataset from JSON inputs.
...
const std::filesystem::path dataset_path =
    "datasets/model/equity/normal_inverse_gaussian/prices/american_puts/"
    "normal_inverse_gaussian_01__american_puts_01__01.json";
const std::filesystem::path catalog_path =
    "catalog/model/equity/normal_inverse_gaussian/prices/american_puts/"
    "normal_inverse_gaussian_01__american_puts_01__01/dataset.yaml";
const std::string url =
    "https://datasets.ai-factory.example/v1/model/equity/normal_inverse_gaussian/prices/american_puts/"
    "normal_inverse_gaussian_01__american_puts_01__01.json";
const std::string numerical_method = "Exact inverse-Gaussian subordination + Longstaff-Schwartz";
const std::string regression_basis = "Spot and log-moneyness six-term basis";

}  // namespace

namespace normal_inverse_gaussian =
...
int main() {
    ...
    namespace normal_inverse_gaussian = model::equity::normal_inverse_gaussian;
    ...
    try {
        ...
        check_cuda(
            ...
            "cudaMalloc NormalInverseGaussian American put prices"
        );
        check_cuda(
            cudaMalloc(
            ...
            "cudaMalloc NormalInverseGaussian American put standard errors"
        );
        ...
        normal_inverse_gaussian::launch_normal_inverse_gaussian_american_option_cuda<OptionSide::put>(
            device_models,
            1U,
            products.data(),
            device_products,
        ...
        check_cuda(cudaDeviceSynchronize(), "NormalInverseGaussian American-put warmup");

        // Plan the reusable workspace and time every production batch.
        wall_start = std::chrono::steady_clock::now();
        execution = normal_inverse_gaussian::launch_normal_inverse_gaussian_american_option_cuda<OptionSide::put>(
            device_models,
            models.size(),
            products.data(),
            device_products,
        ...
        longstaff_schwartz::validate_regression_diagnostics(
            execution, "normal-inverse-Gaussian American put"
        );
        ...
        check_cuda(
            ...
            "cudaMemcpy NormalInverseGaussian American put prices"
        );
        check_cuda(
            cudaMemcpy(
            ...
            "cudaMemcpy NormalInverseGaussian American put standard errors"
        );
    ...
    check_cuda(cudaFree(device_prices), "cudaFree NormalInverseGaussian American put prices");
    check_cuda(
        cudaFree(device_standard_errors),
        "cudaFree NormalInverseGaussian American put standard errors"
    );

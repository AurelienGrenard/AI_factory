// Build one CEV Athena-autocall price dataset from JSON inputs.
...
const std::filesystem::path catalog_path =
    "catalog/model/equity/cev/prices/athena_autocalls/"
    "cev_01__athena_autocalls_01__01/dataset.yaml";
const std::string url =
    "https://mlp.lpma.math.upmc.fr/DataCarlo/Assets/CEV/AthenaAutocall/"
    "cev_01__athena_autocalls_01__01.json";
...
int main() {
    ...
    try {
        ...
        check_cuda(
            ...
            "cudaMalloc CEV Athena autocall prices"
        );
        check_cuda(
            cudaMalloc(
            ...
            "cudaMalloc CEV Athena autocall standard errors"
        );
        ...
        cev::launch_cev_athena_autocall_cuda(
            device_models,
            warmup_row_count,
            device_products,
            warmup_row_count,
        ...
        check_cuda(cudaDeviceSynchronize(), "CEV Athena autocall warmup");
        ...
        check_cuda(
            ...
            "cudaMemcpy CEV Athena autocall prices"
        );
        check_cuda(
            cudaMemcpy(
            ...
            "cudaMemcpy CEV Athena autocall standard errors"
        );
    } catch (...) {
    ...
    check_cuda(cudaFree(device_prices), "cudaFree CEV Athena autocall prices");
    check_cuda(
        cudaFree(device_standard_errors),
        "cudaFree CEV Athena autocall standard errors"
    );

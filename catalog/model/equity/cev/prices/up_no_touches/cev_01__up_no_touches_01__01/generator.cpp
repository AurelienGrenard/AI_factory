// Build one CEV Up-no-touch price dataset from JSON inputs.
#include "common/check_cuda.cuh"
#include "model/equity/cev/up_no_touch.cuh"
...
const std::filesystem::path dataset_path =
    "datasets/model/equity/cev/prices/up_no_touches/"
    "cev_01__up_no_touches_01__01.json";
const std::filesystem::path catalog_path =
    "catalog/model/equity/cev/prices/up_no_touches/"
    "cev_01__up_no_touches_01__01/dataset.yaml";
const std::string url =
    "https://mlp.lpma.math.upmc.fr/DataCarlo/Assets/CEV/UpNoTouch/"
    "cev_01__up_no_touches_01__01.json";
...
int main() {
    ...
    try {
        ...
        check_cuda(
            ...
            "cudaMalloc CEV up no-touch prices"
        );
        check_cuda(
            cudaMalloc(
            ...
            "cudaMalloc CEV up no-touch standard errors"
        );
        ...
        cev::launch_cev_up_no_touch_cuda(
            device_models,
            warmup_row_count,
            device_products,
            warmup_row_count,
        ...
        check_cuda(cudaDeviceSynchronize(), "CEV up no-touch warmup");
        ...
        check_cuda(
            ...
            "cudaMemcpy CEV up no-touch prices"
        );
        check_cuda(
            cudaMemcpy(
            ...
            "cudaMemcpy CEV up no-touch standard errors"
        );
    } catch (...) {
    ...
    check_cuda(cudaFree(device_prices), "cudaFree CEV up no-touch prices");
    check_cuda(
        cudaFree(device_standard_errors),
        "cudaFree CEV up no-touch standard errors"
    );

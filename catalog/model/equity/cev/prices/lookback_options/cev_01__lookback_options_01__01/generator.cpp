// Build one CEV fixed-strike lookback price dataset from JSON inputs.
...
const std::filesystem::path dataset_path =
    "datasets/model/equity/cev/prices/lookback_options/"
    "cev_01__lookback_options_01__01.json";
const std::filesystem::path catalog_path =
    "catalog/model/equity/cev/prices/lookback_options/"
    "cev_01__lookback_options_01__01/dataset.yaml";
const std::string url =
    "https://mlp.lpma.math.upmc.fr/DataCarlo/Assets/CEV/LookbackOption/"
    "cev_01__lookback_options_01__01.json";
...
int main() {
    ...
    try {
        ...
        check_cuda(
            ...
            "cudaMalloc CEV lookback prices"
        );
        check_cuda(
            cudaMalloc(
            ...
            "cudaMalloc CEV lookback standard errors"
        );
        ...
        cev::launch_cev_lookback_option_cuda(
            device_models,
            warmup_row_count,
            device_products,
            warmup_row_count,
        ...
        check_cuda(cudaDeviceSynchronize(), "CEV lookback warmup");
        ...
        check_cuda(
            ...
            "cudaMemcpy CEV lookback prices"
        );
        check_cuda(
            cudaMemcpy(
            ...
            "cudaMemcpy CEV lookback standard errors"
        );
    } catch (...) {
    ...
    check_cuda(cudaFree(device_prices), "cudaFree CEV lookback prices");
    check_cuda(
        cudaFree(device_standard_errors),
        "cudaFree CEV lookback standard errors"
    );

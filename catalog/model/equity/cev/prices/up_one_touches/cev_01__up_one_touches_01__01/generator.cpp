// Build one CEV Up-one-touch price dataset from JSON inputs.
#include "common/check_cuda.cuh"
#include "model/equity/cev/up_one_touch.cuh"
...
const std::filesystem::path dataset_path =
    "datasets/model/equity/cev/prices/up_one_touches/"
    "cev_01__up_one_touches_01__01.json";
const std::filesystem::path catalog_path =
    "catalog/model/equity/cev/prices/up_one_touches/"
    "cev_01__up_one_touches_01__01/dataset.yaml";
const std::string url =
    "https://mlp.lpma.math.upmc.fr/DataCarlo/Assets/CEV/UpOneTouch/"
    "cev_01__up_one_touches_01__01.json";
...
int main() {
    ...
    try {
        ...
        check_cuda(
            ...
            "cudaMalloc CEV up one-touch prices"
        );
        check_cuda(
            cudaMalloc(
            ...
            "cudaMalloc CEV up one-touch standard errors"
        );
        ...
        cev::launch_cev_up_one_touch_cuda(
            device_models,
            warmup_row_count,
            device_products,
            warmup_row_count,
        ...
        check_cuda(cudaDeviceSynchronize(), "CEV up one-touch warmup");
        ...
        check_cuda(
            ...
            "cudaMemcpy CEV up one-touch prices"
        );
        check_cuda(
            cudaMemcpy(
            ...
            "cudaMemcpy CEV up one-touch standard errors"
        );
    } catch (...) {
    ...
    check_cuda(cudaFree(device_prices), "cudaFree CEV up one-touch prices");
    check_cuda(
        cudaFree(device_standard_errors),
        "cudaFree CEV up one-touch standard errors"
    );

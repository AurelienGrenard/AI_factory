// Build one CEV Asian-call price dataset from JSON inputs.
...
const std::filesystem::path dataset_path =
    "datasets/model/equity/cev/prices/asian_calls/"
    "cev_01__asian_calls_01__01.json";
const std::filesystem::path catalog_path =
    "catalog/model/equity/cev/prices/asian_calls/"
    "cev_01__asian_calls_01__01/dataset.yaml";
const std::string url =
    "https://mlp.lpma.math.upmc.fr/DataCarlo/Assets/CEV/AsianCall/"
    "cev_01__asian_calls_01__01.json";
...
int main() {
    ...
    try {
        ...
        check_cuda(
            ...
            "cudaMalloc CEV Asian call prices"
        );
        check_cuda(
            cudaMalloc(
            ...
            "cudaMalloc CEV Asian call standard errors"
        );
        ...
        cev::launch_cev_asian_option_cuda<OptionSide::call>(
            device_models,
            warmup_row_count,
            device_products,
            warmup_row_count,
        ...
        check_cuda(cudaDeviceSynchronize(), "CEV Asian call warmup");
        ...
        check_cuda(
            ...
            "cudaMemcpy CEV Asian call prices"
        );
        check_cuda(
            cudaMemcpy(
            ...
            "cudaMemcpy CEV Asian call standard errors"
        );
    } catch (...) {
    ...
    check_cuda(cudaFree(device_prices), "cudaFree CEV Asian call prices");
    check_cuda(
        cudaFree(device_standard_errors),
        "cudaFree CEV Asian call standard errors"
    );

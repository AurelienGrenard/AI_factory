...
// Generate the caplet dataset and catalog entry.
int main() {
    ...
    const std::string url =
        "https://datasets.ai-factory.example/v1/product/caplets/"
        "caplets_01.json";
    ...
    write_product_dataset(
        ...
        url,
        {
            ...
            {"strike", "Simple annualized caplet strike."},
        ...
        {
            {"expression", "N * delta * max(L(T1; T1, T2) - K, 0)"},
            {"payment_time", "T2"},
            {"normalization", "N = 1"},
        },
        rows

// Generate normalized floorlets with central strikes and sparse tail cases.
...
// Generate the floorlet dataset and catalog entry.
int main() {
    ...
    const std::string url =
        "https://datasets.ai-factory.example/v1/product/floorlets/"
        "floorlets_01.json";
    ...
    write_product_dataset(
        ...
        url,
        {
            ...
            {"strike", "Simple annualized floorlet strike."},
        ...
        {
            {"expression", "N * delta * max(K - L(T1; T1, T2), 0)"},
            {"payment_time", "T2"},
            {"normalization", "N = 1"},
        },
        rows

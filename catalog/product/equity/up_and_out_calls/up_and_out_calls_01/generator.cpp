...
// Generate the Up-and-out-call dataset and catalog entry.
int main() {
    ...
    const std::filesystem::path catalog_path =
        "catalog/product/equity/up_and_out_calls/up_and_out_calls_01/dataset.yaml";
    const std::string url =
        "https://datasets.ai-factory.example/v1/product/up_and_out_calls/"
        "up_and_out_calls_01.json";
    ...
    write_product_dataset(
        ...
        {
            {"expression", "max(S_T - K, 0) if max(S_[0,T]) < barrier; otherwise 0"},

...
// Generate the Forward-start-call dataset and catalog entry.
int main() {
    ...
    const std::filesystem::path catalog_path =
        "catalog/product/equity/forward_start_calls/forward_start_calls_01/dataset.yaml";
    const std::string url =
        "https://datasets.ai-factory.example/v1/product/forward_start_calls/"
        "forward_start_calls_01.json";
    ...
    write_product_dataset(
        ...
        {
            {"expression", "max(S_T - moneyness * S_reset, 0)"},
        },
        rows
    );

// Generate cash-or-nothing puts on the standard equity grid.
...
// Generate the digital-put dataset and catalog entry.
int main() {
    ...
    const std::string url =
        "https://datasets.ai-factory.example/v1/product/digital_puts/"
        "digital_puts_01.json";
    ...
    write_product_dataset(
        ...
        url,
        {
            ...
            {"cash_payoff", "Cash amount paid below the trigger strike."},
        },
        {
            {"expression", "cash_payoff * 1{S_T < K}"},
        },
        rows
    );

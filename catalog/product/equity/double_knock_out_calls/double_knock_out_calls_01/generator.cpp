...
// Generate the Double-knock-out-call dataset and catalog entry.
int main() {
    ...
    const std::filesystem::path catalog_path =
        "catalog/product/equity/double_knock_out_calls/double_knock_out_calls_01/dataset.yaml";
    const std::string url =
        "https://datasets.ai-factory.example/v1/product/double_knock_out_calls/"
        "double_knock_out_calls_01.json";
    ...
    write_product_dataset(
        ...
        {
            {"expression", "max(S_T - K, 0) while S remains between both barriers"},

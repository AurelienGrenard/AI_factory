...
from ai_factory.pytorch.common.fixed_income import (
    cir_bond_coefficients, cir_plus_plus_bond_coefficients,
...
def _price_batch_impl(rows, models, curves, products, *, num_paths, target, target_dt, batch_rows):
    ...
    for offset in range(0, len(rows), batch_rows):
        ...
        initial_factor = _column([m["initial_factor"] for m in ms], target)
        rate = initial_factor.expand(-1, num_paths).clone()
        ...
        for step in range(1, max_step + 1):
            ...
            for row_index, exercise in schedule.get(step, ()):
                ...
                time = exercise_times[row_index : row_index + 1, exercise : exercise + 1]
                discounts[row_index, :, exercise] = cir_plus_plus_path_discount(
                    integral[row_index : row_index + 1], time,
                    initial_factor[row_index : row_index + 1],
                    kappa[row_index : row_index + 1], theta[row_index : row_index + 1],
                    sigma[row_index : row_index + 1], beta0[row_index : row_index + 1],
                    beta1[row_index : row_index + 1], beta2[row_index : row_index + 1],
                    tau[row_index : row_index + 1],
                )[0]

        immediate = torch.zeros_like(states)
        basis_state = torch.zeros_like(states)
        maturities = first[:, :, None] + accrual[:, :, None] * (
            payment_axis[:, None, :] + 1
        )
        bond_a, bond_b = cir_plus_plus_bond_coefficients(
            exercise_times[:, :, None], maturities,
            initial_factor[:, :, None], kappa[:, :, None],
            theta[:, :, None], sigma[:, :, None], beta0[:, :, None],
            beta1[:, :, None], beta2[:, :, None], tau[:, :, None],
        )
...
def price_batch(rows, model_by_id, curve_by_id, product_by_id, *, num_paths, device,
    ...
    return _price_batch_impl(
        rows, model_by_id, curve_by_id, product_by_id, num_paths=num_paths, target=target,

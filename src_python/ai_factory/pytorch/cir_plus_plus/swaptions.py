...
def _price_group(
    ...
    *,
    ...
    for offset in range(0, len(rows), batch_rows):
        ...
        initial_factor = _column(
            [model["initial_factor"] for model in batch_models], target
        )
        ...
        bond_a, bond_b = cir_plus_plus_bond_coefficients(
            expiry, maturities, initial_factor, kappa, theta, volatility,
        ...
        discount = cir_plus_plus_path_discount(
            integral, expiry, initial_factor, kappa, theta, volatility,
            beta0, beta1, beta2, tau,
        )
